...
/**
 * BillDiscountApprovalBanner — v3.74.376 (Stage 5 of 5).
 *
 * Same shape as the invoice / booking banners but scoped to purchase
 * invoices (bills). Renders one of four cards:
 *
 *   amount = 0 OR bill not in draft → render nothing
 *   pending                         → yellow "في انتظار اعتماد"
 *   approved                        → slim green check
 *   rejected                        → red with decision_note
 *   no_request                      → orange fallback
 *
 * onGateChange lets the parent disable the submit-for-receipt action
...
interface Props {
  billId: string
  lang?: string
  onGateChange?: (gate: BillDiscountGate, amount: number) => void
  refreshToken?: number
}

export function BillDiscountApprovalBanner({
  billId, lang = "ar", onGateChange, refreshToken,
}: Props) {
  const isAr = lang !== "en"
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`/api/bills/${billId}/discount-approval`, {
        cache: "no-store",
      })
      if (!res.ok) {
        setState(null)
  ...
  if (state.bill_status !== "draft") return null
        ...
        <CardContent className="py-4 flex items-start gap-3 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-yellow-700 dark:text-yellow-300 mt-1">
              {t("لا يمكن ترحيل فاتورة المورد قبل الاعتماد. يستلم المعتمدون إشعاراً تلقائياً.", "Bill posting is blocked until approval. Approvers receive an automatic notification.")}
            </p>
          </div>
        ...
        <CardContent className="py-4 flex items-start gap-3 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-red-700 dark:text-red-300 mt-2">
              {t("عدّل قيمة الخصم على الفاتورة لإعادة الإرسال للاعتماد، أو ألغِ الخصم لتتمكن من الترحيل مباشرة.", "Edit the discount on the bill to re-submit, or remove it to proceed.")}
            </p>
          </div>
        ...
        <CardContent className="py-3 flex items-center gap-3 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-green-800 dark:text-green-200 mt-0.5">
              {t("يمكن ترحيل الفاتورة الآن.","Bill can be posted.")}
        ...
        <CardContent className="py-4 flex items-start gap-3 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-orange-700 dark:text-orange-300 mt-1">
              {t("احفظ الفاتورة مرة أخرى لإرسال طلب الاعتماد تلقائياً.","Save the bill again to trigger an automatic approval request.")}
            </p>
          </div>
          <Button size="sm" variant="ghost" className="gap-1 text-xs shrink-0" onClick={refresh} disabled={refreshing}>
            <RefreshCw className={`w-3.5 h-3.5 ${refreshing ? "animate-spin" : ""}`} />
