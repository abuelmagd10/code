...
export function SecuritySection({ appLang = 'ar' }: { appLang?: 'ar' | 'en' }) {
  ...
  const pillars = [
    { icon: <Lock className="w-7 h-7" />, title: isAr ? 'عزل بيانات متعدد الشركات' : 'Multi-Tenant Data Isolation', desc: isAr ? 'Row-Level Security يضمن عزل كامل لبيانات كل شركة.' : 'Row-Level Security ensures full data isolation per company.' },
    { icon: <UserCog className="w-7 h-7" />, title: isAr ? 'صلاحيات دقيقة (RBAC)' : 'Granular Permissions (RBAC)', desc: isAr ? 'صلاحيات لكل وحدة وكل إجراء، أدوار مرنة.' : 'Per-module/action permissions, flexible roles.' },
    { icon: <FileLock className="w-7 h-7" />, title: isAr ? 'سير عمل الموافقات' : 'Approval Workflows', desc: isAr ? 'اعتمادات متعددة المستويات للمصروفات والمرتجعات.' : 'Multi-level approvals for expenses and returns.' },
    { icon: <Activity className="w-7 h-7" />, title: isAr ? 'سجل تدقيق كامل' : 'Complete Audit Trail', desc: isAr ? 'كل عملية مسجلة. soft-delete فقط - لا حذف نهائى.' : 'Every operation logged. Soft-delete only.' },
    { icon: <Eye className="w-7 h-7" />, title: isAr ? 'قفل الفترات المحاسبية' : 'Accounting Period Locks', desc: isAr ? 'لا تعديل بعد القفل. IAS 8 prior-period adjustments.' : 'No edits after close. IAS 8 prior-period adjustments.' },
    { icon: <Shield className="w-7 h-7" />, title: isAr ? 'حماية ضد الـ Overdraft' : 'Overdraft Prevention', desc: isAr ? 'منع تلقائى للسحب من حساب فارغ.' : 'Auto-prevent withdrawals from empty accounts.' },
  ]
  return (
    <section id="security" className="py-24 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-900">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {pillars.map((p, i) => (
            <div key={i} className="group p-6 rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-red-500/50 hover:shadow-xl transition-all">
