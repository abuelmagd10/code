...
export function MultiCurrencySection({ appLang = 'ar' }: { appLang?: 'ar' | 'en' }) {
  ...
  const features = [
    { icon: <Globe2 className="w-6 h-6" />, title: isAr ? 'دعم كامل لتعدد العملات' : 'Full Multi-Currency Support', desc: isAr ? 'EGP, USD, EUR, GBP, SAR, AED + أى عملة' : 'EGP, USD, EUR, GBP, SAR, AED + any currency' },
    { icon: <FileCheck className="w-6 h-6" />, title: isAr ? 'متوافق مع IAS 21' : 'IAS 21 Compliant', desc: isAr ? 'تسجيل فروق العملة المحققة وغير المحققة' : 'Realized & unrealized FX recording' },
    { icon: <TrendingUp className="w-6 h-6" />, title: isAr ? 'فروق العملة الآلية' : 'Automated FX Gain/Loss', desc: isAr ? 'تسجيل تلقائى عند الدفع والاسترداد' : 'Auto-post on payments and refunds' },
    { icon: <RefreshCw className="w-6 h-6" />, title: isAr ? 'إعادة تقييم نهاية الفترة' : 'Period-end Revaluation', desc: isAr ? 'Cash + AR + AP، مع قيد عكسى تلقائى' : 'Cash + AR + AP, auto-reversal' },
    { icon: <Banknote className="w-6 h-6" />, title: isAr ? 'حسابات بنكية متعددة العملات' : 'Multi-Currency Bank Accounts', desc: isAr ? 'كل حساب يحتفظ بعملته الأصلية' : 'Each account keeps native + base' },
    { icon: <Coins className="w-6 h-6" />, title: isAr ? 'أسعار صرف Live أو Manual' : 'Live or Manual Exchange Rates', desc: isAr ? 'تحكم كامل لكل شركة' : 'Full per-company control' },
  ]
  return (
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-lg text-gray-600 dark:text-gray-300 mb-8 leading-relaxed">
              {isAr ? 'النظام الوحيد فى منطقتنا الذى يطبق المعايير المحاسبية الدولية (IAS 21) بشكل كامل من تسجيل فروق العملة عند كل معاملة إلى إعادة تقييم نهاية الفترة للذمم المدينة والدائنة والحسابات النقدية كله تلقائى وقابل للتدقيق.' : 'The only system implementing IAS 21 fully: from FX gain/loss recording at every transaction, to period-end revaluation for AR/AP/Cash — all automated and audit-ready.'}
            </p>
          ...
          <div className="relative">
            <div className="relative p-6 rounded-3xl bg-gradient-to-br from-gray-900 to-blue-950 shadow-2xl shadow-purple-500/30 border border-purple-500/20">
              ...
              <div className="p-4 rounded-lg bg-black/30 mb-4 border border-white/10">
                ...
                <div className="flex items-center justify-between mb-2"><span className="text-white text-sm">USD 1,000</span><span className="text-purple-300 text-xs">@ rate 30.00</span></div>
              ...
              <div className="p-4 rounded-lg bg-black/30 mb-4 border border-white/10">
                ...
                <div className="flex items-center justify-between mb-2"><span className="text-white text-sm">USD 1,000</span><span className="text-orange-300 text-xs">@ rate 31.50</span></div>
              ...
              <div className="p-4 rounded-lg bg-gradient-to-br from-purple-500/20 to-pink-500/20 border border-purple-400/50">
                <div className="text-xs font-bold text-purple-200 mb-2">{isAr ? 'قيد تلقائى مُنشأ' : 'Auto-generated entry'}</div>
                ...
                <div className="mt-2 pt-2 border-t border-white/10 text-[10px] text-gray-400">{isAr ? 'مرجع: fx_payment_adjustment · IAS 21 §28' : 'ref: fx_payment_adjustment · IAS 21 §28'}</div>
              </div>
            </div>
          </div>
        </div>
