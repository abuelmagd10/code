  ...
  const GroupAccordion = ({ group, q }: any) => {
    ...
    return (
      <div key={group.key} className="space-y-0.5 sm:space-y-1">
        <EnhancedTooltip functionName={group.key} side="left">
          <button
        ...
        {open && (
          <div className="space-y-0.5 sm:space-y-1">
            {group.items.filter((it: any) => filterAllowed(it.href)).map((it: any) => {
              ...
              const functionName = it.href.split('/').pop()?.split('?')[0] || ''
              
              return (
                <Link key={it.href} href={it.href} prefetch={false}>
                  <EnhancedTooltip functionName={functionName} side="left">
                    <button
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-slate-900 z-[9996] flex items-center justify-between px-4 shadow-lg">
        ...
        {/* زر القائمة مع تلميح */}
        <EnhancedTooltip content="فتح/إغلاق القائمة الجانبية" side="bottom">
          <button
        ...
        <div className="p-3 sm:p-4 md:p-6 pt-2 md:pt-0">
          ...
          <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-slate-700 space-y-3">
            ...
            <Button
              ...
              tooltip={(hydrated && appLanguage === 'en') ? 'Sign out from your account' : 'تسجيل الخروج من حسابك'}
            >
