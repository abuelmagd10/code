...
SELECT 
  ...
  END AS balance,
  CASE 
    WHEN coa.account_type = 'asset' AND 
         (COALESCE(coa.opening_balance, 0) +
          CASE 
            WHEN coa.account_type IN ('asset', 'expense') THEN COALESCE(movements.debit_net, 0)
            ELSE COALESCE(movements.credit_net, 0)
          END) < 0 
    THEN '⚠️ رصيد سالب لحساب أصل'
    WHEN coa.account_type = 'liability' AND 
         (COALESCE(coa.opening_balance, 0) +
          CASE 
            WHEN coa.account_type IN ('asset', 'expense') THEN COALESCE(movements.debit_net, 0)
            ELSE COALESCE(movements.credit_net, 0)
          END) < 0 
    THEN '⚠️ رصيد سالب لحساب التزام'
    ELSE '✅'
  END AS status
FROM chart_of_accounts coa
LEFT JOIN (
...
WHERE coa.account_type IN ('asset', 'liability')
  AND coa.is_active = true
  AND ABS(COALESCE(coa.opening_balance, 0) +
    CASE 
      WHEN coa.account_type IN ('asset', 'expense') THEN COALESCE(movements.debit_net, 0)
      ELSE COALESCE(movements.credit_net, 0)
    END) > 0.01
HAVING 
  (coa.account_type = 'asset' AND 
   (COALESCE(coa.opening_balance, 0) +
    CASE 
      WHEN coa.account_type IN ('asset', 'expense') THEN COALESCE(movements.debit_net, 0)
      ELSE COALESCE(movements.credit_net, 0)
    END) < 0) OR
  (coa.account_type = 'liability' AND 
   (COALESCE(coa.opening_balance, 0) +
    CASE 
      WHEN coa.account_type IN ('asset', 'expense') THEN COALESCE(movements.debit_net, 0)
      ELSE COALESCE(movements.credit_net, 0)
