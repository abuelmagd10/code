-- =====================================================
-- التحقق من فاتورة INV-0003 و COGS Transactions
-- =====================================================
...
-- =====================================================
SELECT 
  '1. معلومات الفاتورة INV-0003' as check_step,
  i.id,
  i.invoice_number,
  i.status,
  i.invoice_date,
...
-- =====================================================
-- 2. المنتجات في الفاتورة
-- =====================================================
SELECT 
  '2. المنتجات في الفاتورة' as check_step,
  ii.id as item_id,
  ii.product_id,
...
SELECT 
  ...
  flc.created_at
FROM invoices i
JOIN fifo_lot_consumptions flc ON flc.reference_id = i.id AND flc.reference_type = 'invoice'
...
SELECT 
  ...
  it.notes
FROM invoices i
JOIN inventory_transactions it ON it.reference_id = i.id AND it.reference_type = 'invoice'
...
WITH invoice_data AS (
  SELECT i.id, i.invoice_number, i.status, i.branch_id, i.cost_center_id, i.warehouse_id
  FROM invoices i
  WHERE i.invoice_number = 'INV-0003'
  LIMIT 1
)
SELECT 
  '6. ملخص شامل للفاتورة INV-0003' as summary,
  id.invoice_number,
  id.status,
  -- عدد المنتجات
