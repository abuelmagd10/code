...
/** قِيسَ يومَ v3.75.59 على الإنتاج. وبيتُ الاختبارِ يقولُ ٥٨٩ — والفرقُ دالّةٌ واحدةٌ لا تخصُّ هذا الحكم. */
const PINNED_INVOKER_UNSET = 588
...
;(async () => {
  ...
  console.log("  وبصلاحيّاتِ مُنادِيها بلا مسارِ بحث: " + data.invokerUnset +
    "   (المُثبَّت " + PINNED_INVOKER_UNSET + " — معدودةٌ لا مسكوتٌ عنها)")
  ...
  if (vInvoker === "grew") {
    problems.push(["زادَ من بصلاحيّاتِ مُنادِيها بلا مسارِ بحث: " + data.invokerUnset + " والمُثبَّتُ " + PINNED_INVOKER_UNSET, [
      "لا ترفعُ صلاحيّةً، لكنّها تقرأُ مزوَّراً وتكتبُ صادقاً — فلا تزيد.",
    ]])
  }
  if (vInvoker === "shrank") {
  ...
  console.log("  ! ومعدودٌ لا مسكوتٌ عنه — " + data.invokerUnset +
    " دالّةً بصلاحيّاتِ مُنادِيها بلا مسارِ بحث: لا ترفعُ صلاحيّةً (فالمُنادى بحقِّه هو)، لكنّها قد تقرأُ مزوَّراً وتكتبُ صادقاً — تُسدَّدُ بدفعتِها.")
  process.exit(0)
