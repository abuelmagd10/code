...
BEGIN
  ...
  -- 6. حذف بنود قيود إعادة التصنيف
  RAISE NOTICE '5️⃣ حذف بنود قيود إعادة التصنيف...';
  DELETE FROM journal_entry_lines
  WHERE journal_entry_id IN (
    SELECT id FROM journal_entries
    WHERE company_id = test_company_id
      AND reference_type IN ('supplier_payment_reclassification', 'supplier_payment_reclassification_reversal')
  );
  GET DIAGNOSTICS deleted_count = ROW_COUNT;
  RAISE NOTICE '   ✅ تم حذف % بند من قيود إعادة التصنيف', deleted_count;

  -- 7. حذف قيود إعادة التصنيف
  RAISE NOTICE '6️⃣ حذف قiود إعادة التصنيف...';
  DELETE FROM journal_entries
  WHERE company_id = test_company_id
    AND reference_type IN ('supplier_payment_reclassification', 'supplier_payment_reclassification_reversal');
  GET DIAGNOSTICS deleted_count = ROW_COUNT;
  RAISE NOTICE '   ✅ تم حذف % قيد إعادة تصنيف', deleted_count;

  -- 8. إعادة تفعيل Trigger
  RAISE NOTICE '7️⃣ إعادة تفعيل Trigger...';
