-- =====================================================
-- التحقق من حالة COGS للفاتورة INV-0006
-- =====================================================
...
WHERE i.invoice_number = 'INV-0006';
...
WHERE p.reference_type = 'invoice' 
  AND p.reference_id = (SELECT id FROM invoices WHERE invoice_number = 'INV-0006')
...
-- =====================================================
SELECT 
  '7. ملخص شامل للفاتورة INV-0006' as summary,
  i.invoice_number,
  i.status,
  ...
  CASE 
    WHEN i.status IN ('paid', 'partially_paid') 
      ...
      AND (SELECT COUNT(*) FROM third_party_inventory WHERE invoice_id = i.id) > 0
    THEN '❌ Third-party items موجودة لكن لا يوجد COGS - يجب استدعاء clearThirdPartyInventory()'
    WHEN i.status IN ('paid', 'partially_paid') 
      AND (SELECT COUNT(*) FROM cogs_transactions WHERE source_id = i.id AND source_type = 'invoice') = 0 
