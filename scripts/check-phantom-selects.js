...
// ٨٤٦: ٣١ منها أُصلحت (أخطاء تسمية، حُلَّت بالاسم المستعار) ⇒ ١٣.
// ٨٤٧: قراءتا `payroll_runs.status` ⇒ ١١. ولم يكن الحل إنشاء العمود: دفتر
//      اليومية هو سجلّ أن المرتب صُرف، و`post_payroll_atomic` يمنع الازدواج
//      به بالفعل — فحُذف الفحص المسبق ولم يُبنَ مصدر حقيقة ثانٍ.
// ٨٤٨: `chart_of_accounts.balance/currency_code` ⇒ ٩. ولا هنا أيضاً أُنشئ
//      عمود: **الرصيد لا يُخزَّن، يُشتَقّ** — افتتاحى + (مدين − دائن) على
//      القيود المرحَّلة، وهى القاعدة المعتمدة فى المشروع أصلاً.
// ٨٤٩: الأخيرة ⇒ **صفر**. وكل واحدة عولجت بما تستحقه، لا بحلٍّ واحد:
//      · اشتراك المستخدمين — نسختان **ميتتان** من ميزة بُنيت لاحقاً بشكل
//        صحيح (`subscription_plans`/`subscriptions`/`billing_invoices`).
//        أُزيلتا بعد التأكد أن لا مستورد ولا مستدعى لهما.
//      · ربط العمولات بالمرتبات — ميزة **حيّة موصولة بالواجهة** بزر فى شاشة
//        المرتبات. الحذف كان سيمحو ميزة تعمل؛ فأُنشئ العمود الناقص، وهو
//        نفسه الحارس الذى يمنع إضافة العمولة للمرتب مرتين.
//      · سكربت الاختبار — كان يفحص الجدول الخطأ بمنطق **لا يمكن أن يفشل**.
//
// ⇒ خط الأساس صفر. أى قراءة جديدة لعمود غير موجود تكسر البناء فوراً.
const BASELINE = Number(process.env.PHANTOM_SELECT_BASELINE ?? 0)
