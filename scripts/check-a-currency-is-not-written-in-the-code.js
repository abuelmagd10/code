 ...
 */
/**
 * v3.75.73 — **وثلاثةُ مواضعَ من الخِيارِ المباشر: الارتدادُ عندَ الفشلِ فى
 * إعادةِ تقييمِ العملة، لا فى القراءةِ الأولى.**
 *
 * تحقيقُ الدَّينِ الأوسعِ المؤجَّلِ من v3.75.71/72 وجدَ فى
 * `lib/fx-revaluation.ts` (الدوالُّ الثلاث: computeFXRevaluation،
 * computeARRevaluation، computeAPRevaluation) أنَّ كلَّ دالّةٍ تنادى
 * `getBaseCurrency()` بيتَ القاعدةِ الواحدِ فى مسارِها الأساسى — لكنَّ كتلةَ
 * `catch` عندَ أىِّ فشلٍ لاحقٍ كانت تُرجعُ `baseCurrency: "EGP"` حرفاً
 * مخترَعاً، بدلَ القيمةِ الحقيقيّةِ التى رُبَّما نجحت قراءتُها قبلَ الفشل، أو
 * فراغٍ صادقٍ إن فشلت القراءةُ نفسُها. صار المتغيِّرُ يُعلَنُ خارجَ `try` فيبقى
 * فى متناوَلِ `catch` بقيمتِه الحقيقيّةِ أو فارغاً — لا اختراع.
 *
 * ومُركَّبُ العطبِ الفعلىُّ ظهرَ عندَ التتبُّعِ حتى الشاشةِ المستهلِكة:
 * `computeFullFXRevaluation` كانت تدمجُ نتائجَ الدوالِّ الثلاثِ بـ
 * `a.baseCurrency || b.baseCurrency || c.baseCurrency` (حكمٌ بـ"له قيمة" لا
 * بـ"نجحَ فعلاً")، فقد تفوزُ عملةٌ مخترَعةٌ من فرعٍ فاشلٍ على عملةٍ حقيقيّةٍ من
 * فرعٍ ناجح؛ وشاشةُ `app/settings/exchange-rates/page.tsx` لم تكن تتحقّقُ من
 * `result.success` إطلاقاً، فتُظهِرُ "لا حاجة لإعادة التقييم" المُطمئِنةَ
 * خطأً عندَ الفشلِ الكاملِ بدلَ كشفِ الخطأ. صار الدمجُ يُفضِّلُ أوّلَ فرعٍ نجح
 * فعلاً، والشاشةُ تعرضُ خطأً صريحاً عندَ الفشلِ (كاملاً أو جزئياً) بدلَ
 * الصمت.
 *
 *   ٣٤  المُثبَّتُ قبلَ الدفعة
 *   −٣  «قيمةٌ تُكتَبُ أو تُرسَل»: lib/fx-revaluation.ts:159،415،513
 *   ───
 *   ٣١  المقيسُ يومَ الشحن
 */
const PINNED = 31
/**
 ...
 */
/**
 * v3.75.73 — **والارتدادُ المزدوَج: عملةٌ محكومةٌ بالفعل، ثم تُختَرَعُ فوقَها
 * حرفاً — وموضعٌ واحدٌ مشتركٌ يُخطئُ لكلِّ شاشةٍ تستدعيه دفعةً واحدة.**
 *
 * تحقيقُ الدَّينِ الأوسعِ المؤجَّلِ من v3.75.71/72 كشفَ نمطاً ثالثاً لم يكن
 * معروفاً من قبل: متغيِّرٌ مصدرُه فعلاً بيتٌ محكومٌ (`readAppCurrency()` أو
 * `getBaseCurrency()`) — لكنَّ الشاشةَ تُعيدُ اختراعَ `'EGP'` فوقَه بعاملِ
 * `||`، كأنَّ البيتَ لم يُنادَ أصلاً. أخطرُها:
 *
 *   - `components/ExchangeRateSelector.tsx` — **المكوَّنُ المشتركُ** الذى
 *     تستدعيه كلُّ شاشاتِ الشركةِ لاختيارِ سعرِ الصرف. عملةُ القاعدةِ وسيطٌ
 *     مُلزَمٌ (prop) تُغذّيه الشاشاتُ من مصدرٍ محكوم، وكان ارتدادُها
 *     الداخلىُّ إلى "EGP" يقرِّرُ isSameCurrency وأىَّ صفٍّ من
 *     exchange_rates يُطلَبُ من القاعدة (from_currency/to_currency) — أى:
 *     **هذا المكوَّنُ نفسُه كان يطلبُ الصفَّ الخطأَ من القاعدةِ لكلِّ شركةٍ
 *     قاعدتُها ليست الجنيه**، فى كلِّ شاشةٍ تستدعيه. صار الغيابُ حالةً
 *     صريحةً (رسالةُ "لم تُحدَّد العملة" بدلَ حسابٍ خاطئ)، لا عملةً مخترَعة.
 *   - `components/customers/customer-refund-dialog.tsx` (موضعان) — يقرِّرانِ
 *     accountNativeAmount المُرسَلَ فعلاً لتُرحَّلَ به قيودُ صرفِ رصيدِ
 *     العميل — رقمٌ محاسبىٌّ، لا عرضَ شاشة.
 *   - `app/banking/page.tsx`، `app/invoices/[id]/page.tsx`،
 *     `app/payments/page.tsx` (٣ مواضعَ)، و
 *     `components/payments/PaymentDetailsModal.tsx` (موضعان) — نفسُ النمطِ
 *     فى مقارناتِ عملةِ الحسابِ واختيارِه، وفى نافذةِ تفاصيلِ الدفعِ (نافذةُ
 *     تحميلٍ عابرةٌ فقط، عرضٌ لا كتابة).
 *
 * وثلاثةُ مسارَاتٍ للفوترةِ (app/api/billing/renew وseats وseats/renew)
 * كانت تقرأُ company.base_currency مباشرةً بلا نداءِ getBaseCurrency()
 * إطلاقاً، لتُغذِّىَ حسابَ سعرٍ حقيقىٍّ يُدفَعُ فعلاً عبر Paymob — صارت تنادى
 * getBaseCurrency() أوّلاً بنفسِ عقيدةِ app/api/billing/preview/route.ts
 * (v3.75.71)؛ والارتدادُ الأخيرُ إلى "USD" **أُبقِىَ عمداً** كحدٍّ أدنى عندَ
 * فشلِ القراءةِ فقط (نفسُ المعنى المقبولِ سلفاً: عملةُ عرضٍ لتسعيرِ منصّة، لا
 * رقمٌ محاسبىٌّ مخترَع) — فلا يُغيِّرُ هذا العددَ المُثبَّتَ هنا، تماماً كما
 * لم يُغيِّرْه إصلاحُ preview/route.ts من قبل.
 *
 * والقياسُ موثَّقٌ لا مُدَّعى: قُورِنَت لائحتا المواضعِ الكاملتانِ (١٦٧ موضعاً
 * قبلَ الدفعة) قبلَ التعديلِ وبعدَه سطراً سطراً، فتأكَّدَ أنّ النقصانَ كلَّه
 * واقعٌ فى الأحدَ عشرَ موضعاً المذكورةِ أعلاه، وأنّ ما بقىَ فى
 * app/payments/page.tsx:4344 (correctionPayment.original_currency ||
 * correctionPayment.currency_code || 'EGP') عملةُ دفعةٍ فرديّةٍ بلا
 * baseCurrency فى سلسلتِه أصلاً — نمطٌ مختلفٌ، فلم يُمَسّ.
 *
 *   ١٣٣  المُثبَّتُ قبلَ الدفعة
 *   −١١  «ارتدادٌ عن قيمةٍ صامتة»: ExchangeRateSelector.tsx، customer-refund-
 *        dialog.tsx (موضعان)، banking/page.tsx، invoices/[id]/page.tsx،
 *        payments/page.tsx (٣ مواضع)، PaymentDetailsModal.tsx (موضعان)
 *   ───
 *   ١٢٢  المقيسُ يومَ الشحن
 *
 * والباقى من الدَّينِ الأوسعِ (١٢٢ موضعاً) عملاتُ مستنداتٍ فرديّةٍ أو ثوابتُ
 * منصّةٍ أو بذورُ حالةٍ فى غالبِها — مؤجَّلٌ عمداً لدفعاتٍ لاحقةٍ تُفرِزُه،
 * **ومعدودٌ لا مسكوتٌ عنه**. ودالّتا getAppCurrency() المهجورتانِ فى
 * lib/currency-utils.ts وlib/currency-converter.ts (صفرُ مستدعٍ حىٍّ لكلٍّ
 * منهما، تحقَّقَ قياسٌ شاملٌ للمستودع) تحملانِ نفسَ نمطِ الارتدادِ المزدوَجِ
 * هذا داخلَهما — مُسمّاتان مؤجَّلتانِ عمداً لدفعةٍ مخصَّصةٍ تعيدُ بناءَ فخِّ
 * هذا الحارسِ الذاتىِّ لحالةِ صفرِ بيوتٍ مُفوَّضة، لا لتُنسَى.
 */
const PINNED_IMPLIED = 122
