// =====================================================
// تنفيذ مباشر لتنظيف القيود العكسية
// Direct Execution of Cleanup Reversal Entries
// =====================================================
...
async function main() {
  ...
  try {
    ...
    console.log('   ⏳ انتظر 5 ثوانٍ ثم اضغط Enter للمتابعة...')
    ...
    for (const entry of allEntriesToDelete) {
      try {
        const entryType = entry.reference_type === 'bill_payment_reversal' 
          ...
          ? 'قيد سداد'
          : 'قيد إعادة تصنيف'
        console.log(`   🔧 حذف ${entryType} ${entry.id}...`)
        ...
        if (delEntryErr) {
          console.log(`      ❌ فشل حذف القيد: ${delEntryErr.message}`)
          errorCount++
        } else {
