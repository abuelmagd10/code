-- =============================================================================
-- تطبيق migrations ربط شركات الشحن بالفروع (20260301)
-- يشغّل بالترتيب: 20260301_001 ثم 20260301_002
-- الاستخدام: انسخ هذا الملف كاملاً إلى Supabase Dashboard → SQL Editor → Run
-- =============================================================================

-- ========== 20260301_001_branch_shipping_providers ==========
...
BEGIN
  ...
  SELECT EXISTS (SELECT 1 FROM public.branch_shipping_providers bsp JOIN public.branches b ON b.id = bsp.branch_id WHERE b.company_id = p_company_id) INTO v_has_any_mapping;
  IF NOT v_has_any_mapping THEN RETURN TRUE; END IF;
  RETURN EXISTS (
    SELECT 1 FROM public.branch_shipping_providers bsp JOIN public.branches b ON b.id = bsp.branch_id
    WHERE bsp.branch_id = p_branch_id AND bsp.shipping_provider_id = p_shipping_provider_id AND b.company_id = p_company_id AND (bsp.is_active IS NULL OR bsp.is_active = TRUE)
  );
END;
$$;
...
CREATE POLICY "branch_shipping_providers_select" ON public.branch_shipping_providers FOR SELECT
  USING (branch_id IN (SELECT b.id FROM public.branches b WHERE b.company_id IN (SELECT company_id FROM public.company_members WHERE user_id = auth.uid())));

DROP POLICY IF EXISTS "branch_shipping_providers_insert" ON public.branch_shipping_providers;
CREATE POLICY "branch_shipping_providers_insert" ON public.branch_shipping_providers FOR INSERT
  WITH CHECK (branch_id IN (SELECT b.id FROM public.branches b JOIN public.company_members cm ON cm.company_id = b.company_id AND cm.user_id = auth.uid() WHERE b.company_id = cm.company_id AND cm.role IN ('owner', 'admin', 'general_manager', 'gm', 'super_admin', 'superadmin', 'generalmanager')));

DROP POLICY IF EXISTS "branch_shipping_providers_update" ON public.branch_shipping_providers;
CREATE POLICY "branch_shipping_providers_update" ON public.branch_shipping_providers FOR UPDATE
  USING (branch_id IN (SELECT b.id FROM public.branches b JOIN public.company_members cm ON cm.company_id = b.company_id AND cm.user_id = auth.uid() WHERE cm.role IN ('owner', 'admin', 'general_manager', 'gm', 'super_admin', 'superadmin', 'generalmanager')));

DROP POLICY IF EXISTS "branch_shipping_providers_delete" ON public.branch_shipping_providers;
CREATE POLICY "branch_shipping_providers_delete" ON public.branch_shipping_providers FOR DELETE
  USING (branch_id IN (SELECT b.id FROM public.branches b JOIN public.company_members cm ON cm.company_id = b.company_id AND cm.user_id = auth.uid() WHERE cm.role IN ('owner', 'admin', 'general_manager', 'gm', 'super_admin', 'superadmin', 'generalmanager')));

-- 4) Trigger على invoices, sales_orders, bills
...
BEGIN
  ...
  IF NEW.branch_id IS NULL THEN RAISE EXCEPTION 'SHIPPING_PROVIDER_BRANCH: branch_id مطلوب عند تحديد شركة الشحن'; END IF;
...
CREATE TRIGGER trg_check_invoice_shipping_provider_branch BEFORE INSERT OR UPDATE OF shipping_provider_id, branch_id ON public.invoices FOR EACH ROW EXECUTE FUNCTION public.check_shipping_provider_branch();

DROP TRIGGER IF EXISTS trg_check_sales_order_shipping_provider_branch ON public.sales_orders;
CREATE TRIGGER trg_check_sales_order_shipping_provider_branch BEFORE INSERT OR UPDATE OF shipping_provider_id, branch_id ON public.sales_orders FOR EACH ROW EXECUTE FUNCTION public.check_shipping_provider_branch();
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'bills' AND column_name = 'shipping_provider_id') THEN
    ...
    CREATE TRIGGER trg_check_bill_shipping_provider_branch BEFORE INSERT OR UPDATE OF shipping_provider_id, branch_id ON public.bills FOR EACH ROW EXECUTE FUNCTION public.check_shipping_provider_branch();
  END IF;
END $$;

-- ========== 20260301_002_branch_shipping_providers_integrity ==========
...
CREATE TRIGGER trg_branch_shipping_provider_same_company
  BEFORE INSERT OR UPDATE OF branch_id, shipping_provider_id ON public.branch_shipping_providers
