...
//
// v3.74.864 — ٢٦٩ ← ٢٦٠. صُنِّفت الـ٢٦٩ بالأثر لا بالعدد، فتبيّن أن ٦٣ منها
// تمسّ المال والدفاتر، و٧١ تسجيلٌ «أطلِق وانسَ» مقصود. وأُغلقت أخطر تسعة:
// كل ما يخصّ **المدفوعات**.
//
// وأخطرها اثنان أذهلانى: `pre-receipt-refund` و`pre-shipment-refund` يحملان
// تعليقاً من v3.74.252 نصّه «لا يُرحَّل القيد إلا بعد نجاح كل ما يعتمد عليه» —
// **والشرط نفسه لم يكن يُفحص قط**. فلو فشل توسيم الدفعة بالإلغاء صامتاً،
// رُحِّل القيد على أى حال: استردادٌ مُقيَّد فى الدفاتر ودفعةٌ أصلية تبدو سارية.
//
// ⇒ **تعليقٌ يصف ضماناً لا يعنى أن الضمان قائم.** والفرق بينهما لا يظهر إلا
//   حين يُقرأ السطر التالى.
//
// والتمييز المطبَّق فى هذا الإصدار:
//   • مسارٌ يمضى للأمام  ⇒ يُرفع الخطأ (نجاحٌ متناقض أسوأ من فشلٍ معلن).
//   • مسارُ تراجُعٍ بعد فشل ⇒ يُسجَّل بمعرّفاته ولا يُرفع، كى لا يُخفى الخطأ
//     الأصلى — لكنه **لا يصمت**.
//
// v3.74.865 — ٢٦٠ ← ٢٤٧. ولم يكن هذا هدفَ الإصدار: كنتُ أُصلح **أعمدة وهمية**
// فى عمليات الإضافة، فوجدتُ أن كل موضعٍ منها يجمع عطبين لا واحداً — اسمَ عمودٍ
// خاطئ، **ونتيجةً مُهملة**. فلمّا صُحّح الاسم وفُحصت النتيجة، سقط ١٣ موضعاً
// من هذا العدّاد تبعاً.
//
// ⇒ **العطب الصامت نادراً ما يأتى وحده.** فالكاتب الذى لم يتحقق من اسم العمود
//   هو نفسه الذى لم يتحقق من نتيجة الكتابة؛ والسبب واحد: لم يكن ثمّة ما يُجبره.
//
// وستةٌ من الثلاثة عشر كانت `try/catch` حول استدعاء supabase-js — وهو الشكل
// الذى يحذّر منه رأس هذا الملف نفسه (سطر ١٧٤): العميل **يُرجع** `{ error }`
// ولا يرفع استثناءً، فلا الـ`catch` يعمل ولا النتيجة تُقرأ. صمتٌ مزدوج.
//
// v3.74.868 — ٢٤٧ ← ٢٢٧. أُغلقت الكتابات الماسّة بالمال والدفاتر، **بعد
// قياس أيُّها موصولٌ أصلاً**. فمن الـ٣٣ موضعاً التى تكتب فى جداول الدفاتر
// تبيّن أن ملفَّين اثنين فقط بلا مستدعٍ (`sales-returns.ts`،
// `supplier-balance.ts`)، والباقى **حىٌّ كلّه**.
//
// ⚠️ وهنا تصحيحٌ لخطأ منّى: قلتُ فى ٨٦٥ إن `purchase-returns-vendor-credits`
// «بلا مستدعٍ». وكان بحثى مقصوراً على `--include=*.ts` ومستدعياه صفحتان
// `.tsx`. ⇒ **بحثُ وصولٍ يستثنى `.tsx` يَصِف الحىَّ ميتاً** — وهو خطأٌ
// يُهوِّن من أثر العطب، وذلك أسوأ اتجاهٍ يخطئ فيه المرء.
//
// والتمييز المطبَّق (درس ٨٦٤) بحاله: مسارٌ يمضى للأمام ⇒ يُرفع الخطأ ·
// مسارُ تراجُعٍ ⇒ يُسجَّل بمعرّفاته ولا يُرفع.
//
// v3.74.870 — ٢٢٧ ← ٢٢٦. لم تُصلَح كتابة، بل **حُذفت وحدتان ميتتان** تحملان
// كتابةً غير مفحوصة (نسخة مكرّرة من خدمة تعديل الفاتورة، ووحدة حوكمةٍ
// مكتوبة بأسلوب Express لا يعمل فى Next.js أصلاً).
// ⇒ الدَّين يقلّ بالحذف كما يقلّ بالإصلاح، والعدّاد لا يفرّق — وهذا صواب:
//   ما زال أحدهما موضعاً كان يمكن أن يُوصَل ويُطلَق صامتاً.
//
// v3.74.871 — ٢٢٦ ← ٢٢٤. `supplier-balance.ts` كان يحمل كتابتين بلا فحص:
// إدراج إشعار الدائن، وتحديثُ مبلغِ إشعارٍ قائم. وأُلغى التحديث أصلاً — فقيدُ
// الإشعار يُنشأ عند الإدراج ولا يتبع التعديل، فكتابة مبلغٍ جديد تجعل المستند
// يقول رقماً والدفاتر تقول غيره.
//
// v3.74.874 — ٢٢٤ ← ٢١١. أُغلقت **كل** الكتابات الماسّة بالمال فى مسارات
// الأعمال الحيّة. والترتيب كان بالقياس: من ٢٢٤ موضعاً، ٢١ يمسّ المال،
// و١٠ منها فى مسارٍ يستعمله الناس اليوم — والباقى فى أدوات إصلاحٍ لا
// تستدعيها صفحة، أو فى دالةٍ ميتة.
//
// وأخطرها موضعان يحذفان **مستنداً مالياً من صفحة**: أمر البيع/الشراء كان
// يحذف فاتورته المسودَّة بلا فحص ثم يحذف نفسه. فلو فشل حذف الفاتورة صامتاً
// حُذف الأمر وبقيت **فاتورةٌ يتيمة** محسوبةٌ فى الأرصدة والتقارير.
//
// v3.74.875 — ٢١١ ← ٢٠١. **بتقاعد ثلاث أدواتِ إصلاح، لا بترقيعها** — وهو
// نفس ما فعله ٧٧٣ بثلاثٍ غيرها، وللسبب نفسه: أداةٌ تُصلح بالكتابة المباشرة
// بُنيت قبل أن تُحكَم القاعدة، فصارت تصطدم بحمايتها **ثم تصمت وتُعلن النجاح**.
//
// وأخطرها كان موصولاً بصفحةٍ إدارية: «إصلاح الدفعات المتبقية» تعتبر كل دفعةٍ
// سالبة بياناتٍ فاسدة فتحذفها حذفاً نهائياً. وفى الإنتاج دفعتان سالبتان،
// **كلتاهما سليمة** ولها قيدٌ محاسبى: إلغاء دفعةٍ باسترداد قبل الشحن،
// وتصحيحُ دفعةِ مورد. ⇒ **السالب لم يعد فساداً؛ صار هو طريقة تسجيل العكس.**
//
// v3.74.876 — ٢٠١ ← ١٩٦. أُزيل نصف `lib/sales-returns.ts`: مسارُ مرتجعٍ
// **لا يصله أحد**، ومعه خمس دوالٍ خاصةٍ به. وكان لغماً لا كوداً خاملاً:
// `processReturnAccounting` تُنشئ قيداً بلا `branch_id` وهو NOT NULL، ثم
// **لا تفحص النتيجة** — فتفشل حتماً وتُخطّى الدفاتر بصمت وتُعيد نجاحاً.
//
// والمسار الحىّ (`prepareSalesReturnData`) باقٍ كما هو: مرتجعات المبيعات
// تمرّ من صندوق الموافقات عبر `accounting-transaction-service`.
//
// v3.74.877 — ١٩٦ ← ١٦٥. **٣١ موضعاً بدالتين**، لا بإحدى وثلاثين رقعة.
//
// أثرُ العملية المالية كان يُكتب فى ثلاثة عشر خدمةً بشكلين متكرّرين، وكلاهما
// يُهمل نتيجته: `upsert` للربط الأمامى، وزوجُ `delete` لتنظيف التراجُع.
// فأُنشئت `lib/services/financial-trace.ts` بدالتين — تماماً كما أُنشئت
// `rollback-journal-entry.ts` فى v3.74.756 فأنهت صمت ستة مواضع دفعةً واحدة.
//
// ⇒ **الشكل المتكرّر يُعالَج بدالةٍ لا بنسخٍ متكرّر للعلاج.** والفارق ليس
//   الاختصار: رقعةٌ فى واحدٍ وثلاثين موضعاً تعنى واحداً وثلاثين فرصةً
//   للسهو، ودالةٌ واحدة تعنى موضعاً واحداً يُراجَع.
//
// والقاعدة محفوظة: `linkTraceEntity` مسارٌ أمامى ⇒ **يرفع** · `purgeTrace`
// مسارُ تراجُع ⇒ **يُسجَّل ولا يرفع** كى لا يُخفى الخطأ الأصلى.
//
// v3.74.878 — ١٦٥ ← ١١٣. **٥٢ موضع تسجيلٍ بدالةٍ واحدة** — ثالث مرّة يُطبَّق
// فيها نفس المنهج: `rollback-journal-entry` (٧٥٦) ثم `financial-trace` (٨٧٧)
// ثم `audit-log-write` هنا.
//
// وتسعةٌ وثلاثون من الخمسين كانت داخل `try/catch` — **وهو أسوأ من غياب
// المعالجة، لأنه يبدو معالجةً وليس كذلك**: supabase-js يُرجع `{ error }`
// ولا يرفع، فلا الـ`catch` يعمل ولا النتيجة تُقرأ. صمتٌ مزدوج.
//
// ⇒ وهذا الشكل ظهر فى **ستة إصداراتٍ متتالية اليوم**، وهو أكثر عطبٍ تكراراً
//   فى المشروع. والدالة لا ترفع: **فشلُ التسجيل لا يُبطل عملاً نجح** — لكنه
//   لا يصمت.
// v3.74.884: ١١٣ ⇒ ١١١ — إغلاق موضعين من مواضع check-impossible-rollback
// أزال معهما كتابتين غير مفحوصتين: الحذف التعويضى لأمر البيع فى
// POST /api/invoices، وحذف حساب الشريك من صفحة المساهمين (الذى كان
// يُرفض بصمتٍ منذ 815 — عيّنة حيّة من هذا الدَّين بالضبط).
//
// v3.74.885: ١١١ ⇒ ٩٣ — عائلة «مزامنة المستندات المرتبطة» كاملة (١٨ موضعاً
// فى ٩ شاشات حية): حذف بنودٍ ثم إعادة إدراجها بلا فحص فى شاشات تعديل
// أوامر البيع/الشراء وفواتير الموردين، ونسخ بنود الفاتورة عند التحويل،
// ووسم «تم التحويل» على الأمر/العرض، وتحديث حالة أمر الشراء بعد الدفعة.
// الفشل الصامت هنا يخرج للعميل مستنداً بلا بنود أو ببنودٍ مكررة أو
// بمستندَين متخالفَين — فصار كل فشلٍ يُقال بالاسم وبما يفعله المستخدم.
//
// v3.74.886: ٩٣ ⇒ ٧٦ — عائلة «الدعوات والعضوية والانضمام» (١٧ موضعاً فى
// ١٠ ملفات): عضوية المالك فى onboarding وsettings كانت داخل try/catch
// **وهمى** (supabase-js لا يرمى) فالفشل = مستخدم عالق بلا عضوية يستمر
// مساره كأن شيئاً لم يكن — صار يُوقِف برسالة مرئية. ووسم الدعوة
// «مقبولة» فى خمسة مسارات، وثلاثية تبديل عملة الأساس (فشلها الصامت =
// شركة بعملتَى أساس)، وتنظيفات الانضمام — كلها تُفحص وتُسجَّل بصوت.
// v3.74.887: ٧٦ ⇒ ٦٤ — عائلة الشحن (١٢ موضعاً): أخطرها تحديث «created»
// بعد نجاح الإنشاء لدى المزود — فشله الصامت يترك السجل pending فيُعاد
// الإنشاء بشحنةٍ (ورسومٍ) مكررة؛ وحفظ حالة الـwebhook — المزود يرسلها
// مرة واحدة وفشلها الصامت يجمّد التتبع أمام العميل (الآن يُرَدّ بغير 200
// ليعيد المزود المحاولة). والسجلات الزمنية غير حاسمة: تُفحص وتُسجَّل.
// v3.74.888: ٦٤ ⇒ ٤٨ — حالات الاسترداد والموافقات (١٦ موضعاً): الشكل
// الواحد الأخطر «المال تحرّك والحالة بقيت pending» ⇒ تنفيذ مزدوج.
// استرداد العميل والمورد يقولان الآن صراحةً عند فشل الوسم: «نُفِّذ فعلاً
// — لا تُعِد التنفيذ». وتكاليف الإعدام تُفحص قبل القيد (FIFO استُهلك)،
// وعكس المساهمة يمنع عكساً ثانياً، وحالات التصنيع لا تتناقض بصمت،
// وإشعارات نقل الصلاحيات خرجت من try/catch الوهمى إلى فحصٍ مسموع.
// v3.74.889: ٤٨ ⇒ ٢٨ — المتفرقات الأمامية (٢٠ موضعاً): أخطرها أمنياً
// سحب وصول الفروع (فشله الصامت يُبقى صلاحيةً كان يجب سحبها — ثلاثة
// مواضع)، ومالياً مبالغ البونص فى كشوف المرتبات وتراجُعات جداول
// الإهلاك (ROLLBACK_INCOMPLETE يُسمّى جداوله). واكتمل بها `app/`
// بالكامل: كل ما تبقى (٢٨) فى lib/ الخلفية.
// v3.74.890: ٢٨ ⇒ **٠** — أُغلق الدَّين كله. الدفعة الأخيرة (الخلفية):
// إلغاءات ما بعد الاسترداد (نمط 888 فى lib/)، أوسمة المعالجة التى فشلها
// الصامت = معالجة مكررة (event-bus، jobs-queue، الحضور — بصمة تُحسب
// مرتين)، حالة الاشتراك نفسها (شركة متعثرة كاملة الصلاحيات أو عميل دفع
// وبقى محجوباً — تُرمى)، لقطة السعر الأصلى قبل التحويل (درس 874: لقطة
// لا تُعاد تُرمى لا تُسجَّل)، تراجُعات ROLLBACK_INCOMPLETE تُسمّى
// ضحاياها، وحتى المُسجِّل نفسه يشكو للكونسول إن سقط.
// من ٢١٣ عند ولادة هذا الحارس إلى صفر. **كل كتابة فى المشروع تُقرأ
// نتيجتها** — وأى كتابة جديدة مهملة تكسر البناء فوراً.
const BASELINE = 0;
