...
foreach ($old in @("push_v3.74.799.ps1","push_v3.74.800.ps1","push_v3.74.801.ps1","push_v3.74.802.ps1")) {
    if (Test-Path $old) { Remove-Item -LiteralPath $old -Force }
}

$v = Get-Content -LiteralPath "lib/version.ts" -Raw
if ($v -match 'APP_VERSION = "3.74.803"') {
    Write-Host "+ 3.74.803" -ForegroundColor Green
} else { Write-Host "X version mismatch" -ForegroundColor Red; exit 1 }

if (Test-Path ".githooks/pre-push") { git config core.hooksPath .githooks 2>&1 | Out-Null }

$cl = Get-Content -LiteralPath "CHANGELOG.md" -Raw
if ($cl -notmatch [regex]::Escape("[3.74.803]")) {
    Write-Host "X CHANGELOG needs a heading containing exactly [3.74.803]" -ForegroundColor Red; exit 1
}
...
# --- 803: the custody principle, complete ---------------------------------------
$mig3 = Get-Content -LiteralPath "supabase/migrations/20260723000008_v3_74_803_custody_principle_complete.sql" -Raw
foreach ($must in @(
    "JOIN public.booking_bundle_selections bbs ON bbs.booking_id = bkg.id",
    "تم إلغاء تحديد الصنف تلقائياً",
    "DELETE FROM public.booking_bundle_selections"
)) {
    if ($mig3 -notmatch [regex]::Escape($must)) {
        Write-Host "X custody-principle migration incomplete: $must" -ForegroundColor Red
        exit 1
    }
}
Write-Host "+ 803: selected optionals gate too; rejection auto-deselects" -ForegroundColor Green
...
git add -- "lib/version.ts" "CHANGELOG.md" `
    ...
    "supabase/migrations/20260723000008_v3_74_803_custody_principle_complete.sql" `
    "push_v3.74.803.ps1" 2>&1 | Out-Null
git add -u -- "push_v3.74.799.ps1" "push_v3.74.800.ps1" "push_v3.74.801.ps1" "push_v3.74.802.ps1" 2>$null
...
if (-not $staged) {
    ...
    $msgPath = Join-Path $env:TEMP "commit_v3_74_803.txt"
    $msgLines = @(
        ...
        '',
        '803 - the owner''s question exposed the trap: with the optional item',
        'REJECTED, completion would still consume it (selection survived',
        'rejection - the system merely advised the employee to deselect);',
        'and a selected optional with NO withdrawal at all consumed stock',
        'with no custodian involvement. Owner decisions: every SELECTED item',
        '(mandatory or optional) needs an approved withdrawal before',
        'execution; rejecting an optional auto-deselects it (automating the',
        'system''s own instruction), guarded to the pre-execution window.',
        'Rehearsed: selected optional w/o withdrawal blocks the gate naming',
        'it; rejection deletes the selection, notifies, and the gate opens.'
    )
