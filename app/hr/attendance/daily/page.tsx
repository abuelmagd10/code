...
import { Plus, Filter, RefreshCw, Calendar as CalendarIcon, Download } from "lucide-react"
...
export default function AttendancePage() {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Date', 'Employee', 'Status', 'Check In', 'Check Out', 'Late (min)', 'Overtime (min)']
    const csvData = attendanceRecords.map(row => {
      const empName = employees.find(e => e.id === row.employee_id)?.full_name || row.employees?.full_name || ''
      return [
        row.day_date,
        empName,
        row.status,
    ...
    link.setAttribute("download", `attendance_export_${filterStartDate}_to_${filterEndDate}.csv`)
