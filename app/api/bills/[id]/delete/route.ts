...
export async function POST(
  ...
  try {
    ...
    const canDelete = await canAction(authSupabase, "bills", "delete")
    if (!canDelete) {
      return NextResponse.json({ success: false, error: "You do not have permission to delete purchase bills" }, { status: 403 })
    }

    const idempotencyKey = resolveFinancialIdempotencyKey(
      request.headers.get("Idempotency-Key"),
      ["bill-delete", context.companyId, id]
    )

    const requestHash = buildFinancialRequestHash({
      operation: "delete_draft_bill",
      billId: id,
    ...
    const result = await service.deleteDraftBill(
      {
        companyId: context.companyId,
        actorId: context.user.id,
    ...
    const message = String(error?.message || "Unexpected error while deleting purchase bill")
