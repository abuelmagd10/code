...
export async function POST(req: NextRequest) {
  try {
    ...
    try { await admin.from('audit_logs').insert({ action: 'INSERT', target_table: 'journal_entries', company_id: companyId, user_id: user.id, record_id: entry?.id, new_data: { year, month, total, entry_id: entry?.id } }) } catch {}
