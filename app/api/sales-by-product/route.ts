/**
 * 📊 Sales by Product Report API - تقرير المبيعات حسب المنتج
 * 
 ...
 * 2. التجميع: حسب المنتج
 * 3. الفلترة: حسب التاريخ، الحالة، المنتج، نوع العنصر
...
export async function GET(req: NextRequest) {
  try {
    ...
    let itemsQuery = supabase
      .from("invoice_items")
      .select("invoice_id, quantity, line_total, product_id, description, products(id, name, sku, item_type)")
    ...
    for (const item of invoiceItems || []) {
      ...
      if (invoiceSet.has((item as any).invoice_id)) {
    ...
    return serverError(`حدث خطأ أثناء جلب تقرير المبيعات حسب المنتج: ${e?.message || "unknown_error"}`)
  }
}
