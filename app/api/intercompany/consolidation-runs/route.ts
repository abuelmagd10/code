...
export async function GET(req: NextRequest) {
  try {
    ...
    const runs = await service.listConsolidationRunsForActor(user.id)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const runType = String(body?.runType || "").trim()
    const asOfTimestamp = String(body?.asOfTimestamp || new Date().toISOString())
    ...
    if (!runType) return badRequestError("runType is required")

    const idempotencyKey = resolveFinancialIdempotencyKey(
      req.headers.get("Idempotency-Key"),
      ["consolidation-run-create", hostCompanyId, consolidationGroupId, periodStart, periodEnd, runType, asOfTimestamp]
    )
    const requestHash = buildFinancialRequestHash(body)

    const service = new IntercompanyService(createServiceClient() as any, authSupabase as any)
    const result = await service.createConsolidationRun(
      {
        hostCompanyId,
