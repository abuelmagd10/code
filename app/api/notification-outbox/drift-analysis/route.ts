...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const service = new NotificationOutboxDriftAnalyzerService(createServiceClient())
    ...
    console.error("[NOTIFICATION_OUTBOX_DRIFT_ANALYSIS]", error)
    return NextResponse.json(
      {
        success: false,
        error: String(
          error?.message || "Failed to analyze notification outbox drift"
        ),
      },
      { status: 500 }
    )
