...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const service = new NotificationOutboxActivationGateService(createServiceClient())
    ...
    console.error("[NOTIFICATION_OUTBOX_ACTIVATION_GATE]", error)
    return NextResponse.json(
      {
        success: false,
        error: String(
          error?.message || "Failed to evaluate notification outbox activation gate"
        ),
      },
      { status: 500 }
    )
