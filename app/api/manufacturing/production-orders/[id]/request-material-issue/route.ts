...
export async function POST(
  ...
  try {
    ...
    // إشعاران لا أربعة — والسبب فى **جانب القراءة**:
    // `NotificationCenter` يعامل (owner / admin / general_manager) كجمهور
    // واحد: كلٌّ منهم يرى إشعارات الآخرين. فإرسال إشعار لكل دور منها يعنى أن
    // المالك يرى **ثلاث نسخ** من نفس الرسالة.
    //
    // وهذا ما حدث فعلاً: نسختُ نمط الإرسال الرباعى من `submit-approval` بلا
    // فحص كيف تُقرأ الإشعارات، فأبلغ المالك أن الرسالة وصلته مكرَّرة.
    // ⇒ **من يكتب لجمهور يقرأ قاعدة الجمهور أولاً.**
    //
    // فواحد للإدارة العليا (يصل ثلاثتهم بحكم قاعدة القراءة)، وواحد لمدير
    // الفرع لأنه ليس منهم ولا يرى إشعاراتهم. والاثنان يغطيان الأدوار الأربعة
    // التى يقبلها `management-approve` بلا تكرار على أحد.
