...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const bill of (data || [])) {
      ...
      grouped[name] = { total: prev.total + Number((bill as any).total_amount || 0), count: prev.count + 1 }
    }
    const result = Object.entries(grouped).map(([supplier_name, v]) => ({ supplier_name, total_purchases: v.total, bill_count: v.count }))
