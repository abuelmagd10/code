...
export async function POST(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const { data: result, error } = await supabase.rpc('no_show_booking_atomic', {
    ...
    try {
      ...
      await notifySvc.notifyBookingNoShow({ bookingId: id, companyId, actorUserId: user.id })
    } catch (err) {
      console.error('[bookings/no-show] notification failed (non-blocking):', err)
    }
