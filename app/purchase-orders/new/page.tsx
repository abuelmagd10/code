...
export default function NewPurchaseOrderPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ po_number is auto-generated by database trigger (auto_generate_po_number)
      ...
      const { data: poData, error: poError } = await supabase.from("purchase_orders").insert({
        ...
        // po_number: auto-generated by database trigger
      ...
      // Enterprise Requirement: Audit Log for PO Creation
      const { data: { user } } = await supabase.auth.getUser()
      if (user && companyId) {
        await supabase.from("audit_logs").insert({
          ...
          entity_id: poData.id,
          new_values: { ...poData, status: isAdmin ? "draft" : "pending_approval" },
      ...
      if (!isAdmin) {
        ...
        try {
          ...
          console.log(`✅ PO approval notifications sent for PO ${poData.po_number}`)
        } catch (notifErr) {
          console.error('⚠️ PO approval notification failed (non-critical):', notifErr)
        }
      } else {
        // Admin creates PO -> immediately draft status -> CREATE BILL
        // ✅ bill_number is auto-generated by database trigger (auto_generate_bill_number)
        // Create linked bill (draft)
        const { data: billData, error: billError } = await supabase.from("bills").insert({
          company_id: companyId,
          supplier_id: formData.supplier_id,
          // bill_number: auto-generated by database trigger
