...
interface ServiceRevRow {
  ...
  avg_rating: number; completion_rate: string
}

export default function RevenueByServicePage() {
  const router = useRouter()
  const [data,      setData]      = useState<ServiceRevRow[]>([])
  const [summary,   setSummary]   = useState({ total_services: 0, total_bookings: 0, total_revenue: 0, total_collected: 0 })
  ...
  const loadData = async () => {
    setIsLoading(true)
    try {
      const params = new URLSearchParams({ from, to, service_type: svcType })
      const res  = await fetch(`/api/reports/bookings/revenue-by-service?${params}`)
      const json = await res.json()
      setData(json.data   ?? [])
      setSummary(json.summary ?? { total_services: 0, total_bookings: 0, total_revenue: 0, total_collected: 0 })
    } finally { setIsLoading(false) }
  }

  useEffect(() => { loadData() }, [from, to, svcType])

  const handleExport = () => {
    const header = ["service_name", "service_type", "category", "total_bookings", "completed", "cancelled", "no_show", "completion_rate", "total_revenue", "total_collected", "avg_rating"]
    const rows   = data.map((r) => [r.service_name, r.service_type, r.category ?? "", r.total_bookings, r.completed_bookings, r.cancelled_bookings, r.no_show_bookings, r.completion_rate, r.total_revenue.toFixed(2), r.total_collected.toFixed(2), r.avg_rating.toFixed(2)])
    const csv    = [header.join(","), ...rows.map((r) => r.join(","))].join("\n")
    const blob   = new Blob(["﻿" + csv], { type: "text/csv;charset=utf-8;" })
    const url    = URL.createObjectURL(blob)
    const a      = document.createElement("a"); a.href = url; a.download = `revenue-by-service-${from}-${to}.csv`; a.click()
      ...
      <main className="flex-1 md:mr-64 p-4 md:p-8 pt-20 md:pt-8">
        <div className="space-y-6">
            ...
            <CardContent className="py-6">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t("Revenue by Service", "الإيرادات حسب الخدمة")}</h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t("Completed booking revenue per service", "إيرادات الحجوزات المكتملة لكل خدمة")}</p>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: t("Services", "الخدمات"),      value: summary.total_services,                  icon: CalendarDays, color: "text-orange-500" },
              { label: t("Bookings", "الحجوزات"),      value: summary.total_bookings,                  icon: TrendingUp,   color: "text-blue-500" },
              { label: t("Total Revenue", "إجمالي الإيرادات"), value: fmt.format(summary.total_revenue),   icon: DollarSign,   color: "text-green-500" },
              { label: t("Collected", "المحصّل"),       value: fmt.format(summary.total_collected), icon: Star,         color: "text-purple-500" },
          ...
          <Card className="dark:bg-gray-800">
            <CardContent className="py-4">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  <Label className="text-xs">{t("Service Type", "نوع الخدمة")}</Label>
                  <Select value={svcType} onValueChange={setSvcType}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">{t("All", "الكل")}</SelectItem>
                      <SelectItem value="appointment">{t("Appointment", "موعد")}</SelectItem>
                      <SelectItem value="class">{t("Class", "حصة")}</SelectItem>
                      <SelectItem value="package">{t("Package", "باقة")}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
          ...
          {data.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card className="dark:bg-gray-800">
                <CardHeader><CardTitle>{t("Revenue by Service", "الإيرادات حسب الخدمة")}</CardTitle></CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={data.slice(0, 10) as any[]}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="service_name" angle={-30} textAnchor="end" height={70} tick={{ fontSize: 11 }} />
                      <YAxis />
                      <Tooltip formatter={(v: number) => fmt.format(v)} />
                      <Legend />
                      <Bar dataKey="total_revenue"   fill="#f97316" name={t("Revenue", "الإيرادات")} />
              ...
              <Card className="dark:bg-gray-800">
                <CardHeader><CardTitle>{t("Bookings Distribution", "توزيع الحجوزات")}</CardTitle></CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie data={data.slice(0, 8) as any[]} dataKey="total_revenue" nameKey="service_name" cx="50%" cy="50%" outerRadius={100}
                        label={(e: any) => e.service_name}>
          ...
          <Card className="dark:bg-gray-800">
            <CardHeader>
              <CardTitle>{t("Service Revenue Details", "تفاصيل إيرادات الخدمات")} ({data.length})</CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {data.map((row, i) => (
                        <tr key={row.service_id ?? i} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-slate-700">
                          ...
                          <td className="py-3 px-2 text-center">
                            <span className={`font-semibold ${Number(row.completion_rate) >= 70 ? "text-green-600" : Number(row.completion_rate) >= 50 ? "text-amber-600" : "text-red-500"}`}>
                              {row.completion_rate}%
                            </span>
                          </td>
