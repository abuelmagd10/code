...
export default function NewInvoicePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const _canSkipSO = _role === 'owner' || _role === 'admin' || _role === 'general_manager'
    ...
    setTimeout(async () => {
      try {
        ...
        const itemsToSend = invoiceItems
          ...
          .map((item) => {
            ...
            return {
              ...
              // v3.74.403 - persist tax code link
              tax_code_id: item.tax_code_id || null,
              discount_percent: item.discount_percent ?? 0,
              line_total: netLine,
              returned_quantity: 0,
        ...
        // 🚀 Single atomic API call — no partial inserts possible
        ...
        if (!res.ok || !result.success) {
          console.error("Invoice API error:", result)
          toast({
            title: appLang === 'en' ? "Save failed" : "فشل الحفظ",
            description: result.error || result.message || (appLang === 'en' ? "Unknown error" : "خطأ غير معروف"),
            variant: "destructive",
          })
          startTransition(() => { setIsSaving(false) })
          return
        ...
        try {
          const { getActiveCompanyId } = await import("@/lib/company")
          const notifCompanyId = await getActiveCompanyId(supabase)
          if (notifCompanyId && invoiceId) {
            createNotification({
              companyId: notifCompanyId,
              referenceType: 'invoice',
              referenceId: invoiceId,
              title: appLang === 'en' ? 'New Sales Invoice' : 'فاتورة مبيعات جديدة',
              message: appLang === 'en' ? 'A new invoice has been created' : 'تم إنشاء فاتورة جديدة',
              createdBy: '',
              branchId: branchId || undefined,
              costCenterId: costCenterId || undefined,
              assignedToRole: 'accountant',
              priority: 'normal',
              eventKey: `invoice:${invoiceId}:created`,
              severity: 'info',
              category: 'sales'
            }).catch(console.error)
          }
        ...
        const msg = error?.message || (appLang === 'en' ? "Error creating invoice" : "خطأ في إنشاء الفاتورة")
        startTransition(() => { setIsSaving(false) })
        toast({ title: appLang === 'en' ? "Save failed" : "فشل الحفظ", description: msg, variant: "destructive" })
      }
    }, 0)
  }
              ...
              <CardContent>
                <div className="space-y-2" data-ai-help="invoices.sales_order">
                  ...
                  <Select value={salesOrderId} onValueChange={handleSalesOrderChange}>
                    <SelectTrigger className={`${(!salesOrderId && !(['owner','admin','general_manager'].includes(String(userContext?.role || '').toLowerCase()))) ? 'border-red-500' : ''}`}>
                  ...
                  {!salesOrderId && !(['owner','admin','general_manager'].includes(String(userContext?.role || '').toLowerCase())) && (
