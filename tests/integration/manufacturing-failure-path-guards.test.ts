...
const shouldRunManufacturingFailurePathGuards =
  hasTestSupabaseCredentials() &&
...
const describeFailurePathGuards = shouldRunManufacturingFailurePathGuards ? describe : describe.skip
  ...
  if (!warehouseId) {
    const { data: insertedWarehouse, error } = await supabase
      ...
      .insert({
        ...
        is_main: true,
        is_active: true,
        notes: "Failure-path integration warehouse",
      })
      .select("id")
      .single()
...
async function setupReleasedAndSyncedOrder(tag: string) {
  ...
  const { error: seedInventoryError } = await supabase.from("inventory_transactions").insert({
    ...
    reference_type: "failure_path_seed",
...
describeFailurePathGuards("Manufacturing failure-path guards integration", () => {
  beforeAll(() => {
  ...
  it(
    "rejects over-issue above remaining requirement without partial persistence",
    async () => {
      const tag = `FP-OVER-ISSUE-${Date.now()}`
      ...
      const issueResponse = await issueMaterialsRoute(
        makeJsonRequest(
          ...
          "POST",
          {
            command_key: `${tag}-ISSUE-OVER`,
            notes: `${tag} attempt to over issue`,
            lines: [
              {
      ...
      expect(String(issueJson.error || "")).toContain("remaining")
      ...
      expect(reservationAfter.status).toBe("partially_reserved")
      expect(Number(reservationAfter.requested_qty || 0)).toBe(20)
      expect(Number(reservationAfter.reserved_qty || 0)).toBe(10)
      expect(Number(reservationAfter.consumed_qty || 0)).toBe(0)
      expect(Number(reservationAfter.released_qty || 0)).toBe(0)
      expect(Number(allocationAfter?.allocated_qty || 0)).toBe(10)
      expect(Number(allocationAfter?.consumed_qty || 0)).toBe(0)
      expect(Number(allocationAfter?.released_qty || 0)).toBe(0)
      expect(Number(rmSnapshotAfter?.on_hand_quantity || 0)).toBe(10)
      expect(Number(rmSnapshotAfter?.reserved_quantity || 0)).toBe(10)
      expect(Number(rmSnapshotAfter?.free_quantity || 0)).toBe(0)
    },
    120000
  )

  it(
    "rejects receipt before in-progress without partial persistence",
    async () => {
      const tag = `FP-RECEIPT-TIMING-${Date.now()}`
      const scenario = await setupReleasedAndSyncedOrder(tag)
      currentApiContext = scenario
      ...
      expect(orderBefore.status).toBe("released")
      expect(Number(fgSnapshotBefore?.on_hand_quantity || 0)).toBe(0)
      expect(Number(fgSnapshotBefore?.reserved_quantity || 0)).toBe(0)
      expect(Number(fgSnapshotBefore?.free_quantity || 0)).toBe(0)
      expect(reservationBefore.status).toBe("partially_reserved")
      ...
      const receiptResponse = await receiptOutputRoute(
        makeJsonRequest(
          ...
          "POST",
          {
            command_key: `${tag}-RECEIPT-EARLY`,
            notes: `${tag} invalid early receipt`,
            received_qty: 1,
          }
        ),
        { params: Promise.resolve({ id: scenario.productionOrderId }) }
      ...
      expect(String(receiptJson.error || "")).toContain("in progress")

      const [
        { data: orderAfter, error: orderAfterError },
        { data: receiptEvents, error: receiptEventsError },
        { data: receiptLines, error: receiptLinesError },
        { count: receiptTxnCount, error: receiptTxnCountError },
        { count: fgProductionLotCount, error: fgProductionLotCountError },
      ...
      if (receiptTxnCountError) throw new Error(receiptTxnCountError.message)
      if (fgProductionLotCountError) throw new Error(fgProductionLotCountError.message)
      ...
      expect(Number(receiptTxnCount || 0)).toBe(0)
      expect(Number(fgProductionLotCount || 0)).toBe(0)
      expect(Number(fgSnapshotAfter?.on_hand_quantity || 0)).toBe(0)
      expect(Number(fgSnapshotAfter?.reserved_quantity || 0)).toBe(0)
      expect(Number(fgSnapshotAfter?.free_quantity || 0)).toBe(0)
      expect(reservationAfter.status).toBe("partially_reserved")
      expect(Number(reservationAfter.requested_qty || 0)).toBe(20)
      expect(Number(reservationAfter.reserved_qty || 0)).toBe(10)
      expect(Number(reservationAfter.consumed_qty || 0)).toBe(0)
