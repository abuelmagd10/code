-- Migration: Auto-Generate Bills as pending_receipt
-- Description: Updates the approve_purchase_order_atomic RPC to auto-generate bills with pending_receipt status instead of draft, passing the redundant admin approval step.
...
BEGIN
    ...
    IF v_user_role NOT IN ('admin', 'owner', 'general_manager', 'gm', 'super_admin') THEN
         RETURN jsonb_build_object('success', false, 'error', 'Insufficient permissions to approve or reject POs');
    END IF;
    ...
    -- Global roles (admin, owner, general_manager) bypass branch isolation for PO approval/rejection.
    IF v_user_role NOT IN ('admin', 'owner', 'general_manager', 'gm', 'super_admin') THEN
        IF v_user_branch IS NOT NULL AND v_po.branch_id IS NOT NULL AND v_user_branch != v_po.branch_id THEN
             RETURN jsonb_build_object('success', false, 'error', 'Branch isolation violation: Cannot approve or reject PO for a different branch');
        END IF;
    END IF;
    ...
    IF p_action = 'approve' THEN
        v_new_status := 'approved'; -- Change from draft to approved
        v_audit_action := 'APPROVE'; -- Must match check constraint
        ...
        -- Auto-Create Pending Receipt Bill
        v_new_bill_id := gen_random_uuid();
        
        -- Generate Bill Number based on company (using a separate integer variable to prevent type mismatch)
        ...
        INSERT INTO public.bills (
            ...
            tax_amount,
            total_amount,
            status,             -- changed from draft to pending_receipt
            approval_status,    -- newly added to ensure it passes approval step inherently
            created_at,
            is_deleted,
            purchase_order_id,
            branch_id,
            ...
            v_po.tax_amount,
            v_po.total_amount,
            'pending_receipt',  -- direct to warehouse validation
            'approved',         -- auto-approved as admin already approved PO
            NOW(),
            false,
            v_po.id,
            v_po.branch_id,
    ...
    ELSIF p_action = 'reject' THEN
        ...
        v_audit_action := 'UPDATE'; -- Must match check constraint
