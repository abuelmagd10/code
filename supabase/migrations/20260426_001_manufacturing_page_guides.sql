...
(
  ...
  '["استعرض القائمة لرؤية جميع هياكل المواد المسجلة وحالة كل منها (تشغيلي / موقف)", "ابحث عن هيكل مواد محدد باسم المنتج أو الكود المرجعي باستخدام خانة البحث", "اضغط «إنشاء هيكل مواد جديد» لإضافة وصفة إنتاجية لمنتج جديد", "اضغط على أي هيكل في القائمة لفتح صفحة تفاصيله وإدارة مكوناته ومراجعة إصداراته", "راجع بطاقات الإحصائيات في الأعلى لمعرفة إجمالي الهياكل والنشطة منها"]',
  '["Browse the list to see all registered BOMs and each one''s status (active / inactive)", "Search for a specific BOM by product name or reference code", "Click «Create New BOM» to add a production recipe for a new product", "Click any BOM in the list to open its detail page and manage components and versions", "Review the statistics cards at the top for total BOMs and active count"]',
...
(
  ...
  '["راجع البيانات الأساسية في الأعلى: اسم المنتج، الفرع، الكود المرجعي، والحالة الحالية", "انتقل لقسم «المكونات» واضغط «إضافة مكون» لإضافة مادة خام", "لكل مكون: اختر المادة الخام من قائمة المنتجات وأدخل الكمية اللازمة لإنتاج وحدة واحدة", "كرّر الخطوة السابقة حتى تُضيف جميع المواد الخام التي تدخل في الوصفة", "إذا تغيرت الوصفة لاحقاً: أنشئ «إصداراً جديداً» من الزر المخصص له بدلاً من تعديل الإصدار القديم", "فعّل الهيكل أو أوقفه حسب جاهزيته للاستخدام في أوامر التصنيع"]',
  '["Review the header details: product name, branch, reference code, and current status", "Go to the «Components» section and click «Add Component» to add a raw material", "For each component: select the raw material from the product list and enter the required quantity per unit produced", "Repeat until all raw materials needed for the recipe are added", "If the recipe changes later: create a «New Version» using its dedicated button — do not edit the existing version", "Activate or deactivate the BOM based on its readiness for use in production orders"]',
...
(
  ...
  '["استعرض قائمة مسارات العمل وحالة كل مسار (تشغيلي / موقف)", "ابحث عن مسار معين بالاسم أو الكود المرجعي", "اضغط «إنشاء مسار عمل جديد» لإضافة خطوات إنتاج لمنتج", "افتح أي مسار من القائمة لعرض عملياته التفصيلية أو تعديل خطواته", "راجع الإحصائيات لمعرفة إجمالي المسارات المسجلة والنشطة"]',
  '["Browse the routing list and check each routing''s status (active / inactive)", "Search for a specific routing by name or reference code", "Click «Create New Routing» to add production steps for a product", "Open any routing to view its detailed operations or modify its steps", "Review the statistics for total and active routing counts"]',
...
(
  ...
  '["راجع معلومات المسار الأساسية: الاسم، الكود، والمنتج المرتبط به", "انتقل لقسم «العمليات» واضغط «إضافة عملية» لإضافة خطوة إنتاج جديدة", "لكل عملية: أدخل اسماً واضحاً يصف ما يحدث فيها (مثال: خلط المكونات، تعبئة، تغليف)", "حدد الوقت المقدر لتنفيذ كل عملية بالدقائق أو الساعات", "رتّب العمليات بالترتيب الصحيح الذي تحدث فيه فعلياً داخل المصنع", "فعّل المسار بعد الانتهاء لجعله متاحاً في أوامر التصنيع"]',
  '["Review the routing header: name, code, and linked product", "Go to the «Operations» section and click «Add Operation» to add a new production step", "For each operation: enter a clear name describing what happens (e.g., Mixing, Filling, Packaging)", "Specify the estimated time to complete each operation in minutes or hours", "Order the operations in the correct sequence they actually occur in the factory", "Activate the routing once complete to make it available in production orders"]',
...
(
  ...
  'This is your main production control panel — showing all current and past production orders with each one''s status. From here you get a bird''s eye view of your production line: what is in progress, what is completed, and what has not started yet.',

  '["استعرض بطاقات الإحصائيات في الأعلى: إجمالي الأوامر، النشطة منها، والمكتملة", "راجع قائمة الأوامر ولاحظ حالة كل أمر: مسودة / جاهز / قيد التنفيذ / مكتمل / ملغى", "استخدم فلاتر البحث لتصفية الأوامر حسب المنتج أو الحالة أو الفرع أو تاريخ الإنشاء", "اضغط «إنشاء أمر إنتاج جديد» لبدء دورة إنتاج لمنتج معين بكمية محددة", "اضغط على أي أمر في القائمة لفتح صفحة تفاصيله ومتابعة تقدمه أو تنفيذ الإجراءات عليه"]',
  '["Review the statistics cards at the top: total orders, active, and completed", "Look at the order list and note each order''s status: draft / ready / in progress / completed / cancelled", "Use the search filters to filter orders by product, status, branch, or creation date", "Click «Create New Production Order» to start a production cycle for a product and quantity", "Click any order to open its detail page, track progress, or perform actions on it"]',
  ...
  ARRAY[
    ...
    'Monitor orders in "in progress" status regularly to ensure production stays on schedule'
  ]
),
...
(
  ...
  '["راجع ملخص الأمر في الأعلى: المنتج، الكمية المطلوبة، هيكل المواد، ومسار العمل المرتبط", "تحقق من بطاقات الحالة: الكمية المخططة، المنجزة، والمتبقية", "راجع قسم «المكونات» للتأكد من توفر المواد الخام المطلوبة في المخزن", "اضغط «إصدار» عندما تتأكد من جاهزية المواد والفريق للبدء — الأمر يصبح رسمياً بعدها", "اضغط «بدء التنفيذ» عندما يبدأ فريق الإنتاج فعلياً في العمل", "تابع تقدم العمليات من تبويب «العمليات» وحدّث حالة كل خطوة", "عند الانتهاء من التصنيع: اضغط «اكتمال» لتسجيل النتيجة وتحديث المخزون تلقائياً"]',
  '["Review the order summary at the top: product, required quantity, BOM, and linked routing", "Check the status cards: planned quantity, completed, and remaining", "Review the «Components» section to confirm required raw materials are available in stock", "Click «Release» when materials and team are ready to start — the order becomes official at this point", "Click «Start Execution» when the production team actually begins working", "Track operation progress from the «Operations» tab and update each step''s status", "When manufacturing is done: click «Complete» to record the result and automatically update inventory"]',
  ...
  ARRAY[
    'Clicking «Complete» creates an automatic inventory movement: raw materials are consumed and the finished product is added — this step is irreversible',
    'You can only cancel an order before clicking «Start Execution» — after that it must be completed or formally stopped',
    'If the production recipe has changed, verify the BOM is updated before clicking «Complete»'
  ]
)
