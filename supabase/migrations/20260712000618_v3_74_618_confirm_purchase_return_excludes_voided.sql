-- v3.74.618 — confirm_purchase_return_delivery_v3: the pre-apply overpayment
-- guard summed approved payment_allocations without excluding voided payments,
-- so a voided payment could wrongly block confirming a purchase return.
-- Applied to production via mcp; this mirrors it.
CREATE OR REPLACE FUNCTION public.confirm_purchase_return_delivery_v3(p_purchase_return_id uuid, p_confirmed_by uuid, p_notes text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
