-- Fix get_user_notifications return types and audit_logs action constraint

-- 1. Restore the correct return types for get_user_notifications
...
CREATE OR REPLACE FUNCTION public.get_user_notifications(p_user_id uuid, p_company_id uuid, p_branch_id uuid DEFAULT NULL::uuid, p_warehouse_id uuid DEFAULT NULL::uuid, p_status character varying DEFAULT NULL::character varying, p_severity character varying DEFAULT NULL::character varying, p_category character varying DEFAULT NULL::character varying)
 RETURNS TABLE(id uuid, reference_type character varying, reference_id uuid, title character varying, message text, priority character varying, status character varying, created_at timestamp with time zone, branch_name character varying, warehouse_name character varying, severity character varying, category character varying, event_key character varying)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
...
-- 2. Update approve_purchase_order_atomic to use standard audit actions
...
BEGIN
    ...
    IF p_action = 'approve' THEN
        ...
        v_audit_action := 'UPDATE'; -- Using standard action
    ...
    -- 6. Audit Logging Using Correct Columns and Standard Action
...
-- 3. Update approve_purchase_return_atomic to use standard audit actions
...
BEGIN
  ...
  IF p_action = 'approve' THEN
    ...
    v_audit_action := 'UPDATE'; -- Standard action
