-- Enterprise PR Fix 1+2+3+4+5 — Final Consolidated confirm_purchase_return_delivery_v2
-- This replaces all previous versions.
-- Includes:
--   FIX 1: Multi-currency FX Gain/Loss via exchange_rate_at_return vs bills.exchange_rate
--   FIX 2: financial_status (not_applicable / pending_refund / refund_recorded)
--   FIX 3: vendor_credits ALWAYS created when v_vendor_credit_amount > 0
--   FIX 4: pg_advisory_xact_lock + SELECT bills FOR UPDATE (concurrency control)
--   FIX 5: v_vc_debit renamed to v_vendor_credit_amount
...
BEGIN
  ...
  IF v_bill_id IS NOT NULL THEN
    ...
    INTO v_bill_total, v_bill_paid, v_rate_at_invoice
  ...
  IF v_bill_id IS NOT NULL THEN
    UPDATE bills SET
      ...
      return_status = CASE WHEN (COALESCE(returned_amount, 0) + v_pr.total_amount) >= COALESCE(total_amount, 0) THEN 'fully_returned' ELSE 'partially_returned' END,
      updated_at = NOW()
    WHERE id = v_bill_id;
  END IF;
