-- ============================================================
-- Fix confirm_purchase_return_delivery_v2 — error 42703
-- vendor_credits does NOT have these columns:
--   warehouse_id, original_currency, exchange_rate_used, exchange_rate_id
-- Removed them from the vendor_credits INSERT statement.
-- ============================================================
...
BEGIN
  ...
  IF v_pr.settlement_method IN ('debit_note', 'credit') AND COALESCE(v_pr.total_amount, 0) > 0 THEN
    IF NOT EXISTS (
      ...
      -- ✅ Only insert columns that exist in the vendor_credits table
...
COMMENT ON FUNCTION public.confirm_purchase_return_delivery_v2(uuid, uuid, text) IS
  'Warehouse confirms PR delivery: posts JE, deducts inventory, updates bill, creates vendor credit. Fixed to match actual vendor_credits schema (no warehouse_id/original_currency/exchange_rate columns).';
