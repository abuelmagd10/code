-- =============================================================================
-- Migration: 20260318100000_restore_post_bill_in_rpc
-- Purpose  : Restore 'post_bill' transaction type support in post_purchase_transaction
--            Migration 20260302_003 replaced the function but removed 'post_bill' case.
--            This migration adds it back to the 12-parameter variant.
-- =============================================================================
...
BEGIN
  ...
  IF p_transaction_type = 'post_bill' THEN
    ...
    -- B. Insert Journal Entry (AP vs Inventory/Purchases)
    IF p_journal_entry IS NOT NULL THEN
      ...
      -- Touch bill updated_at
      UPDATE bills SET updated_at = NOW() WHERE id = p_bill_id;
    END IF;

    -- C. Update Bill Status (if requested)
  ...
  -- ── PURCHASE RETURN (prefer process_purchase_return_atomic) ─────────────────
  ELSIF p_transaction_type = 'purchase_return' THEN
    ...
    IF p_purchase_return IS NOT NULL THEN
      INSERT INTO purchase_returns (
        ...
        p_purchase_return->>'reason', p_purchase_return->>'notes',
...
COMMENT ON FUNCTION post_purchase_transaction(TEXT, UUID, UUID, JSONB, JSONB, JSONB, JSONB, JSONB, JSONB, JSONB, JSONB, JSONB) IS
  'Purchase transaction RPC supporting post_bill (receipt approval), bill creation, and purchase returns.
   For purchase returns, prefer process_purchase_return_atomic which has row-level locking.';
