-- =============================================================================
-- إصلاح: قيد الدفع التلقائي يجب أن يُدخل القيد كـ draft ثم الأسطر ثم يرحّل
-- Fix: auto_create_payment_journal must insert entry as draft, add lines, then post
-- (الـ trigger enforce_posted_entry_lines_no_edit يمنع إضافة أسطر لقالب مرحّل)
-- =============================================================================
...
BEGIN
  ...
  -- فاتورة مبيعات: إنشاء القيد كـ draft ثم إضافة الأسطر ثم ترحيل (لتجنب trigger منع إضافة أسطر لقالب مرحّل)
  IF NEW.invoice_id IS NOT NULL THEN
