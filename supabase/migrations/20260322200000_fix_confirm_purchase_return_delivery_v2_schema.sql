-- ==============================================================================
-- إصلاح confirm_purchase_return_delivery_v2 (PostgREST error 42703 = undefined_column)
-- الدالة القديمة (phase1) تستهدف أعمدة غير موجودة في المخطط الحالي مثل:
--   inventory_transactions.unit_cost / total_cost
--   journal_entry_lines.original_currency / exchange_rate_used
--   vendor_credits.reference_number / remaining_amount
-- كما أن journal_entries يتطلب branch_id (NOT NULL) ولا يجب إنشاء قيد مكرر إذا وُجدت مسودة.
-- ==============================================================================
  ...
  LOOP
    IF v_item.product_id IS NOT NULL THEN
      INSERT INTO inventory_transactions (
        ...
        COALESCE(v_pr.return_date::timestamptz, NOW())
      );
    END IF;
...
COMMENT ON FUNCTION public.confirm_purchase_return_delivery_v2(uuid, uuid, text) IS
  'Warehouse confirms PR: post draft JE or create GL; inventory/bill/vendor credit; aligned with current schema (no missing columns).';
