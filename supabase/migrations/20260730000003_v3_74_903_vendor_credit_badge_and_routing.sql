-- =============================================================================
-- v3.74.903 — «فتح المرجع» يعرف أنواع مصفوفة الإشعار الدائن + شارة العدّ فى الشريط
-- =============================================================================
-- ملاحظتا المالك الحيتان بعد أول إخطار مصفوفةٍ حقيقى (CR-51543):
--   1) زر «فتح المرجع» صامت — أنواع vendor_credit_pending/approved/rejected
--      غابت عن خريطة توجيه الإخطارات (lib/notification-routing.ts —
--      أُصلحت هناك: المعلق ⇒ صندوق الموافقات تبويب vc؛ القراران ⇒ صفحة
--      الإشعار).
--   2) لا عدّ للإشعارات المعلقة فى شارة «الموافقات» بالشريط الجانبى —
--      هذا الملف يضيف مفتاح vendor_credit_pending إلى دالة الشارات:
--      للمالك والمدير العام فقط (مصفوفة 900 حرفياً)، ويُستبعد ما أنشأه
--      المستخدم نفسه — فصل المهام: لا يُعدُّ له ما لا يملك قراره.
-- تحقق حى بعد التطبيق: شارة المالك على شركة تست = 1 (إشعار CR-51543).
-- =============================================================================
...
BEGIN
  ...
  IF v_role IN ('owner','general_manager') THEN
    ...
    v_result := v_result || jsonb_build_object('vendor_credit_pending', v_n);
  END IF;
