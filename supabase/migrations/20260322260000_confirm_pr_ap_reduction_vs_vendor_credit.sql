-- ==============================================================================
-- Enterprise: مرتجع مشتريات — تخفيض الذمم الدائنة (AP) أولاً، ثم Vendor Credit للفائض فقط
-- السيناريو: فاتورة 50 غير مدفوعة + مرتجع 20 → Dr AP 20 (لا vendor_credit document)
--            فاتورة مدفوعة بالكامل → إشعار دائن كامل
--            جزئي: يُخفّض AP المتبقي ثم الفائض كـ vendor credit
-- مصدر الحقيقة: bills.total_amount - bills.paid_amount = open AP قبل هذا المرتجع
-- ==============================================================================
...
BEGIN
  ...
  -- فتح AP المتبقي على الفاتورة (قبل تطبيق هذا المرتجع): total_amount − paid_amount
  v_bill_total := 0;
  v_bill_paid := 0;
  ...
  -- vendor_credits: فقط للمبلغ الذي يتجاوز الذمم المفتوحة على الفاتورة، وضمن طرق التسوية المعتمدة
...
COMMENT ON FUNCTION public.confirm_purchase_return_delivery_v2(uuid, uuid, text) IS
  'Warehouse confirms PR: JE splits Dr AP (open bill balance) vs Dr vendor_credit_liability (excess); vendor_credits row only for excess when settlement is debit_note/credit.';
