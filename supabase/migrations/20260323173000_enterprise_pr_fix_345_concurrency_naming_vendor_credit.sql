-- ===================================================================
-- Enterprise PR Upgrade: Fix 3 + Fix 4 + Fix 5
-- Fix 4: Concurrency locking on bills + advisory lock
-- Fix 5: v_vc_debit → v_vendor_credit_amount (naming clarity)
-- Fix 3: vendor_credits record ALWAYS created when GL has vendor credit entry
-- ===================================================================
...
DECLARE
  ...
  v_vendor_credit_amount    NUMERIC(18, 4);   -- FIX 5: was v_vc_debit
  v_vc_ratio                NUMERIC(18, 8);
  v_vc_sub                  NUMERIC(18, 4);
  v_vc_tax                  NUMERIC(18, 4);
  v_round_fix               NUMERIC(18, 4);
BEGIN
  -- FIX 4: Advisory lock — prevent concurrent returns on same PR
  ...
  v_transition := transition_purchase_return_state(
    p_purchase_return_id, v_company_id, p_confirmed_by, 'completed', p_notes
  );
  ...
  -- Compute open AP balance (FIX 4: FOR UPDATE on bills)
  v_bill_total := 0;
  v_bill_paid  := 0;
  IF v_bill_id IS NOT NULL THEN
    SELECT COALESCE(b.total_amount, 0), COALESCE(b.paid_amount, 0)
    INTO v_bill_total, v_bill_paid
    FROM bills b
    WHERE b.id = v_bill_id AND b.company_id = v_company_id
    FOR UPDATE;

    IF NOT FOUND THEN
      v_bill_id := NULL; v_bill_total := 0; v_bill_paid := 0;
    END IF;
  END IF;
  ...
  IF v_bill_id IS NULL THEN
    v_ap_reduction         := 0;
    v_vendor_credit_amount := v_return_total;
  ELSE
  ...
  IF v_je_id IS NOT NULL THEN
    ...
    SET status = 'posted', branch_id = COALESCE(branch_id, v_branch_id),
  ...
  ELSE
    ...
    WHERE company_id = v_company_id AND sub_type = 'inventory' AND COALESCE(is_active, true) LIMIT 1;
    SELECT id INTO v_vat_account_id FROM chart_of_accounts
    WHERE company_id = v_company_id AND sub_type = 'vat_input' AND COALESCE(is_active, true) LIMIT 1;
    SELECT id INTO v_ap_account_id FROM chart_of_accounts
    WHERE company_id = v_company_id AND (sub_type = 'accounts_payable' OR sub_type = 'ap') AND COALESCE(is_active, true)
    ORDER BY CASE WHEN sub_type = 'accounts_payable' THEN 0 ELSE 1 END LIMIT 1;
    SELECT id INTO v_vc_liability_account_id FROM chart_of_accounts
    WHERE company_id = v_company_id AND sub_type = 'vendor_credit_liability' AND COALESCE(is_active, true) LIMIT 1;
    SELECT id INTO v_fallback_liability_id FROM chart_of_accounts
    WHERE company_id = v_company_id AND COALESCE(is_active, true)
      AND (sub_type = 'vendor_credit_liability' OR sub_type = 'accounts_payable' OR sub_type = 'ap') LIMIT 1;

    IF v_ap_reduction > 0 AND v_ap_account_id IS NULL THEN
      v_vendor_credit_amount := v_vendor_credit_amount + v_ap_reduction; v_ap_reduction := 0;
    END IF;
    IF v_vendor_credit_amount > 0 AND v_vc_liability_account_id IS NULL AND v_ap_account_id IS NOT NULL THEN
      v_ap_reduction := v_ap_reduction + v_vendor_credit_amount; v_vendor_credit_amount := 0;
    END IF;
    ...
    INSERT INTO journal_entries (
      ...
      v_company_id, v_branch_id, v_cc_id, 'purchase_return', p_purchase_return_id,
      COALESCE(v_pr.return_date, CURRENT_DATE),
      'مرتجع مشتريات ' || COALESCE(v_pr.return_number, ''), 'posted'
    ...
    IF v_return_total > 0 THEN
      ...
      IF v_vendor_credit_amount > 0 THEN
        DECLARE v_vc_gl_account UUID;
        BEGIN
          v_vc_gl_account := COALESCE(v_vc_liability_account_id, v_fallback_liability_id);
          IF v_vc_gl_account IS NULL THEN
            RAISE EXCEPTION 'No liability account (AP / vendor credit) found for company %', v_company_id;
          END IF;
          INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit_amount, credit_amount, description, branch_id, cost_center_id)
          VALUES (v_je_id, v_vc_gl_account, v_vendor_credit_amount, 0,
            CASE WHEN v_vc_liability_account_id IS NOT NULL
              THEN 'إشعار دائن مورد — جزء يتجاوز الرصيد المفتوح على الفاتورة'
              ELSE 'تسوية مرتجع مشتريات (حساب احتياطي)' END,
            v_branch_id, v_cc_id);
        END;
      END IF;
    END IF;
    IF v_inventory_account_id IS NOT NULL AND COALESCE(v_pr.subtotal, 0) > 0 THEN
      INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit_amount, credit_amount, description, branch_id, cost_center_id)
      VALUES (v_je_id, v_inventory_account_id, 0, v_pr.subtotal, 'مخزون مرتجع للمورد', v_branch_id, v_cc_id);
    END IF;
    IF v_vat_account_id IS NOT NULL AND COALESCE(v_pr.tax_amount, 0) > 0 THEN
      INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit_amount, credit_amount, description, branch_id, cost_center_id)
      VALUES (v_je_id, v_vat_account_id, 0, v_pr.tax_amount, 'عكس ضريبة المشتريات', v_branch_id, v_cc_id);
    END IF;
  END IF;

  PERFORM set_config('app.allow_direct_post', 'false', true);

  UPDATE purchase_return_items SET is_deducted = true
  WHERE purchase_return_id = p_purchase_return_id AND COALESCE(is_deducted, false) = false;
  ...
  FOR v_item IN SELECT * FROM purchase_return_items WHERE purchase_return_id = p_purchase_return_id LOOP
    IF v_item.product_id IS NOT NULL THEN
      INSERT INTO inventory_transactions (
        ...
        'مرتجع مشتريات للمورد' || CASE WHEN v_pr.return_date IS NOT NULL
          THEN ' — تاريخ المرتجع: ' || v_pr.return_date::text ELSE '' END,
        v_branch_id, v_cc_id, v_pr.warehouse_id
      );
    END IF;
    IF v_item.bill_item_id IS NOT NULL THEN
      UPDATE bill_items SET returned_quantity = COALESCE(returned_quantity, 0) + v_item.quantity
  ...
  -- FIX 3: ALWAYS create vendor_credits when v_vendor_credit_amount > 0
  ...
  THEN
    ...
    v_round_fix := v_vendor_credit_amount - (v_vc_sub + v_vc_tax);
    ...
    INSERT INTO vendor_credits (
      ...
      v_vc_sub, v_vc_tax, v_vendor_credit_amount, 0,
      v_branch_id, v_cc_id,
      CASE
        WHEN v_pr.settlement_method IN ('cash', 'bank_transfer')
          THEN 'إشعار دائن — في انتظار استرداد نقدي/بنكي ' || COALESCE(v_pr.return_number, '')
        ELSE 'إشعار دائن — جزء المرتجع فوق رصيد AP المفتوح ' || COALESCE(v_pr.return_number, '')
      END
    ...
    SELECT v_credit_id, pri.product_id, pri.description, pri.quantity, pri.unit_price,
           pri.tax_rate, pri.discount_percent, ROUND(COALESCE(pri.line_total, 0) * v_vc_ratio, 2)
    FROM purchase_return_items pri WHERE pri.purchase_return_id = p_purchase_return_id;
  END IF;

  UPDATE purchase_returns SET workflow_status = 'completed', updated_at = NOW()
  WHERE id = p_purchase_return_id;
