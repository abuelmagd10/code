-- =============================================================================
-- Migration: 20260318110000_fix_post_bill_allow_direct_post
-- Purpose  : Fix post_purchase_transaction RPC to set app.allow_direct_post='true'
--            before inserting into journal_entries.
--            Required because enforce_je_integrity trigger blocks direct INSERT
--            of journal entries with status='posted' unless this flag is set.
-- =============================================================================
...
BEGIN
  ...
  IF p_transaction_type = 'post_bill' THEN
    ...
    IF p_inventory_transactions IS NOT NULL AND jsonb_array_length(p_inventory_transactions) > 0 THEN
      ...
      SELECT
        p_company_id,
        NULLIF(t->>'branch_id', '')::UUID, NULLIF(t->>'warehouse_id', '')::UUID,
        NULLIF(t->>'cost_center_id', '')::UUID, NULLIF(t->>'product_id', '')::UUID,
        t->>'transaction_type', COALESCE((t->>'quantity_change')::NUMERIC, 0),
        NULLIF(t->>'unit_cost', '')::NUMERIC, NULLIF(t->>'total_cost', '')::NUMERIC,
        COALESCE(NULLIF(t->>'reference_id', '')::UUID, p_bill_id),
        COALESCE(NULLIF(t->>'reference_type', ''), 'bill'),
        NULLIF(t->>'journal_entry_id', '')::UUID, t->>'notes',
    ...
    IF p_journal_entry IS NOT NULL THEN
      ...
      IF p_journal_entry->'lines' IS NOT NULL THEN
        ...
        SELECT v_je_id, (l->>'account_id')::UUID, l->>'description',
          COALESCE((l->>'debit_amount')::NUMERIC, 0), COALESCE((l->>'credit_amount')::NUMERIC, 0),
          NULLIF(l->>'branch_id', '')::UUID, NULLIF(l->>'cost_center_id', '')::UUID
    ...
    IF p_bill_update IS NOT NULL AND p_bill_id IS NOT NULL THEN
      UPDATE bills SET status = COALESCE(NULLIF(p_bill_update->>'status', ''), status), updated_at = NOW()
      WHERE id = p_bill_id;
    END IF;
  ...
  ELSIF p_transaction_type = 'bill' THEN
    INSERT INTO bills (
      ...
      bill_number, bill_date, due_date, status, subtotal, tax_amount, total_amount, paid_amount, notes
    )
    SELECT p_company_id, (p_bill_data->>'supplier_id')::UUID,
      NULLIF(p_bill_data->>'branch_id', '')::UUID, NULLIF(p_bill_data->>'cost_center_id', '')::UUID,
      NULLIF(p_bill_data->>'warehouse_id', '')::UUID, p_bill_data->>'bill_number',
      (p_bill_data->>'bill_date')::DATE, NULLIF(p_bill_data->>'due_date', '')::DATE,
      COALESCE(NULLIF(p_bill_data->>'status', ''), 'draft'),
      COALESCE((p_bill_data->>'subtotal')::NUMERIC, 0), COALESCE((p_bill_data->>'tax_amount')::NUMERIC, 0),
      COALESCE((p_bill_data->>'total_amount')::NUMERIC, 0), 0, p_bill_data->>'notes'
    RETURNING id INTO v_bill_id;
    v_result := jsonb_set(v_result, '{bill_id}', to_jsonb(v_bill_id));

    IF p_bill_items IS NOT NULL THEN
      INSERT INTO bill_items (bill_id, product_id, description, quantity, unit_price, tax_rate, discount_percent, line_total)
      SELECT v_bill_id, NULLIF(bi->>'product_id', '')::UUID, bi->>'description',
    ...
    IF p_journal_entry IS NOT NULL THEN
      INSERT INTO journal_entries (
        company_id, branch_id, cost_center_id, entry_date, description, reference_id, reference_type, status
      ) VALUES (
        p_company_id, NULLIF(p_journal_entry->>'branch_id', '')::UUID,
        NULLIF(p_journal_entry->>'cost_center_id', '')::UUID,
        (p_journal_entry->>'entry_date')::DATE, p_journal_entry->>'description',
        v_bill_id, 'bill', COALESCE(NULLIF(p_journal_entry->>'status', ''), 'posted')
      ) RETURNING id INTO v_je_id;
      IF p_journal_entry->'lines' IS NOT NULL THEN
    ...
    IF p_inventory_transactions IS NOT NULL THEN
      INSERT INTO inventory_transactions (
        company_id, product_id, transaction_type, quantity_change, reference_id, reference_type,
        journal_entry_id, notes, branch_id, cost_center_id, warehouse_id, transaction_date
      )
      SELECT p_company_id, NULLIF(inv_tx->>'product_id', '')::UUID, inv_tx->>'transaction_type',
        COALESCE((inv_tx->>'quantity_change')::NUMERIC, 0), v_bill_id, 'bill', v_je_id, inv_tx->>'notes',
        NULLIF(inv_tx->>'branch_id', '')::UUID, NULLIF(inv_tx->>'cost_center_id', '')::UUID,
        NULLIF(inv_tx->>'warehouse_id', '')::UUID, COALESCE((inv_tx->>'transaction_date')::DATE, CURRENT_DATE)
  ...
  ELSIF p_transaction_type = 'purchase_return' THEN
    IF p_journal_entry IS NOT NULL THEN
      INSERT INTO journal_entries (
        ...
        NULLIF(p_journal_entry->>'cost_center_id', '')::UUID,
        (p_journal_entry->>'entry_date')::DATE, p_journal_entry->>'description',
        p_bill_id, 'purchase_return', COALESCE(NULLIF(p_journal_entry->>'status', ''), 'posted')
      ) RETURNING id INTO v_je_id;
      IF p_journal_entry->'lines' IS NOT NULL THEN
        INSERT INTO journal_entry_lines (journal_entry_id, account_id, description, debit_amount, credit_amount)
        SELECT v_je_id, (jl->>'account_id')::UUID, jl->>'description',
    ...
    IF p_purchase_return IS NOT NULL THEN
      INSERT INTO purchase_returns (
        company_id, supplier_id, bill_id, journal_entry_id, return_number, return_date, status,
        subtotal, tax_amount, total_amount, settlement_method, reason, notes, branch_id, cost_center_id, warehouse_id
      ) VALUES (
        p_company_id, NULLIF(p_purchase_return->>'supplier_id', '')::UUID, p_bill_id, v_je_id,
        p_purchase_return->>'return_number', (p_purchase_return->>'return_date')::DATE,
        COALESCE(NULLIF(p_purchase_return->>'status', ''), 'completed'),
        COALESCE((p_purchase_return->>'subtotal')::NUMERIC, 0), COALESCE((p_purchase_return->>'tax_amount')::NUMERIC, 0),
        COALESCE((p_purchase_return->>'total_amount')::NUMERIC, 0),
        p_purchase_return->>'settlement_method', p_purchase_return->>'reason', p_purchase_return->>'notes',
        NULLIF(p_purchase_return->>'branch_id', '')::UUID, NULLIF(p_purchase_return->>'cost_center_id', '')::UUID,
        NULLIF(p_purchase_return->>'warehouse_id', '')::UUID
      ) RETURNING id INTO v_pr_id;
      v_result := jsonb_set(v_result, '{purchase_return_id}', to_jsonb(v_pr_id));
      IF v_je_id IS NOT NULL THEN UPDATE journal_entries SET reference_id = v_pr_id WHERE id = v_je_id; END IF;
    END IF;

    IF p_vendor_credit IS NOT NULL THEN
      INSERT INTO vendor_credits (
        company_id, supplier_id, bill_id, source_purchase_return_id, source_purchase_invoice_id,
        journal_entry_id, credit_number, credit_date, status, subtotal, tax_amount, total_amount, applied_amount,
        branch_id, cost_center_id, warehouse_id, notes
      ) VALUES (
        p_company_id, NULLIF(p_vendor_credit->>'supplier_id', '')::UUID, p_bill_id, v_pr_id, p_bill_id, v_je_id,
        p_vendor_credit->>'credit_number', COALESCE((p_vendor_credit->>'credit_date')::DATE, CURRENT_DATE), 'open',
        COALESCE((p_vendor_credit->>'subtotal')::NUMERIC, 0), COALESCE((p_vendor_credit->>'tax_amount')::NUMERIC, 0),
        COALESCE((p_vendor_credit->>'total_amount')::NUMERIC, 0), 0,
        NULLIF(p_vendor_credit->>'branch_id', '')::UUID, NULLIF(p_vendor_credit->>'cost_center_id', '')::UUID,
    ...
    IF p_inventory_transactions IS NOT NULL THEN
      ...
      SELECT p_company_id, NULLIF(inv_tx->>'product_id', '')::UUID, inv_tx->>'transaction_type',
        COALESCE((inv_tx->>'quantity_change')::NUMERIC, 0), COALESCE(v_pr_id, p_bill_id), 'purchase_return',
        v_je_id, inv_tx->>'notes', NULLIF(inv_tx->>'branch_id', '')::UUID, NULLIF(inv_tx->>'cost_center_id', '')::UUID,
        NULLIF(inv_tx->>'warehouse_id', '')::UUID, COALESCE((inv_tx->>'transaction_date')::DATE, CURRENT_DATE)
      FROM jsonb_array_elements(p_inventory_transactions) AS inv_tx;
    END IF;
    ...
    IF p_bill_update IS NOT NULL AND p_bill_id IS NOT NULL THEN
      UPDATE bills SET
        ...
        status = COALESCE(NULLIF(p_bill_update->>'status', ''), status), updated_at = NOW()
      WHERE id = p_bill_id;
    END IF;
    ...
    IF p_update_source->'bill_items_update' IS NOT NULL THEN
      FOR item_update IN SELECT * FROM jsonb_array_elements(p_update_source->'bill_items_update') LOOP
        UPDATE bill_items
        SET returned_quantity = COALESCE(returned_quantity, 0) + COALESCE((item_update->>'returned_quantity')::NUMERIC, 0)
...
COMMENT ON FUNCTION post_purchase_transaction(TEXT, UUID, UUID, JSONB, JSONB, JSONB, JSONB, JSONB, JSONB, JSONB, JSONB, JSONB) IS
  'Purchase transaction RPC. Sets app.allow_direct_post=true to bypass enforce_je_integrity trigger.
   Supports: post_bill (receipt approval with inventory+journal), bill (create new), purchase_return.';
