...
async function testFilter() {
  ...
  async function testOmit(fieldName) {
    ...
    const payload = {
      ...
      title: 'Test Omit ' + fieldName,
      message: 'Test message',
      priority: 'high',
      status: 'unread',
      created_at: new Date().toISOString(),
      event_key: 'test_omit_' + fieldName + '_' + testId,
      severity: 'warning',
      category: 'approvals',
      channel: 'in_app',
    ...
    await s.from('notifications').insert(payload);
    const res = await s.rpc('get_user_notifications', {
      p_user_id: accountantId,
      p_company_id: companyId,
      p_branch_id: branchId
    });
    console.log(`Omitting ${fieldName} => Found: ${res.data?.some(x => x.id === testId)}`);
    await s.from('notifications').delete().eq('id', testId);
  }

  await testOmit('warehouse_id');
  await testOmit('message');
  await testOmit('priority');
  await testOmit('severity');
  await testOmit('category');
  await testOmit('channel');
  await testOmit('retry_count');
}
testFilter();
