...
async function testFilter() {
  ...
  async function testOmit(fieldName) {
    ...
    const payload = {
      ...
      title: 'Test Null ' + fieldName,
      message: 'Test message',
      status: 'unread',
      event_key: 'test_null_' + fieldName + '_' + testId,
      created_by: accountantId,
    };
    
    payload[fieldName] = null; // explicitly set to null
    ...
    console.log(`Setting ${fieldName} = null => Found: ${res.data?.some(x => x.id === testId)}`);
    await s.from('notifications').delete().eq('id', testId);
  }

  await testOmit('message');
  await testOmit('created_by');
}
testFilter();
