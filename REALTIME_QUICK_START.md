...
2. اختر مشروعك
3. اضغط على **Database** من القائمة الجانبية
4. اضغط على **Replication** من القائمة الفرعية
5. ابحث عن الجدول المطلوب (مثل `inventory_write_offs`)
6. فعّل مفتاح التبديل في عمود **Realtime**

**راجع `SUPABASE_REALTIME_SETUP.md` للدليل التفصيلي**

#### الطريقة 2: من SQL Editor (متقدم)

1. اذهب إلى **SQL Editor** في Supabase Dashboard
2. انسخ محتوى `scripts/enable_realtime_tables.sql`
...
|--------|----------|--------|
| `notifications` | ⭐⭐⭐⭐⭐ | ✅ مطلوب (يعمل حالياً) |
| `inventory_write_offs` | ⭐⭐⭐⭐⭐ | ✅ مطلوب (للإهلاك) |
| `inventory_transactions` | ⭐⭐⭐⭐ | ⏳ اختياري |
| `purchase_orders` | ⭐⭐⭐⭐ | ⏳ اختياري |
| `sales_orders` | ⭐⭐⭐⭐ | ⏳ اختياري |
| `invoices` | ⭐⭐⭐⭐ | ⏳ اختياري |
| `approval_workflows` | ⭐⭐⭐ | ⏳ اختياري |
