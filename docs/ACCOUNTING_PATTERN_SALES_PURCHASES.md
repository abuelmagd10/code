...
### 📌 النمط المحاسبي الصارم (ERP Professional)

**❌ لا COGS في أي مرحلة**
- COGS يُحسب عند الحاجة من `cost_price × quantity` المباع
- لا قيد `invoice_cogs` أو `sales_return_cogs` أو `credit_note_cogs`
...
**Paid / Partially Paid**
- ✅ إنشاء قيد الدفع فقط (`reference_type = 'invoice_payment'`).
...
**مرتجع فاتورة Sent**
- ✅ إعادة الكميات المرتجعة إلى المخزون (`inventory_transactions` من نوع `"sale_return"`).
- ❌ لا قيد محاسبي.
- ❌ لا Customer Credit.

**مرتجع فاتورة Paid / Partially Paid**
- ✅ إعادة الكميات المرتجعة إلى المخزون (`sale_return`).
- ✅ إنشاء قيد مرتجع (`reference_type = 'sales_return'`).
- ✅ إنشاء رصيد دائن للعميل (`customer_credits`) إذا المدفوع > الصافي بعد المرتجع.
- ❌ لا COGS reversal.
