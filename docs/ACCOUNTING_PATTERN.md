...
## 🛡️ فحوصات سلامة البيانات المحاسبية (Integrity Checks)

### 📌 القواعد الأساسية:

#### 1️⃣ حساب صافي الربح (Net Income):
```javascript
// ✅ صحيح
netIncome = income - expense

// ❌ خطأ (لا تستخدم الجمع!)
netIncome = income + expense  // خطأ جسيم!
```

#### 2️⃣ طبيعة الحسابات (Normal Balance):
| نوع الحساب | الطبيعة | حساب الرصيد |
|------------|---------|-------------|
| **الأصول (asset)** | مدين (debit) | `debit - credit` |
| **المصروفات (expense)** | مدين (debit) | `debit - credit` |
| **الالتزامات (liability)** | دائن (credit) | `credit - debit` |
| **حقوق الملكية (equity)** | دائن (credit) | `credit - debit` |
| **الإيرادات (income)** | دائن (credit) | `credit - debit` |

#### 3️⃣ المعادلة المحاسبية:
```
الأصول = الالتزامات + حقوق الملكية + صافي الربح
الأصول = الالتزامات + حقوق الملكية + (الإيرادات - المصروفات)
```

### 🛡️ Database Constraints & Triggers:

| الحماية | الوظيفة |
|---------|---------|
| `NOT NULL normal_balance` | كل حساب يجب أن يكون له طبيعة محددة |
| `chk_normal_balance` | القيم المسموحة: `debit` أو `credit` فقط |
| `trg_validate_normal_balance` | التحقق من تطابق الطبيعة مع نوع الحساب |
| `trg_check_journal_balance` | تحذير عند القيود غير المتوازنة |
