...
Sentry.init({
  ...
  ignoreErrors: [
    // Supabase RLS noise — those are expected on access-denied; we already
    // surface them as 403 to the user.
    "JWT expired",
    "No API key found in request",
  ],

  beforeSend(event) {
    // Strip Authorization headers in case they get into breadcrumbs
