...
## [3.74.833] - 2026-07-26 — الإصدار يُجمّد لقطة المواد ويحجز المخزون · ولا استلام بلا صرف

**أخطر ما ظهر فى الاختبار الحى حتى الآن.**

### الفجوة الأولى: لقطة المواد لم تكن تُنشأ فى الاستخدام الفعلى **أبداً**

«لقطة احتياجات المواد» هى قائمة المواد **مجمَّدة على الأمر** لحظة إصداره
(فتعديل القائمة غداً لا يُغيّر أمراً جارياً)، وعليها يُبنى **حجز المخزون**.

كانت تُنشأ فى مسار واحد: `POST .../[id]/sync-materials` — و**لا يناديه إلا
ملفات الاختبار**. لا زر فى الشاشة، ولا مسار آخر، ولا الإصدار. فعملياً:

- لا لقطة ⇒ **صرف الخامات يتعذّر** و**استلام المنتج يتعذّر**، برسالة
  إنجليزية خام فى آخر السلسلة.
- لا حجز ⇒ **نفس المخزون قابل للبيع أو الصرف لأمر آخر** بينما هو محسوب لأمر
  إنتاج جارٍ — التزام مزدوج بنفس الكمية.

⚠️ وأخطر ما فيها أن **اختبار المسار الذهبى كان أخضر** — لأنه ينادى
`sync-materials` بنفسه. اختبار يمرّ على طريق **لا يسلكه أى مستخدم**.

**العلاج**: الإصدار يُجمّد اللقطة ويحجز المخزون **فى نفس المعاملة** — إما أن
يُصدَر الأمر ومواده محجوزة أو لا يُصدَر. وشروط التجميد تُفحص **قبل** الإصدار
...
دالة الاستلام لم تتحقق أبداً من صرف أى خامة. ولو حدث:

| الأثر | |
|---|---|
| تكلفة المنتج = تكلفة التحويل فقط | **مخزون تام ناقص القيمة** |
| الخامات تبقى كأنها لم تُستهلك | **مخزون خام مبالغ فيه** |
| والنتيجة | **ربح غير حقيقى عند البيع** |

حارس جديد يمنع ذلك برسالة تقول ما يُفعل («اصرف الخامات أولاً»)، ونفس المنع
أُضيف فى المسار (422) فيُرفض **الطلب عند إنشائه** بدل أن يُرسل إشعاراً
لمسؤول المخزن بطلب مستحيل التنفيذ.

### إصلاح البيانات القائمة

`MPO-202607-000029` كان «قيد التنفيذ» بلقطة صفرية — مُصدَر بلا حجز. حُضِّرت
احتياجاته: **٢ مكوّنات (زيت تصنيع ١ · قاعدة ماتور ١)** وحجز
**fully_reserved**. والترحيل يفعل الشىء نفسه لأى أمر آخر فى نفس الحالة،
ويُسجّل تحذيراً لما يتعذّر بدل أن يتوقف.

---

## [3.74.832] - 2026-07-26 — تعديل أمر معتمد يُعيده للاعتماد (كان مستحيلاً بحارس)

**تكملة 830**: بعد إصلاح الأعمدة الوهمية صار القيد يُكتب على الأعمدة
الصحيحة — فظهر العائق التالى، **وهو منطقى لا تقنى**:
`انتقال غير مسموح: القديم=approved، الجديد=pending_approval`

### تصميمان متعارضان فى نفس النظام

- **المسار** يقول: تعديل أمر معتمد **يُبطل اعتماده** ويُعيده للدورة — وهذا
  ضابط سليم: الاعتماد أُعطى لبيانات معيّنة، فإن تغيّرت صار الاعتماد يغطى
  **ما لم يُعتمد**.
- **جدول الانتقالات فى القاعدة** يعتبر `approved` **حالة نهائية**، فيستحيل
  الإرجاع ويفشل التعديل كله.

⇒ دورة «عدّل ⇒ يعود للاعتماد» **لم تكن ممكنة أصلاً** — لا بالأعمدة الوهمية
وحدها (830)، بل بحارس يرفض الانتقال نفسه.

### العلاج — مسموح، ومحدود بالمسودة

| الحالة | التعديل |
|---|---|
| **قبل الإصدار** (مسودة) | يُبطل الاعتماد ويُعيد الدورة ✔ |
| **بعد الإصدار** | **مرفوض** — المواد تحركت، فلا يُعاد فتح الاعتماد بأثر رجعى؛ ألغِ الأمر وأنشئ غيره |

والحد آمن **مضاعفاً**: حارس تجميد الترويسة يمنع أصلاً تعديل مستودع أو قائمة
مواد على أمر مُصدَر.

**تحقق على قاعدة الاختبار** — ما يبقى مرفوضاً: معتمد ⇒ مرفوض ✗ ·
معتمد ⇒ مسودة ✗. وما يُسمح: مسودة ⇒ بانتظار ✓ · بانتظار ⇒ معتمد ✓ ·
مرفوض ⇒ بانتظار ✓ · **معتمد ⇒ بانتظار ✓ (الجديد)**.

ورسائل الحارس الثلاث صارت ثنائية اللغة.

---

## [3.74.831] - 2026-07-26 — ملف القفل لم يُرفع: البناء يسقط قبل أن يبدأ

الـCI يفشل عند أول خطوة، `npm ci`:
`Missing: pg@8.22.0 from lock file`

### السبب — الجذر نفسه للمرة الثالثة فى يومين
...
`npm install` — يرفض أى تفاوت ويُسقط البناء كله **قبل أن يبدأ**.

**وهذه ثالث صورة لنفس الجذر**: سكربتات النشر تستخدم
`git add -- <ملفات محددة>` — انضباط يمنع رفع النسخ الاحتياطية والأسرار
**ويُحفظ** — لكنه **يُسقط صامتاً كل ملف لا يذكره أحد**:

| # | الضحية | النشرة |
|---|---|---|
| 1 | `scripts/ai-governance-audit.js` — لم يُرفع أصلاً | 828 |
| 2 | `scripts/check-phantom-columns.js` — أمسكه الحارس قبل النشر | 830 |
| 3 | **`package-lock.json`** | 831 |

### العلاج

- **ملف القفل رُفع.**
- **حارس ثالث** `check-lockfile-in-sync.js`، وفيه الدرس المستفاد من
  محاولة أولى قاصرة: مقارنة **ملفات العمل** لا تكفى — فهى متطابقة على جهاز
  المطور دائماً. السؤال الحقيقى ليس «هل الملفان متطابقان عندى؟» بل **«هل
  التطابق سيصل؟»**. فالحارس يفحص الاثنين: تطابق التبعيات، **وأن ملف القفل
  ليس فيه تعديل غير مرفوع** (`git diff` لا `git status` — فالمجهَّز سيصل،
  وغير المجهَّز وحده هو الخطر).

### وصورة رابعة ظهرت فوراً: مدخلات السكربت لم تصل

بعد رفع سكربت التدقيق (828) فشل الـCI ثانيةً — لأن السكربت **يقرأ**
`knowledge/api/routes.md` وهو غير مرفوع أيضاً:
`ENOENT: no such file or directory`.

**والملف موجود فعلاً** — كتالوج حقيقى بـ**1,349 سطراً يوثّق 437 مساراً**.
لذلك رُفض الحل السريع المقترح (إنشاء ملف فارغ): التدقيق يستعمله لكشف
المسارات غير المستخدمة، فملف فارغ يجعله يعتبر **كل** المسارات مهجورة —
مئات النتائج الزائفة. الصواب رفع الملف القائم.

`check-referenced-scripts-tracked.js` صار يفحص **ما تقرؤه السكربتات** لا
السكربتات وحدها (`knowledge/` · `docs/` · `supabase/schema/`) — ويُجرّد
التعليقات قبل الفحص، لأن التعليق الشارح يقتبس المسار فيُبلَّغ عن مرجع لا
وجود له فى الكود.

---

## [3.74.830] - 2026-07-26 — 🚨 كتابة أعمدة لا وجود لها: دورة إعادة الاعتماد لم تعمل قط

**كُشف أثناء الاختبار الحى** — حفظ مستودع الاستلام على أمر إنتاج **معتمد**:
```
Could not find the 'cycle_no' column of 'manufacturing_production_orders'
```

### السبب

المسار كان يكتب — عند تعديل سجل معتمد — **ثلاثة أعمدة لا وجود لها**:
`cycle_no` (مكانها جدول `approval_history` وحده) و`po_approved_by`
و`po_approved_at` (الأعمدة الحقيقية `approved_by` و`approved_at`).

⇒ **دورة «عدّل ⇒ يعود للاعتماد» لم تعمل ولا مرة** — وليس فى أوامر الإنتاج
وحدها، بل فى **نسخ قوائم المواد ونسخ المسارات** أيضاً: ثلاثة مسارات بنفس
العطب حرفياً.

**ولماذا عاشت طويلاً؟** لأنها لا تفشل إلا **وقت التشغيل**، وفقط على المسار
الذى **يكون السجل فيه معتمداً بالفعل** — أندر الحالات أثناء التطوير.

### العلاج

- الثلاثة تكتب الآن الأعمدة الحقيقية، و**رقم الدورة يُشتق من
  `approval_history`** (مصدره الوحيد) بدل عمود وهمى كان يعطى 2 دائماً.
- **حارس جديد** `check-phantom-columns.js`: يقارن كل `.update({...})` فى
  مسارات الـAPI بأعمدة الجدول من snapshot المخطط، على **خط أساس (56)** —
  فأى كتابة **جديدة** لعمود وهمى تكسر البناء فوراً.

### وما كشفه الحارس فوراً

**~45 كتابة أخرى** لأعمدة وهمية — تحققتُ من **22 منها على قاعدة الإنتاج
مباشرةً: كلها مفقودة فعلاً**. أخطرها مسارات يستعملها المستخدم:
**قبول دعوة العضوية** · **ربط العمولات بالمرتب** · **إقفال الفترة
المحاسبية** · **إدارة الاشتراك** · ومديول **MRP** بستة أعمدة.

مسجَّلة كمشروع معالجة بند بند (لا دفعة واحدة) فى دفتر التسليم.

---

## [3.74.829] - 2026-07-26 — إصدار أمر الإنتاج: مطلوب مخفى خلف «إعدادات متقدمة»

**كُشف أثناء الاختبار الحى** — ضغط «إصدار الأمر للتنفيذ» فظهر:
`Production order release requires issue and receipt warehouses.`

### السبب ليس فى الحارس بل فى النموذج

مستودع **صرف الخامات** ومستودع **استلام المنتج التام** شرطان للإصدار —
بدونهما لا يعرف النظام من أين تخرج المواد ولا أين يدخل المنتج. ومع ذلك
كانا مدفونين داخل قسم **مطوى** اسمه «إعدادات متقدمة»، **بلا نجمة، بلا
تحقق**. فيُنشئ المستخدم الأمر ويبدو سليماً (مسودة · معتمد)، ثم يصطدم
بالجدار عند التنفيذ — برسالة إنجليزية تذكر معرّف الأمر ولا تقول **أى**
مستودع ينقص ولا **أين** يُضبط.

(الأمر MPO-202607-000029 كان ينقصه مستودع الاستلام وحده.)

### العلاج بثلاث طبقات

1. **النموذج**: القسم لم يعد «متقدماً» — صار **«المستودعات (مطلوبة)»**
   مفتوحاً افتراضياً، بنجمة حمراء على كل حقل، مع تحقق يمنع الإنشاء ناقصاً
   **ويسمّى الناقص تحديداً** ويفتح القسم عنده.
2. **المسار**: يفحص المستودعين قبل النداء ويقول أيهما ينقص وأين يُضبط.
3. **الحارس**: رسائله الثلاث (أمر غير مسودة · مستودعان ناقصان · بلا خطوات)
   صارت **ثنائية اللغة** بكود `check_violation`.

**الدرس**: الحقل المطلوب لا يُخفى خلف قسم مطوى — طىّ المطلوب يجعل المستخدم
يكمل النموذج وهو يظن أنه أتمّه.

---

## [3.74.828] - 2026-07-26 — 🚨 الـCI كان يفشل لأن سكربتاً لم يُرفع أصلاً

**ملاحظة المالك**: وظيفة `AI Governance Audit` تفشل، ومعها **يُتخطى النشر**.

### السبب — ليس عطباً فى الكود

```
Error: Cannot find module '/home/runner/work/code/code/scripts/ai-governance-audit.js'
```

الملف موجود على جهاز التطوير و**غير مرفوع إلى git إطلاقاً**. الـCI يستدعى
سكربتاً لا وجود له فى المستودع.

**ولماذا سقط؟** لأن سكربتات النشر تستخدم `git add -- <ملفات محددة>` عمداً —
انضباط أنقذنا مراراً من رفع نسخة احتياطية أو ملف أسرار. لكن ثمنه أن أى ملف
**جديد** لا يذكره أحد فى القائمة **يسقط صامتاً**. الانضباط سليم؛ الناقص كان
حارساً يمسك ما يسقط منه.

### العلاج

- **الملف رُفع** (`scripts/ai-governance-audit.js`).
- **حارس جديد** `check-referenced-scripts-tracked.js`: يقرأ كل سكربت يناديه
  `package.json` أو `.github/workflows/*.yml`، ويفشل إن كان موجوداً محلياً
  وغير متتبَّع فى git — أو مذكوراً ولا وجود له. يسبق التدقيق فى الـCI، فيسمّى
  الملف الناقص صراحةً بدل أثر مكدس Node.
- **ما أمسكه الحارس فوراً**: أمرا npm `audit:fix` و`inventory:fix` يشيران
  لملفين **غير موجودين إطلاقاً** — أوامر ميتة تتظاهر بالوجود؛ حُذفا.

### وتدقيق الحوكمة: من «صفر مطلق» إلى «لا ديون جديدة»

كان يُسقط الـCI عند أى CRITICAL، والمشروع يحمل **ثلاثة** موروثة. وحين يفشل
كل commit، لا يعود الفشل يعنى شيئاً. صار على خط أساس (**3**) كنظيره فى
`check-unchecked-writes` — وأى CRITICAL **جديد** يكسر البناء فوراً.

**والثلاثة مسجَّلة لا مقبولة** (مشروع تأمين مستقل فى دفتر التسليم):

| المسار | الخطر |
|---|---|
| `/api/subscription/create` | **الأخطر** — إنشاء اشتراك بلا مصادقة إطلاقاً |
| `/api/biometric/attendance/push` | جهاز البصمة يدفع حضوراً بلا سرّ جهاز |
| `/api/bills/:id/journal-entry-id` | يكشف رقم قيد أى فاتورة بتخمين المعرّف |

---

## [3.74.827] - 2026-07-26 — مركز العمل: رسالة تفهمها بدل خطأ إنجليزى خام

**كُشف أثناء الاختبار الحى** — المالك يعدّل مركز عمل ليضبط أسعار الأجور،
فتظهر رسالة حمراء بالإنجليزية كما خرجت من قاعدة البيانات حرفياً:
`capacity_uom and nominal_capacity_per_hour must both be null or both be provided.`

### السبب

«وحدة القياس» و«الطاقة فى الساعة» **زوج** — وحارس القاعدة محقّ فى رفض أن
يُملأ أحدهما دون الآخر (طاقة بلا وحدة رقم بلا معنى). لكن مسارَى الإنشاء
والتعديل كانا يُطبّعان كلاً منهما **على حدة**، فيمر نصف الزوج ويصطدم
بالحارس — فيرى المستخدم رسالة بلغة لا يقرؤها ولا تدله على أى حقل يصلح.

### العلاج بطبقتين

1. **المسار**: يُفحص الزوج **قبل** إرسال الطلب، برسالة عربية تسمّى الحقلين
   وتقول ماذا يفعل — «أكمل الناقص أو امسح الاثنين». ولا يُسقط ما كتبه
   المستخدم صامتاً.
2. **الحارس**: رسائله الست صارت **ثنائية اللغة** بكود `check_violation` —
   فلو تسرّب أى مسار مستقبلى من الفحص، تصل رسالة مفهومة لا طلسم.

---

## [3.74.826] - 2026-07-25 — الحجوزات تفتح على «الحالات النشطة» لا على السجل كله

بعد إصلاح 825 بقى السبب قائماً: **الزحام نفسه**. خمسة من ثمانية حجوزات فى
شركة المالك ملغاة — أى **٦٢٪ من التقويم سجلّ تاريخى** يدفع الحجوزات الحقيقية
خلف التمرير.

**العلاج على مستوى النظام لا على مستوى العادة**: خيار جديد **«الحالات النشطة
(بدون ملغى)»** صار **الافتراضى** فى شاشة الحجوزات — يستبعد `cancelled`
و`no_show`. و«جميع الحالات» باقية لمن يريد السجل كاملاً، وكل حالة مفردة كما
هى.

يسرى على العروض الثلاثة معاً (جدول · تقويم · كانبان) لأنها تقرأ نفس الفلتر.

---

## [3.74.825] - 2026-07-25 — تقويم الحجوزات: «+N أخرى» صارت تمريراً

**ملاحظة المالك**: يوم الخميس ١٦ فيه أربعة حجوزات، يعرض التقويم ثلاثة ثم
«+1 أخرى» — والرابع لا سبيل لرؤيته.

### الفجوة

خلية اليوم كانت تقص القائمة عند ثلاثة (`slice(0, 3)`) وتكتب «+N أخرى»
**كنص جامد لا يُنقر ولا يفتح شيئاً** — فالحجز الرابع مخفى تماماً من التقويم،
ولا طريقة للوصول إليه إلا بمغادرة العرض كله إلى الجدول أو الكانبان.

### العلاج

- تُعرض **كل** حجوزات اليوم داخل الخلية مع **تمرير رأسى**، وارتفاع محدود
  (132px) فلا تتمدد الخلية وتكسر شبكة الشهر.
- **عدّاد** بجوار اسم اليوم يظهر عند تجاوز ثلاثة — فتعرف أن هناك ما يستحق
  التمرير بدل أن تكتشفه بالمصادفة.
- شريط تمرير رفيع (1.5px) يتضح عند المرور، ويحترم الوضعين الفاتح والداكن
  واتجاه RTL.

---

## [3.74.824] - 2026-07-25 — 🚨 قيود التصنيع كانت تُولد ميتة ولا تُرحَّل أبداً

وصل إليه الخيط من سؤال بسيط أثناء مراجعة التقارير الاسترشادية: **لماذا يوجد
قيدان بحالة «مسودة» فى قاعدة الإنتاج؟**

### الفجوة

`createEntryHeader` تُنشئ قيد التصنيع بحالة `draft`، **ولم يكن فى الملف كله
سطر واحد يحوّله إلى `posted`**:
...
### ولماذا بدت الدورة التجريبية سليمة؟

لأن حارساً قديماً كان يُرحّل قيد الاستلام بالتوازى (JE-000056). وفى **814**
أسقطناه بوصفه «ازدواجاً» وعكسنا قيده — **وهو فى الحقيقة كان المُرحِّل
الوحيد**. صحّت الأرصدة **بالمصادفة وحدها**: صرف مواد 60 لم يُرحَّل،
واستلام 60 رُحّل ثم عُكس ⇒ الصافى صفر — وهو نفس صافى الدورة الصحيحة **لأن
تكلفة التحويل كانت صفراً**. مع أول مركز عمل بسعر حقيقى (بعد 818) كان الفارق
سيظهر انحرافاً دائماً.

### العلاج

- **الكود**: `postEntryHeader` تُرحّل القيد بعد إدراج سطوره، وتتحقق من
  التوازن **من قاعدة البيانات** لا من المصفوفة فى الذاكرة، وتتراجع عن القيد
  بالكامل إن اختل — فلا رأس قيد يتيم ولا قيد مشوَّه.
- **أربعة تقارير محاسبية** كانت تستثنى المحذوف **ولا تفلتر الحالة** ⇒
  المسودات تتسرب للأرصدة: أرصدة البنوك · **شاشة تحديث الأرصدة (وهى تكتب
  الأرصدة المخزَّنة!)** · فروق الصرف · كشف حركة البنك (بلا أى فلتر). أُضيف
  `status='posted'` و`is_deleted` لكلٍّ منها.

### إصلاح البيانات

المسودتان العالقتان أُلغيتا مع **توثيق السبب داخل وصف القيد نفسه**: أثر
الدورة مُثبت بالفعل عبر JE-000056/57، فترحيلهما الآن يضاعف.

**التحقق**: صفر مسودات حية · ميزان المراجعة **0.00** · المخزون **140.77** =
تقييم الدفعات **140.77** ✓

---

## [3.74.823] - 2026-07-25 — أجور الإنتاج «محمَّلة» لا «مستحقة» + قالب الحسابات

### الفجوة: التزام وهمى + ازدواج تكلفة العمالة

قيد استلام المنتج التام كان يُدائن تكلفة العمالة على **«الرواتب والأجور
المستحقة 2130»** — أى **التزام جديد**. بينما مسار المرتبات يُحمّل الأجر
كاملاً على «الرواتب والأجور 5210» ويدفعه نقداً:

1. **ازدواج تكلفة العمالة**: مرة مصروفاً فى قائمة الدخل، ومرة داخل قيمة
   المخزون ⇒ التكلفة تظهر مرتين والربح مشوَّه.
2. **التزام وهمى دائم**: 2130 يتضخم مع كل أمر إنتاج **ولا مسار يسدده** —
   فتبدو الشركة مدينة لموظفيها بمبالغ **سبق أن دفعتها فعلاً**.

### المعالجة القياسية (Applied Labour)

حساب **مقابل للمصروف** «أجور محمَّلة على الإنتاج 5415» — نظير «أعباء صناعية
محملة 5410» الموجود أصلاً بنفس المنطق:

```
مدين  مخزون المنتج التام        (مواد + تحويل)
    دائن إنتاج تحت التشغيل        (المواد)
    دائن 5415 أجور محمَّلة        (العمالة)
    دائن 5410 أعباء صناعية محملة  (الأعباء)
```

فيُخفَّض صافى مصروف الأجور بما استُوعب منه فى قيمة المخزون: **لا ازدواج ولا
التزام وهمى**، والفرق بين المدفوع والمستوعب يبقى ظاهراً كانحراف عمالة.

### قالب الحسابات — فجوة كانت ستضرب كل شركة جديدة

حسابات نشرات اليوم كانت تُنشأ للشركات القائمة فقط. بلا إضافتها للقالب تولد
كل **شركة جديدة** بلا: تأمينات مستحقة (817)، عمولات مستحقة ومصروف عمولات
(822)، وأجور محمَّلة (823) — فتصطدم بأول دورة رواتب أو عمولة أو أمر إنتاج.
أُضيفت الأربعة للقالب **ولملف البذر** معاً.

**البيانات**: لا سطر واحد رُحّل على 2130 من التصنيع فى أى شركة (أسعار مراكز
العمل كانت صفرية) ⇒ لا تصحيح مطلوب. وقائى بالكامل.

---

## [3.74.822] - 2026-07-25 — عمولات البيع تدخل الدفاتر أخيراً

### الفجوة: العمولة تُحسب وتُخزَّن ولا تدخل الدفاتر إطلاقاً

- **لا مصروف عمولات** ⇒ الربح يبدو أعلى من حقيقته حتى شهر الصرف.
- **لا التزام تجاه الموظف** ⇒ الشركة مدينة بمبلغ **لا يظهر فى أى تقرير ولا
  فى الميزانية**.
- **ولا عكس عند مرتجع المبيعات**: الاسترداد كان يُسجَّل صفاً سالباً فى
  الجدول بلا أى أثر دفترى.

### الدورة الكاملة بثلاث محطات

| المحطة | القيد |
|---|---|
| **عند البيع** | مدين 5215 عمولات ومكافآت البيع / دائن 2136 عمولات مستحقة |
| **عند المرتجع** | مدين 2136 / دائن 5215 (عكس بنفس النسبة) |
| **عند الصرف مع المرتب** | مصروف المرتبات **يستبعد** العمولة، وتُحمَّل على **الالتزام** — فلا ازدواج |

بروفة كاملة على قاعدة الاختبار (ثم rollback): عمولة 200 ⇒ الاستحقاق
[5215 مدين 200 / 2136 دائن 200] ✓ · نداء مكرر أعاد نفس القيد ✓ · ثم مرتب
أساسى 5,000 + عمولة 200 ⇒ **5210 مدين 5,000 فقط** · **2136 مدين 200** ·
**1110 دائن 5,200** — متوازن، والمصروف اعتُرف به **مرة واحدة** ✓

### ✅ إصلاح البيانات

عمولتان بـ**7.00** لكل منهما (INV-2026-00002/3) كانتا معلقتين بلا أثر دفترى
⇒ أُثبتتا (**JE-000065** و **JE-000066**).

**التحقق**: ميزان المراجعة **0.00** ✓ · **التزام العمولات 14.00 ظاهر الآن
فى ميزانيتك** ✓ · صافى الربح **2,420.17** — أى الربح الحقيقى بعد الاعتراف
بالتزاماتك، بدل 2,434.03 الذى كان يتجاهلها.

---

## [3.74.821] - 2026-07-25 — مرتجع المشتريات: مالك واحد للدفعات + إغلاق فجوة الـ0.14

### 🔴 الفجوة الأولى: خفض مزدوج لدفعات المخزون (لم تُختبر بعد)

كانت الدفعات تنقص عند المرتجع **مرتين**:

1. الحارس `trg_fifo_on_purchase_return` المعلَّق على حركة المخزون نفسها (منذ v3.74.702).
2. بلوك مكرر **داخل** `confirm_purchase_return_delivery_v2` يخفض من دفعات الفاتورة يدوياً.

⇒ أى مرتجع يمر بالمسار الحى يستهلك **ضعف الكمية**: مخزون ينضب من العدم،
تكلفة بضاعة مباعة متضخّمة، ثم أخطاء «كمية غير كافية». لم يظهر بعد لأن
المرتجعين الوحيدين نُفذا **قبل** وجود الحارس — والمرتجع القادم كان سيصطدم به.

**مالك واحد للعملية: الحارس** (درس 804 — الكود الزومبى يُعطَّل صراحةً لا
يُترك يعمل بالتوازى). بروفة على قاعدة الاختبار: دفعة 10 وحدات، مرتجع 2 ⇒
المتبقى **8** وسطر استهلاك **واحد** بتكلفة **2.40** ✓

### الفجوة الثانية: القيد بسعر المستند والدفعات بتكلفتها الحقيقية

المورد يرد **ثمن البضاعة** ولا يرد **الشحن المرسمل**، فالفرق صار يُعترف به
**مصروف نقل مشتريات (5140)** بدل تركه انحرافاً صامتاً:

| | مدين | دائن |
|---|---|---|
| الموردون | سعر المستند (ما يرده المورد) | |
| المخزون | | تكلفة FIFO الحقيقية |
| فرق شحن مرتجع | الفارق | |

السطران المضافان متساويان مديناً ودائناً ⇒ القيد متوازن بالبناء.

### ✅ إصلاح البيانات: فجوة الـ0.14 أُغلقت نهائياً

حساب المخزون **140.63** مقابل تقييم الدفعات **140.77** — فجوة لاحقتنا عبر
جلسات. مصدرها: مرتجعان (PRET-5689 و PRET-79328) نُفذا 3 و9 يوليو **قبل**
إصلاح v3.74.702، فسجّلا خروج البضاعة بسعر المستند بدل تكلفة الدفعة.

صُحّحت بقيد تسوية تقييم مخزون (**JE-000064**) لا بتعديل قيد مرحّل:
مدين المخزون 0.14 / دائن مصاريف نقل المشتريات 0.14.
...
## [3.74.820] - 2026-07-25 — ضريبة المدخلات على المصروفات

### الفجوة: لا حقل ضريبة على المصروفات إطلاقاً

قيد المصروف كان سطرين: مدين المصروف بكامل المبلغ / دائن الخزنة بكامله.
فشركة مسجَّلة ضريبياً تدفع مصروفاً بضريبة 14%:

1. **مصروفها متضخّم** بقيمة الضريبة — والضريبة ليست مصروفاً بل **حق لدى
   مصلحة الضرائب** يُخصم من ضريبة المخرجات.
2. **حق الخصم يضيع تماماً**: تقرير ضريبة المدخلات لا يرى المصروفات إطلاقاً
   ⇒ **الشركة تدفع ضريبة أكبر من المستحق عليها فعلاً**.
3. ربحها المحاسبى أقل من الحقيقة بمقدار الضريبة المحمَّلة خطأً.

### العلاج

حقل «ضريبة مدخلات قابلة للخصم» داخل المبلغ (المبلغ يبقى الإجمالى المدفوع)،
والقيد صار ثلاثة أسطر:

| الحساب | مدين | دائن |
|---|---|---|
| حساب المصروف | المبلغ − الضريبة | |
| ضريبة القيمة المضافة - مدخلات | الضريبة | |
| الخزنة/البنك | | الإجمالى المدفوع |

بروفة حية على قاعدة الاختبار: مصروف **1,140** منه **140** ضريبة ⇒ المصاريف
الإدارية 1,000 مدين · ضريبة المدخلات 140 مدين · الصندوق 1,140 دائن —
متوازن ✓ (ثم rollback).

قيد `tax_amount <= amount` يمنع ضريبة أكبر من المبلغ، والحقل يبدأ صفراً لكل
الصفوف القائمة ⇒ **لا قيد تاريخى يتغير** (المصروف الوحيد المرحّل بلا ضريبة).

---

## [3.74.819] - 2026-07-25 — الأصول الثابتة تُثبَّت دفترياً + إبطال مسارين قديمين

### 🚨 شاشة الأصول لم تكن تُرحّل أى قيد اقتناء إطلاقاً

تُدخل الأصل فيُنشأ صف وجدول إهلاك — **بينما الدفاتر لا تعرف عنه شيئاً**:

- النقدية **لا تنقص** رغم دفع ثمن الأصل.
- الأصل **لا يظهر فى الميزانية**.
- ثم يبدأ الإهلاك بالترحيل **على أصل لم يُثبت قط** ⇒ الميزانية تحمل مجمَّع
  إهلاك بلا أصل يقابله، ومصروف إهلاك بلا سند.

**قرار المالك**: يدعم النظام **المسارين باختيار المستخدم** فى شاشة الأصل:

| الاختيار | ماذا يحدث |
|---|---|
| **مُقتنى عبر فاتورة مشتريات** | لا يُرحَّل قيد من هنا — دورة المشتريات أثبتته بموردها وضريبتها وسدادها المحكوم. وترحيله ثانية **مرفوض** بحارس صريح. |
| **تسجيل مباشر** | تختار حساب السداد (خزنة/بنك/مورد) فيُرحّل النظام: **مدين الأصل / دائن الحساب المختار** بالفرع ومركز التكلفة. |

### الحارس الحاسم

`ASSET_NOT_CAPITALISED` — **لا يمكن تفعيل أصل قبل إثباته دفترياً**: إمّا
مربوط بفاتورته أو له قيد اقتناء مُرحَّل. فيستحيل بنيوياً أن يُهلَك أصل لا
وجود له فى الدفاتر.

بروفة على قاعدة الاختبار (ثم rollback) — خمس نتائج: تفعيل قبل الإثبات
**رُفض** ✓ · الاقتناء **مدين 100,000 / دائن 100,000** ✓ · نداء مكرر **أعاد
نفس القيد** ✓ · تفعيل بعد الإثبات **مرّ** ✓ · أصل من فاتورة **رُفض** ✓.

### إضافات الأصل: الحساب المقابل لم يعد مثبّتاً

`register_asset_addition` كان نصّها: *"CREDIT: Bank/Cash (Hardcoded for
prototype)"* على «1110» — فأى إضافة مموّلة من البنك أو على حساب مورد كانت
**تُخصم من الخزنة**. وبلا فرع ولا بوابة ترحيل ولا تحقق فترة ولا حارس صلاحية
شركة. صار الحساب معاملاً، وأُضيفت الأربعة.

### إبطال مسار المسحوبات القديم

`record_shareholder_drawing_atomic` كانت تُدخل السحب بحالة «مُرحَّل» فوراً:
**بلا اعتماد، بلا حارس دور، بلا فحص سحب على المكشوف، بلا فصل مهام** — التفاف
كامل على المسار الحى. استُبدلت برسالة صريحة توجّه لشاشة المسحوبات.

### البيانات

صفر أصول ثابتة وصفر مسحوبات فى الإنتاج ⇒ **لا شىء تاريخى يحتاج تصحيحاً**.
الإصلاح كله وقائى، يصل قبل أول أصل حقيقى وأول مسحوبة.

---

## [3.74.818] - 2026-07-25 — دفعة المنتج التام تحمل تكلفة التحويل

### الفجوة: الدفاتر تحمّل شيئاً والمخزون يُسعَّر بشىء آخر

- **القيد** (`postProductReceiptJournal`) يُدين «المنتج التام» بـ**المواد +
  تكلفة التحويل** (أجور + أعباء صناعية) — وهو الصحيح بمعيار IAS 2، ويُنشئ
  التزام الأجور وحساب الأعباء المحمَّلة.
- **بينما** الدالة الذرية للاستلام تُسعّر حركة المخزون **ودفعة FIFO** من
  تكلفة المواد **وحدها**.

**ثلاث نتائج عند تشغيل أسعار مراكز العمل:**

1. **انحراف دائم** بين حساب المخزون وتقييم الدفعات = تكلفة التحويل، يتراكم
   مع كل أمر إنتاج ويستحيل تفسيره لاحقاً.
2. **تكلفة البضاعة المباعة ناقصة** عند بيع المنتج التام ⇒ **ربح مبالغ فيه**
   فى قائمة الدخل — وهو أخطر أثر على الإطلاق.
3. تقارير تكلفة المنتج تُظهره أرخص من حقيقته ⇒ قرارات تسعير مبنية على وهم.

**لماذا لم يظهر فى الدورة التجريبية؟** مراكز العمل كانت بأسعار صفرية،
فدخل المنتج التام بـ60 (مواد فقط) وخرجت تكلفته 60 — تطابقا **بالمصادفة**.
أول مركز عمل بسعر أجر حقيقى كان سيفتح الفجوة.

### العلاج

`mpoe_conversion_cost` — توأم دقيق فى SQL لمعادلة `calculateConversionCost`
فى الواجهة (العمالة ÷ 60 × السعر × معامل الكفاءة، ودقائق الآلة ÷ 60 ×
أسعار الآلة والأعباء، للعمليات المكتملة وحدها). تُضاف قيمتها لتكلفة
الاستلام **قبل** إنشاء حركة المخزون ودفعة FIFO ⇒ المصدران يتطابقان
**بالبناء لا بالمصادفة**. والاستلام الجزئى يوزّع المواد والتحويل معاً بنسبة
الكمية المستلمة.

طُبِّق بالمرساة الموثقة (تحقق أن المرساة فريدة تماماً + علامة idempotency)
على القاعدتين، ومرساة الإعلان أُخذت بالسطر الكامل لا بالاسم — درس فخ
المرساة غير الفريدة.

---

## [3.74.817] - 2026-07-25 — المرتبات: المديول لم يكن يعمل أصلاً، وقيده كان ناقصاً

أخطر ما فى مراجعة المديولات، لأنه يتكرر لكل موظف كل شهر.

### 🚨 الاكتشاف الأول: صرف المرتبات لم ينجح ولا مرة واحدة

`post_payroll_atomic` كانت تُدخل القيد مباشرة **بلا فتح بوابة الترحيل**،
فيصدّها حارس `enforce_je_integrity` برسالة `DIRECT_POST_BLOCKED`. الدليل
القاطع: **صفر قيود** من نوع `payroll_payment` فى قاعدة الإنتاج رغم وجود
كشوف مرتبات. الدوال الذرية النظيرة تفتح البوابة صراحة داخل معاملتها؛ هذه
أُغفلت. أُضيفت البوابة وتُغلق فور الانتهاء.

### 🚨 الاكتشاف الثانى: القيد كان بالصافى لا بالإجمالى

كان من سطرين: مدين مصروف المرتبات = **صافى** المرتب / دائن الخزنة = الصافى.
ثلاث نتائج خاطئة **لا يكشفها ميزان المراجعة** (لأنه متوازن!):

1. **مصروف المرتبات مُنقَص بقيمة كل الاستقطاعات** — التأمينات والضرائب
   والسلف جزء من تكلفة العمالة على الشركة، لا تختفى لأن الموظف لم يقبضها.
2. **التزامات الشركة تجاه التأمينات والضرائب لا تظهر إطلاقاً** — ما استُقطع
   دَين على الشركة حتى تورده، وكان يختفى من الميزانية.
3. **السلفة المستقطعة لا تُسدَّد دفترياً** — حساب «سلف الموظفين» يبقى بكامل
   رصيده رغم سداد الموظف ⇒ أصل وهمى.

وبلا `branch_id` على أى سطر ⇒ ربحية الفروع تستبعد الأجور، وهى عادةً أكبر
بند مصروف.

**القيد بعد الإصلاح** (بروفة حية على قاعدة الاختبار: إجمالى 10,000):

| الحساب | مدين | دائن |
|---|---|---|
| 5210 الرواتب والأجور (مقسَّم بفرع كل موظف) | 10,000 | |
| 1170 سلف ومقدمات للموظفين | | 1,000 |
| 2135 تأمينات اجتماعية مستحقة *(حساب جديد)* | | 1,200 |
| 2125 استقطاعات مستحقة أخرى | | 500 |
| 1110 الصندوق (الصافى المدفوع) | | 7,300 |
| **المجموع** | **10,000** | **10,000** ✓ |

والتوازن **بالبناء**؛ وإن خالفت بيانات الكشف ذلك يُرفض الصرف برسالة صريحة
ثنائية اللغة بدل ترحيل قيد مشوّه.

### نزع فتيل الدالة الثانية

`post_payroll_run_atomic` — نصّها يعترف حرفياً: *"SIMPLIFICATION: We Don't
handle Deductions logic deeply here"* — تُدين المستحقات وتُدائن الصافى ⇒ قيد
غير متوازن مع أى استقطاع. لا تستدعيها الواجهة، لكنها مسدس محشوّ؛ استُبدلت
برسالة صريحة توجّه للمسار الصحيح.

### البيانات

كشوف المرتبات الأربعة فى الإنتاج كلها بلا استقطاعات (الصافى = الإجمالى)
ولم يُرحَّل لها قيد قط ⇒ **لا قيد تاريخى يحتاج تصحيحاً**. الإصلاح وقائى
بالكامل قبل أول دورة رواتب حقيقية. وحساب «2135 تأمينات اجتماعية مستحقة»
أُنشئ لكل الشركات الأربع.

---

## [3.74.816] - 2026-07-25 — حارسا الحساب التجميعى + إصلاح البيانات القائمة

**توضيح المالك للقاعدة المؤسِّسة**: الأساس إصلاح الفجوة فى النظام حتى لا
تتكرر، **مع إصلاح البيانات القائمة فى الشركات** — لأن نظافة البيانات هى
الدليل العملى على أن الإصلاح نجح على مستوى المشروع لا على الورق.

### الفجوة — الترحيل على رأس شجرة الحسابات

أثناء التحقق من إصلاح تصنيف الإيراد (815-د) ظهر أن ثلاثة أصناف مربوطة
بـ«4000 الإيرادات» و«5000 المصروفات» — **رأسا الشجرة**، حسابان تجميعيان
بحتان لا يُرحَّل عليهما فى أى نظام محاسبى. ولأن بانى القيد فى القاعدة يقرأ
حساب إيراد الصنف، كانت أول فاتورة لأحدها ستضع الإيراد على رأس الشجرة
فتنهار قائمة الدخل (المجموع يساوى نفسه مرتين).

**النظام** — حارسان بثنائية اللغة (هجرة 20260725000004):

- `fn_guard_no_root_account_posting` على `journal_entry_lines` (INSERT فقط،
  فلا يُبطل سطراً تاريخياً).
- `fn_guard_product_accounts_not_root` على `products`.
- التمييز دقيق: 4100/4200/5100/5200 لها أبناء (مردودات/خصومات) لكنها حسابات
  ترحيل مشروعة — الرفض لرأس الشجرة وحده (`parent_id IS NULL` وله أبناء).
- بروفة على قاعدة الاختبار: ترحيل على رئيسى **رُفض** ✓ · على تفصيلى **مرّ** ✓
  · ربط صنف برئيسى **رُفض** ✓ (ثم rollback).

**البيانات** — لكل الشركات لا شركة بعينها:

- ثلاثة أصناف نُقلت من 4000/5000 إلى 4100/5100.
- خامات بسعر بيع ⇒ صفر (نظير إخفاء الحقل فى 815).
- كل مساهم قائم بلا حساب رأس مال/مسحوبات صارت له حساباته مربوطة
  (شركة ثانية كان شريكها بلا حسابات).
- **قيد إعادة تصنيف JE-000063**: خدمة «تقشير» 500 كانت فى «إيرادات المبيعات»
  فنُقلت لـ«إيرادات الخدمات» — بقيد جديد لا بتعديل قيد مرحّل.
  الأثر على الربح والنقدية **صفر**؛ التبويب فقط.

**التحقق بعد الإصلاح**: ميزان المراجعة 0.00 · صافى الربح 2,434.03 دون تغيير
· إيرادات الخدمات 1,900.00 (500+700+700) · إيرادات المبيعات 791.39 · صفر
أصناف على رأس الشجرة · صفر خامات بسعر · صفر مساهم بلا حسابات.

---

## [3.74.815] - 2026-07-25 — مراجعة التقارير والمديولات المحاسبية: سبع فجوات نظام

قاعدة الجلسة (قرار المالك): **«نحن لا نصلح بيانات شركة — نصلح النظام حتى
لا تتكرر الفجوة فى المستقبل.»** كل بند أدناه إصلاح يسرى على كل شركة.

### (أ) فاحص انحراف تقييم المخزون كان يقارن بالسعر الثابت

`ic_inventory_valuation_drift` قارن رصيد حساب المخزون بـ`cost_price` المخزَّن
على الصنف بدل القيمة الحقيقية للدفعات، فأنذر بانحراف 116.63 وهمى. صار يقارن
`SUM(remaining_quantity × unit_cost)` من `fifo_cost_lots` — وهو المصدر نفسه
الذى يقوّم به النظام مخزونه.

### (ب) تقرير التغير فى حقوق الملكية كان يحتسب القيود المؤرشفة

`lib/equity-reporting-service.ts` كان الوحيد بلا فلتر `is_deleted`، فأرباحه
المحتجزة تخالف الميزانية وقائمة الدخل. أُضيف الفلتر المعتمد فى بقية التقارير.

### (ج) «الملخص المبسط» كان يُسقط إيراد الخدمات كاملاً

`app/api/simple-report/route.ts` التقط حساب المبيعات وحده، فعرض 1,049 بينما
الإيراد الحقيقى 2,434.03. صار يجمع كل حسابات `account_type='income'`، وتغيّر
عنوان البند من «المبيعات» إلى «الإيرادات» ليصدق على البضاعة والخدمة معاً.

### (د) بانيا قيد الفاتورة كانا يصنّفان الإيراد تصنيفين مختلفين

الدليل فى الدفاتر: `INV-2026-00001` (خدمة) رُحّلت لـ«إيرادات المبيعات 4100»
بينما `INV-2026-00002/3` لنفس الخدمة رُحّلتا لـ«إيرادات الخدمات 4200» — لأن
`execute_sales_invoice_accounting` توزّع على `products.income_account_id`
بينما `prepareInvoiceRevenueJournal` تكبّ كل شىء فى حساب المبيعات العام.
البانى فى TS صار يوزّع بالتناسب على حساب إيراد كل صنف، وأكبر مجموعة تبتلع
فروق التقريب فيساوى مجموع الأسطر صافى القيد حرفياً.

### (هـ) إشعارات اعتمادات التصنيع — هجرة 20260725000003

- **قائمة المواد (BOM) بلا حارس إشعار إطلاقاً**: من يرسل نسخة للاعتماد لا
  يصل المالك أى إشعار (المسار وأمر الإنتاج والاستلام لها حوارس منذ 813/814).
- **القرار لا يعود لصاحب الطلب** فى أى من الثلاثة: يوافق المعتمِد أو يرفض
  فلا يعلم المُنشئ. حارس موحّد `manufacturing_notify_decision_trg` يقرأ اسم
  عمود الحالة من `TG_ARGV` عبر `to_jsonb` — فيخدم `status` (BOM) و
  `approval_status` (المسار/الأمر) بلا استثناءات، ويحمل سبب الرفض.
- بروفة على قاعدة الاختبار: طلب=1 للمالك، قرار=1 للمُنشئ، ثم rollback.

### (و) تقرير مخزون المخازن: تدفق التصنيع كان مخفياً + زر تصدير معطّل

عمودان جديدان («إنتاج تام وارد» / «استهلاك تصنيع») يفصلان حركتَى التصنيع
داخل الوارد والصادر — كان المنتج التام يبدو ظاهراً من العدم والخامات مختفية
بلا سبب. وزر التصدير كان معروضاً بلا وظيفة؛ صار يصدّر CSV بترميز يفتح فى
إكسل العربية سليماً.

### (ز) المواد الخام كانت تُعرض للبيع

`item_type` لكل الأصناف المخزنية = `product`، والتمييز الحقيقى فى
`product_type` — فكانت الخامات تظهر فى أصناف الفاتورة وأمر البيع وإضافات
الحجز، ويُطلب لها «سعر بيع» إلزامى. الآن: الحقل مخفى ومصفَّر للخامة، وهى
مستبعدة من شاشات البيع الأربع (الشرط يُبقى الأصناف القديمة بلا تصنيف).

### (ح) الإشعارات الضائعة أثناء انقطاع القناة

موثّق حياً: إشعار وُلد 17:18:32 واشتراك المالك عاد 17:18:36 — فضاع. البث
الحى لا يعوّض ما فات، فصار كل اشتراك ناجح يُطلق **catch-up** يعيد حساب
العداد ويجدّد الصندوق المفتوح. و`CHANNEL_ERROR` العابر صار تحذيراً هادئاً
بالعربية بدل خطأ أحمر يُفزع المستخدم.

---

## [3.74.814] - 2026-07-24 — ختام دورة التصنيع: عطبان هيكليان + زران

الدورة الصناعية الأولى اكتملت حياً (خامات ← BOM ← مسار ← أمر ← صرف
بمرحلتين ← استلام تام ← **بيع بتكلفة 60.00 حرفياً وربح 440**) — وفى
طريقها أربعة إصلاحات:

### (أ) أعمدة مرحلة اعتماد الإدارة — هجرة 20260724000007

مسار «اعتماد الإدارة» لصرف المواد كان يكتب فى أعمدة لم تُنشأ قط
(management_approved_by/at/notes) وقيد حالة لا يعرف 'management_approved'
— فانفجر أول اعتماد. الأعمدة أضيفت والقيد وُسع (مطبقة على القاعدتين).

### (ب) إسقاط المرحّل المزدوج لقيد الاستلام — هجرة 20260724000008

أول استلام إنتاج ولّد قيدين لنفس الحدث (حارس قديم + المسار الحديث) —
مخزون تام 120 مقابل دفعة FIFO بـ60 وتحت التشغيل سالب. الحارس أُسقط
نهائياً (درس 804)، والازدواج عُكس بقيد أصولى موثق عبر بوابة
app.allow_direct_post احتراماً لحارس القيود المرحّلة. تحقق: تحت
التشغيل = 0.00 والمخزون = تقييم FIFO.

### (ج) زر «إرسال للاعتماد» المفقود فى شاشة أمر الإنتاج

نفس نمط 813 (المسار): الخادم يرفض الإصدار قبل الاعتماد والزر غير
موجود. أُضيف للمسودات (والأمر العالق كان أُرسل يدوياً بالدالة الذرية).

### (د) زر إرسال المسار يقرأ حالة الاعتماد

ملاحظة المالك «هل يظل الزر فعالاً بعد الاعتماد؟» — الزر كان يقرأ حالة
دورة الحياة فقط (تبقى مسودة حتى التفعيل) فبقى ظاهراً بعد الاعتماد.
الآن يختفى بعد الإرسال/الاعتماد ويعود عند الرفض فقط.

### مؤجل لـ815 (مسجل)

عمودا تقرير المخزون (إنتاج وارد/استهلاك تصنيع — الأرصدة صحيحة والعرض
ناقص)، إشعارات اعتمادات التصنيع (الصندوق يعمل والجرس صامت)، إخفاء سعر
البيع للخامات + استبعادها من شاشات البيع، catch-up الإشعارات عند إعادة
الاشتراك، إسكات ضجيج الانقطاع العابر.

---

## [3.74.813] - 2026-07-24 — ثلاث عثرات تصنيع اصطادها الاختبار الحى

### (أ) نافذة سريان نسخة الـBOM — هجرة 20260724000006

«سارى من/حتى» بنفس اللحظة كانت تموت برمز 23514 غامض. الآن: رفض مبكر
برسالة ثنائية واضحة (دالة القاعدة — مطبقة على القاعدتين) + تحقق مطابق
فى الواجهة قبل الإرسال.

### (ب) زر «إرسال للاعتماد» المفقود فى شاشة مسار التصنيع

الخادم كان يرفض التفعيل بحق («لم تُرسل للاعتماد بعد» 409) بينما الشاشة
لا تعرض زر الإرسال أصلاً — مسار submit-approval موجود فى الـAPI بلا
مستدعٍ من الواجهة. أُضيف الزر (يظهر للمسودة فقط) + دالة العميل
submitRoutingVersion. (نسخة ROUT-001 v1 العالقة أُرسلت يدوياً بنفس
الدالة الذرية لفك حصار الاختبار.)

### (ج) كود العملية تلقائى

طلب المالك: الخطوة الجديدة تولد كودها بالنمط الصناعى OP-010 / OP-020
(قفزات عشرات تسمح بالإقحام لاحقاً)، مع بقاء الحقل قابلاً للتعديل.

---

## [3.74.812] - 2026-07-24 — المسمى الوظيفى يواكب الدور + مسؤول التصنيع يرى المنتجات

### (أ) «عمود الوظيفة لم يتغير» — تشخيص المالك بنفسه

تغيير الدور من الإعدادات يحدّث `company_members.role` (الحاكم الحقيقى
للصلاحيات — وقد تغيّر بنجاح) لكنه لا يلمس `employees.job_title` (نص حر
تعرضه قائمة الموظفين) — فبدا للجميع أن التغيير لم يحدث. مسار
member-role يزامن الآن المسمى مع عنوان الدور الجديد تلقائياً (وفشل
المزامنة لا يفشل تغيير الدور). بيانات محمد الصاوى صُححت يدوياً فوراً.

### (ب) صلاحيات مسؤول التصنيع — هجرة 20260724000005

الدور كان بلا قراءة للمنتجات أصلاً — فبانى قوائم المكونات ما كان
سيعرض شيئاً. منح قراءة فقط، مطبق على القاعدتين.

### (ج) «القائمة تعرض التصنيع لكن أى صفحة لا تفتح»

خريطة مسار←مورد فى حارس الصفحات كانت تعرف 3 صفحات تصنيع فقط؛ البقية
(صرف المواد، استلام الإنتاج، النسخ، MRP، الإقفال) تسقط لمورد وهمى
'manufacturing' لا يملكه أى دور → طرد فورى حتى للمخوَّلين. أُضيف مدخل
مظلة `/manufacturing → manufacturing_boms` يغطى المديول كله.

---

## [3.74.811] - 2026-07-24 — مدير الفرع يضيف منتجات + رفض الصلاحيات لم يعد ينفجر

### ملاحظة المالك (بداية اختبار التصنيع)

إضافة المنتج تنجح للمالك وتفشل لمدير الفرع بخطأ 500 بجسد فارغ
(«Unexpected end of JSON input»).

### عيبان بجذر واحد فى المشهد

1. **أسطولى — apiGuard يعيد كائناً خاماً بدل Response** فى ثلاثة فروع
   رفض (unauthorized / غير مرتبط بالشركة / صلاحيات غير كافية) — فأى
   رفض صلاحية عبر الحارس كان ينفجر فى Next برسالة «Expected a Response
   object but received 'object'» → 500 فارغ بدل 401/403 JSON نظيفة.
   (سجل Vercel كشفها حرفياً). أُصلحت الفروع الثلاثة بلفها فى
   ErrorHandler.handle.
2. **تعارض طبقتين**: قائمة السماح فى مسار الإنشاء (675، بمبدأ المالك)
   تشمل مدير الفرع، بينما company_role_permissions يمنعه بـ
   can_write=false — فالحارس يصده قبل بلوغ القائمة. هجرة
   20260724000004 تمنح manager الكتابة والتعديل على products فى كل
   الشركات (مطبقة على القاعدتين). الحذف يبقى للإدارة العليا.

---

## [3.74.810] - 2026-07-24 — حزمة ملاحظات المالك الست

### (أ) إشعار اعتماد الدفعة: مرسل واحد بدل ثلاثة

المالك عدّ 3 إشعارات لطلب اعتماد دفعة واحدة. المصادر: حارس القاعدة
payment_supplier_notify_approval_trg (الأصح — مالك+GM مباشرة، يعمل لكل
مسارات الإنشاء — **يبقى**) + إرسال فى مسار API + إرسال قديم من واجهة
التوزيع المجمع (لأدوار admin+GM بالوراثة) — **أُزيلا**. دفعات العملاء
لم تُمس (لا حارس قاعدى لها).

### (ب) الحسابات الحرجة إلزامية — هجرة 20260724000003

طلب المالك نصاً. الشركات القائمة علّمت حساباتها الحرجة is_system،
والحارس صار يمنع: الحذف والتعطيل والأرشفة ونزع الصفة. (الثغرة الأخطر
كانت التعطيل: يكسر الترحيل كالحذف وكان مسموحاً). وبملاحظة المالك
اللاحقة «الرسالة حسب اللغة المختارة»: القاعدة لا تعرف لغة الواجهة،
فالرسائل **ثنائية اللغة** (عربى | إنجليزى) فى نص واحد.

### (ج) قوائم الفواتير غير المسددة تعرض المستحق الفعلى

ملاحظة المالك: الفاتورة ذات الدفعة المعلقة كانت تظهر «مستحقة بالكامل»
بزر اختيار نشط، والمحاسب لا يكتشف إلا برفض الخادم (559). الآن: طرح
المخصصات المعلقة، شارة «⏳ معلقة»، تعطيل الاختيار عند صفر، والتعبئة
التلقائية بالفعلى.

### (د) سطر «التعديل 0.00» فى صفحة فاتورة المشتريات

يظهر فقط لقيم موروثة غير صفرية (الخانة أُلغيت فى 788).

### (هـ) ضجيج 403 discount-approvals

كل تحميل صفحة لغير المخوّل (مسئول مخزن/محاسب) كان يسجل خطأ شبكة أحمر.
المسار يعيد الآن قائمة فارغة نظيفة `can_approve:false` — لا تسريب
(القائمة الفارغة بلا معلومة) ولا ضجيج.

### (و) سكربت التسجيل كان يحذف كاش النسخة الحالية

قائمة إبقائه مبنية على v4.0.0 بينما أسماء الكاش صارت v4.4.0-<ts> —
فكان يحذف الكاش النشط فى كل تحميل ويعيد تنزيل كل الملفات (مرصود فى
كونسول المالك). حُذفت إدارة الكاش من الصفحة نهائياً — activate فى
الـSW هو المنظف الوحيد.

---

## [3.74.809] - 2026-07-24 — كل نشرة تصل فعلاً لمتصفحات المستخدمين

### الجذر الأعمق لكل أعراض «النسخة القديمة» — مصطاد من كونسول مسئول المخزن

متصفحه كان يعمل ببناء أقدم بنشرتين من الإنتاج. السبب: sw.js كان يولّد
رقم نسخته بـ`Date.now()` **وقت التشغيل** — أى أن بايتات الملف متطابقة
عبر كل النشرات، فالمتصفح لا يكتشف تحديثاً أبداً، وآليات إعادة التحميل
التلقائى (updatefound / SW_UPDATED) لا تنطلق قط، ويبقى كل تبويب مفتوح
على النسخة القديمة للأبد. هذا حجب إصلاح الإشعارات الحية (808) وفسّر كل
حالات «اعمل ريفرش أولاً».

### الإصلاح — SW v4.4.0

- `scripts/stamp-sw-version.js` يختم النسخة **وقت البناء** (يعمل ضمن
  `npm run build` عبر run-next-build.js) — كل نشرة تغيّر بايتات sw.js ←
  المتصفح يكتشف العامل الجديد ← skipWaiting + SW_UPDATED ← إعادة تحميل
  تلقائية للنسخة الجديدة.
- مكسب جانبى: اسم كاش ثابت داخل النشرة الواحدة (كان يتغير مع كل
  استيقاظ للعامل).
- **مرة أخيرة فقط**: المستخدمون الحاليون يحتاجون ريفرش يدوياً واحداً
  لالتقاط sw.js الجديد؛ بعدها التحديث تلقائى للأبد.

### إصلاح ثانٍ — دالة الرصيد المتاح (هجرة 20260724000002)

400 متكرر فى لوحة أصناف الحجز: الدالة تعلن integer والـview يعيد bigint
(42804). أُسقطت وأعيد إنشاؤها بالأنواع الصحيحة + SECURITY DEFINER مع
assert_company_access + REVOKE anon (نمط 805). تحقق حى: أرصدة المشتريات
الجديدة تظهر (زيت=2، booto=2) والشركة الأجنبية تُصد.

---

## [3.74.808] - 2026-07-24 — ملاحظة مسئول المشتريات تسافر للمعتمِد

### ملاحظة المالك

«الملحوظة التى كتبها مسئول المشتريات فى امر الشراء من اين يطلع عليها
المالك او المدير العام للاعتماد؟» — الملاحظة كانت تُحفظ فى القاعدة ولا
تظهر لا فى الإشعار ولا فى صفحة الأمر. المعتمِد يقرر أعمى عن المبرر.

### الإصلاح — مرآة 795 (المبيعات) على المشتريات

- **الإشعار**: طلب/إعادة طلب اعتماد أمر الشراء يقتبس ملاحظة المُنشئ
  حتى 200 حرف («— ملاحظة المُنشِئ: "..."») فى المسارين (الإنشاء
  والتعديل).
- **صفحة أمر الشراء**: بطاقة «ملاحظة مُنشئ الأمر» تظهر أعلى الصفحة حين
  توجد ملاحظة — كانت الصفحة تجلب الحقل ولا تعرضه.
...
### إصلاح ثالث — قيد استلام فاتورة المشتريات لا يعرف خصم المستند

أول فاتورة مشتريات بخصم مستند (BILL-0004، خصم 10% بعد الضريبة) صدّها
حارس التوازن: مدين 41.04 (مخزون 35 + ضريبة 4.90 + شحن 1.00 + ضريبة شحن
0.14) مقابل دائن 37.04 (المورد) — **الفرق 4.00 هو الخصم بالضبط**. بانى
القيد (الذى تعلم الشحن والتسوية فى 499) لم يتعلم الخصم قط.

الإصلاح فى السلسلة كاملة:

- `getAccrualAccountMapping`: حساب `purchase_discount` الجديد
  (sub_type: purchase_discounts — 5130 «خصم المشتريات المكتسب» فى
  الدليل المزروع، نظير sales_discount من 784).
- `prepareBillPosting` (الحى) و`prepareBillPostingFromPayload`
  (الـreplay): خصم المستند يُشتق من الإجماليات المستقرة
  `(subtotal+shipping+adjustment+tax) − total` — يصح لأى نوع/موضع خصم —
  ويُرحّل **دائن 5130**. عمداً لا يُصفّى فى المخزون: دفعات FIFO تحمل
  تكاليف البنود فقط، وتخفيض حساب المخزون كان سيفصل GL عن تقييم FIFO.
- لا حساب؟ فشل مبكر برسالة عربية واضحة (لا ترحيل أعمى).
- المسارات الثلاثة مغطاة: confirm-receipt، الاستلام التلقائى
  (bill-receipt-workflow)، والـreplay.
- **اكتشاف جانبى من سؤال المالك**: بذر الشركة الجديدة كان يزرع 5130
  **بلا sub_type** — فأى شركة جديدة كانت ستفشل فى ترحيل فاتورة مشتريات
  بخصم. أُصلح البذر (الشركات الأربع القائمة سليمة — لديها sub_type
  بالفعل).

### ملفات

- lib/services/purchase-order-notification.service.ts — حقل notes +
  اقتباسه فى الرسالة (عربى/إنجليزى)
- app/api/purchase-orders/[id]/notifications/route.ts — جلب notes
  وتمريرها
- app/api/purchase-orders/route.ts — تمرير notes فى مسار الإنشاء
- app/purchase-orders/[id]/page.tsx — بطاقة الملاحظة

---

## [3.74.807] - 2026-07-24 — بوابة المقاعد تفحص الشركة الصحيحة

### ملاحظة المالك

مسئول المشتريات مُنع من الدخول لشركة «تست» برسالة «مقعدك رقم #1 انتهت
صلاحيته فى ٢٢ يوليو» — بينما صفحة إدارة المقاعد تعرض مقعده نشطاً حتى
٢٩ يوليو. صفحة الإدارة كانت الصادقة.

### السبب الجذرى

المستخدم عضو فى شركتين. دالة البوابة `get_user_company_status` كانت
تحوى قراءتين `LIMIT 1` بلا ترتيب: اختيار العضوية عشوائى، والأخطر أن
البحث عن رخصة المقعد **غير مقيد بالشركة** — فالتقط الرخصة المنتهية من
شركته الأخرى («توب تانك»، انتهت ٢٢ يوليو) وحظر بها جلسة «تست».

### الإصلاح — هجرة `20260724000001`

- اختيار العضوية صار حتمياً: المالك أولاً، ثم عضوية برخصة سارية، ثم
  الأقدم — فالمحظور فى شركة والنشط فى أخرى يدخل بالنشطة.
- البحث عن الرخصة مقيد بنفس الشركة المفحوصة.
- `get_user_seat_license` (نفس النمط، غير مستدعاة حالياً) حُصّنت بترتيب
  حتمى.

### التحقق (اختبار ثم إنتاج، حياً)

- goldwallet: غير محظور، شركة تست، صلاحية ٢٩ يوليو ✓
- بروفة بكل الرخص منتهية: محظور بشركة متسقة مع رخصتها (rollback) ✓
- المالك: لا يُحظر أبداً ✓

---

## [3.74.806] - 2026-07-23 — الاكتمال يوقّع اعتماده

### ملاحظة المالك

بطاقة اعتماد الصرف فى فاتورة الحجز تعرض «تم بواسطة: -» و«تاريخ القرار:
-» — الاكتمال كان يعتمد الصرف بلا توقيع.

### المنطق والتنفيذ

توقيع المنفذ **صادق**: بوابات العهدة (802/803) تضمن أن مسئول المخزن
اعتمد سحب كل صنف مطلوب **قبل** أن يبدأ التنفيذ أصلاً — فاكتمال الخدمة هو
الإذن الختامى. الاكتمال الآن يسجل: الحالة والمعتمِد (المنفذ) والتاريخ؛
وشفاءٌ لمرة واحدة وقّع الفواتير القائمة من بيانات حجوزها (تحقق حى:
INV-2026-00002 تعرض الآن «خالد عجلان» بتاريخ). لا تغيير واجهة — الصفحة
كانت تقرأ الحقول الصحيحة وتنتظر امتلاءها.

---

## [3.74.805] - 2026-07-23 — الفاحص الأمنى يصطاد بوابتنا الجديدة + ملاحظتا المالك

### الاصطياد الأمنى (الفاحص يثبت جدارته)

أول إنذار حقيقى بعد اللوحة النظيفة: `ic_anon_reachable_readers` أمسك
**بوابة العهدة المولودة فى 802** — SECURITY DEFINER بحق نداء PUBLIC
الافتراضى فى Postgres، تقرأ بيانات شركة، بلا فحص هوية. **أُغلقت**: سحب
الحق من PUBLIC وanon (للمسجلين فقط) + فحص انتماء الشركة داخلها — وعاد
الفاحص إلى صفر إنذارات.

### ملاحظتا المالك (اختبار الحجز الحى)

1. **إشعار «حجز مكتمل — فاتورة صادرة» يوجّه المحاسب للوحة التحكم**:
   كان يشير إلى الحجز — صفحة خارج دوره فارتد. صار يشير إلى **الفاتورة**
   (مساحة عمله) فيهبط عليها مباشرة.
2. **شارة «بانتظار اعتماد التسليم» على فاتورة معتمدة الصرف**: القائمة
   كانت تقدّم `approval_status` (يبقى «معلقاً» افتراضياً فى فواتير
   الحجز) على `warehouse_status` الحقيقى. الأولوية انعكست للحقل الحاكم.
   (و«مرتجع كامل» التى بدت شارة اتضح أنها **زر إجراء** مشروع.)

---

## [3.74.804] - 2026-07-23 — إسقاط الحوارس الزومبى (اعتراف وإصلاح جذرى)

### ما حدث — بشفافية

هجرة 796 ختمت ردمها بأمر `ENABLE TRIGGER USER` على جدول الفواتير — وهو
أمر يوقظ **كل** الحوارس، ومنها ثلاثة حوارس محاسبية قديمة ساذجة كانت
**معطلة عمداً** منذ زمن. الحارس الميت عاد فاعترض تحويل فاتورة الحجز
مسودة←مرسلة عند التنفيذ (DIRECT_POST_BLOCKED — رصده المالك حياً على
BKG-2026-00007)، وكان سيعترض أيضاً أى إعادة إرسال بعد رفض صرف. لا ضرر
على البيانات: الاعتراض أجهض العملية ذرياً.

### الدرس والعلاج الجذرى

«معطل» لغمٌ لأى `ENABLE TRIGGER USER` مستقبلى. الثلاثة متجاوَزون أصلاً
(حساباتهم ساذجة: لا خصوم ولا أسعار شاملة ولا ضريبة شحن — مسارات القيود
الحقيقية هى المنفذات الذرية و`execute_sales_invoice_accounting`).
**أُسقطوا نهائياً** من القاعدتين، وتحقق بعدها: **صفر حوارس غير مفعّلة
فى النظام كله** — لا ألغام باقية.

---

## [3.74.803] - 2026-07-23 — مبدأ العهدة مكتملاً: المحدد كله بإذن حارسه

### سؤال المالك الذى كشف الفخ

«لو اعتمد مسئول المخزن الإلزامى ورفض الاختيارى ونُفذت الخدمة — ماذا
يُخصم من المخزن؟» الجواب المرعب قبل الإصلاح: **الاثنان** — رفض السحب لم
يكن يلغى تحديد الصنف، والنظام يكتفى بوصية للموظف «ألغِ تحديد الصنف»؛
ولو نسيها خرجت بضاعة رفض حارسُها تسليمها. والأدهى: اختيارى محدد **بلا
طلب سحب أصلاً** كان يُخصم عند الاكتمال دون علم مسئول المخزن.

### قرارا المالك (نصاً وتنفيذاً)

1. **«كل صنف محدد يحتاج اعتماد سحب قبل التنفيذ»** — البوابة المشتركة
   (802) وسّعت: الإلزامى دائماً + الاختيارى المحدد، وكلاهما يحجب الزر
   والتفعيل باسمه حتى يعتمد الحارس.
2. **رفض الاختيارى يلغى تحديده تلقائياً** (أتمتة وصية النظام ذاتها) —
   الرسالة صارت «تم إلغاء تحديد الصنف تلقائياً — أكمل الخدمة بدونه»،
   والاكتمال يستهلك ما صرّح به الحارس فقط، والبوابة تنفتح طبيعياً.
   (محمى بنافذة ما قبل التنفيذ — حجز مكتمل لا يُمس.)

### البروفة (نسخة الاختبار، ارتدت)

اختيارى محدد بلا سحب: البوابة تحجب مسمّيةً «booto» ✓؛ رفضه: التحديد
حُذف، الإشعار بالنص الجديد، والبوابة انفتحت (الإلزامى معتمد) ✓.

الهجرة مطبَّقة على القاعدتين. (تُدفع مع 800–802 فى دفعة واحدة.)

---

## [3.74.802] - 2026-07-23 — اعتماد سحب الإلزامى شرطُ التنفيذ

### قاعدة المالك (نصاً)

«زر تنفيذ الخدمة يجب أن يظهر بعد اعتماد مسئول المخزن للمنتجات المرتبطة
الإلزامية» — ومنطق النظام نفسه كان يعِد بها: رسالة رفض الإلزامى تقول
«لا يمكن تنفيذ الحجز بدونه». هذا الإصدار يجعل الوعد قابلاً للإنفاذ.
(الاختيارى لا يحجب — الخدمة تمضى بدونه كما صُمم.)

### التنفيذ — طبقتان من دالة مشتركة واحدة

`booking_mandatory_custody_gate(booking_id)` تعيد {جاهز، الناقص[]}:

1. **حارس قاعدة البيانات** (الإنفاذ الحقيقى): التفعيل يُصد برسالة عربية
   **تسمّى الأصناف الناقصة**: «لا يمكن تنفيذ الخدمة قبل اعتماد مسؤول
   المخزن سحب الأصناف الإلزامية: 5 كيلو زيت ماتور».
2. **الزر يقفل بتلميح** يعرض نفس الأسماء وشارة «بانتظار اعتماد السحب» —
   ويفشل مفتوحاً عند تعذر القراءة لأن حارس الخادم هو البوابة الفعلية.

### البروفة (نسخة الاختبار، ارتدت)

حجز مؤكد بصنف إلزامى بلا سحب معتمد: البوابة {جاهز:لا، الناقص:[الزيت]} ✓
التفعيل مصدود بالرسالة المسماة ✓ وبعد اعتماد السحب: {جاهز:نعم} ✓.

الهجرة مطبَّقة على القاعدتين. (تُدفع مع 800 و801 فى دفعة واحدة.)

---

## [3.74.801] - 2026-07-23 — زر «تنفيذ الخدمة» يظهر للحجز المؤكد

### المفارقة التى رصدها المالك

بعد إصلاح التأكيد (799) اختفى زر «تنفيذ الخدمة» من حجز خالد المؤكد —
لأن شرط الزر كان مكتوباً **حول العطب القديم**: «مسودة + مختومة بالتأكيد»
(الحالة التى كان يخلّفها التأكيد المعطوب). فلما صار التأكيد يحوّل الحالة
فعلاً، كذب الشرط واختفى الزر لحظة أن أصبح مشروعاً.

### التنفيذ

الشرط صار: الحالة «مؤكد» (مع إبقاء «مسودة مختومة» تسامحاً لأى صف قديم
غير مشفى). دالتا القاعدة سليمتان أصلاً: البدء يشترط «مؤكد» والتفعيل
يمنع الحالات النهائية فقط.

(يُدفع مع 800 فى دفعة واحدة — سكربت 800 تعطل بمحارف backtick قبل الدفع.)

---

## [3.74.800] - 2026-07-23 — إشعار قرار السحب يوجّه المنفذ إلى حجزه

### ما رصده المالك حياً

خالد ضغط إشعار «تم اعتماد سحب المنتج» فهبط على **صندوق الموافقات** —
صفحة أفعالها لمسئول المخزن، بينما خطوة المنفذ التالية على **الحجز** نفسه
(بدء التنفيذ).

### التنفيذ

توجيه الإشعارات صار واعياً بنوع الحدث: إشعار **القرار** (اعتماد/رفض/
إلغاء تلقائى) يفتح صفحة الحجز مباشرة — رقم الحجز يُقرأ من ذيل مفتاح
الحدث؛ إشعار **الطلب** لمسئول المخزن يبقى على تبويب صندوق الموافقات.
ومفتاح الإلغاء التلقائى (797) أُلحق به رقم الحجز ليتوجه هو الآخر.

**مسجل للتحسين لاحقاً:** تسمية المنتج داخل نصوص إشعارات السحب
(«سحب: 5 كيلو زيت ×1» بدل «منتج»).

---

## [3.74.799] - 2026-07-23 — التأكيد يؤكد فعلاً، والمنفذ يعرف من يخدم

### ما رصده المالك حياً على BKG-2026-00007

1. الشارة تقول «مسودة» تحت سطر «تم التأكيد» — وقاعدة البيانات أكدت:
   دالة التأكيد كانت تختم التاريخ وترسل الإشعار **وتنسى تحويل الحالة**
   (توثيقها يعِد بـ draft→confirmed والتحويل غائب من جسدها). الموظف كان
   سيُصد عند «تنفيذ الخدمة» لأن آلة الحالات تشترط confirmed قبل البدء.
2. العميل يظهر «—» فى صفحة **الموظف المنفذ** رغم وجوده — صلاحية الموظف
   على جدول العملاء «منشئه فقط»، فالعرض أعاد الاسم فارغاً لعميل حجزٍ
   الموظفُ نفسه مكلف بتنفيذه.

### الإصلاح

1. **الدالة تحوّل الحالة فعلاً**، وفحص التكرار صار على الحالة لا الختم —
   فإعادة النقر تشفى حجزاً مختوماً عالقاً فى مسودة؛ وردمٌ لمرة واحدة شفى
   كل العالقين (ومنهم BKG-2026-00007) عبر المسار الشرعى المسجل فى تاريخ
   الحالات.
2. **مسار جلب الحجز يكمل هوية العميل خادمياً** (اسم/هاتف/بريد فقط) لمن
   ثبت حقه فى قراءة الحجز ذاته — هوية من يخدمه جزء من حجزه.

الهجرة سارية على القاعدتين فوراً؛ إصلاح الهوية يحتاج النشر.

---

## [3.74.798] - 2026-07-23 — عملاء الحجز الجديد يتبعون الفرع المختار

### ملاحظة المالك (افتتاحية اختبار دورة الحجز)

مسئول الحجز غير المرتبط بفرع يختار الفرع أولاً — لكن قائمة العملاء كانت
تعرض **عملاء كل الفروع**، فيمكنه اختيار عميل فرع آخر بالخطأ ولا يكتشف
إلا عند الحفظ برسالة حارس العزل الخام (بعد ملء النموذج كله).

### التنفيذ

نفس قاعدة الخدمات طُبقت على العملاء: اختر الفرع أولاً ← القائمة تعرض
عملاء الفرع المختار (+ غير المرتبطين بفرع — الحارس يقبلهم)؛ تغيير الفرع
يمسح اختيار العميل كما يمسح الخدمة؛ والنص الإرشادى حُدّث. الأدوار
المرتبطة بفرع لم تتغير قوائمها (مرشَّحة أصلاً من الصفحة).

الحارس الخلفى (CUSTOMER_BRANCH_ISOLATION) يبقى خط الدفاع الأخير كما هو.

---

## [3.74.797] - 2026-07-23 — §3.8c مغلقة: دورة حياة عهدة الحجز محكمة

### ما وجدته المراجعة الشاملة (مفاجأة سارة)

البروتوكول المصمم **سليم**: العهدة الخارجة تعود للمخزن عند التنفيذ
فيُخصم الاستهلاك مرة واحدة بالضبط؛ الإلغاء يطلب إرجاع العهدة عبر دورة
الاعتماد؛ وتعديلات المنتجات المباعة (الإضافات والباقات) **موصولة فعلاً**
بجسر `resync_booking_invoice` الكامل (يعيد بناء بنود الفاتورة ويشعر
المحاسب والإدارة).

### الثقوب الحقيقية — قصة BKG-2026-00006 حرفياً

1. **الاكتمال يترك طلبات السحب المعلقة حية** — طلب يُعتمد بعده يُخرج
   بضاعة لعهدة لن يستهلكها أحد أبداً (والفاتورة «لا تقرأه» لأنه بحق لم
   يعد هناك ما يُقرأ — الاستهلاك سُجّل لحظة التنفيذ).
2. **الإلغاء يتركها حية أيضاً** — نفس الفخ.
3. **البت بلا حارس حالة** — يقبل اعتماد طلب لحجز منتهى.

### الإصلاح (ثلاث رقع فى قاعدة البيانات)

دالة `fn_void_pending_booking_withdrawals`: تُلغى الطلبات المعلقة
تلقائياً برسالة مفسّرة وإشعار للطالب — يستدعيها **الاكتمال والإلغاء**
كلاهما؛ وحارس فى دالة البت: اعتماد طلب لحجز منتهى يُرفض برسالة «الطلب
أصبح لاغياً: ارفضه بدلاً من اعتماده» (الرفض يبقى متاحاً دائماً).

### البروفة (نسخة الاختبار، ارتدت — بعد مطابقة حوارسها المعطلة بالإنتاج)

حجز قيد التنفيذ + طلب معلق ← اكتمال: الطلب أُلغى تلقائياً بالنص المفسِّر
**والفاتورة وُلدت** ✓؛ طلب جديد على الحجز المكتمل ← الاعتماد **صُد**
بالرسالة العربية والرفض نجح ✓؛ حجز بطلب معلق ← إلغاء: الطلب أُلغى ✓.

الهجرة مطبَّقة على القاعدتين — سارية فوراً بلا نشر. **بهذا تُغلق §3.8c.**

---

## [3.74.796] - 2026-07-23 — لوحة سلامة نظيفة لأول مرة: جلسة دفتر التسليم

### ما أُنجز فى هذه الجلسة (بعد نسخة احتياطية مؤكدة إلى OneDrive)

1. **النسخة الاحتياطية**: 17.39MB ← مضغوطة 2.12MB ← منسوخة لـ OneDrive
   بتحقق بايتى.
2. **نظير المشتريات لإشعار الصرف المتكرر**: فُحص ووُجد **محصّناً أصلاً**
   (يبنى مفتاح كل إشعار بمعرّف جولة جديد) — لا إصلاح.
3. **تسوية FIFO التاريخية**: تسوية **الكميات نظيفة 100%** فى كل الشركات؛
   انحراف **القيمة** انكمش من −5.41 إلى −0.14 (بقايا تقريب داخل هامش
   تسامح الفاحص السليم أصلاً) — أُغلق البند.
4. **BKG-2026-00006**: أُلغى بقرار المالك — العهدة كانت مرجعة فعلاً
   ورصيدها الدفترى صفر؛ الإلغاء نُفذ بتجاوز موثَّق لآلة الحالات (مكتمل
   نهائى بحق) مع سطر تاريخ حالة باسم المالك والسبب.
5. **فاحص الضريبة (المريض الحقيقى الوحيد من الثلاثة)**: معادلته كانت
   عمياء عن أربع حقائق — خصم البنود، الأسعار الشاملة، خصم المستند قبل
   الضريبة، وضريبة الشحن — فكل فاتورة **صحيحة** بأى منها بدت «منحرفة»
   (زائفتا 8.11 و11.2). المعادلة الجديدة تعيها كلها. ومعها ردم فجوة
   بيانات: فاتورتان تاريخيتان فقط بالنظام كله حملتا ضريبة شحن 14% فى
   إجماليهما قبل أن يُخزَّن المعدل — استُكمل تخزينه.

### التحقق

نسخة الاختبار: صفر إنذارات زائفة على كل الفواتير الحقيقية، وفاتورة
مُفسدة عمداً (ضريبة 99.99) **اصطادها** الفاحص بفارق 95.65 ✓.
الإنتاج بعد التطبيق: **فواحص الضريبة والتقييم والحجوزات الثلاثة = صفر
إنذارات** — أول لوحة سلامة نظيفة بالكامل.

### المتبقى فى دفتر التسليم

فجوة الحجز⇄الفاتورة (§3.8c — الأكبر)، وتشخيص الإشعارات الحية (بانتظار
سطر كونسول المالك)، و272 كتابة غير مفحوصة موروثة (متتبعة).

---

## [3.74.795] - 2026-07-23 — ملاحظة الموظف تسافر مع التعديل

### ملاحظة المالك (أثناء اختبار دورة الرفض)

الموظف كتب فى ملاحظات أمر البيع **لماذا** عدّل («اقنع العميل بالمنتج») —
والمحاسب لم يرها. نصف القصة كان غائباً عن المكتب الذى يقرر إعادة الإرسال.

### التنفيذ (فى حارس المزامنة نفسه، بنفس بوابة النافذة الآمنة)

1. ملاحظات أمر البيع تُنسخ إلى **ملاحظات الفاتورة** المرتبطة.
2. نص إشعار «أعد الإرسال» يقتبسها حرفياً: «ملاحظة الموظف: "..."» (حتى
   200 حرف).

### البروفة (نسخة الاختبار، ارتدت)

تعديل كمية + ملاحظة بعد الرفض: البنود انعكست (2)، ملاحظات الفاتورة حملت
النص، والإشعار اقتبسه بالنص الكامل ✓.

الهجرة مطبَّقة على القاعدتين — سارية فوراً بلا نشر.

---

## [3.74.794] - 2026-07-23 — سياق الاعتماد يسكن حيث يبدأ الإجراء

### اقتراح المالك (أثناء اختبار دورة الرفض)

إشعار الرفض ينقل الموظف إلى **أمر البيع** — فلماذا لا يجد هناك نفس بطاقتى
الفاتورة: قرار الصرف (ومن رفضه ولماذا) واعتماد الخصم؟

### التنفيذ

صفحة تفاصيل أمر البيع صارت تعرض بطاقتين بجوار معلومات الأمر:

1. **صرف الفاتورة المرتبطة**: الحالة (تم الصرف / مرفوض / بانتظار)، من
   قرر، متى، وسبب الرفض — مع إرشاد العمل نفسه: «عدّل هذا الأمر وتعديلك
   يسرى على الفاتورة تلقائياً ثم يعيد المحاسب إرسالها».
2. **اعتماد الخصم**: الحالة والقيمة ومن اعتمد ومتى وملاحظة القرار —
   نفس بطاقة الفاتورة (v3.74.791).

### وفى نفس الإصدار — إجمالى البند «£0.00»

جدول بنود الأمر كان يقرأ حقلى total/subtotal للبند، وتعديل الأمر يحفظ
line_total فقط — فظهر بند حقيقى بإجمالى صفر (رصده المالك على SO-0003).
الخلية الآن تتدرج: total ← line_total×(1+الضريبة) ← حساب كامل من الكمية
والسعر والخصم والضريبة.

---

## [3.74.793] - 2026-07-23 — إعادة الإرسال تولّد إشعار صرف جديداً

### ما كشفه المالك (الإرسال الثانى لـ INV-00003)

بعد الرفض الأول وإعادة «تحديد كمرسلة»، **لم يصل مسئول المخزن إشعار جديد**
— الظاهر لديه كان إشعار الجولة الأولى القديم فقط.

### العلة

منع تكرار الإشعارات يعمل بفئتين: فئة «مخزون» (الحالية لإشعار الصرف) تجد
إشعار الجولة الأولى بنفس المفتاح — حتى وهو منفَّذ — **فتمتنع عن إنشاء
جديد للأبد**؛ بينما فئة «موافقات» تحديداً تؤرشف النسخة القديمة وتنشئ
جديدة غير مقروءة — وهو بالضبط سلوك طلبِ موافقةٍ يتكرر مشروعاً مع كل جولة
(رفض ← تعديل ← إعادة إرسال).

### الإصلاح

إشعار «فاتورة جاهزة للشحن» صار بفئة «موافقات» — وهى فئته الصادقة أصلاً
(طلب اعتماد صرف). كل إعادة إرسال تؤرشف القديم وتولّد جديداً حياً.

**مسجل للتسليم:** فحص نظيره فى المشتريات («مطلوب اعتماد استلام
البضاعة») لنفس العائلة؛ وتشخيص وصول الإشعارات الحى (Realtime) معلق على
دليل الكونسول من المالك.

---

## [3.74.792] - 2026-07-23 — عيبا الرفض المكتشفان حياً: السبب الضائع والإشعار الغائب

### ما كشفه اختبار المالك على INV-00003

مسئول المخزن رفض الصرف **بسبب مكتوب**، لكن: (1) إشعار «عدّل أمر البيع»
لم يصل موظف الفرع — وصل بدله الإشعار الاحتياطى القديم للمحاسب؛
(2) سبب الرفض ظهر «لا توجد ملاحظات» فى الفاتورة والإشعارات معاً.

### العلتان

1. **حجب الصلاحيات الصامت:** طبقة الخدمة كانت تقرأ أمر البيع بسياق
   **مسئول المخزن** منفّذ الرفض، وسياسات RLS تخفى أوامر البيع عن دوره —
   فعادت القراءة فارغة بصمت وسقط النظام للاحتياطى. نُقل إشعار المصدر إلى
   دالة الرفض ذاتها (`reject_sales_delivery` — SECURITY DEFINER ترى
   المستندات مهما كان المنفّذ): أمر بيع ← منشئه؛ فاتورة خدمة ← **الموظف
   منفذ الخدمة** (تصحيح المالك)؛ والدالة تعيد علماً يمنع الاحتياطى من
   الازدواج — الاحتياطى الآن للفواتير المستقلة بلا مصدر فقط.
2. **فقدان السبب على حدود API:** نافذة الرفض ترسل السبب باسم
   `rejection_reason` والخادم كان يقرأ `notes` فقط — كل سبب مكتوب كان
   يموت على الحدود. المسار يقرأ الاسمين الآن.

### البروفة (نسخة الاختبار، ارتدت)

رفض بسبب «العميل عدّل الكمية»: الفاتورة مسودة، السبب محفوظ فى الفاتورة،
إشعار «عدّل أمر البيع» وصل منشئ الأمر بالسبب كاملاً، والعلم مرفوع ✓.

هجرة الدالة مطبَّقة على القاعدتين (سارية فوراً)؛ إصلاح المسار يحتاج النشر.

---

## [3.74.791] - 2026-07-23 — صفحة الفاتورة تعرض اعتماد الخصم

### سؤال المالك (أثناء اختبار INV-00003)

«لماذا لم تحتوِ تفاصيل الفاتورة على اعتماد الخصم ومن قام بالاعتماد لكى
يراه المحاسب ويعلم أن الفاتورة التى بها خصم تم اعتمادها؟»

### التنفيذ

بطاقة «حالة الاعتماد» القائمة تخص **إخراج المخزن** فقط. أُضيفت بطاقة
جديدة «**اعتماد الخصم**» فى تفاصيل الفاتورة تعرض: الحالة (معتمَد/مرفوض/
بانتظار/ملغى)، قيمة الخصم المعتمدة، **اعتُمد بواسطة** (بالاسم)، تاريخ
القرار، وملاحظة القرار — مع بيان المصدر: «موروث من أمر البيع (SO-x) —
القرار الواحد يعيش على الأمر» للفواتير التابعة (بنية v3.74.782)، أو
«اعتماد خاص بهذه الفاتورة» للمستقلة. البطاقة تظهر فقط حين مرّ المستند
بدورة الخصم، وتُخفى من الطباعة.

القراءة متاحة لكل أعضاء الشركة (سياسة القراءة على طلبات الاعتماد مقيدة
بالشركة فقط) — فالمحاسب يرى الاعتماد دون أن يملك حق البت.

---

## [3.74.790] - 2026-07-22 — مدير الفرع يعلم بقرارات الخصم فى فرعه

### قاعدة المالك (نصاً)

عند مراجعة خريطة إشعارات رفض خصم SO-0003: «اعتقد ان مدير الفرع يجب ان
يعلم ما يتم فى فرعه».

### التنفيذ

قرار الخصم (اعتماد أو رفض) على أى مستند — أمر بيع، فاتورة، أمر شراء,
فاتورة شراء، حجز — يرسل الآن إشعار علم إلى **مدير فرع المستند** عبر
قناته القائمة: «نشاط فرعك: تم رفض الخصم على طلب المبيعات ... السبب:
...». طلبات الاعتماد لا تحمل رقم الفرع، فيُشتق من المستند نفسه حسب
نوعه. الإشعار محاط بحماية: فشله لا يُفشل القرار أبداً.

### البروفة (نسخة الاختبار، ارتدت)

رفض تجريبى لخصم أمر بيع: الموظف الطالب وصله الرفض بالسبب ✓، مدير الفرع
وصله «نشاط فرعك: تم رفض الخصم» بنفس السبب ✓.

### وفى نفس الإصدار — سد طريق مسدود اكتشفته مراجعة خريطة الرفض

إرشاد الرفض يقول للموظف: «احذف الخصم أو غيّره». **تغييره** يفتح دورة
اعتماد جديدة اعتمادها يولّد الفاتورة ✓. أما **حذفه كلياً** فكان طريقاً
مسدوداً مؤكداً من علّتين متراكبتين:

1. فرع الصفر فى تقييم خصم أمر البيع يلغى الطلب المعلق ويعود — لا شىء
   يولّد الفاتورة بعدها أبداً.
2. وحتى لو حاول الإنشاء: حارس «الخصم المرفوض» كان يمنع حفظ الفاتورة
   **دون النظر** إلى أن خصمها الحالى صفر — يحجب فاتورة لا تحمل الخصم
   المرفوض أصلاً. (ونفس العمى فى توأمه بدورة المشتريات — أُصلحا معاً.)

بعد الرقعتين، أثبتت البروفة الدورة كاملة: خصم 5 ← رفض المالك ← الموظف
صفّر الخصم ← **الفاتورة ولدت تلقائياً** ببنودها وبصفر خصم وإشعاراتها ✓.
وسؤال المالك عن الخصم المختلط (بنود + مستند): تصفير أحدهما مع بقاء
الآخر يُبقى الإجمالى المجمَّع > صفر ← دورة اعتماد جديدة تصلك — لا انسداد
فى هذا المسار أصلاً.

الهجرات الثلاث مطبَّقة على قاعدتى الاختبار والإنتاج — سارية فوراً بلا نشر.

---

## [3.74.789] - 2026-07-22 — إنشاء أمر البيع يُشعر مدير الفرع فقط

### سؤال المالك

«ما فائدة إشعار (أمر بيع جديد) إلى المالك؟» — وصله مع SO-0003 رغم أن
النظام ناداه فعلاً بما يخصه (طلب اعتماد الخصم).

### القرار والتنفيذ

الإنشاء الروتينى شأن **مدير الفرع** التشغيلى — وهو يملك أصلاً إشعاره
الخاص من حارس قاعدة البيانات («نشاط فرعك: تم إنشاء طلب مبيعات»). المالك
والمدير العام يُنادَيان فقط حين يكون القرار قرارهما (اعتماد خصم) أو عند
الاستثناءات (رفض مخزن، انحرافات).

لذا **حُذف البث القيادى** «أمر بيع جديد» من خدمة إشعارات أمر البيع
كلياً — لا أُعيد توجيهه لمدير الفرع لأن ذلك كان سيضاعف إشعاراته عن كل
أمر. النتيجة: قناة واحدة صحيحة لكل دور.

---

## [3.74.788] - 2026-07-22 — إلغاء خانة «التعديل/التسوية» من الدورات الثلاث

### سؤال المالك

«خانة التعديل فى أمر البيع — هل هى واجبة الوجود وما سببها العملى أم أنها
مخالفة؟»

### الجواب الذى كشفته المراجعة

غرضها المشروع عالمياً تسوية كسور القروش، **لكنها فى نظامنا كانت ثغرة
مفتوحة**: دالة تقييم خصم أمر البيع لا تنظر إليها إطلاقاً — تعديل
**بالسالب** = خصم مقنّع يلتف كلياً حول قاعدة «الخصم يُعتمد مرة واحدة»،
ومحرك القيود (منذ 784) يسجّله فى الدفاتر على حساب خصم المبيعات حرفياً.
والتعديل بالموجب إيراد بلا مصدر.

### قرار المالك (نصاً)

«إزالة الخانة نهائياً وكذلك إذا وُجدت فى دورة المشتريات أيضاً وكذلك دورة
الخدمات».

### التنفيذ

- **الواجهة:** حقل الإدخال أُزيل من 7 شاشات (أمر بيع جديد/تعديل، فاتورة
  جديدة/تعديل، فاتورة شراء تعديل، أمر شراء جديد/تعديل، إشعار دائن مورد).
  المستندات التاريخية التى تحمل قيمة تعرضها **للقراءة فقط** بوسم «تاريخى».
- **قاعدة البيانات (الحارس الحقيقى):** `trg_block_manual_adjustment` على
  الجداول الخمسة (أوامر البيع، الفواتير، فواتير الشراء، أوامر الشراء,
  إشعارات المورد) يرفض أى قيمة غير صفرية جديدة أو معدَّلة برسالة عربية،
  مهما كان مصدر الطلب. التصفير مسموح دائماً والتاريخى محفوظ.

### البروفة (نسخة الاختبار)

إدخال بقيمة −5 ← مرفوض ✓، إدخال بصفر ← يمر ✓، تعديل قيمة إلى غير صفر ←
مرفوض ✓. الهجرة مطبَّقة على قاعدتى الاختبار والإنتاج.

---

## [3.74.787] - 2026-07-22 — رفض الصرف: التعديل يبدأ من المصدر والفاتورة تتبع

### قاعدة المالك (نصاً)

عند رفض مسئول المخزن صرف البضاعة (العميل رجع فى منتج أو عدّل عدداً عند
التسليم): إشعارات **للعلم** للمالك والمدير العام ومدير الفرع، وإشعار
**إجراء** إلى **منشئ أمر البيع** ليعدّل أمر البيع — «والذى بدوره يعود هذا
التعديل على الفاتورة المرتبطة» — ثم إشعار محاسب الفرع بالتعديل ليضغط
«كمرسلة» مرة أخرى وتُعاد الدورة حتى تكتمل. ولفاتورة الخدمة: الإجراء يصل
إلى **الموظف منفذ الخدمة** (المعين على تنفيذ الحجز — تصحيح من المالك؛
منشئ سجل الحجز احتياط فقط إن لم يوجد منفذ) ليعدّل المنتجات **المباعة**
فى أمر الحجز (المنتجات المستهلكة فى تنفيذ الخدمة خارج الدورة — استُخدمت
بالفعل).

### ما بُنى

1. **إعادة توجيه إشعار الرفض** (خدمة الرفض): الإجراء إلى منشئ أمر البيع
   («عدّل أمر البيع وسيسرى تعديلك على الفاتورة تلقائياً») أو منشئ أمر
   الحجز لفواتير الخدمة؛ فاتورة مستقلة بلا مصدر تبقى على إشعار المرسل
   القديم. إشعارات العلم للمحاسب والإدارة (مالك/مدير عام/مدير فرع) كما هى.

2. **مزامنة البنود من أمر البيع إلى الفاتورة** — الفجوة التى كانت تمنع
   الدورة أصلاً: تعديل بنود أمر البيع لم يكن يصل إلى بنود الفاتورة
   إطلاقاً (المزامنة الموجودة رأسية فقط). حارس جديد يعيد بناء بنود
   الفاتورة من بنود الأمر **داخل النافذة الآمنة حصراً**: فاتورة
   مسودة/غير مرسلة، **بلا قيد إيراد مرحّل** — فاتورة سلّمت بضاعتها لا
   يمكن أن تُمس بالبنية. (الإجماليات كانت تتزامن رأسياً من قبل.)

3. **إشعار المحاسب بعد التعديل**: يولد من المزامنة نفسها حين تكون
   الفاتورة مرفوضة الصرف — «انعكس التعديل على الفاتورة، أعد الإرسال» —
   وإعادة الإرسال تعيد الصرف إلى «معلّق» وتُخطر مسئول المخزن من جديد
   (مسار إعادة الإرسال القائم).

### البروفة (نسخة الاختبار، ارتدت بالكامل)

أمر بيع ← فاتورة تلقائية ← إرسال ← رفض (مسودة/مرفوض) ← تعديل الكمية
1→3 فى الأمر ← بنود الفاتورة صارت 3 ✓ ← إشعار المحاسب وُلد ✓ ← وبعد
تسجيل قيد مرحّل للفاتورة، تعديل جديد (9) **لم يمسها** ✓.

الهجرة مطبَّقة على قاعدتى الاختبار والإنتاج.

**خارج نطاق هذا الإصدار (مسجل):** انعكاس تعديل أمر الحجز على فاتورة
الخدمة يعتمد على مسار الحجز الحالى — فجوة مزامنة الحجز⇄الفاتورة المعروفة
(§3.8c) تُعالج فى إصدار لاحق؛ حقول عرض الخصم على الفاتورة قد تحتاج تحديثاً
مع مزامنة الرأس.

---

## [3.74.786] - 2026-07-22 — طبقات FIFO كانت تُخصم مرتين فى كل عملية بيع

### كيف انكشفت

مسئول المخزن ضغط «اعتماد الصرف» لأول مرة تحت دورة v3.74.785 فرفضتها
قاعدة البيانات: `fifo_cost_lots violates chk_quantities`. الطبقة بها 1
والفاتورة تبيع 1 — فمن أين التجاوز؟

### العلّة — منذ v3.74.702

حارس قيد التكلفة (`trg_auto_cogs_on_sale`) يستدعى `consume_fifo_lots`
التى **تسجّل استهلاكاً وتخصم من الطبقة**، والمعاملة الذرية تطبّق حمولة
استهلاك صريحة من الخادم **فتخصم الطبقات نفسها مرة ثانية**. كل بيع مرّ
بالمسار الذرى منذ 702 خصم طبقات التكلفة مرتين — بصمت ما دام فى المخزون
فائض يمتص الخصم الثانى (وهذا هو أصل انحراف «تقييم المخزون −5.41» الذى
رصده فاحص السلامة)، وبصوت عالٍ الآن: 1 − 1 (الحارس) − 1 (الحمولة) = −1.

### الإصلاح — مبدأ المستهلِك الواحد

`post_accounting_event` يرفع راية محلية للمعاملة حين تحمل الحمولة
استهلاكاً صريحاً؛ فيتحول الحارس إلى **التسعير قراءةً فقط**
(`calculate_fifo_cogs` — نفس الطبقات، نفس الترتيب، نفس المبلغ) ويترك
الخصم للحمولة وحدها. المسارات القديمة بلا حمولة يبقى الحارس مستهلكها
الوحيد — لا سلوك انكسر.

### البروفة (نسخة الاختبار، ارتدت)

اعتماد صرف كامل بحمولة صريحة: خصم واحد بالضبط (1→0)، سطر استهلاك واحد،
قيد تكلفة صحيح بتكلفة الطبقة الحقيقية، قيد الإيراد المؤجل مرحّل متوازن.
السلوك المعطوب كان سينهار (−1).

الهجرة مطبَّقة على قاعدتى الاختبار والإنتاج — إصلاح فورى بلا انتظار نشر.

**للجلسات القادمة:** الخصم المزدوج منذ 702 ترك أرصدة طبقات منخفضة عن
الحقيقة تاريخياً — تلزم جولة تسوية (FIFO مقابل حركات المخزون) مسجلة فى
دفتر التسليم.

---

## [3.74.785] - 2026-07-22 — الإيراد يُعترف به عند التسليم، لا قبله

### قاعدة المالك (نصاً)

«القيد المحاسبى يجب أن يتم **بعد اعتماد مسئول المخزن التابع للفرع بالتسليم
للعميل**» — و«التحصيل بعد التسليم فقط». وهذا يوافق IFRS 15: الإيراد عند
انتقال السيطرة على البضاعة.

### ما كان يحدث

«تحديد كمرسلة» كانت تسجّل قيد الإيراد **فوراً** — قبل أن تغادر البضاعة
المخزن. ولو رفض مسئول المخزن التسليم لاحقاً (بلا دفعات) كانت الفاتورة
ترجع مسودة **ويبقى قيد الإيراد حياً فى الدفاتر** — قيد يتيم لفاتورة
مسودة (عيب كامن كشفته المراجعة الشاملة، تُصلحه القاعدة الجديدة بالبنية
لا بالترقيع).

### التسلسل الجديد

1. المحاسب «تحديد كمرسلة» ← حالة + إشعار «فاتورة جاهزة للشحن» لمسئول
   المخزن. **لا قيد.**
2. مسئول المخزن يعتمد التسليم ← قيد الإيراد المتوازن + إخراج المخزون +
   قيد التكلفة **فى معاملة واحدة** (`approve_sales_delivery_v2`) — يكتمل
   الكل أو يرتد الكل.
3. الرفض قبل الاعتماد ← رجوع نظيف للمسودة، لا قيد يُعكس.
4. حارس جديد فى قاعدة البيانات (`trg_payment_requires_revenue_je`): لا
   دفعة على فاتورة بلا قيد إيراد مرحَّل — «التحصيل بعد التسليم فقط».

قرارات المالك المكمّلة: فاتورة الخدمات شقّان — منتجات مستهلكة فى تنفيذ
الخدمة تُعترف تكلفتها فور التنفيذ (مسار عهدة الحجز القائم)، ومنتجات
مباعة تنتظر اعتماد التسليم؛ فاتورة خدمات فقط قيدها عند «مرسلة» (لا بضاعة
تنتظر — شرط التأجيل يقرأ `warehouse_status='pending'` تحديداً كى لا
تُترك فاتورة خدمات بمزود شحن بلا قيد إلى الأبد)؛ فرع بلا مسئول مخزن يبقى
الاعتماد التلقائى الفورى (القيد والإخراج معاً). فاحص التحقق المحاسبى
عُلِّم القاعدة كى لا يعدّ فاتورة فى طابور الشحن «فاتورة بلا قيد».

### البروفة (نسخة الاختبار، ارتدت بالكامل)

إرسال بلا قيد ✓ — اعتماد ينشئ قيداً مرحَّلاً متوازناً 798/798 مع
`warehouse_status=approved` فى معاملة واحدة ✓ — دفعة قبل التسليم مرفوضة
بالرسالة العربية ✓ — دفعة بعد الاعتماد تمر ✓.

الهجرة مطبَّقة على قاعدتى الاختبار والإنتاج.

---

## [3.74.784] - 2026-07-22 — فاتورة بخصم «بعد الضريبة» لم يكن يمكن ترحيلها قط

### كيف انكشفت

خاتمة اختبار المالك للدورة الجديدة: المحاسب ضغط «تحديد كمرسلة» على INV-00002
(خصم ١٠٪ بعد الضريبة) فرفضها حارس التوازن:

> `UNBALANCED_JOURNAL_PAYLOAD: Debit=274.60 Credit=294.00`

**الفجوة ١٩٫٤٠ = الخصم بعد الضريبة بالضبط.**

### العلّة

بانى قيد الإيراد (`prepareInvoiceRevenueJournal`) يقيّد المدين بإجمالى
الفاتورة (الصافى النهائى) والدائن بمكوّنات **ما قبل الخصم**: الإيراد
والضريبة والشحن — **ولا يقرأ الخصم إطلاقاً**.

خصم «قبل الضريبة» كان ينجو مصادفةً لأن المجموع الفرعى يصله صافياً أصلاً.
أما «بعد الضريبة» فلا شىء فى الجانب الدائن ينقص، **فكل فاتورة كهذه تصل
الحارس غير متوازنة ولا يمكن ترحيلها أبداً** — علّة من فئة «مسار لم يُختبر
قط»، الخامسة هذا الأسبوع.

**والحارس الذى رفضها هو البطل هنا:** قيد أعوج لم يلمس الدفاتر.

### الإصلاح — يتوازن بالبناء لا بالأمل

بعد أسطر الإيراد/الضريبة/الشحن، تُحسب الفجوة **من نفس الأرقام الموضوعة على
الأسطر**: `(إيراد+ضريبة+شحن) − الإجمالى`.

- فجوة موجبة (خصم بعد الضريبة) ← **مدين «خصم المبيعات المسموح به»** —
  حساب 4120 القائم فى دليلك (`sub_type='sales_discounts'`) فيظهر الخصم
  كحساب مقابل للإيراد كما يجب؛ وإن غاب الحساب فى شركة ما، يُصفّى على
  الإيراد فيبقى القيد متوازناً.
- فجوة سالبة (تسوية رافعة للإجمالى) ← دائن «تسوية على الفاتورة».

مع تسجيل قيم العملة الأصلية للفواتير الأجنبية (IAS 21) كباقى الأسطر.

**لا فحص سالب فى أى حارس لهذا الإصدار** — الدرس ثابت.

---

## [3.74.783] - 2026-07-22 — المحاسب يُشعَر بالفواتير، لا بأوامر البيع

### قاعدة المالك (نصاً، أثناء اختبار الدورة الجديدة)

> «ما أهمية إرسال إشعار إلى المحاسب بإنشاء أمر البيع وهو ليس من اختصاصه،
> وهو يبدأ عمله بعد إنشاء الفاتورة المرتبطة بأمر البيع؟»

### ما كان يحدث

`SalesOrderNotificationService` من بقايا الدورة القديمة: إن وُجدت فاتورة أشعر
المحاسب بها، **وإلا تراجع لإشعاره بالأمر المجرد**. فى دورة الاعتماد الواحد
(v3.74.782) الأمر ذو الخصم المعلّق **بلا فاتورة بعد — وقد يُرفض ولا يصير
فاتورة أبداً** — فكان الإشعار ضجيجاً عن مستند خارج اختصاصه.

### الإصلاح

حُذف مسار التراجع: **لا إشعار للمحاسب إلا بفاتورة.** أمر بلا خصم ← تولد
فاتورته فوراً ويُشعر بها من هنا؛ أمر بخصم ← يُشعر عند ولادتها من الاعتماد
(مسار قائم، **مُثبَت من اختبار المالك الفعلى**: وصله «فاتورة مبيعات جديدة
تحتاج إجراء» بعد الاعتماد). إشعار القيادة بالأمر الجديد باقٍ كما هو —
فذلك اختصاصها.

### مسجَّل من نفس جلسة الاختبار (للتسليم، تجميلية)

نص إشعار الفاتورة عرض «بقيمة 0.00» لأنه صيغ فى اللحظة التى سبقت إعادة حساب
الإجمالى — **بيانات الفاتورة نفسها صحيحة (٢٧٤٫٦٠، مُتحقَّق)**. عائلة «الصفر
المؤقت» ذاتها، فى موضع رابع.

---

## [3.74.782] - 2026-07-22 — خصم واحد، اعتماد واحد: أمر البيع هو المصدر

### مواصفة المالك (نصاً، ٢١ يوليو)

> «الخصم يُعتمد مرة واحدة — على أمر البيع. لو اعتُمد تُنشأ الفاتورة التابعة.
> الموظف ليس له حق الاطلاع على الفاتورة؛ محاسب الفرع يستكملها بلا تعديل.
> وفواتير البيع ليس لها اعتماد.»

### ما كان يحدث (مُثبَت من الإنتاج، SO-0002 ← INV-00002)

إنشاء أمر بيع بخصم كان يفعل كل هذا فى طلب واحد: **٤٥٣.** يُقدَّم طلب اعتماد
خصم الأمر ← **٨٣٥.** تُنشأ الفاتورة فوراً — **والقرار لم يُتخذ** — ويُقدَّم
لها طلب اعتماد **ثانٍ**. ثم يُحاصَر المحاسب برسالة 409 تدلّه على صندوق موافقات
**يرفضه بـ403**. وقراران مستقلان لنفس العشرين جنيهاً يمكن أن يتناقضا.

**والمفارقة:** آلية التخطى (`app.skip_discount_approval`) **مصمَّمة وموثَّقة فى
ترحيل قديم ولم تُوصَّل قط** — بحث شامل: لا يضبطها أى كود.

### التنفيذ — ست رقع، كلها فى قاعدة البيانات، بلا لمس للواجهات

1. **لا فاتورة قبل القرار.** الإنشاء التلقائى يتخطّى بأدب (يُرجع
   `skipped` والمسار الأمامى يتعامل معها دون أى تعديل — مُتحقَّق).
2. **حاجز قاعدة بيانات:** أى إدراج فاتورة لأمر خصمه معلّق يُرفض — يغطى كل
   المسارات بما فيها التحويل اليدوى.
3. **القرار هو المفتاح:** الاعتماد **يُنشئ الفاتورة بنفسه** (وذرّياً يُلغى أى
   توأم قديم معلّق كوارثٍ للقرار). الرفض ← لا فاتورة، والموظف يعدّل الأمر،
   وتعديله يُقدّم طلباً جديداً تلقائياً (محفِّز قائم، مُجرَّب).
4. **«إجمالى: ٠٫٠٠» فى صندوق الموافقات:** كان يلتقط `total_amount` الذى يتركه
   النظام صفراً والحقيقى فى `total`. و`COALESCE` وحدها لا تنفع — **الصفر ليس
   فارغاً** — فـ`NULLIF` أولاً. الآن يظهر الإجمالى الحقيقى.
5. **علّة كامنة مكشوفة بالبروفة:** تعديل مادى لمسودة **بلا خصم** كان ينهار على
   قيد `CHECK (discount_value > 0)` — محفِّز التعديل يُدرج صفراً بلا حارس.
6. **فواتير البيع التابعة لا تُنشئ طلبات إطلاقاً** — من أى مسار، بما فيه محفِّز
   التعديل الذى كان مصدر التوأم فى الإنتاج (يلتقط تحوّل الإجماليات أثناء
   الإنشاء نفسه كأنه «تعديل مادى»). الحوكمة محفوظة: حارس الترحيل يشترط تطابق
   خصم الفاتورة مع المعتمَد على الأمر.

**ورسالة المحاسب** صارت تسمّى صاحب القرار: «بانتظار اعتماد المالك / المدير
العام» بدل توجيهه إلى باب مغلق. وفحص جديد فى مسار الترحيل يقرأ حالة **أمر
البيع** مباشرة بعد أن لم يعد للفاتورة صفوف خاصة.

### البروفة — على نسخة مطابقة من الإنتاج، ثمانية سيناريوهات

أمر بخصم لا يُنشئ فاتورة ✓ الاعتماد يُنشئها وصفر توائم ✓ الرفض يمنعها ✓
تعديل الموظف يعيد الطلب ✓ الفاتورة المباشرة قبل البت مرفوضة ✓ أمر بلا خصم
يمرّ فوراً ✓ الإجمالى ٥٠٠ لا صفر ✓ تعديل مسودة بلا خصم لم يعد ينهار ✓

وثلاثة دروس بروفة: نسخ أمر **بلا بنود** فجّر سلسلة محفِّزات لا يراها الواقع؛
وراية جلسة تسرّبت **داخل معاملة الاختبار** فعطّلت فحصاً (منفصلة الطلبات فى
الواقع)؛ وتعادل التوقيتات داخل المعاملة جعل «الأحدث» عشوائياً — كلها وثِّقت.

### أرقام الإنتاج قبل وبعد

**متطابقة**: ٩٠ قيداً، ٢٬٦٤١٬٣٥٠٫٨٥ مديناً ودائناً. تعديل دورة اعتماد لا يمسّ
دفاتر.

### خارج النطاق، مُسجَّل للتسليم

تعديل شحن فاتورة تابعة كشف محفِّزاً يقرأ حقلاً غير موجود (`last_edited_by`) —
علّة قائمة مستقلة تحتاج جلستها.

---

## [3.74.781] - 2026-07-21 — مرتجعات المبيعات: ميزة لم تعمل ولا مرة

### الاكتشاف

**لم يكتمل مرتجع مبيعات واحد فى هذا النظام إطلاقاً** — `sales_returns` فارغ.
لم يكن ذلك لقلة الاستخدام، بل **لأن الميزة لا يمكن أن تعمل**.

حارسان يتصادمان: `sales_return_approval_insert_trg` يرفض أى حالة غير
`draft`/`pending_approval` ما لم يكن `created_by_user_id` مالكاً أو مديراً
عاماً — و`post_accounting_event` **لم يكن يُدرج ذلك العمود إطلاقاً**، والدورة
تُدخل الحالة `completed`. فـ`v_role` كان دائماً فارغاً، والرفض دائماً، والمعاملة
كلها تتراجع.

**ولو مُلئ العمود لما كفى:** مَن ينفّذ الخطوة الأخيرة هو **أمين المخزن**، لا
المالك. الحارس لا يستطيع التمييز بين الدورة المعتمَدة وبين شخص يصنع مرتجعاً
منتهياً بيده.

**أُثبت لا استُنتج:** تشغيل نفس إدراج الدورة على نسخة مستعادة من الإنتاج يُعيد
إنتاج الرفض حرفياً.

**الحل:** الدورة تُعلن عن نفسها بـ`app.sales_return_workflow`، يُضبط داخل
`post_accounting_event` لعمر معاملتها فقط. لا يمكن ضبطه من المتصفح، ويموت مع
المعاملة — نفس آلية `app.allow_direct_post` المستخدمة أصلاً. **والمرتجع المباشر
بلا اعتماد ما زال مرفوضاً**، وهو مُختبَر فى نفس البروفة.

### وخلف ذلك الباب المقفول، خمسة عيوب أخرى

**١. عكس التكلفة كان بسعر البطاقة لا التكلفة الحقيقية.** المُحفِّز يسأل عن تاريخ
الاستهلاك برقم **المرتجع**، وهو مُسجَّل برقم **الفاتورة**. فلا يجد شيئاً أبداً،
ويسقط إلى `products.cost_price` — **نفس العيب الذى أزال أربع دوال فى
`3.74.726` و`3.74.759`**. الفرع الصحيح كان غير قابل للوصول.

**٢. الطبقات تُعاد مرتين.** `restore_fifo_lots_on_return` تُعيدها صحيحةً، ثم
`lib/sales-returns.ts` يبنى عكساً **كاملاً لكل أصناف الفاتورة** يُطبَّق فوقها.
أُزيلت طبقة الكود: **قاعدة البيانات تملك العملية، ووجود مالكَين هو المشكلة**.

**٣. سجل التكلفة والقيد يُحسبان من رقمين مختلفين.** صار المُحفِّز يكتب السجل
بنفسه من نفس الرقم الذى يُرحّله. **لا يمكن أن يختلفا لأنه رقم واحد.**

**٤. فحص المطابقة لا يرى المرتجعات.** يقارن بقيود من نوع `sale_return_cogs`
والمُحفِّز يكتب `cogs_return`. أُعيدت التسمية بعد التأكد أن الاسم القديم غير
مذكور فى أى كود أو دالة أو صف.

**٥. المُحفِّز يقرأ عموداً غير موجود** (`transaction_date`؛ الجدول فيه
`created_at`). كان سيفشل فور بلوغه. **عائقان متراكبان.**

### وثلاثة روابط لم تُكتب قط

- `sales_return_items.invoice_item_id` — **عطّل صامتاً نصف حارس الكمية**: الحارس
  يجمع هذا العمود، فيحصل على صفر دائماً، فلا يقيّد الكمية إلا الطلبات المعلّقة.
  **مرتجعات متتابعة كان يمكنها تجاوز الكمية المباعة.** الرقم كان يتدفق فى كل
  الطبقات؛ الإدراج وحده أسقطه.
- `sales_returns.journal_entry_id` — القيد موجود، ولا شىء يشير إليه.
- `invoice_items.returned_quantity` — فارغ دائماً، و`fix-inventory` يطابق عليه.

### وباب خلفى، وسباق

`POST /api/sales-returns` كان يُدرج جسم الطلب مباشرة فى `sales_returns`: **بلا
بنود، ولا مخزون، ولا قيد، ولا اعتماد.** أُوقف بـ410 مع بقاء المصادقة أولاً.

وطلبات المرتجع كانت «اقرأ ثم أدرج» بلا شىء بينهما. فهرس فريد يمنع الآن طلبين
مفتوحين لفاتورة واحدة، **مع السماح بطلب لاحق بعد إغلاق السابق** (مُختبَر).

### أخطاء وقعتُ فيها أثناء الإصلاح — أمسكها الاختبار لا القراءة

**١. برّأتُ المُحفِّز خطأً** بعد قراءة نصفه، ثم أدانه تدفّق البيانات.
**٢. نقلتُ عيب العمود غير الموجود إلى نسختى** حرفياً؛ ظهر عند أول إدراج فعلى.
**٣. اخترتُ قيماً سالبة** لسجل التكلفة، **وقيود الجدول ترفضها** — فهو يخزّن
مقادير والاتجاه يحمله نوع المصدر. تغيير قيد على جدول مالى ليناسب اختيارى كان
سيكون عكس الصواب.
**٤. أول مُرحِّل لم يكن آمناً للتكرار** — إعادة تشغيله كانت ستُضيف كل تعديل
مرتين.

**وأُبلغتُ بعيب خامس «خطير» فى عمود غير موجود — فتحققتُ، والعمود موجود.**

### التحقق

**أول مرتجع مبيعات ناجح فى تاريخ المشروع**، على نسخة مطابقة من الإنتاج، بتسع
فحوص: اكتمال المرتجع · طبقة واحدة مُعادة لا اثنتان · **التكلفة ٣٠٠ الحقيقية لا
٩٩٩ سعر البطاقة** · نوع القيد · تطابق السجل مع القيد · صفر ملاحظات فى فحص
المطابقة · **رفض المرتجع المباشر** · توازن الدفاتر.

وجُعل سعر البطاقة مخالفاً **عمداً**، لأنهما متطابقان فى بياناتك ولو اختُبر عليها
**لما أثبت شيئاً**.

أرصدة الإنتاج قبل وبعد التطبيق: **متطابقة** (٢٬٦٤١٬٣٥٠٫٨٥). وأُزيلت ٦ بنود
مرتجع يتيمة من ديسمبر ٢٠٢٥ بعد التأكد من خلوّها من أى أثر مالى.

### ما لم يُنجز بعد

خطوات ما بعد المعاملة (الاسترداد النقدى، تحديث الحالة، المكافآت) ما زالت خارج
الحماية وفشلها يُسجَّل فقط. **إصلاحها إعادة تصميم لطريقة ردّ النقد، لا تعديل
محدود**، ولها إصدارها الخاص.

---

## [3.74.780] - 2026-07-21 — الحارس نفسه كان أعمى عن نصف عمله

### كيف انكشف

فى الإصدار السابق أُزيلت **ستّ** كتابات غير مفحوصة من شاشات المصروفات.
والفاحص أبلغ **١٤٥ قبلها و١٤٥ بعدها**. لو قُبل ذلك على أنه «لا تغيير يُذكر»
لبقى العطل إلى اليوم.

### العطل

القاعدة كانت تبحث **فى سطر واحد**، مثبّتة بـ `^\s*await`. فلا ترى إلا:

```
await supabase.from("t").update({ ... })
```

بينما الأسلوب السائد فى هذا المشروع موزّع على أسطر:
...
وكل واحدة من هذه كانت **غير مرئية**. ومعها كل ما لا يبدأ سطره، مثل:

```
try { await admin.from("audit_logs").insert({ ... }) } catch {}
```

**وهذا هو بعينه ما وُجد هذا الفاحص ليمنعه:** أداة تُبلغ بالنجاح وهى تؤدى جزءاً
من عملها. وكانت الحارس على كل إصدار منذ كُتبت.

### تصحيح للسجل

ذُكر فى ملاحظات `3.74.779` أن العدد الحقيقى **٢٨١** وأن **١٣٦** غير مرئية.
**هذا الرقم كان خاطئاً.** قيس على عجل بلا تنظيف للتعليقات، فعدّ أمثلة الشيفرة
داخل التوثيق — بما فيها الأمثلة فى ترويسة هذا الملف نفسه.

**العدد الصحيح: ٢٧٢، منها ١٢٧ لم تكن مرئية.**

### القاعدة الجديدة

الفحص صار **على مستوى الجملة لا السطر**. والأصل أن الكتابة **غير مفحوصة**، ولا
تُستثنى إلا إذا كان أحدهم يستقبل نتيجتها فعلاً (إسناد، أو وسيط، أو `return`،
أو `.then`).

**ولُف الفحص حول try/catch لا يجعلها مفحوصة، والسكربت يقول ذلك صراحةً:**
`supabase-js` لا تُطلق خطأً عند فشل الكتابة، فالـ`catch` لا يعمل أصلاً. وهذه
أخدع الأشكال فى المشروع كله.

### ثلاثة أخطاء وقعتُ فيها أثناء الإصلاح، وكلها أمسكها الاختبار

**١. استثناء `.then` بالقرب لا بالانتماء.** أول نسخة بحثت عن `.then/.catch`
فى ٨٠٠ حرف تالية، **فأخفت خمس كتابات حقيقية** — من بينها **حذف فاتورة وبنودها**
فى `sales-orders`. الذى طابقتُه كان `response.json().catch(...)` على بُعد ٢٣
سطراً، **نداءً مختلفاً تماماً**. وهذا نفس الخطأ المتكرر: *القريب من الهدف ليس
الهدف*. الآن يُمشى إلى نهاية الجملة فعلياً بتتبّع الأقواس.

**٢. الاعتماد على الفاصلة المنقوطة.** القاعدة اشترطت `;` أو `{` قبل `await`.
**والمشروع لا يستخدم الفاصلة المنقوطة**، فجملة تلى جملة بسطر جديد فقط. كُشف حين
شُغّل الفاحص المُصلَح على شيفرة المصروفات **قبل** الإصدار السابق: وجد ٣ من ٦.

**٣. `"نص".includes("")` تُرجع صحيحاً.** فصار «لا شىء قبل الكتابة» يُقرأ على أنه
«أحدهم يستقبلها». **أمسكه الاختبار الذاتى من أول تشغيل.**

### التحقق

**اختبار رجوع حاسم:** شُغّل الفاحص المُصلَح على شيفرة المصروفات قبل `3.74.779`
— **فوجد الستّ كلها، فى أسطرها بالضبط** (٤٨٠، ٤٩٣، ٥٠١، ٤٠٦، ٤٤٥، ٤٧٢).

وفُحصت عيّنة من الـ١٢٧ المكتشفة يدوياً: **تحديثات رواتب، وحذف فواتير وبنودها،
ورفض طلبات استرداد، وتحديث شعار شركة، وقسائم خصم** — كلها بلا فحص.

وفُحص الاتجاه المعاكس: **صفر حالة** فيها نتيجة مُستقبَلة وحُسبت خطأً.

### إضافات دائمة

- `--list` لعرض كل حالة. **أُضيف لأن مقارنة نسختين من القاعدة مستحيلة حين يكون
  الناتج رقماً فقط — والرقم وحده هو ما أخفى العطل طوال هذه المدة.**
- اختبارات ذاتية جديدة تُثبّت الأشكال الثلاثة التى كسرت القاعدة، فى الاتجاهين.

### الرقم

**١٤٥ ← ٢٧٢.** لم تُضَف كتابة واحدة؛ **صارت مرئية فقط.**

---

## [3.74.779] - 2026-07-21 — المرحلة ٣: المصروفات تنتقل إلى الخادم

### المشكلة

المصروفات لم يكن لها **أى مسار خادم**. كل شىء يحدث فى متصفح الموظف، عبر
ثلاثة مسارات منفصلة تكتب فى الدفاتر المحاسبية مباشرة.

والخطر لم يكن فى القيد نفسه — القيد كان يُكتب بعملية واحدة آمنة. الخطر فى
**الخطوة التى تليه**: أمر ثانٍ منفصل يربط المصروف بقيده، **بلا فحص إطلاقاً**.
انقطاعٌ فى تلك اللحظة يترك القيد مُسجَّلاً فى الدفاتر والمصروف يبدو غير مدفوع،
والموظف يرى «تم الاعتماد بنجاح».

وهذا الاتجاه **لم يكن له فحص واحد فى النظام كله**. الاتجاه المعاكس له فحص
(`ic_expense_no_journal`). هذا لا.

ومسار الاعتماد التلقائى للمالك الوحيد كان الأسوأ: **ثلاثة أوامر غير مفحوصة**،
من بينها أمر «التراجع» نفسه — أى أن رسالة «تم التراجع» كانت قد تظهر بلا تراجع.

### الحل

**القيد والربط صارا عملية واحدة داخل قاعدة البيانات.** لا يمكن أن ينفصلا، ليس
لأن الفحص تحسّن بل لأن الانفصال لم يعد ممكناً. ولم يعد هناك «تراجع» أصلاً،
لأن نصف الاعتماد لم يعد موجوداً.

**الدوال الجديدة**

- `post_expense_atomic` — يكتب القيد، ويربط المصروف، ويسجّل الأثر، ويقبل مرجع
  السداد، كلها فى معاملة واحدة. مُتماثل: الضغط مرتين يعطى قيداً واحداً.
- `approve_expense_atomic` — الاعتماد والترحيل معاً.
- `reject_expense_atomic` — الرفض، **وله تحقّق صلاحية لأول مرة**.
- `expense_actor_may_approve` — مصدر واحد لقاعدة الصلاحية.

**المسارات الجديدة** — أول مسارات خادم فى وحدة المصروفات على الإطلاق:
`/api/expenses/[id]/approve`, `/reject`, `/post`.

### ملاحظتان تستحقان التسجيل

**١. `create_journal_entry_atomic` لا تُطلق أخطاءها — تُعيدها.** جسمها ينتهى بـ
`EXCEPTION WHEN OTHERS THEN RETURN success:false`. فالمستدعى الذى يقرأ «أنها
عادت» لا يعرف شيئاً، وكان سيربط المصروف بقيدٍ لم يُكتب. **نفس نمط الكتابة غير
المفحوصة، طبقةً أعمق.** لذلك تفحص الدالة الجديدة القيمة المُعادة وتُطلق
الاستثناء بنفسها.

**٢. الحماية كانت موجودة أكثر مما ظُنّ.** قبل بناء حماية من تعديل أو حذف مصروف
مُرحَّل، جرى فحص إن كانت موجودة — **فوُجدت وتعمل** (`trg_block_expense_delete`,
`trg_block_expense_immutable_edits`). **كادت تُبنى نسخة ثانية من حارس قائم.**

لكن الفحص كشف ثغرة حقيقية: `trg_expense_paid_requires_journal` كان **عند
التعديل فقط لا عند الإنشاء**، فمصروف يمكن أن يُولد «مدفوعاً» بلا قيد. **سُدَّت،
مع التأكد أن الإنشاء العادى ما زال يعمل.**

### قرار المالك — ٢١ يوليو ٢٠٢٦

**الاعتماد للمالك والمدير العام فقط.** وهذا **أضيق** مما كان (كان يشمل `admin`).
جرى التحقق من الإنتاج قبل التغيير: **لا يوجد `admin` واحد فى أى من الشركات
الأربع**، وكل شركة تحتفظ بمعتمِد. لا أحد يفقد صلاحيته.

و`manager` مُستبعَد — وهذا يحسم تناقضاً ثلاثياً: كان يُرسَل له إشعار «مصروف
ينتظر اعتمادك»، ثم يرفضه زر الواجهة، ثم ترفضه سياسة قاعدة البيانات.

**فصل المهام لم يتغيّر عمداً**: `trg_expense_sod_guard` ما زال يحكم الاعتماد
الذاتى كما كان.

### الانتحال

الفاعل هو `auth.uid()` كلما وُجدت جلسة. `p_actor_id` يُقبل **فقط** حين تكون
`auth.uid()` فارغة. بدون ذلك، مستخدم مسجَّل يستدعى الدالة مباشرة كان يمكنه
تسجيل الاعتماد **باسم شخص آخر**.

### ما جرى التحقق منه

بروفة كاملة على **نسخة مطابقة من الإنتاج** (٧ مصروفات، ٨٩ قيداً، ٩٢٢ سطراً)،
ثم تحقّق على الإنتاج نفسه بإلغاء تلقائى:

اعتماد كامل · الضغط مرتين · مرجع السداد على مصروف مُرحَّل · الترحيل والمرجع معاً ·
موظف يحاول الاعتماد (مرفوض) · موظف يحاول الرفض (مرفوض) · حالة خاطئة · رفض بلا
سبب · إنشاء «مدفوع» بلا قيد (مرفوض) · حذف مصروف مُرحَّل (مرفوض) · **إنشاء مصروف
عادى (يعمل)** · الدفاتر متوازنة.

**وأهمها:** أُفشلت خطوة الربط عمداً — **فلم يبقَ قيد يتيم واحد**.

أرصدة الإنتاج قبل وبعد: **متطابقة إلى القرش** (٢٬٦٤١٬٢٢٥٫٨٥ مدين ودائن).

### ما بقى فى المتصفح عمداً

الإشعارات. تغييرها يعنى نقل سباكة إشعارات فى نفس الإصدار الذى ينقل المحاسبة،
وفشل إشعار ليس مشكلة دفاتر. **شىء واحد فى كل مرة.**

---

## [3.74.777] - 2026-07-21 — ✅ اكتملت العائلة: خمسة من خمسة

**تصحيح المدفوعات** — للعملاء وللموردين — صار يُسجّل من نفّذه.

تصحيح الدفعة **يُلغى دفعة ويُرحّل بديلتها**: قيدان محاسبيان ودفعتان، من قرار واحد.
**ولم يكن شىء من ذلك يُسجّل صاحب القرار.**

الآن كل ما أنتجته العملية مربوطٌ بأثر واحد: **الطلب · الدفعة الأصلية · دفعة
الإلغاء · الدفعة البديلة · قيد الإلغاء · القيد الجديد**.

### قرارٌ معاكس لِما فعلناه أمس — والسبب مختلف لا متناقض

فى v3.74.774 وضعتُ الأثر **داخل دالة الترحيل** وكتبتُ أن الكتابة عند المُنادى
«تتطلب العثور على كل المُنادين، وهذا موضع أخطائى».

**هنا وضعتُه عند المُنادى.** والفارق ليس تناقضاً:

| | عهدة الحجوزات | تصحيح المدفوعات |
|---|---|---|
| المُنادون | عدة، **بينهم تلقائى** | **صفر فى قاعدة البيانات**، ومسارٌ واحد — مُتحقَّق من `pg_proc` |
| حجم الدالة | ٥ كيلوبايت | **١٤ و١٢ كيلوبايت** |
| تكتب القيد | عبر دالة مساعدة | **مباشرةً** |

**إعادة كتابة ٢٦ كيلوبايت من كود مالى لإدراج أربعة أسطر** هى بالضبط خطر النسخ
اليدوى الذى بُنيت `append-function-to-migration.js` لتجنّبه أمس.

**والدالتان تُعيدان كل معرّف أنشأتاه** — فالروابط تصف **ما حدث فعلاً**، لا ما طُلب.

### ومُساعد مشترك بدل النسختين ١٦ و١٧

`lib/financial-trace.ts`.

الفحص وجد `createTrace`/`linkTrace` **مُعادة كتابتها فى خمس عشرة خدمة**، **وقد
تباعدت فعلاً**: واحدة تُرتّب نتائجها بالتاريخ وأخرى لا، فتُعيدان صفوفاً مختلفة عند
تعدد الروابط من نفس النوع. **عيبٌ كامن، وكل نسخة جديدة تُضاعفه.**

فحين احتاج موضعان جديدان للتتبُّع، **كُتب المُساعد بدل أن تصير سبعة عشر**.

**ولم تُمسّ الخمس عشرة القائمة** — توحيدها يعنى اختيار سلوك واحد لكل الخدمات
المالية دفعةً واحدة، ويستحق جلسته واختباره.

### وثلاث قواعد مكتوبة فى المُساعد

| القاعدة | لماذا |
|---|---|
| **التتبُّع لا يُعطّل العملية أبداً** | كل فشل يُسجَّل ويُبتلع. سجلٌّ قادر على إفشال عملية مالية مشكلةٌ أكبر من سطر ناقص |
| **الفاعل يُسجَّل بصدق، ولو كان معدوماً** | `null` لا حساب خدمة بديل. سجلٌّ يخترع فاعلاً أسوأ من سجلٍّ يعترف |
| **الروابط من الناتج الفعلى** | تصحيحٌ لم يُنتج قيد إلغاء لا يحمل رابطاً يدّعى أنه أنتجه |
...
| خروج العهدة | ✅ |
| إرجاع العهدة | ✅ |
| تكلفة استهلاك الخدمة | ✅ |
| **تصحيح مدفوعات العملاء** | ✅ |
| **تصحيح مدفوعات الموردين** | ✅ |

---

## [3.74.776] - 2026-07-21 — إرجاع العهدة وتكلفة الخدمة صارا مُتتبَّعين

> يشمل هذا الإصدار الترحيلين **3.74.775** (إرجاع العهدة) و**3.74.776**
> (تكلفة استهلاك الخدمة). كُتب العنوان أولاً بصيغة `[3.74.775 + 3.74.776]`
> **فرفضه خطاف ما قبل الدفع** — وهو يشترط رقم الإصدار الحالى وحده. الخطاف محقّ:
> الإصدار المُعلن فى `version.ts` واحد، والعنوان يجب أن يطابقه.

**ثلاثة مسارات من خمسة اكتملت**، كلها مُجرَّبة على نسخة من الإنتاج قبل تطبيقها عليه.

| المسار | ما يُسجَّل الآن |
|---|---|
| خروج العهدة *(٧٧٤)* | الحجز · طلب السحب · حركة المخزون · قيد اليومية |
| **إرجاع العهدة** *(٧٧٥)* | نفسها |
| **تكلفة استهلاك الخدمة** *(٧٧٦)* | الفاتورة · **كل صف استهلاك** · قيد اليومية |

### الاختبار كان دورة كاملة، لا خطوة معزولة

إرجاع العهدة **بلا معنى منفرداً**، فجُرِّب كما يحدث فعلاً: **خروج ثم إرجاع**.
...
### وتكلفة الخدمة لها شكل مختلف — والفارق مقصود

تُعالج **دفعة صفوف** وتُنتج **قيداً واحداً**. فالأثر يُفتح **بعد** تحديد الدفعة لا قبل
أول كتابة.

**استهلاك FIFO يسبق الأثر** إذن — لكن **كل صف مُستهلَك مربوطٌ به**، فالعملية تبقى
قابلة لإعادة البناء كاملةً.

والبديل — فتح الأثر أولاً — يعنى مفتاح تكرار **لا يُميّز إعادة المزامنة عن الترحيل
الأصلى**، وهى الثغرة التى كُتب v3.74.705 لإغلاقها.

**اخترتُ الربط الصحيح على الترتيب الأنيق، وسجّلتُ المقايضة** بدل تركها لمن يكتشفها.

### وحارسان رفضا اختبارى — بحق

| الرفض | السبب |
|---|---|
| `Cost center does not belong to company` | مركز تكلفة من شركة أخرى |
| `لا يُمكِن خَصم ١ — الرَّصيد الحالى ٠` | رصيد فرع صفر |

**رفضان صحيحان**، ودليل إضافى أن الحماية حول هذه المسارات تعمل. أعدتُ الاختبار على
منتج له رصيد ١٧٨ فعلى: **تكلفة ٦٠٠، قيد أُنشئ، والصف مُختَم**.

### وأداة جديدة: `append-function-to-migration.js`

**الترحيل سجلٌّ لما طُبِّق.** ولو أعدتُ كتابة ٢٠٠ سطر بيدى، لسجّل الملف **ما كتبتُه
أنا** لا ما يعمل عندك — وهما شىءٌ واحد **حتى لا يكونا**، وسطرٌ مُبدَّل فى دالة تكتب
فى الدفتر **لا يُعلن عن نفسه**.

الأداة تقرأ التعريف من `pg_get_functiondef`. **فلا يمكن للملف أن يخالف الإنتاج،
لأنه لم يكن نسخةً أصلاً.**

**وترفض العمل عند وجود توقيعات متعددة** — التوقيعات المتعددة أوقعتنى مرتين اليوم.

### تحقُّق بعد التطبيق

الثلاث: `SECURITY DEFINER` سليم · الحُرّاس قائمة · منطق FIFO محفوظ ·
**كتلتا حماية فى كلٍّ** · **صفر وصول لغير المسجَّل**.

### المتبقى فى هذه العائلة

`execute_payment_correction` و`execute_vendor_payment_correction` — نفس النمط.

---

## [3.74.774] - 2026-07-21 — عهدة الحجوزات صارت تُسجّل من نقلها

### ما أُصلح

خروج عهدة الفنّى — نقل بضاعة من مخزنك إلى يد فنّى — **لم يكن يُسجّل من نفّذه**.
سؤال المدقق «من أخرج هذه المواد ومتى وبأى إذن؟» **لم يكن له جواب فى البيانات**.

الآن العملية كاملة مربوطة بأثر واحد:

```
الأثر  : booking_stock_withdrawal → booking_custody_out
الروابط: الحجز | طلب السحب | حركة المخزون | قيد اليومية
```

### قرار التصميم الذى يستحق التسجيل

**الأثر يُكتب داخل دالة الترحيل، لا فى مُنادِيها.**

الدالة تُستدعى من مسارين على الأقل: قرار مسؤول المخزن، **والاعتماد التلقائى** حين
لا يوجد مسؤول مخزن للفرع. الكتابة فى موضع الترحيل **تُغطى كل مُنادٍ، بما فيهم من
يُضاف لاحقاً**.

والبديل — الكتابة عند كل مُنادٍ — يتطلب **العثور على كل المُنادين بشكل صحيح**،
وهذه الجلسة أثبتت مراراً أن هذا بالضبط موضع أخطائى.

### والفاعل قد يكون NULL — عمداً

فى مسار الاعتماد التلقائى **لا إنسان قرّر**. فيُسجَّل `NULL`، ويحمل الأثر علامة
`auto_approved_no_store_manager`.

**أثرٌ يخترع فاعلاً أسوأ من أثرٍ يعترف أنه لا فاعل.**

### والتتبُّع لا يوقف العملية أبداً

كل نداء أثر مُغلَّف. لو فشل التدقيق، **تمرّ حركة المخزون والقيد** ويُسجَّل تحذير.

**سجل تدقيق قادر على تعطيل عملية مادية مشكلةٌ أكبر من سطر تدقيق ناقص** — الفنّى
واقفٌ يحمل القطعة.

### 🔵 وأول تغيير فى تاريخ المشروع يُجرَّب قبل تطبيقه

طُبِّق على **قاعدة الاختبار** وشُغِّل على سحب اصطناعى **قبل أن يرى الإنتاج شيئاً**:

```
ok=true  valued=true  حركة مخزون +١
links: booking | booking_stock_withdrawal/source |
       inventory_transaction | journal_entry
```

**وقاعدة الاختبار موجودة بسبب عمل النسخ الاحتياطية فى الإصدارات ٧٦٨-٧٧٢.**
عائدها الثانى خلال يوم.

### ما لم يتغيّر — وتُحقّق منه بعد التطبيق

| | |
|---|---|
| تقييم FIFO (v3.74.703) | سليم |
| حارس الشركة (v3.74.749) | سليم |
| `SECURITY DEFINER` | سليم |
| المُنادون | ٢، يعملان |
| وصول غير المسجَّل | **صفر** |

### وما لم يُفعل عمداً

**الستة قيود التاريخية لم تُملأ بأثر رجعى.** الأثر يُسجّل **من نفّذ** العملية،
واختراع فاعل لحدث ماضٍ **كذبٌ فى سجل التدقيق**. تبقى ظاهرة كملاحظات.

---

## [3.74.773] - 2026-07-21 — إيقاف ثلاث أدوات تُفسد الدفتر وتقول «تم»

**١٧٩ ← ١٤٥ كتابة غير مفحوصة.** أُزيلت ٣٤، كلها من الفئة الحرجة.

### ما أُوقف ولماذا

**`repair-invoice`** — تحذف سطور قيد الفاتورة وحركات مخزونها ثم تُعيد بناءها.
**١١ كتابة غير مفحوصة.** وفشلٌ فى المنتصف يترك دفتر الفاتورة **محذوفاً ولم يُعَد**،
والرد **٢٠٠**.

**`fix-sent-invoice-journals`** — تُنشئ رأس القيد ثم تُدخل سطوره بلا فحص. فينشأ
**قيد مُرحَّل بلا سطور**: موجود فى الدفتر، يحمل مرجعاً، ولا يُحرّك شيئاً. وعدة دوال
فيها تُرجع `true` بلا شرط، فيَعُدّ التقرير الفاتورة «مُصلَحة».

**صفحة «موازنة قيود الفواتير»** — الأخطر.

### الاكتشاف الذى غيّر التصنيف

الدالة **لم تكن خلف زر فقط — كانت تعمل تلقائياً عند فتح الصفحة**:

```js
useEffect(() => {
  await fixUnbalancedInvoiceJournals()   // ← بلا زر، بلا تأكيد
  ...
}, [loading])
```

**مجرد استعراض التقرير كان محاولة كتابة فى دفترك.**

وما تكتبه **سطور أحادية الجانب**: سطر دائن للإيراد، وسطر مدين للذمم، **بشرطين
مستقلين** — أحدهما قد يعمل دون الآخر. والثانى يُحسب من أرقام **قبل** الإدخال
الأول، فيصير قديماً.

**هذا لا يُصلح المحاسبة — يُجبر الدفتر على التوازن باختراع إيراد وذمم.**

### وأُثبت أنها لم تعمل قط

جُرِّبت على **نسخة مستعادة من الإنتاج** فى قاعدة الاختبار:
...
**والكتابة غير مفحوصة، فابتُلع الرفض.** وكل قيودك مُرحَّلة (اختبار «لا مسودات»
يمرّ) — أى أنها **فشلت فى كل مرة، بصمت، طوال وجودها**.

هذه ثالث أداة اليوم تُبلّغ بالنجاح وهى عاجزة.

### ما حُفظ

**تقرير لقطة الأرصدة سليم ولم يُمسّ** — الدالة كانت دخيلة عليه، لا جزءاً منه.

**وفحص الهوية بقى أولاً** فى المسارين، فغير المسجَّل ما زال يتلقى ٤٠١ لا ٤١٠ —
وهو ما تؤكده اختبارات الأمان.

### والثلاثة كانت تحتسب التكلفة من بطاقة المنتج

نفس عيب أربع دوال أُزيلت فى v3.74.726 و759.

---

## [3.74.772] - 2026-07-21 — ✅ أول نسخة احتياطية مُثبتة فى تاريخ المشروع

### النتيجة

```
الجداول      ٢٤٩ / ٢٤٩        الدوال      ١٢٠٨ / ١٢٠٤
السياسات     ٧٩٩ / ٧٩٧        المشغّلات    ٥٠٧ / ٥٠١

الشركات        ٤ / ٤          الفواتير       ١٨ / ١٨
العملاء       ٢٢ / ٢٢          بنود الفواتير ١٦١ / ١٦١
قيود اليومية  ٨٩ / ٨٩          سطور القيود  ٩٢٢ / ٩٢٢
المدفوعات     ٣٠ / ٣٠          دليل الحسابات ٣٦٧ / ٣٦٧
```

**تسعة جداول محاسبية، تطابق تام صفاً بصف.**

### نجاحٌ بالصدفة، صار نجاحاً بالتصميم

أمر تعطيل فحص المراجع كان يُمرَّر هكذا:

```
-c "SET session_replication_role = 'replica';"
```

وعلى Windows تعمل هذه الأوامر عبر الصدفة، **فقُطع الوسيط عند أول مسافة** ووصلت
`psql` كلمة **`SET`** وحدها:
...
**والبيانات حُمّلت رغم ذلك** — لأن الترتيب الذى اختاره `pg_dump` صادف ألا يخرق
أى مرجع دائرى.

**نجاحٌ حقيقى لسبب خاطئ.** لا فحص يمسك هذا: كل شىء يبدو صحيحاً، والحماية التى
ظننّاها تعمل **لم تكن مُفعَّلة أصلاً**. وكانت ستصمد حتى يوم يختلف فيه ترتيب النسخة
قليلاً — **أى يوم الكارثة بالضبط**.

الإعداد ينتقل الآن **داخل رابط الاتصال** (`options=`)، فلا تلمسه الصدفة إطلاقاً.

**وكشفه أن المالك أرسل الناتج كاملاً** بدل الاكتفاء بالسطر الأخير الأخضر.

### وحلقة لا تنتهى

مسح قاعدة الاختبار توقّف عند ٩٦ دالة **إلى الأبد**: ١٤٩ دالة مملوكة لامتدادات
Postgres لا يمكن حذفها، وحلقتى تطلب نفس الخمسين وتفشل وتُعيد الطلب.

**شرط خروجى كان نجاح العمل، ولم أضع شرطاً لِمَ لا ينجح.**

أُضيف شرطان: استبعاد دوال الامتدادات، **والتوقف عند أى دورة بلا تقدّم** — والثانى
يحمى من أسباب لم أفكر فيها بعد.

**ولم يُبلّغ عنه فحص.** المالك قال «مستمر على ذلك فترة طويلة» — **مرتين اليوم**.
الصمت الذى يبدو عملاً هو آخر ما تراه الأدوات.

### واحتجاز مخرجات عطّل شبكة أمان

تحويل مخرجات `psql` إلى أنبوب علّق السكربت: استعادة مخطط ٣.٥ ميجابايت تُصدر
طوفان إشعارات، والعملية تنتظر أنبوباً لا يُفرَّغ. **العمل كان قد اكتمل** — ٢٤٩
جدولاً و١٢٠٨ دالة — **والسكربت وحده هو العالق**.

**والأخطر:** حين حوّلتُ المخرجات للشاشة فى سكربت النسخ أمس، **عطّلتُ إعادة
المحاولة دون أن أنتبه** — فهى تقرأ نص الخطأ، ولم يعد يصل إليها. ولهذا لم تظهر
محاولة إعادة واحدة عند انقطاع الاتصال. **شبكة أمان لا ترى السقوط ليست شبكة أمان.**

الحل مختلف لكل سكربت لأن الظرف مختلف: النسخ يحتجز (عشرات الأسطر، والاحتجاز
ضرورى للكشف)، والتحقق يُمرِّر (آلاف الإشعارات).

### ورقم متوقَّع خاطئ

الحد الأدنى للصفوف بُنى على إحصاء يشمل مخططات `auth` و`storage` — **٢٠ ألف صف
لا يشملها النسخ أصلاً**. فأعلن نسخةً سليمة فاشلة.

استُبدل التقدير بـ**عدّ فعلى لتسعة جداول محاسبية**، لا يعتمد على إحصاءات تتقادم.

---

## [3.74.771] - 2026-07-21 — نسخة تُثبت نفسها، ونبضة تكشف صمتها

### أولاً: نتيجة الاختبار الحاسم

استُعيدت نسخة `pg_dump` فى قاعدة نظيفة:

| | لقطة المستودع | **`pg_dump`** | الإنتاج |
|---|---|---|---|
| الجداول | ٢٤٣ | **٢٤٩** | ٢٤٩ |
| الدوال | ١٠٥٣ | **١٢٠٨** | ١٢٠٤ |
| السياسات | ٤٢٩ | **٧٩٩** | ٧٩٧ |
| **المشغّلات** | **٥** | **٥٠٧** | ٥٠١ |

**٥ ← ٥٠٧.** البنية تُستعاد بالكامل. السؤال الذى شغل شهوراً له إجابة.

### وأداة التحقق كذبت

طبعت **«يمكنك الاعتماد على هذه النسخة»** بينما `rows = 0` — **لم يُستعد صفٌّ واحد**.

بُنيت هذه الأداة تحديداً لتمسك «ما يبدو ناجحاً وليس كذلك»، **ففعلَته بنفسها**:
فحصت الأرقام المُبهرة وتجاهلت الرقم الوحيد الذى يقول هل يوجد شىء بالداخل.

**ترفض الآن أى استعادة تعود بأقل من نصف الصفوف المتوقعة.**

### السبب: `system_logs`

| الجدول | الصفوف | النسبة |
|---|---|---|
| **`system_logs`** | **٢٠٩٬٠٩٢** | **٨٦.٩٪** |
| كل شىء آخر | ٣٠٬٧٨٠ | ١٣٪ |

جملة `INSERT` واحدة بـ**١٠٠٬٠٠٠ صف**، ومات الاتصال عند نهايتها بالضبط
(السطر ١٣٢٦٦٠). **النسخة كانت تفشل بسبب سجلّ طلبات API، لا بسبب الدفاتر** —
سطور القيود ٩٢٢ صفاً فقط.

استُثنى. **و`audit_logs` باقٍ عمداً** — وهو ما تستثنيه نسخة التطبيق.

### ومُرشِّح كاد يُفسد البيانات

المُرشِّح الأول أزال **٣ رؤوس `INSERT`** وترك **٢٠٨٬٥٩١ سطر قيم يتيماً** — كانت
ستلتصق بالجملة السابقة، **فتُحشر سجلات API داخل جدول محاسبى**. نسخة تُستعاد
بنجاح تام ببيانات مغشوشة.

**كشفه أن الملف لم يصغر.** مُرشِّح يدّعى إزالة ٨٧٪ ولا يُنقص الحجم لم يُزل شيئاً.
صار واعياً بحدود الجُمل، **ويرفض المتابعة إن أزال أقل من ١٠٠٠ سطر**.

### 🔴 والأهم: نبضة الفحص الليلى

صباح اليوم: **صفر سجل** من الفحص الليلى. والسبب أن الحلقة تدور على الشركات
**التى لها ملاحظات** — وقد نظّفناها كلها أمس.

**فصار هذان متطابقين تماماً:**

```
«النظام سليم»        ← صفر سجل
«المهمة توقفت»       ← صفر سجل
```

**مراقبٌ لا يستطيع الإبلاغ عن حالته ليس مراقباً.** وهذا نفس عيب v3.74.753 بثوب
آخر: حينها كانت تعمل ولا تستطيع الكتابة، والآن تكتب ولا شىء لديها لتقوله —
**والصمت يُقرأ واحداً فى الحالتين**.

**نبضة واحدة لكل شركة، كل ليلة.** فغياب السجل صباحاً صار يعنى شيئاً واحداً:
**المهمة لم تعمل.**

### وأربع عقبات أخرى، كلها كانت ستقابلك أثناء كارثة

| العقبة | الحل |
|---|---|
| خيار `--exclude-table-data` غير مدعوم | السكربت **يسأل الأداة عن خياراتها** بدل افتراضها |
| الاتصال يسقط عشوائياً | إعادة محاولة ٣ مرات — **للانقطاع فقط**، لا للأخطاء الحقيقية |
| `psql` غير مثبت | استُخدم من **داخل حاوية Supabase المُنزَّلة أصلاً** |
| `DROP SCHEMA` يتجاوز حدّ الأقفال | حذف على دفعات |
| Docker متوقف بعد إعادة التشغيل | فحص مسبق يقولها **فى سطر واحد** بدل ٦٠ سطر خطأ |

### وتسريبان لكلمات المرور، أُغلقا

رابط الاتصال صار وسيطاً فى الأوامر، **فصار جزءاً من كل رسالة خطأ**. فُحص **كل
موضع يصل فيه خطأ إلى الشاشة** فى السكربتات الأربعة — ووُجد **موضعان مكشوفان**،
أحدهما فى سكربت لا علاقة له بالتغيير.

**الفارق فى الطريقة:** التسريبان الأولان أمس اكتُشفا **بعد طباعة كلمة المرور**.
هذان اكتُشفا **بالبحث عن كل المسارات الممكنة قبل التشغيل**.

---

## [3.74.770] - 2026-07-20 — 🔴 لم تكن هناك نسخة احتياطية إطلاقاً

### الاكتشاف

لوحة Supabase تقول صراحةً:

> **Free Plan does not include project backups**

وقبله بساعة أثبتنا أن لقطة المستودع **لا تُعيد بناء قاعدة البيانات**.

**النتيجة المُجمَّعة:** لم تكن هناك نسخة احتياطية عاملة لقاعدة الإنتاج **إطلاقاً**.
لا منقوصة — **معدومة**. ٢٧٠ ميجابايت من بيانات محاسبية حيّة، ٢٤٩ جدولاً، نحو
٢٤٠ ألف صف، ولا شىء يُعيدها.

كنا نعمل طوال اليوم على افتراض أن لقطتنا **إضافةٌ** لنسخ المنصة. **لم تكن هناك
نسخ منصة.**

### قرار المالك

الترقية إلى Pro **مؤجَّلة حتى يُحقق المشروع إيراداً** — قرار سليم. فبُنى المسار
المجانى الكامل.

### ما بُنى

**`scripts/backup-production.js`** — نسخ حقيقى بـ`pg_dump` عبر Supabase CLI:
`roles.sql` و`schema.sql` و`data.sql`.

يرفض العمل إن لم يكن مربوطاً بالإنتاج، ويُعلن النسخة **ناقصة** إن فشل أى جزء،
**ويرفض ناتجاً أصغر من ٥ ميجابايت** مقابل قاعدة ٢٧٠ ميجابايت — لأن **ملفاً صغيراً
ينجح بلا خطأ أخطر من فشل صريح**، وهو نمط اليوم كله.

**`scripts/verify-backup.js`** — يستعيد النسخة فى قاعدة الاختبار **ويقارن الأعداد
بالإنتاج**.

> عدّ العبارات الناجحة يخبرك أن الملف قُرئ.
> **مقارنة النتيجة بالإنتاج تخبرك هل تستطيع العمل عليها غداً.**

لقطة المستودع فشلت بـ٤٨٧١ عبارة **ناجحة** — ولهذا لا يُكتفى بعدّ النجاح.

**`docs/DISASTER_RECOVERY.md`** — التهيئة، النسخ، التحقق، الوتيرة، وخطوات
الاستعادة عند الكارثة.

### وسؤال المالك الذى غيّر التوثيق

**«هل ما تبنيه مختلف عن النسخ الموجودة فى الإعدادات؟»**

سؤالٌ كان يجب أن أفحصه **قبل** أن أبنى. فحصتُه بعده:
...
**نسخة التطبيق تنسخ البيانات لا البنية، وتستعيد إلى قاعدة موجودة.** فإن ضاعت
القاعدة نفسها، **لا يوجد ما تُستعاد إليه**.

**الأولى تحمى بياناتك، والثانى يحمى نظامك.** ولا يُغنى أحدهما عن الآخر —
والمقارنة صارت جزءاً دائماً من التوثيق.

**وملاحظة كشفها الفحص:** `audit_logs` مستثنى من نسخة التطبيق عمداً — **سجل
التدقيق لا يُنسخ فيها إطلاقاً**، وهو أول ما يطلبه مدقق.

### وثغرة أُغلقت قبل أن تُفتح

مجلد `backups/` **لم يكن مستثنى من git**. وملفات `pg_dump` تحوى **كل صف من بيانات
عملائك وفواتيرك ورواتبك** — رفعها يعنى نشرها.

أُضيف الاستثناء **قبل أخذ أول نسخة**، واختُبر بملف فعلى.

**وفُحص ما كان موجوداً:** ملف واحد من يناير، ٣١٢ بايت، ثلاثة جداول **كلها فارغة**.
لا تسريب. أُزيل من التتبُّع.

---

## [3.74.769] - 2026-07-20 — 🔴 النسخة الاحتياطية لا تُعيد بناء قاعدة البيانات

### السؤال الذى لم يُطرح منذ شهور

تعليق `dump-db-schema.js` كان يقول منذ كُتب:

> *«غير مُثبت أنه يُعيد بناء قاعدة البيانات وحده؛ ذلك يتطلب استعادته فى مشروع
> نظيف ومقارنته. حتى يُجرى هذا الاختبار ويُجتاز، عاملها كـ«نرى ما فى الإنتاج»
> لا «نستطيع إعادة بنائه».»*

**أُجرى الاختبار. ولم يُجتَز.**
...
| الجداول | ٢٤٩ | ٢٤٣ |
| الدوال | ١٢٠٤ | ١٠٥٣ |
| سياسات الحماية | ٧٩٧ | **٤٢٩** |
| **المشغّلات** | **٥٠١** | **٥** |

**خمسة مشغّلات من ٥٠١.** المشغّلات هى موضع ترحيل التكلفة، وفرض توازن القيود،
واستهلاك دفعات FIFO. قاعدة مُستعادة من هذا الملف **تقبل البيانات وتتوقف عن
المحاسبة بصمت** — وبأقل من نصف حمايتها، أى **تتسرب البيانات بين الشركات**.

### وأخطر ما فى النتيجة ليس الفشل

**٤٨٧١ عبارة نجحت.** أى أن النظام المُستعاد **كان سيبدو عاملاً**.

فشلٌ صريح يوقفك ويجعلك تبحث. **نجاحٌ ظاهرى يجعلك تستأنف العمل على قاعدة معطوبة.**

### الأسباب — بنيوية لا تُرقَّع

| # | السبب | المثال |
|---|---|---|
| ١ | **لا ترتيب للاعتماديات** | المفاتيح الأجنبية قبل الأساسية التى تشير إليها |
| ٢ | الامتدادات لا تُكتب | `type "vector" does not exist` |
| ٣ | الأنواع المخصصة لا تُكتب | `type "discount_document_type" does not exist` |
| ٤ | المتتاليات لا تُكتب | `system_audit_log_id_seq does not exist` |
| ٥ | قيم افتراضية تستدعى دوالاً لم تُحمَّل بعد | `ir_generate_reservation_number()` |

**وعيب أمانة يستحق سطره:** عمودا `audit_logs.entity` و`entity_id` محسوبان
(`GENERATED ALWAYS`)، والملف يكتبهما كـ`DEFAULT`. **لحسن الحظ رفضهما Postgres** —
لو قُبلا لاختلف سلوك الجدول عن الإنتاج **دون أن يُبلّغ أحد**.

### ما الذى يبقى نافعاً

الملف **سطح مقارنة ممتاز، لا سكربت استعادة** — وقد سدّد ثمنه اليوم: كشف نسخةً
كانت ما زالت تمنح ثلاث دوال خطرة لغير المسجَّل بعد حذفها.

**للتعافى الحقيقى: `pg_dump`** — عبر `supabase db dump` — يحلّ الترتيب والامتدادات
والأنواع والمتتاليات **بحكم تصميمه**. ولا يجوز توسيع سكربتنا ليصير `pg_dump`.

### أربع عقبات قبل أن يبدأ الاختبار أصلاً

رمز خاص فى كلمة مرور القاعدة، ثم `#` يقطع سطر `.env` بصمت، ثم خادم يعمل بـIPv6
فقط لا تصله شبكة IPv4.

**كل واحدة منها كانت ستواجهك أثناء كارثة حقيقية** — وأنت تحت الضغط، وبلا تفسير.

---

## [3.74.768] - 2026-07-20 — النسخة الاحتياطية تطابق الواقع تماماً

### لماذا هذا الإصدار

أجرينا اليوم **٢٠ تغييراً** على قاعدة البيانات عبر ٢٢ ترحيلاً. النسخة المحفوظة
فى المستودع — **مصدر إعادة البناء عند الكارثة** — لم تكن تعرف أياً منها.

فى v3.74.760 اكتشفنا أن نسخة متأخرة **كانت ستُعيد ثلاث دوال خطرة وصلاحياتها
المفتوحة** بعد ساعة من حذفها. أصلحتُها حينها **جراحياً بيدى**؛ هذا الإصدار
يستبدلها بتوليد آلى كامل من الواقع.

### التطابق يُغلق تماماً

| | |
|---|---|
| دوال حية فى قاعدة البيانات | **١٢٠٤** |
| تعريفات فى `functions.sql` | **١٢٠٤** |

**أول تطابق تام اليوم.** فى v3.74.760 كان ١٢٠١ مقابل ١٢٠٢ (ناقص واحد).

*(السكربت يطبع «1214 routines» من عدّاده الداخلى — العدد المرجعى من
`pg_proc` هو ١٢٠٤، وهو ما يُقارَن.)*

### ما تأكد فى الملفين

**غائبة كما يجب** — الدوال الثلاث التى تُعيد كتابة التكلفة بسعر خاطئ:
`fix_all_historical_cogs`، `fix_cogs_clean`، `recalculate_cogs`
(وسابقتها `fix_historical_cogs`) — **صفر أثر فى الملفين**.

**حاضرة كما يجب** — الحُرّاس الثلاثة الجدد:
`ic_anon_reachable_writers`، `ic_anon_reachable_readers`،
`get_db_governance_state`.

**صلاحيات `anon` المسحوبة** — الميزانية، قائمة الدخل، ميزان المراجعة،
فتح فترة مقفلة، اعتماد دفع مورد: **صفر منح**.

**صلاحيات `anon` المحفوظة عمداً** — مُحدِّد المحاولات، بحث تسجيل الدخول،
دالة سياسات الحماية: **موجودة**.

**فحص التأخّر:** ٦٢٤ ملف ترحيل، ولا شىء فى اللقطة يصف ما أزاله ترحيل.

### الأثر

**استعادة قاعدة البيانات من هذا المستودع تُعيد الآن ما نملكه فعلاً** — لا حالة
الأمس بثغراتها.

يبقى أن هذا **سجل أمانة لا خطة تعافٍ مُثبتة**: لم يُستعد فى مشروع نظيف قط.
إثبات ذلك يحتاج قاعدة بيانات اختبار منفصلة — وهى قرار المالك.

---

## [3.74.767] - 2026-07-20 — ٤٨ إنذار أثر تتبُّع، منها ١٦ حقيقى

### القاعدة الجديدة

القيد **قابل للتدقيق** إذا كان له أثرٌ خاص به **أو** كان لعمليته الأم أثر.

الفاحص كان يطلب أثراً **مباشراً على كل قيد**. فأنتج ٤٨ ملاحظة، **٣٢ منها ليست
فجوات إطلاقاً**.

### لماذا لم تكن فجوات

قيد تكلفة البضاعة **ابنٌ لفاتورته** — يُنشئه مُشغِّل تلقائى فى قاعدة البيانات،
**والمُشغِّل لا يعرف سياق العملية** فلا يستطيع كتابة أثر. لكن الفاتورة فوقه تحمل
الأثر كاملاً، وكل قيد منها **قابل للوصول عبرها**.

نفس الشىء لقيود السداد وفواتير المشتريات وعكوس القيود.

### والستة عشر الباقية حقيقية — ومركّزة

| المسار | القيود |
|---|---|
| **المصروفات** | ٧ |
| **عهدة الحجوزات** (صرف/إرجاع/تسوية) | ٦ |
| إعادة تصنيف مصروف، إعادة ترحيل سداد، تكلفة استهلاك خدمة | ٣ |

**هذه ما يسأل عنها المدقق:** «من صرف هذا المبلغ ومتى وبأى إذن؟» — والبيانات
لا تجيب. وكانت **مدفونة تحت ضعف عددها من الضجيج**.

### التحقق قبل النشر

| | |
|---|---|
| ملاحظات قبل الإصلاح | ٤٨ |
| صمتت بحق (الأم مُتتبَّعة) | **٣٢** |
| **ما زالت ظاهرة** | **١٦** |
| المسارات المُبلَّغة | `expense`، `booking_custody_*`، وثلاثة متفرقة |

**لا فجوة حقيقية واحدة اختفت.**

### وإن تعذّر الفحص

لو فشلت قراءة أثر العملية الأم، يُبلّغ الفاحص عن **تعذُّر التحقق** بدل أن يعود
لاعتبار كل قيد يتيماً. نفس مبدأ v3.74.764 و765.

### ما لم يُفعل، عمداً

**لم أُضف تسجيل الأثر لمسارَى المصروفات وعهدة الحجوزات.** ذلك يعدّل كوداً يكتب
فى الدفتر، وهو عمل جلسة جديدة بذهن صافٍ — لا الإصدار الخامس والأربعين فى يوم
واحد. الفجوة الآن **مرئية ومعدودة ومُسمّاة** بدل أن تكون مجهولة.

---

## [3.74.766] - 2026-07-20 — الـ٥٢ الأخيرة، ولم تكن ما وصفتُه

### تصحيح ما قلتُه للمالك

وصفتُ الباقى بأنه **«دوال تُعيد نعم/لا، منخفضة الخطورة»**. **كان وصفاً خاطئاً.**
المجموعة تضم:

```
approve_supplier_payment_atomic     اعتماد دفع مورد
approve_sales_return_atomic         اعتماد مرتجع مبيعات
approve_production_order_atomic     اعتماد أمر إنتاج
post_invoice_atomic_v2              ترحيل فاتورة
post_manual_journal_draft           ترحيل قيد يدوى
unlock_accounting_period            ← فتح فترة محاسبية مقفلة
```

**هذه تُغيّر الحالة، لا تقرأ.** صُنِّفت «قراءة» لأن مُرشِّحى يبحث عن
`INSERT/UPDATE/DELETE` **فى نص الدالة**، وهى **تُفوّض الكتابة** لدوال أخرى فيبدو
نصّها نظيفاً.

**الغلاف ليس قارئاً.** نفس خطأ اليوم المتكرر — مطابقة الشكل الظاهر بدل الحقيقة —
لكنه هذه المرة جاء **فى صورة تصنيف لا فى صورة نمط**.

### ثم بالغتُ فى الاتجاه المعاكس

رأيتُ `post_accounting_event` يكتب مباشرةً بلا فحص دور، **فأعلنتُ أنه كاتبٌ فى
الدفتر بلا حماية ومفتوح للجميع**.

**وهذا خطأ أيضاً.** النسخة التى تكتب (١٢ وسيطاً) تحمل `assert_company_access` —
الحارس الذى بنيتُه اليوم — ومسح v3.74.759 **استثناها بحق**. النسخة الأخرى بلا
حماية لكنها **لا تكتب**، بل تُفوّض للمحمية.

**الإنذار جاء من استعلام أغفلتُ فى أعمدته الحارسَ الذى قضيتُ اليوم فى تركيبه.**

قلّلتُ ثم بالغتُ، خلال دقائق.

### الموقف الدقيق بعد فحص الاتجاهين

| الحقيقة | التفصيل |
|---|---|
| **ولا واحدة تستخدم `auth.uid()`** | تقرأ `p_user_id` من **وسائط النداء** وتفحص دوره — فيمرّر المهاجم معرّف المالك |
| بعضها يُفوّض لدوال **محمية** | `assert_company_access` يعمل، فالضرر محدود |
| **لا مُنادى شرعى واحد بلا تسجيل دخول** | التطبيق يستدعيها كلها بجلسة |

**كونها قابلة للنداء أصلاً هو العيب** — بغضّ النظر عن مدى ما يبلغه المهاجم.

### التحقق

| | |
|---|---|
| الانحراف فى لوحة التحكم | **٠** |
| `run_all_integrity_checks` | **٠ ملاحظات** |
| العمليات الحسّاسة الخمس أمام غير المسجَّل | **٠ من ٥** |
| نفسها أمام التطبيق | **٥ من ٥** |
| تسجيل الدخول + مُحدِّد المحاولات + سياسات الحماية | **محفوظة** |

---

## [3.74.765] - 2026-07-20 — المانع الأخير كان فاتورة خدمات

### ما بقى بعد v3.74.764

إصدار سابق رفع النتيجة من ٦٣٪ إلى ٩٥٪ وأزال خمسة موانع وهمية. بقى **مانع واحد**،
ووصفتُه حينها بأنه «المشكلة الحقيقية الوحيدة»:

> «فاتورة واحدة من ٣ بلا قيد تكلفة — الربح فى قائمة الدخل مضخّم»

### وهو ليس حقيقياً كذلك

الفاتورة `INV-2026-00001`:
...
**الخدمة لا تكلفة بضاعة لها.** لا يوجد ما يُسجَّل، ولن يوجد أبداً.

والرسالة كانت تقول للمالك **«الربح فى قائمة الدخل مضخّم»** — وهذا **غير صحيح**.
لا مخزون خرج، ولا تكلفة نقصت، ولا ربح تضخّم.

### السبب

الاختبار يفحص **كل** فاتورة نشطة، بلا تمييز بين ما يبيع مخزوناً وما يبيع خدمة.
ففاتورة خدمات تُحسب «ناقصة التكلفة» **إلى الأبد** — وكونها حرجة، **تحظر الإقفال
السنوى إلى الأبد**.

والمفارقة أن **بقية الكود يعرف الفارق**: التقرير التشخيصى فى
`app/api/fix-cogs-accounting` يستثنى `item_type = 'service'` صراحةً منذ v3.74.726.
هذا الاختبار وحده لم يكن يعرف.

### التحقق قبل التعديل

لم أُصلح ثم أتحقق، بل حاكيتُ المنطق المُصحَّح على **كل الشركات**:

| الشركة | فواتير نشطة | تبيع مخزوناً | خدمات مستثناة | **تنقصها التكلفة فعلاً** |
|---|---|---|---|---|
| 8ef6…526 | ٣ | ٢ | ١ | **٠** |
| b0de…254 | ١٤ | ١٤ | ٠ | **٠** |

**لا فجوة تكلفة واحدة فى النظام كله.**

### وإن تعذّر الفحص

لو فشلت قراءة سطور الفواتير، يقول الاختبار **«تعذّر التحقق»** ولا يحظر الإقفال —
بدل أن يعدّ كل فاتورة ناقصة. نفس مبدأ v3.74.764.

---

## [3.74.764] - 2026-07-20 — الإقفال السنوى كان محظوراً بخمس مشكلات غير موجودة

### ما كانت الصفحة تقول

```
٦٣٪ — تم اكتشاف مشكلات حرجة — الإقفال السنوي محظور
١٢ من ١٩ اختبار اجتاز · ٥ حرج · ٢ تحذير
```

### ما هو صحيح فعلاً

**١٧ من ١٩. ولا شىء يستوجب حظر الإقفال.**

| الاختبار | يقول | الواقع (من `pg_catalog`) |
|---|---|---|
| ١٢ مشغّلات الحوكمة | ٤ مفقودة | **الأربعة موجودة** — ضمن ٣٥ مشغّلاً على جداول القيود |
| ١٣ جدول Idempotency | مفقود | **موجود، ١١ عموداً** |
| ١٤ دوال الحماية الذرية | ٤ مفقودة | **الأربعة موجودة** |
| ١٦ دوال الأداء | ٣ مفقودة | **الثلاثة موجودة** |
| ١٨ درع الحماية | ٥ مكونات مفقودة | **الخمسة موجودة** |

### السبب

إحدى عشرة مرة، بهذا الشكل:
...
**PostgREST لا يستطيع تقديم `information_schema`** — ليست جدولاً مكشوفاً.
كل استعلام منها **فشل**. والكود يأخذ `{ data }` **ويُسقط `{ error }`**، فيعود
`null`، فيُقرأ الفراغ على أنه «غير موجود».

أربعة من هذه الاختبارات **حرجة**، والحرج **يحظر الإقفال السنوى**.

**فحصٌ لا يستطيع العمل يجب أن يقول «تعذّر التحقق». قول «مفقود» أسوأ من الصمت،
لأن أحداً سيتصرف بناءً عليه.**

### التناقض الذى كان ظاهراً فى نفس التقرير

الاختبار **١١ اجتاز**: «لا توجد قيود مكررة — Trigger منع التكرار فعّال».
الاختبار **١٢ فشل**: «`trg_prevent_duplicate_journal_entry` مفقود».

نفس المشغّل، فى نفس الصفحة، مرة يعمل ومرة مفقود. الفارق أن **١١ يفحص السلوك**
و**١٢ يفحص الاسم عبر قناة معطلة**. السلوك كان الحقيقة.

### المشكلة الحقيقية الوحيدة

**الاختبار ٥: فاتورة واحدة من ٣ بلا قيد تكلفة — الربح فى قائمة الدخل مضخّم.**
من الحركات السابقة لنظام FIFO. كانت **جالسة بين خمس ملاحظات كاذبة**.

### الإصلاح

دالة واحدة `get_db_governance_state()` تقرأ `pg_catalog` (المقروء دائماً)
وتُعيد حقائق الوجود. لا تأخذ وسائط ولا تكشف بيانات شركة.

وإن فشلت هى نفسها، **تقول الاختبارات «تعذّر التحقق» ولا يحظر أىٌّ منها الإقفال**.

### ما تأكد أيضاً

ميزان المراجعة متوازن (٠.٠٠)، الميزانية متوازنة (٣١٣١٠.٤٤ = ٣١٣١٠.٤٤)،
٣١ قيداً كلها متوازنة، لا قيود مكررة، لا مسودات، ومخزون GL مطابق لـFIFO.

**واختُبر التطبيق حياً:** التقارير والميزانية والإشعارات ولوحات السلامة —
كل النداءات **٢٠٠**، ولا خطأ صلاحيات واحد بعد تغييرات اليوم على نحو ١٠٠ دالة.

---

## [3.74.763] - 2026-07-20 — أفسدتُ لوحة السلامة بالأمس، وأصلحتُها اليوم

### الخطأ أولاً

الفحص الذى أضفتُه فى v3.74.762 كان يُعيد **صفاً لكل دالة: ١٠٤ صفوف**. وهو
معلومة عن البنية، **متطابقة لكل الشركات** — أى ١٠٤ × ٤ شركات = **٤١٦ صفاً كل
ليلة، كلها نفس الشىء، ولا واحد منها عن بيانات أحد**.

والأسوأ أنه صار **الشىء الوحيد الظاهر فى اللوحة**:

```
SELECT check_code, count(*) FROM run_all_integrity_checks(<company>) GROUP BY 1;
→ anon_reachable_readers | 104        ولا شىء غيره
```

كل فحوص المحاسبة والمخزون تُعيد صفراً — بحق — لكن صمتها كان **مدفوناً تحت
الضجيج**. مشكلة حقيقية تظهر غداً كانت ستكون **صفاً واحداً بين ١٠٥**.

**ما كشفه:** أربع شركات مختلفة تُعيد الرقم ١٠٤ بالضبط. البيانات المحاسبية لا
تفعل ذلك.

هذا نفس فخ الضجيج الذى تجنّبتُه فى v3.74.759 (باستثناء دوال المشغّلات) — ثم
وقعتُ فيه من الاتجاه المعاكس بعد إصدارين. **فحصٌ يصرخ بنفس الشىء كل ليلة ليس
فحصاً، بل ورق حائط، والناس تتوقف عن قراءة ورق الحائط.**

صار **صفاً واحداً** يحمل العدد وخمسة أمثلة. ١٠٤ ← ١ لكل شركة.

### ثم العمل المقصود

**٥٢ فحص سلامة (`ic_*`) أُغلقت أمام غير المسجَّل.** كانت تكشف لأى شخص مشاكل
شركتك: مخزون سالب، قيود غير متوازنة، ازدواج تخصيص مدفوعات، سلامة أرصدة العملاء.

**استُثنيت اثنتان** تستدعيهما سياسات حماية الصفوف — والمُرشِّح وجدهما بنفسه من
جداول النظام، لم أُسمِّهما له.

### الإثبات

لم أكتفِ بفحص الصلاحيات، بل **شغّلتُ المُجمِّع فعلاً بعد الإغلاق**:

| الفحص | النتيجة |
|---|---|
| `run_all_integrity_checks` بعد إغلاق ٥٢ دالة يستدعيها | يعمل |
| `ic_*` معطّلة للتطبيق | **٠** |
| `ic_*` معطّلة للمهمة الليلية | **٠** |
| دالة سياسات الحماية | محفوظة |
| تسجيل الدخول | محفوظ |
| دوال قراءة ما زالت مفتوحة | ١٠٤ ← **٥٢** |

---

## [3.74.762] - 2026-07-20 — الميزانية العمومية كانت مقروءة بلا تسجيل دخول

### النصف الذى لم أفحصه

كل مسوحى هذا الأسبوع بحثت عن دوال **تكتب**. ولا واحد نظر إلى دوال **تقرأ**.
فبقيت نحو ١٨٠ دالة قراءة تتجاوز حماية الصفوف ومتاحة للمفتاح العام — الذى
يُشحن داخل كل صفحة متصفح. منها:

```
get_balance_sheet(company_id, date)
get_income_statement(company_id, date, date)
get_trial_balance(company_id, date)        ← وثلاث نسخ
get_audit_trail_report(...)  search_audit_trail(...)
get_employee_commission_summary_for_payroll(...)
```

أى أن **من يملك معرّف شركة يستطيع قراءة ميزانيتها وقائمة دخلها وسجل تدقيقها
وعمولات موظفيها — بلا تسجيل دخول**. ومعرّفات الشركات ليست سراً: تظهر فى الروابط
والتصديرات، وكل موظف سابق يعرف معرّف شركته.

**أُغلقت ٤٦ دالة** — كل ما يخص القوائم المالية والتدقيق والرواتب.

### كيف أخطأتُ فيها أولاً

الترحيل الأول كتب `REVOKE ... FROM anon` فقط. **نُفِّذ بنجاح، ٤٦ أمراً بلا خطأ
واحد، ولم يُغلق شيئاً تقريباً.** لأن السحب من `anon` لا يُزيل صلاحية ممنوحة
لـ`PUBLIC`، و`anon` يرثها.

اكتشفتُه لأننى **فحصتُ النتيجة بعد التنفيذ** فوجدتُ ٨ من أهم الدوال ما زالت
مفتوحة. v3.74.759 كتبها صحيحة (`anon, PUBLIC`) وأنا أسقطتُ نصفها هنا.

**ترحيلٌ ينجح ولا يفعل شيئاً** — نفس النمط الذى تدور حوله كل هذه الجلسة.

### ما أُبقى عمداً

| البند | السبب |
|---|---|
| ٩ دوال تستدعيها سياسات حماية الصفوف | تُقيَّم بصلاحية السائل — سحبها **يكسر** الحماية لا يشددها |
| دوال تسجيل الدخول والتسجيل | تعمل **قبل** وجود جلسة |

الأولى حُدِّدت من `pg_policy` برمجياً، لا بقراءة الأسماء.

### ما تبقّى، معلناً لا مخفياً

**١٠٤ دالة قراءة ما زالت مفتوحة** — عائلات `ic_*` و`can_*` و`check_*`.
تكشف أقل بكثير من ميزانية (أرقام ونعم/لا)، لكنها بيانات شركة بلا جلسة.

نجت لأن مُرشِّحى اقتصر على بادئات أسماء — وهو حاجز أمان مقصود: كنسة شاملة على
١٨٠ دالة هى بالضبط ما يُصلح شيئاً ويُعطِّل آخر.

سُجِّلت كفحص `anon_reachable_readers` **يظهر فى لوحة التحكم**، فالدَّين مُسمّى
ومعدود لا موصوف بأنه منتهٍ. أُثبت بالتخريب: ١٠٤ ← ١٠٥ عند إعادة منح الميزانية.

---

## [3.74.761] - 2026-07-20 — 🔴 نقطة نهاية عامة كانت تحذف كل حسابات المستخدمين

### ما وُجد

دالة حافة منشورة باسم `delete-non-vitaslims-users`، بإعداد `verify_jwt = false`
و**بلا أى تحقق من الهوية داخل الكود**. تتجاهل الطلب تماماً (`async (_req)`)،
تبنى عميلاً بصلاحية `service_role`، تجلب **كل مستخدمى المشروع**، وتحذف كل من
ليس ضمن **خمسة معرّفات مكتوبة يدوياً**.

**طلب HTTP واحد، من أى شخص، بلا كلمة مرور → حذف كل حسابات العملاء نهائياً.**

عنوانها كان متوقَّعاً بالكامل من اسم المشروع. لم تكن ثغرة تسريب بيانات، بل
**تدمير حسابات لا رجعة فيه**.

### ما فُعل فوراً

أُعيد نشرها بـ`verify_jwt = true` **و**بجسم يرفض كل طلب برمز 410 — سدٌّ مزدوج.
لم أنتظر إذناً: حذف الحسابات لا يُستعاد، وكل ساعة إضافية كانت مخاطرة بلا مقابل.

**الخطوة الأخيرة تخصك:** احذف الدالة نهائياً من لوحة Supabase
(Edge Functions ← delete-non-vitaslims-users ← Delete). ما فعلتُه يسدّ الثقب،
والحذف هو الإصلاح الحقيقى.

### لماذا لم تُكتشف طوال هذا الوقت

| السبب | التفصيل |
|---|---|
| **لم تكن فى المستودع قط** | نُشرت مباشرةً على الإنتاج، فلم يراجعها أحد أبداً |
| **كل مسوحى فحصت دوال Postgres** | ودالة الحافة ليست دالة Postgres |

الاثنتان الأخريان (`notification-escalation` و`update-exchange-rates`) **موجودتان
فى المستودع، وكلتاهما مكتوبتان بحذر**. الفارق ليس صدفة.

هذا **نفس درس v3.74.760** حرفياً: ما ليس فى المستودع لا يُراجَع. وجدتُه هناك فى
النسخة المحفوظة، وهنا فى دالة حافة — وفى الحالتين كان الغياب هو الخطر نفسه.

### ملاحظتان أصغر على الاثنتين الأخريين

`notification-escalation` تعليقها يقول «حماية: فقط طلبات Cron أو مُصادق عليها»،
والكود يفحص **نوع الطلب فقط** (GET أو POST). الحماية مذكورة وغير موجودة.
أثرها محدود: تُنشئ إشعارات تصعيد فقط، ومحكومة بمفاتيح تكرار وبحدّ ٣ مستويات.

`update-exchange-rates` تفحص السر `if (expectedSecret)` — أى **تسمح بالمرور إن لم
يُضبط السر أصلاً**. أثرها محدود أيضاً: لا تقبل أسعاراً من الطلب بل تجلبها من
مصدر عام. **تحقّق من ضبط `CRON_SECRET`** فى إعدادات الدوال.

---

## [3.74.760] - 2026-07-20 — النسخة المحفوظة كانت ستُعيد ما حذفناه للتو

### الخطر

إصدار v3.74.759 حذف ثلاث دوال خطرة من قاعدة البيانات. لكن **النسخة المحفوظة فى
المستودع** — التى نعتمد عليها لإعادة بناء النظام عند الكارثة — ظلت تحمل:
...
أى أن **إعادة البناء من النسخة الاحتياطية كانت ستُعيد الدوال الثلاث وصلاحياتها
المفتوحة** — بالضبط ما أزلناه قبل ساعة. **نسخة احتياطية قديمة أخطر من عدمها،
لأنها موثوقة.**

أُزيلت الثلاث من الملفين، و١٣ منحة `anon` معها. تحقق حسابى مُغلق:
١٢٠١ محفوظة + الحارس الجديد = ١٢٠٢ حية. ومنحة مُحدِّد المحاولات باقية كما يجب.

### الحارس الجديد `check-schema-snapshot-fresh`

يرفض النشر لو وصفت النسخة المحفوظة شيئاً أزاله ترحيل. **يعمل بلا اتصال بقاعدة
البيانات**، فيصلح للتشغيل الآلى.

### ثلاثة أخطاء فى الحارس نفسه، وكيف ظهرت

| الخطأ | الأثر | كيف اكتُشف |
|---|---|---|
| عالجتُ كل `CREATE` ثم كل `DROP` داخل الملف | ملفٌ يحذف نسخة قديمة ويُنشئ بديلها سُجّل «محذوف» → **٨ إنذارات كاذبة** | سألتُ قاعدة البيانات: الثمانى **موجودة كلها** |
| طابقتُ **الاسم** لا **التوقيع** | نسخة محذوفة وأخرى باقية بنفس الاسم لا تُميَّزان → إنذار كاذب | `create_notification` حية |
| ظننتُ الباقى خطأً حقيقياً | — | `create_product_atomic` بـ١٦ وسيطاً **حية** |

الثالث ليس خطأً بل **سقف الفحص**: ٤٩ ملفاً فقط من ٦٦١ نسخة مُطبَّقة موجودة فى
المجلد — الباقى طُبِّق عبر محرر SQL. فدالة قد يحذفها ملف ويُعيدها ترحيل لا يراه
الفحص. سُجِّلت كاستثناء **موثّق ومُتحقَّق منه حياً**، لا كإسكات.

**لو صدّقتُ إنذاراتى الثمانية** لحذفتُ ثمانى دوال عاملة من النسخة الاحتياطية.
قاعدة البيانات هى التى منعت ذلك، لا الفحص.

---

## [3.74.759] - 2026-07-20 — ثلاث دوال تُعيد كتابة التكلفة، بلا تسجيل دخول

### ما وُجد

فى v3.74.726 حذفتُ `fix_historical_cogs` لأنها تحتسب تكلفة البضاعة من بطاقة المنتج
بدل دفعات FIFO. **حذفتُ الدالة ولم أبحث عن بقية أخواتها.** ثلاث نجت:

| الدالة | نطاقها | الأثر |
|---|---|---|
| `fix_all_historical_cogs()` | حلقة على **كل الشركات** فى قاعدة البيانات | قيود تكلفة بسعر خاطئ |
| `fix_cogs_clean()` | كل فاتورة مدفوعة، **بلا تصفية بالشركة** | نفس الأثر |
| `recalculate_cogs()` | نفسها، **وبلا شرط «أُنشئ من قبل»** | كل تشغيل يُضيف قيداً جديداً — نداءان **يُضاعفان التكلفة** |

الثلاث `SECURITY DEFINER` (لا تُطبَّق عليها حماية الصفوف) و`EXECUTE` ممنوح لـ `anon`
— أى **قابلة للنداء من الإنترنت بلا تسجيل دخول**. ولا شىء فى النظام يستدعيها.

### لماذا لم تظهر فى أربعة مسوح سابقة

كل الرواصد السابقة (v3.74.727 و746 و748 و751) تشترط **وسيطاً من نوع uuid**.
هذه الدوال **بلا وسائط**. كل راصد قال «نظيف»، وكل واحد كان صادقاً داخل نطاقه.

وعمودٌ فى مسحى قال `filters_by_company = true` لأن النمط طابَقَ
`coa.company_id = company_record.id` — وهو بحثٌ **داخل حلقة تدور على كل الشركات**.
العَلَم قال «مُقيَّدة بشركة»، والدالة تجوب قاعدة البيانات كلها. **قراءة المصدر** هى ما كشفه.

### ما مُنع أيضاً

`cleanup_old_security_events(0)` كانت تمسح **سجل الأحداث الأمنية بالكامل**، ومتاحة
لغير المسجَّل. و`get_activity_summary` تُعيد «من فعل ماذا» داخل **أى شركة** يُمرَّر معرفها.
١٣ دالة مُنعت عن `anon`.

**استثناء واحد مقصود:** `check_and_increment_rate_limit`. تحديد المحاولات يجب أن يعمل
**قبل** تسجيل الدخول، و`lib/rate-limit.ts` **يسمح بالمرور عند الخطأ** — فمنعها كان
سيُعطِّل الحماية عند صفحة الدخول بالذات، بصمت.

### ما تراه أنت

زر «تصحيح البيانات القديمة» فى الإعدادات كان **مُعطَّلاً منذ ٣٣ إصداراً** — يستدعى دالة
محذوفة ويُرجع خطأً فى كل مرة. لم يُلاحَظ لأن زراً يُعطى خطأً يبدو كزر لم يُضغط.
أُوقف الآن رسمياً برسالة واضحة. التقرير الذى **يقرأ ولا يكتب** باقٍ.

**لا بديل، عمداً:** الحركات السابقة لنظام FIFO لا تُصحَّح بزر — التصحيح السليم يستهلك
الدفعات بترتيبها، وخطؤه يُفسد الربح الإجمالى بصمت.

### الحارس الجديد

`ic_anon_reachable_writers` يرصد **الشكل** لا الاسم: تتجاوز حماية الصفوف + متاحة لغير
المسجَّل + تكتب + لا تسأل عن هوية المُنادى. أُثبت **بالتخريب** لا بالادعاء:
...
## [3.74.758] - 2026-07-20 — أصلحت كوداً ميتاً، وكشفه العدّاد

### ما أُصلح فعلاً
**إلغاء إهلاك مُرحَّل** — خمسة مواضع تراجع، أخطرها اثنان:

| الموضع | ما يحدث لو فشل صامتاً |
|---|---|
| بعد نجاح السطر الأول | **قيد عكسى نصف متوازن** فى الدفتر |
| بعد نجاح السطرين | القيد العكسى **والأصل** قائمان → الإهلاك مُلغى مرتين |

**وعكس مساهمة رأس المال** — حذف غير مفحوص، **وترحيل غير مفحوص**: لو فشل الترحيل، يظهر فى التطبيق أن المساهمة عُكست **بينما الدفتر يحملها كاملة**.

```
١٨٩ → ١٧٩
```

### وبندان من قائمتى لم يحتاجا شيئاً
**`hr/payroll/payments`** — **يفحص الحذفين بالفعل**. أدرجته لأننى **خلطت مخرجات بحثين مختلفين** ولم أُعِد قراءته.

### والأهم: أصلحت كوداً لا يعمل
`app/invoices/[id]/edit/page.tsx` — قرأت الكود، شخّصت عيباً مقنعاً (**حذف يفشل ثم إعادة ترحيل ← قيود مكررة**)، وكتبت إصلاحاً وشرحاً واثقاً.

**والكود كله داخل تعليق كتلى** من السطر ٥٤٣ إلى ١٠٢٤، ورأسه يقول:

> *«Legacy direct UI mutation path retained as a reference only … Do not re-enable this block»*

**وفوقه `return` مباشرة.** التعديلات الحيّة تمرّ عبر الـAPI.

### وكيف انكشف
العدّاد **لم يتغيّر** بعد «الإصلاح». ولم أُمرّر ذلك:

```
فحصى اليدوى (بلا تتبّع التعليقات):  ٥ مواضع
الماسح (بتتبّعها):                    ٠
```

> **الماسح كان مُحقاً، وفحصى اليدوى هو الخاطئ.** ولو صدّقت رقمى بدل رقمه، لكنت أعلنت إصلاح عيب لا وجود له.

**ثامنة عشرة — وأوضحها:** قرأتُ ما يبدو كوداً حيّاً، وتجاهلت `return` فوقه وتعليقاً يقول «لا تُعِد تفعيله».
...
## [3.74.757] - 2026-07-20 — تعليقات تَعِد بما لا تضمنه

### أكملت الفئة
خمسة ملفات، أحد عشر موضعاً. **وكلها بشكل مختلف عن الست السابقة:** القيد أُنشئ، ثم **فشل إدراج السطور**، فتُحذف الرأس وحدها.

### والتعليقات نفسها هى الشاهد
الكاتب كان **مُدركاً تماماً** للخطر:

| الملف | ما يقوله التعليق |
|---|---|
| `manufacturing-accounting` | *«Best effort: try to remove the orphan header»* |
| `sales-return-cash-disbursement` | *«Roll back the JE shell so it doesn't sit there as orphan draft»* |
| `pre-shipment-refund` | *«No half-state is left behind»* |

> **ثلاثة تعليقات تَعِد بنتيجة، وثلاث كتابات لا تتحقق من حدوثها.**

«أفضل جهد» يعنى **أن أحداً لا يعلم حين يفشل الجهد**. و«لا يبقى نصف حالة» **كانت نيّة لا واقعاً**.

### وواحدة أخطر من الحذف
```js
await admin.from('journal_entries').update({ status: 'posted' })
```
**فى مسار صرف نقدى لمرتجع مبيعات.**

> لو فشل هذا صامتاً: **النقد خرج فعلاً، والسطور موجودة، والدفتر لا يحتسبها** — لأن القيد بقى مسودة.

صار يفشل بوضوح: *«أُنشئ القيد ولم يُرحَّل»*.

### الحصيلة
```
٢١٣ → ٢٠١ → ١٨٩
```
**كل تراجع لقيد محاسبى فى `lib/` يُبلّغ عن فشله الآن.**

### وما بقى — بأسمائه
`fixed-assets/depreciation` (٥ مواضع) · `shareholders/contributions/reverse` · `hr/payroll/payments` · أدوات `fix-*`

**أشكالها مختلفة، وتحتاج قراءة فردية.** والباقى بعدها سجلات مراجعة.

### Files
- خمسة ملفات فى `lib/` · `scripts/check-unchecked-writes.js`

---

## [3.74.756] - 2026-07-20 — التراجع الذى لا يتراجع

### الفئة الخطرة من الـ٢٠١
ست خدمات تُلغى قيداً محاسبياً نصف مكتوب داخل `catch`:

```ts
} catch (error) {
  if (journalEntryId) {
    await admin.from("journal_entry_lines").delete()...
    await admin.from("journal_entries").delete()...
  }
  throw error          // ← الخطأ الأصلى يُطلَق
}
```

**ولو فشل الحذف؟** لا شىء. **والخطأ الأصلى يُطلَق فوقه** — فالمُنادى يرى السبب الأول ولا يعلم أن التنظيف فشل أيضاً.

> **فيبقى فى دفترك قيدٌ من عملية كان يُفترض أنها أُلغيت.** لا يوازن شيئاً، ولا مستند يشير إليه، **ولا أحد يُخبَر.**

### المُساعد المشترك
`rollbackJournalEntry` — يحذف السطور ثم الرأس، **ويفحص كليهما**.

**والترتيب مقصود:** السطور أولاً. العكس يترك سطوراً تشير إلى رأس اختفى، لو فشل الحذف الثانى.

### ولماذا يُسجّل ولا يُطلق استثناءً
إطلاق استثناء هنا **يستبدل الخطأ الأصلى** — سبب فشل العملية أساساً — **بخطأ تنظيف**، فيفقد المُنادى التشخيص الذى يهمّه فعلاً.

فيُسجَّل بعلامة ثابتة قابلة للبحث:
```
[ROLLBACK_INCOMPLETE] ...
```
**وهى ما تبحث عنه فى السجلات حين يظهر قيد لا يُفسّره مستند.**

### الست المُصلَحة
`manual-journal` · `customer-refund` · `customer-voucher` · `shareholder-capital` · `bank-transfer` · `supplier-refund-receipt`

```
٢١٣ → ٢٠١
```

### وخمس باقية فى نفس الفئة — بأسمائها
`period-closing` · `pre-receipt-refund` · `pre-shipment-refund` · `sales-return-cash-disbursement` · `manufacturing-accounting`

**لم أُعمّم عليها** لأن أشكالها مختلفة، والتعميم على ما لم أقرأه هو ما أوقعنى فى ستة عشر خطأً اليوم.

**والباقى بعدها سجلات مراجعة** — فشلها سطر ضائع، لا دفتر مختل.

### Files
- `lib/services/rollback-journal-entry.ts` (جديد)
- ست خدمات أوامر · `scripts/check-unchecked-writes.js`

---

## [3.74.755] - 2026-07-20 — النمط تكرر أربع مرات، فأوقفت نموّه

### الجذر المشترك
أربعة أعطاب اليوم، **كلها من سبب واحد**:

```js
await supabase.from("t").insert({...})     // النتيجة تُهمَل
```

> **مكتبة Supabase لا تُطلق استثناءً عند فشل الكتابة** — بل تُعيد `{ error }`. فالكتابة الفاشلة **لا تفعل شيئاً بصمت**، والكود يمضى وكأنها نجحت.

| الإصدار | ما فشل صامتاً |
|---|---|
| ٧٢٦ | أداة إصلاح تستدعى دالة غير موجودة وتقول «تم بنجاح» |
| ٧٤٣ | تغيير فرع العميل — **فشل منذ يوم كُتب الحارس** |
| ٧٥٣ | فحص السلامة اليومى — **صفر سجل، صفر إشعار، على الإطلاق** |
| ٧٥٤ | تذكير تقييم العملات — **لم يصلك ولا مرة** |

**ولا واحد منها أعلن عن نفسه. كلها ردّت بالنجاح.**

### الحجم — وتصحيح لتقديرى
قلت **٩٦**. الرقم الحقيقى **٢١٣**.

بحثى غطّى `app` فقط بامتداد `.ts`، **وأهمل `lib` بالكامل** — أى أكثر من نصف الكود. **والسكربت وحده هو ما أظهر الرقم الصحيح**، وإلا لبقى الأساس خاطئاً بـ١١٧ إلى الأبد.

> **سادسة عشرة من نفس الجذر: نطاق بحث ضيّق يُقدَّم كأنه إحصاء كامل.**

### وليست كلها سواء
أغلب القائمة **سجلات مراجعة** — فشلها مزعج لا خطير.

**لكن فيها ما هو أخطر بكثير:** حذف قيود محاسبية فى **مسارات التراجع**:
```
...
> **إن فشل حذفٌ من هذه صامتاً، فالتراجع لا يحدث — ويبقى قيد نصف مكتوب ناجياً من عملية فاشلة.**

**هذه مسألة سلامة محاسبية لا تسجيل.** وهى أولوية الجلسة القادمة، لا القائمة من أعلاها.

### ولن أُعدّل ٢١٣ موضعاً
**هذه بالضبط الكنسة التى تكسر شيئاً وهى تُصلح آخر** — وقد أخطأتُ بها اليوم أكثر من مرة.

فالسقّاطة، كما نجحت مع فحص العزل:
- **مخالفة جديدة → يفشل البناء فوراً**
- **نقصان العدد → يفشل أيضاً**، حتى يُثبَّت المكسب فى الأساس ولا يعود الدَّين متسللاً

### واختبار ذاتى يمنع انزلاق القاعدة
سبع حالات تُثبّت الطرفين: الأشكال المعطوبة **يجب أن تُرصَد**، والصحيحة **يجب أن تمرّ**.

**وحالتان منها للتعليقات تحديداً** — لأن حرّاساً كتبتُها هذا الأسبوع **رفضت توثيقها الخاص** حين طابقت النثر كأنه كود.

### الفارق الذى تعلّمته اليوم
> **الكتابة الفاشلة والكتابة التى لا عمل لها تبدوان متطابقتين — ما لم تسأل عن النتيجة.**

### Files
- `scripts/check-unchecked-writes.js` · `.github/workflows/ci.yml` · `package.json`

---

## [3.74.754] - 2026-07-20 — وسألت السؤال نفسه عن بقية المهام

### بعد إصلاح فحص السلامة، عمّمت السؤال
**«أى مهمة مجدولة تركت أثراً فعلاً؟»**

| المهمة | الأثر |
|---|---|
| النسخ الاحتياطى اليومى | ١٢٩ سجلاً، آخرها اليوم ✓ |
| تجديد الاشتراكات | يعمل ✓ |
| تذكير الحجوزات | ٥٣ إشعاراً فى ٣٠ يوماً ✓ |
| إنشاء الفترات المحاسبية | ٥٠ فترة ✓ |
| **تذكير إعادة تقييم العملات** | **صفر** |

### ولم أفترض أن الصفر يعنى عطباً
قد يكون **لا عمل لها**. فتحققت: **٩١ حساباً بعملة أجنبية** فى قاعدتك، بعملتَى أساس مختلفتين.

**فللمهمة عمل حقيقى — ولم تُنتج شيئاً فى شهر.**

### نفس النمط بالحرف
```js
category: 'accounting',   // ليست ضمن الفئات المسموحة
// reference_id مفقود      // NOT NULL
// kind, retry_count, created_by مفقودة
await supabase.from('notifications').insert({...})   // بلا فحص للنتيجة
```

**اختبرتُه بالشكل الذى ترسله فعلاً:**
```
كما ترسله المهمة: CONFIRMED broken — reference_id لا يقبل NULL
بعد التصحيح:      PASS
```

> **فمنذ كُتبت هذه المهمة، وأنت لم تتلقَّ تذكيراً واحداً بإعادة تقييم عملاتك قبل إقفال الشهر.**

### والفئة الصحيحة اخترتها بالتحقق
`accounting` ليست فى القيد. المسموح: `finance` · `inventory` · `sales` · `approvals` · `system` · `billing` · `hr` · `manufacturing` · `branch_activity` · `accountant_action`.

**إعادة تقييم العملات شأن مالى ← `finance`.**

### والصمت أُغلق هنا أيضاً
تُسجَّل أخطاء الكتابة، **وترد المهمة بخطأ صريح** إن فشلت أى منها — بدل «٢٠٠ وكل شىء تمام».

### الدرس المُعمَّم
**خمس مهام تعمل، واثنتان كانتا صامتتين** — ولم يكن أحد ليعرف، لأن **المهمة الفاشلة والمهمة التى لا عمل لها تبدوان متطابقتين من الخارج**.

> **الفارق الوحيد: أن تسأل «هل تركت أثراً؟» بدل «هل ردّت بنجاح؟»**
...
## [3.74.753] - 2026-07-20 — بنينا كل هذه الفحوص، وقناة إبلاغك كانت ميتة

### السؤال الذى أوصل إليه
بنينا اليوم عشرات الفحوص. فسألت السؤال الطبيعى: **هل تصل إليك؟**

الحلقة تبدو مكتملة: فحص → cron يومى ١:٣٠ → إشعار للمالك.

### والجواب
```
سجلات فحص السلامة:     صفر
إشعارات فحص السلامة:   صفر
إشعارات فئة «system»:  صفر — على الإطلاق
```

**بينما `backup_auto_export` يعمل** — فالـcron يعمل والمفتاح مضبوط. **الفحص وحده هو الذى لم يُسجّل شيئاً قط.**

### أربعة أعطاب فى كتابة واحدة
```js
entity_type: "system_integrity",   // عمود غير موجود
entity_id: companyId,              // عمود مُولَّد تلقائياً
// target_table مفقود               // وهو NOT NULL
```
**ولا فحص لنتيجة الكتابة** — فالفشل يمر بلا صوت.

**نفس عطب v3.74.743 بالحرف** — الكتابة على أعمدة مُولَّدة.

### وأربعة أخرى فى الإشعار
`notifications` **لا تملك عمود `user_id`** أصلاً (المستلمون فى جدول منفصل)، و**سبعة أعمدة إلزامية** لم تُرسَل: `channel`, `created_by`, `kind`, `reference_id`, `reference_type`, `retry_count`, `severity`.

> **فالـcron يحسب الانحرافات بدقة كل ليلة، ثم يفشل فى تسجيلها وفى إبلاغك، ويعود بـ«نجاح».**

**رابع شىء اليوم يُبلغ نجاحاً وهو لا يفعل شيئاً.**

### وخطآن فى إصلاحى أنا — أمسكهما الاختبار
لم أشحن الإصلاح مباشرة. جرّبت الشكل الجديد أولاً:

| ما جرّبته | النتيجة |
|---|---|
| الشكل القديم | **مؤكَّد معطوب** ✓ تشخيصى صحيح |
| شكلى الجديد للسجل | **فشل** — `system_integrity_check` غير مسموح فى قيد الإجراءات |
| شكلى الجديد للإشعار | **فشل** — `action_url` غير موجود، **نسختُه من الكود المعطوب بلا تحقق** |

**لو شحنت «الإصلاح» لبقيت القناة ميتة، ولظننتُها حيّة.**
...
### والأهم: الصمت لم يعد ممكناً
الـcron الآن **يفحص نتيجة كل كتابة**، وإن فشلت أى منها **يرد بخطأ 500 ويذكرها**.

> **مهمة مجدولة تكتب لا شىء وتقول «تم بنجاح» أسوأ من مهمة لا تعمل** — فالثانية يُلاحظ غيابها.

### Files
- `app/api/cron/system-integrity/route.ts`
- `supabase/migrations/20260720000012_v3_74_753_allow_system_integrity_audit_action.sql`

---

## [3.74.752] - 2026-07-20 — لم أجد عيباً، فوجدت ما هو أهم

### الوحدات الخاملة: **سليمة**
بدأت أفحص هل تستطيع دوال الرواتب والإهلاك الترحيل فى **فترة محاسبية مُقفلة**.

**وأخطأت ثلاث مرات متتالية فى نفس التحقيق:**

| # | الخطأ |
|---|---|
| ١ | بحثت عن `validate_transaction_date` — والاسم `validate_transaction_period` |
| ٢ | استنتجت أن `post_depreciation` لا تفحص — وهى تُنادى `require_open_financial_period_db` |
| ٣ | **افترضت أن كل دالة يجب أن تفحص بنفسها** |

**والثالث هو الأهم — لأنه افتراض عن التصميم لا عن اسم.**

### الحقيقة: الإنفاذ مركزى
`trg_period_lock_header` و`trg_period_lock_lines` تعملان **قبل كل كتابة** على جدولَى القيود. فالفترة المُقفلة ترفض القيد **أياً كانت الدالة التى تحاول**.
...
> **كنت أُفتّش فى الطبقة الخطأ.** أسأل «هل تفحص كل دالة؟» بينما النظام يفرضها مركزياً — **وهو التصميم الأفضل.**

### لكن الإنفاذ المركزى يُركّز الخطر
**٢٦ مُشغِّلاً** تحمل الآن حمايات لا يُعيد فحصها شىء آخر: أقفال الفترات · توازن القيد المزدوج · ترحيل COGS من FIFO · عزل الفروع.

> **و`DISABLE TRIGGER` جملة واحدة.** يُعطّلها أحد أثناء إصلاح بيانات، وينسى إعادتها — **فتختفى الحماية بصمت، وتستمر الكتابة، ولا شىء يُبلّغ.**

### الفاحص الجديد
يتحقق أن الستة والعشرين **موجودة ومُفعَّلة**. ويفحص **بالدالة لا باسم المُشغِّل** — فإعادة التسمية لا تُطلق إنذاراً كاذباً، والمهم أن الحماية **مُلحَقة وتعمل**.

### وأثبتُّ أنه يعضّ
عطّلت قفل الفترة عمداً:
```
قبل:      CLEAN
معطَّلاً:  حماية مُعطَّلة: قفل الفترات المحاسبية (رأس القيد)
بعد:      CLEAN
```

**فاحص لم يره أحد يعمل، لا يعرف أحد أنه يعمل.**

### Files
- `supabase/migrations/20260720000011_v3_74_752_ic_critical_triggers.sql`

---

## [3.74.751] - 2026-07-20 — صفر · وتصحيح لما قلته بالأمس

### أولاً: تصحيح
قلت فى v3.74.750 إن `restore_fifo_lots_on_return` و`reverse_fifo_consumption` **«لا تُحلّان بالبناء»** لأنهما تأخذان مرجعاً متعدد الأنواع.

**كان ذلك خاطئاً — وخاطئاً فى الاتجاه الأسوأ:**

> حوّلتُ **«لم أبحث بما يكفى»** إلى **«هذا مستحيل»**. وهكذا يُهجَر العمل بدل أن يُنجَز.

| الدالة | الحل الذى كان أمامى |
|---|---|
| `restore_fifo_lots_on_return` | تأخذ **`p_product_id`**، و`products` تحمل `company_id` |
| `reverse_fifo_consumption` | تكتب فى `fifo_lot_consumptions`، **وفيها `company_id` و`reference_id` معاً** |

**الجزء «متعدد الأنواع» لم يكن مهماً أصلاً.** ثبّتُّ نظرى عليه وتوقفت عن القراءة.

### والثلاث الأخرى احتاجت عمود مفتاح غير `id`
`transaction_id` و`tenant_id`. **و`tenant_id` هو الشركة فعلاً** — تحققت بالمطابقة لا بالاسم.

### ثم وسّعت السؤال بعد أن قال الفاحص CLEAN
**على مبدأ أن التقرير النظيف يساوى قيمة سؤاله.** المسح الأوسع أعطى ٣٠ مرشَّحاً: **٢٩ محصورة بالخادم منذ v3.74.728**، وواحدة حقيقية:

```
recompute_account_balances_for_date(target_company uuid, target_date date)
```

**تُعيد حساب أرصدة الحسابات، وكانت قابلة للنداء بلا تسجيل دخول.**

### ولماذا أفلتت من كل مسح
```
v3.74.727 يبحث عن  '%company_id%'   → "target_company"
v3.74.746 يبحث عن  '_id uuid'       → "target_company"
الفاحص ورث الشرطين                  → CLEAN
```

**الوسيط يعنى الشركة بالضبط. لكنه لا يُكتب كما افترضت.**

> **وثلاثة فحوص اتفقت على الخطأ لأنها ورثت الافتراض نفسه.** ثانية عشرة من نفس الجذر.

**فصار الفاحص يقبل أى وسيط `uuid`** — الهجاء لم يعد مهماً.

### الحصيلة النهائية
```
٤٨ → ٣٣ → ٢٩ → ١٣ → ٥ → ٠
دفتر الأستاذ: ١٩ / ١٩ ✓
CLEAN تحت أوسع تعريف
```

### Files
- `supabase/migrations/20260720000009_*.sql` · `20260720000010_*.sql`

---

## [3.74.750] - 2026-07-20 — «أى شركة؟» كان السؤال الخطأ

### قراءة الثلاث عشرة فرادى كشفت عيباً من نوع آخر
خمس دوال **لا علاقة لها بالشركات أصلاً**:
...
**كلها تأخذ معرّف المستخدم من المُنادى ولا تقارنه بالجلسة.**

> **`update_username` تُغيّر اسم أى مستخدم.** والباقى يجعل مستخدماً يقرأ إشعارات غيره أو يؤرشفها.

### ولماذا كان حجبها عن المسح السابق صواباً
`assert_company_access` **كان سيمرّرها بسعادة** — فالمُنادى **عضو فى الشركة فعلاً**، وكل ما فى الأمر أنه **يتصرف بهوية شخص آخر داخلها**.

> **فحص الشركة هنا كان سيبدو حمايةً ولا يحمى شيئاً.** وهذا أسوأ من غياب الفحص.

**السؤال الصحيح ليس «أى شركة؟» بل «هل هذا أنت؟»**

فصار `assert_is_self` — واختُبر:
```
نداء الخادم        → مسموح
أنت على نفسك       → مسموح
أنت على غيرك       → مرفوض
...
### وثلاث مرتبطة بفاتورة — حُرست بالطريقة المعتادة

### وخمس متبقية — بأسمائها وأسبابها
| الدالة | لماذا لم تُحرَس |
|---|---|
| `append_financial_audit_flag` | جدول `p_transaction_id` لم يُقرأ بعد |
| `enqueue_notification_outbox_event` | تأخذ `p_tenant_id` — يحتاج تأكيد أنه الشركة |
| `link_financial_operation_trace` | تُنادى من عميل المستخدم والخادم معاً |
| `restore_fifo_lots_on_return` | **مرجع متعدد الأنواع** — لا جدول واحد |
| `reverse_fifo_consumption` | نفس السبب |

**الأخيرتان لا تُحلّان بالبناء:** تأخذان `p_reference_type` و`p_reference_id`. **وتخمين جدول لمرجع متعدد الأنواع هو الطريق إلى حراسة الصف الخطأ.**

### والفاحص تعلّم الحارس الجديد
عرض **١٠** بعد الإصلاح لا ٥ — لأنه **لا يعرف `assert_is_self`**. نفس ما حدث فى v3.74.729 حرفياً.

> **عدّاد يتجاهل حارساً جديداً يجعل التقدم الحقيقى يبدو عدماً.**
...
### Files
- `supabase/migrations/20260720000008_v3_74_750_self_scope_and_invoice_scope.sql`

---

## [3.74.749] - 2026-07-20 — ١٧ جسم دالة، وخطآن أمسكهما الفحص لا أنا

### ١٦ دالة (١٧ جسماً) محروسة
موافقات · تقييم خصومات · حركات عهدة · إقفال فترات · نقل صلاحيات. **لا تمسّ الدفتر**، لكن العيب نفسه: موظف فى شركة يتصرف فى سجل شركة أخرى.

### وخطآن فى محاولتى الأولى — كلاهما أوقف الترحيل بدل أن يمر
**١ — اسم وسيط خاطئ.** كتبت `p_purchase_return_id` لـ`resubmit_purchase_return`، **واسمه الحقيقى `p_return_id`**.

استنتجته من استعلام سابق يستخرج **أول وسيط** ثم نسخته يدوياً. **والفحص الذى أضفته رفض الترحيل كله** بدل حقن نداء لا يُترجَم.

> **هذا وحده سبب أنه لم يُشحَن معطوباً.**

**٢ — نسختان من نفس الدالة.** `close_accounting_period` لها **توقيعان**. واختيارها بالاسم مع `LIMIT 1` كان **سيُصلح واحدة ويترك الأخرى مكشوفة صامتة**.

الحلقة صارت تدور على **صفوف `pg_proc`** لا على الأسماء — فتشمل كل النسخ. **وتحقق: نسختا `close_accounting_period` محروستان.**

### ثلاث عشرة متروكة عمداً — ومذكورة بالاسم
إشعارات · تتبّع مالى · FIFO · `update_username`.

**والسبب ليس الكسل:** بعضها **ليس مرتبطاً بشركة أصلاً**. `update_username` تخص **مستخدماً**، و`mark_notification_as_read` تخص **مُستلِماً**.

> **حراستها بالشركة ستكون فحصاً خاطئاً، لا فحصاً ناقصاً.** وهذا أسوأ — لأنه يبدو حمايةً وليس كذلك.

**تحتاج قراءة فردية.**

### اختبار سلوكى على فرع حقيقى
```
نداء الخادم      → مسموح
عضو الشركة       → مسموح
شركة أخرى        → مرفوض
...
### Files
- `supabase/migrations/20260720000007_v3_74_749_guard_remaining_row_id_writers.sql`

---

## [3.74.748] - 2026-07-20 — دفتر الأستاذ مُغلق بالكامل: ١٩/١٩

### الأربع المتبقية — والسبب الحقيقى لتأجيلها
قلت فى v3.74.747 إنها **«تُحمّل بياناتها بطريقة أخرى»**. **لم يكن ذلك دقيقاً.**

الحقيقة أنها تستخدم **أسماء مستعارة**:
```sql
FROM purchase_returns pr WHERE pr.id = ...
```

وتعبيرى كان يطلب `FROM <جدول> WHERE` **بلا اسم مستعار**. **لا شىء غريب فيها — نمطى أنا كان ضيقاً**، ووصفت قصورى وكأنه خاصية فيها.

| الدالة | الجدول |
|---|---|
| `confirm_purchase_return_delivery` | `purchase_returns` |
| `execute_customer_refund` | `customer_refund_requests` |
| `delete_fixed_asset_completely` | `fixed_assets` |
| `force_delete_all_depreciation_schedules` | `fixed_assets` |

### وعيب حقيقى فى دالتَى حذف الأصول
كلتاهما تجمعان قيود إهلاك الأصل **وتحذفانها** — السطور ثم القيود.

و`force_delete_all_depreciation_schedules` **تعدّ المُرحَّل منها فى متغير `v_posted_count`** — **ثم تمضى ولا تستخدمه أبداً**. العدّ يُجمَع ولا يُبنى عليه شىء.

> **حذف قيد إهلاك مُرحَّل يمحو أن الإهلاك حُسِب أصلاً.** فتختلف الحسابات عن سجل الأصول **ولا شىء يُفسّر السبب**.

**الحل يُبقى الحالة المشروعة ويرفض المدمّرة:** حذف أصل أُنشئ بالخطأ ولم يُرحَّل — **مسموح**. حذف إهلاك مُرحَّل — **مرفوض برسالة تشرح أن التصحيح بقيد عكسى**.

الوحدة تحمل **صفر أصول**، فلا شىء قائم يتأثر. **هذا عن أول مرة تُستخدم فيها.**

### إيجابية كاذبة أمسكتها فى استعلامى أنا
بعد الحقن، أظهر فحصى **ثلاث دوال تمسّ الدفتر بلا حارس**. فحصتها: **كلها قراءة فقط** — `assert_no_posted_depreciation` (الحارس الذى كتبته للتوّ) و`can_access_journal_lines` و`verify_depreciation_deleted`.

**السبب:** أسقطت شرط الكتابة من ذلك الاستعلام. **لا ثغرة — استعلامى هو الخطأ.**

### الحصيلة
```
دوال تمسّ دفتر الأستاذ:  ١٩ / ١٩ محروسة  ✓
العدّاد الكلى:            ٤٨ → ٢٩
```

**الباقى ٢٩ لا تمسّ الدفتر** — إشعارات وموافقات وصيانة FIFO.

### Files
- `supabase/migrations/20260720000006_v3_74_748_guard_last_four_ledger_functions.sql`

---

## [3.74.747] - 2026-07-20 — البند الحى سليم، و١٥ دالة محاسبية مؤمَّنة

### أولاً: `confirm_purchase_return_delivery_v2` — **لا تحتاج إصلاحاً**
كنت قد أبلغتك أننى **لم أحسم أمرها**. تتبّعتها الآن، والنتيجة: **سليمة**.
...
IF v_pr.workflow_status NOT IN ('pending_warehouse','pending_approval') THEN
  RAISE EXCEPTION 'Return is not pending warehouse confirmation...';
```

الصف **مقفول**، والاستدعاء الثانى **يفشل قبل الحذف بثلاثة آلاف حرف**. فالقيد الذى يُعاد بناؤه **مسودة بحكم التعريف** — والحذف هو «إعادة بناء سطور مسودة»، لا محو تاريخ.

**وتأكيد من البيانات:** صفر مرتجع معلَّق يحمل قيداً مُرحَّلاً.

**ولماذا لم أجد الحارس أول مرة:** بحثت عن `status` و`delivery_status`، **والعمود اسمه `workflow_status`**. حادية عشرة من نفس الجذر — وهى السبب الذى جعلنى أرفض إصدار حكم حينها.

### ثانياً: ١٥ دالة تمسّ دفتر الأستاذ — مؤمَّنة الآن
منها `record_payment` و`execute_sales_invoice_accounting` و`post_payroll_run_atomic` و`create_reversal_entry`.

**المشكلة:** لا تملك `company_id` لتُمرّره للحارس — تستقبل معرّف **فاتورة** أو **مسيّر رواتب**.

**الحل:** `assert_company_access_by_row(الجدول, معرّف السجل)` — يستنتج الشركة من السجل ثم يتحقق.

### وكيف عرفت جدول كل دالة — وهو بيت القصيد
**أول محاولة:** اشتققت الجدول من اسم الوسيط بالجمع — `p_asset_id` ← `assets`، `p_schedule_id` ← `schedules`.

**فـ٢٩ من ٤٨ أشارت إلى جداول لا وجود لها.**

> **التخمين من الأسماء هو نفسه العادة التى أنتجت النقطة العمياء التى جاء هذا الإصدار ليُغلقها.**

**فقرأت الجدول من جسم الدالة نفسه:**
```
FROM <الجدول> WHERE id = <وسيط الدالة>
```

**دليل، لا استنتاج.** ١٥ من ١٩ تُصرّح بجدولها هكذا. **والأربع الباقية تُحمّل بيانتها بطريقة أخرى — تُراجَع فرادى ولا تُخمَّن.**

**وتحققت أن الجداول الأحد عشر تحمل `id` و`company_id`** قبل أن أكتب حرفاً.

### اختبار سلوكى على فاتورة حقيقية
```
نداء الخادم بلا هوية        → مسموح   (مسارات الـAPI تعمل)
عضو فى شركة الفاتورة        → مسموح
مستخدم من شركة أخرى         → مرفوض
```
...
٤٨ → ٣٣
```
الباقى: **٤ تمسّ الدفتر** وتحتاج مراجعة فردية، و**٢٩ لا تمسّه**.

### Files
- `supabase/migrations/20260720000005_v3_74_747_assert_company_access_by_row.sql`

---

## [3.74.746] - 2026-07-20 — «CLEAN» كانت صحيحة فى نطاقها، وخاطئة فى الواقع

### بدأت بمسح الوحدات الخاملة
الأصول الثابتة · الرواتب · التحويلات · إشعارات الدائن. **كلها صفر صفوف** — لم تُستخدم قط.

فالسؤال ليس «هل تعمل؟» بل **«ماذا ستفعل بحساباتك أول مرة يستخدمها عميل؟»**

### فوجدت ما هو أكبر منها
```
delete_fixed_asset_completely(p_asset_id)
post_payroll_run_atomic(p_payroll_run_id, ...)
record_payment(p_invoice_id, p_amount, ...)
execute_sales_invoice_accounting(p_invoice_id)
```

**كلها قابلة للنداء بلا تسجيل دخول.**

> **`record_payment` وحدها تعنى: أى شخص، بلا حساب، يُسجّل دفعة على أى فاتورة فى أى شركة.**

### ولماذا لم يرها مسحى
مسح **v3.74.727** سحب `anon` من كل دالة تأخذ **`company_id`** فى وسائطها. **والنطاق كان خاطئاً.**

> **الدالة لا تحتاج `company_id` لتصل إلى بيانات شركة — يكفيها معرّف سجل، تعرف الشركة منه.**

```
٤٨ دالة فى النقطة العمياء
 ٤٨ منها قابلة للنداء بلا تسجيل دخول
 ١٩ تمسّ دفتر الأستاذ
  ٣ تحذف قيوداً محاسبية
```

### والفاحص كان يقول CLEAN طوال الوقت
لأنه ورث **نفس الافتراض**. وقد أبلغتك بذلك بالأمس.

**الفاحص كان دقيقاً داخل نطاقه، والنطاق هو العيب.**

> **تقرير نظيف يساوى بالضبط قيمة السؤال الذى وراءه.**

### المرحلة الأولى — نُفِّذت
```
anon:           ٤٨ → ٠
authenticated:  ٤٨ → ٤٨   (التطبيق كما هو)
```

**وتحققت قبل السحب** أن أياً من الـ٤٨ ليس مساراً قبل تسجيل الدخول — كلها موافقات وإشعارات وصيانة FIFO ومرتجعات وترحيل.

### والفاحص صار يفرّق بين ثلاث حالات
لأن علاج كل منها مختلف:

| الحالة | العلاج |
|---|---|
| قابلة لـ`anon` | سحب الصلاحية |
| تأخذ `company_id` | `assert_company_access` |
| **تأخذ معرّف سجل** | **تستنتج الشركة من السجل أولاً، ثم تتحقق** |

**والثالثة لا تقبل `assert_company_access(p_company_id)`** — لا تملك `company_id` أصلاً لتُمرّره. **كل دالة تحتاج عملاً مقصوداً، لا كنسة.**

اللوحة تعرض الآن **٤٨ — قيد المعالجة**، بدل صمتٍ كاذب.

### والوحدات الخاملة نفسها
لم تُفحَص محاسبياً بعد. لكن `delete_fixed_asset_completely` و`force_delete_all_depreciation_schedules` **تحذفان قيوداً محاسبية بدل عكسها** — نفس نمط `fix-bill-return` الذى أوقفناه فى v3.74.733. **بند تالٍ.**

### Files
- `supabase/migrations/20260720000003_*.sql` · `20260720000004_*.sql`

---

## [3.74.745] - 2026-07-20 — تجهيز آخر بند: قاعدة اختبارات مُثبَتة

### لوحة التحكم نظيفة
```
٧ ← ٣ ← ٠     CLEAN — no findings
```
تحقق من الجانبين: الفاحص صامت، **والعملاء الثلاثة تحت ملكية المالك فعلاً فى القاعدة** — لا مجرد اختفاء إنذار.

### البند الأخير — ولا أستطيع تنفيذه وحدى
إنشاء مشروع Supabase **قرار تكلفة يخص المالك**. فأعددت كل ما عداه ليصير نصيبه دقائق.

### ولماذا هذا البند يساوى بندين
| ما يفتحه | لماذا |
|---|---|
| ٥٨ حالة اختبار معطَّلة | تعمل وتصير بوابة نشر |
| **النسخة المرجعية للقاعدة** | **تُثبَت** لا تبقى مجرد ملف |

والثانى هو الأهم. `schema.sql` و`functions.sql` يحفظان الجداول والسياسات والمُشغِّلات والصلاحيات — **ولم يُثبَت قط أنهما يُعيدان البناء**.

> **استعادتهما فى مشروع فارغ هى الإثبات نفسه.** وإن فشلت، **نكتشف ذلك على مشروع فارغ لا على كارثة حقيقية.**

### `npm run test:db:verify`
يفحص **قبل** أن تلمس الاختبارات القاعدة:

1. أن الهدف **ليس الإنتاج**
2. أنه قابل للوصول والمفتاح صحيح
3. أن **المخطط موجود**
4. أن الاستعادة **مكتملة** لا جزئية

**والفحص الثالث يستحق التوضيح:** لو وُجِّهت الاختبارات إلى مشروع **فارغ**، لفشلت كلها برسائل «جدول غير موجود» — **وبدت كأنها عيوب فى الكود**. وقد يُنفَق يوم على تشخيص خاطئ قبل أن ينتبه أحد أن الاستعادة لم تحدث أصلاً.

**الخطآن يفشلان فى اتجاهين متضادين، والسكربت يقول لك أيهما عندك فى أمر واحد.**

### تحقق بالتشغيل
```
بلا متغيرات      → exit 1 ✓
مُوجَّه للإنتاج    → exit 1 ✓ «points at the PRODUCTION project»
```

### وكتلة ميتة أزلتها من سكربتى
كتبت استعلاماً لعدّ الدوال **ثم أهملت نتيجته**. أزلته: عدد الدوال مُستنتَج أصلاً من نجاح `export_public_schema()`.

**ومِسبار تُهمَل نتيجته أسوأ من غيابه** — وهذا نفس ما عالجناه فى v3.74.740 و.741 و.742.

### Files
- `scripts/verify-test-database.js` · `docs/TEST_DATABASE_SETUP.md` · `package.json`

---

## [3.74.744] - 2026-07-20 — إصلاحى الناقص من v3.74.725

### التصحيح نجح كما تنبّأت
```
٧ ← ٣
```
والثلاثة هم المتوقعون بالاسم: بيومى مصطفى · مجمد زيتون · محمد بسيونى — **كلهم فى مدينة نصر، ومنشئهم واحد انتقل إلى الفرع الرئيسي**.

### وقبل أن أرشدك، فحصت المسار
الفاحص نفسه يقترح: *«Reassign ownership via Settings > Users > Transfer Ownership»*.

**فذهبت أتأكد أن المسار سالك — ولم يكن.**

قائمة **الموظفين الهدف** تستثنى المستخدم الحالى:
```ts
members.filter(m => m.user_id !== selectedSourceUser && !m.is_current)
```

**والوجهة الصحيحة الوحيدة لهؤلاء الثلاثة هى أنت** — فرعهم **بلا موظف واحد**.

### وهذا عيبى أنا، نصف مُصلَح
فى **v3.74.725** جعلت المالك قابلاً للاختيار كـ**مصدر**، لأن النقل التلقائى يُودع العملاء لديه فيحتاج تسليمهم. **وكتبت حينها:**

> «العملاء يُودَعون حيث لا تصل إليهم أى قائمة»

**ثم تركت الاتجاه الآخر مغلقاً.** فالنقل **إلى** المالك — وهو **جوهر فكرة الإيداع** — ظل مستحيلاً يدوياً.

نجا الأمر لأن الكود يفعله **تلقائياً** عند تغيير الفرع. **أما ما سبق الميزة فظل عالقاً** — وهؤلاء الثلاثة بالضبط كذلك.

### الدرس
أصلحت العَرَض الذى رأيته، لا **التماثل** الذى يقتضيه. والقائمتان **مرآتان**: كل قيد على إحداهما يجب أن يُراجَع على الأخرى.

### وعاشرة، فى الحارس نفسه
أول تشغيل رفض كوداً **سليماً**: «قائمة الهدف لا تقبل المستخدم الحالى» — وهى تقبله.

قسّمت الملف على عبارة **«الموظفين الهدف»**، وهى **تتكرر أربع مرات** — مرتان فى رسائل خطأ قبل الواجهة بثلاثة آلاف سطر. فأخذ الحارس **المقطع بين الرسالتين**، ولم يرَ القائمة إطلاقاً.

صار التثبيت على نص **يظهر مرة واحدة**، **والحارس يتحقق أولاً من أنه فريد** — فلو تكرر مستقبلاً، يقول ذلك بدل أن يقرأ المنطقة الخطأ صامتاً.

**عاشر مرة فى يومين أُطابق ما يُشبه المطلوب.** والنمط لم يعد مصادفة — **هو الطريقة التى تفشل بها الحرّاس افتراضياً**، ولهذا صار كل حارس يُثبَّت على شىء فريد ويُختبَر فى الاتجاهين.

### تحقق من سلامة المسار كاملاً
| ما فحصته | النتيجة |
|---|---|
| منشئ الثلاثة | مستخدم واحد (staff) |
| الوجهة | المالك |
| مبدأ العينين | **owner واحد فقط** → استثناء المسؤول الأوحد ينطبق |

**فتستطيع اعتماد طلبك بنفسك، والمسار سالك من طرفه إلى طرفه.**

### بعد النشر
**الإعدادات ← المستخدمون ← نقل الصلاحيات:**
...
ثم اعتمد الطلب. **والنتيجة المتوقعة: ٣ ← ٠.**
...
## [3.74.743] - 2026-07-20 — صلاحية ممنوحة رسمياً ولا سبيل لممارستها

### كيف بدأت
طلبتَ تصحيح فرعَى عميلين، فأرشدتك إلى «تعديل العميل». **وكان إرشادى خاطئاً** — النموذج ردّ:

> 🔒 يمكن تعديل حقول العنوان فقط. الحقول الأخرى مقفلة.

ففحصت. **ووجدت أربع طبقات، كل واحدة معقولة وحدها، ومعاً تجعل صلاحية ممنوحة صراحةً مستحيلة التنفيذ.**

### الطبقات الأربع

**١ — الواجهة تُخفى المنتقى عند التعديل.** مشروط بـ`!editingCustomer` — يظهر عند الإنشاء فقط.

**٢ — والقفل لا يعرف الأدوار.** `isCustomerLocked` يقفل على الجميع، **بمن فيهم المالك**.

**٣ — والـAPI يمنح ثم يمنع.** يُصرّح للمالك بتغيير `branch_id` **ويُسجّله**، ثم بعد أسطر يرفض الطلب لأن `branch_id` ليس حقل عنوان: *«يمكنك تعديل العنوان فقط»*. **رفض الصلاحية التى منحها للتوّ.**

**٤ — والكتابة بصلاحية الخادم تمحو الهوية.** الحارس فى القاعدة يقرأ `auth.uid()`، والـAPI يكتب بمفتاح الخدمة — **فلا هوية**، فيعتبره `staff` ويرفض.

### ثم الطبقة الخامسة — وهى الحاسمة
حين تجاوزت الأربع، ظهر:
```
cannot insert a non-DEFAULT value into column "entity"
```

الحارس يكتب قيد المراجعة على عمودَى `entity` و`entity_id` — **وكلاهما `GENERATED ALWAYS`**، مرآتان لعمودين آخرين. **فترفضهما القاعدة، والاستثناء يُلغى التعديل كله.**

> **أى أن الصلاحية ليست صعبة المنال — بل معطوبة.** كل محاولة من أى مالك منذ كُتب هذا الحارس **فشلت**، برسالة لا تذكر فرعاً ولا صلاحية.

**ولم يكشفها إلا محاكاة التعديل بهوية المالك فعلاً**، بدل افتراض أن فحص الدور هو العائق الوحيد. **فحص الدور نجح — وقتلها كتابة سجل المراجعة.**

### الإصلاح
| الطبقة | التغيير |
|---|---|
| الواجهة | المنتقى يظهر عند التعديل لمن يملك الصلاحية، بقيمته الحالية وتنبيه واضح |
| القفل | يبقى على كل شىء **إلا** الفرع لمن يملكه |
| الـAPI | حقل حوكمة مُصرَّح به لم يعد يُحسب «غير عنوان» فى بوابة الفواتير |
| الكتابة | تمرّ بجلسة المستخدم لا بمفتاح الخدمة، فيرى الحارس صاحبها الحقيقى |
| القاعدة | القيد يُكتب على الأعمدة الحقيقية، فتملأ المُولَّدة نفسها |

### وخطأ ارتكبته أثناء الإصلاح
اختصرت اسم الإجراء إلى `customer_branch_changed` — **وهو مُقيَّد بـ`audit_logs_action_check`**. فاستبدلت عطباً بعطب.

**ما يتحقق منه شىء آخر ليس حراً فى التجميل.**

### تحقق بالتشغيل لا بالقراءة
```
المالك يغيّر الفرع        → مسموح · وقيد المراجعة مكتوب (entity=customers)
الموظف يغيّر الفرع        → GOVERNANCE_VIOLATION
لا تغيير فعلى فى الفرع    → يمرّ دون مساس
```

**واختبار سابق أعطى «الموظف مسموح له» — لأننى ضبطت الفرع على قيمته الحالية**، فعاد الحارس مبكراً. الخطأ كان فى اختبارى لا فى الحارس.

### بعد النشر
**العملاء ← عدّل ← الفرع:**
- `ahmed abuelmagd` → **الفرع الرئيسي**
- `بيومى مصطفى` → **مدينة نصر**

والنتيجة **٧ ← ٣** كما شرحت، والثلاثة الباقية عملاء فرع مدينة نصر الخالى من الموظفين.

### Files
- `components/customers/customer-form-dialog.tsx` · `app/api/customers/update/route.ts`
- `supabase/migrations/20260720000002_v3_74_743_fix_customer_branch_audit_insert.sql`

---

## [3.74.742] - 2026-07-20 — الحارس الذى بنيته للتوّ كان يقرأ الرقم الخطأ

### ظهر فى مخرجات نشر v3.74.741
```
Test Files  2 passed | 4 skipped (6)
      Tests  50 passed | 32 todo (82)
+ 2 real tests pass, the rest are declared todo
```

**«٢ ناجحة»؟** الرقم الصحيح **٥٠**. حارسى قرأ سطر **الملفات** لا سطر **الاختبارات**.

### ولماذا لم يكن خطأً تجميلياً
الحارس مبنى ليمنع عودة الاختبارات الوهمية:

> إن تجاوز عدد الناجحة ٦٠، أوقف النشر.

فلو عادت الحالات الاثنتان والثلاثون إلى «النجاح»، لصار السطر:
```
Test Files  6 passed (6)
      Tests  82 passed (82)
```
**وحارسى يقرأ ٦.** و**٦ < ٦٠**، فيمرّ.

> **الحارس المبنى لمنع عودة العدّاد الكاذب كان سيسمح بها بالضبط.**

### تحققت بدل أن أُصلح وأمضى
```
الرقم الحالى  → المجرد: ٢    · المُثبَّت: ٥٠  ✓
لو عاد الـ٨٢  → المجرد: ٦ (يمرّ ✗) · المُثبَّت: ٨٢ (يُمسَك ✓)
```

**التثبيت على `Tests\s+(\d+)\s+passed`.**

### ثامن مرة، ونفس الجذر
| # | ما طابقته | فماذا التقط |
|---|---|---|
| ٧٢٦ · ٧٢٧ · ٧٣٣ | أسماء دوال ومسارات | تعليقاتى |
| ٧٣٥ | كلمة `TRIGGER` | صلاحيات الجداول |
| ٧٣٨ | اسم متغير | إصلاحى أنا |
| ٧٤٠ | `argv[1]` | مسار السكربت |
| **٧٤٢** | **`(\d+) passed`** | **سطر الملفات** |

**ثمانى مرات، ونمط واحد: التقطتُ ما يُشبه المطلوب لا المطلوب.**

### وتاسعة، فى نفس السكربت
أول تشغيل بعد التثبيت **رفض النشر**: «سطر الاختبارات لا يذكر todo» — **وهو يذكره أمام عينى فى المخرجات.**

السبب: `vitest` **يُلوّن ملخّصه**. فبين الكلمات **رموز ألوان غير مرئية**، وأى نمط يعبرها يفشل.

> **ما تراه على الشاشة ليس ما فى النص.**

فصار السكربت **يُجرّد رموز الألوان أولاً**. وحالات الاختبار الآن **مُلوَّنة كما يُخرجها vitest فعلاً** — لأن النسخة النصية المجرّدة **كانت تنجح بينما الحقيقية تفشل**.

والفارق الوحيد أن السابقة كانت **تُنذر خطأً** — فتُكشف فوراً. **وهذه كانت تسكت خطأً** — ولا تُكشف أبداً.

> **الحارس الصامت هو الأخطر.** ولم يكشفه إلا أننى قرأت رقماً بدا غريباً فى مخرجات نشر ناجح.
...
## [3.74.741] - 2026-07-20 — ٣٢ اختباراً «ناجحاً» لا يفحص شيئاً

### لاحظتها فى مخرجات النشر نفسها
سطر فى نتيجة `v3.74.740`:
```
stderr | tests/critical/inventory.test.ts
Supabase credentials not found - skipping tests
 ✓ tests/critical/inventory.test.ts (9 tests)
```

**«تخطّى الاختبارات» ثم ✓ تسعة ناجحة.** و**٨٢ اختباراً فى ٥٥ مللى ثانية** — رقم مستحيل لاختبارات تلمس قاعدة بيانات.

### فحصت، فوجدت
```
٨٢ حالة فى tests/critical
٣٢ منها:  expect(true).toBe(true)   // Placeholder
```

**أربعة ملفات كاملة، كل حالة فيها TODO:**

| الملف | الحالات |
|---|---|
| `security.test.ts` | ٦ — **كلها وهمية** |
| `invoices.test.ts` | ١١ — **كلها وهمية** |
| `inventory.test.ts` | ٩ — **كلها وهمية** |
| `journal.test.ts` | ٦ — **كلها وهمية** |

### وأنا قلت لك بالأمس إن هذه هى الحارس الحقيقى
**كان كلامى خاطئاً.** الحارس الحقيقى **٥٠ حالة** فى ملفَّين (`equity-governance` و`financial-integrity`) — منطق سليم يُختبر فعلاً.

### ولماذا هذا أسوأ من غياب الملف
ملف اسمه `security.test.ts` يقول **«٦ ناجحة»** وهو لا يفحص شيئاً — **وهو أول ما ينظر إليه من يسأل «هل الصلاحيات مغطاة؟»**

> **الملف الغائب يُثير السؤال. والنجاح الكاذب يُجيبه إجابة خاطئة.**

و`invoices.test.ts` تعلو رأسَه العبارة: *«أى فشل فى هذه الاختبارات يُعتبر BUG وظيفى»* — **بينما لا شىء فيه يستطيع الفشل**.

و`api-security.test.ts` كان يحمل حالتين وهميتين لفحص **الخاصية التى انكسرت فعلاً**: «لا تقبل `companyId` من الرابط». وهى بالضبط ثغرة `bonuses` التى أصلحناها فى v3.74.737. **العلامة الخضراء كانت هناك طوال الوقت.**

### العلاج: `it.todo`
لم أحذف الأسماء — **فهى مواصفة دقيقة لثوابت حقيقية**، وكتابتها لها قيمة قبل وجود أجسامها.

لكن `vitest` سيعرضها الآن **TODO لا PASSED**. **العدّاد توقف عن الكذب.**

```
قبل:  82 passed
بعد:  50 passed · 32 todo
```

### وما هو مُغطّى فعلاً — بغير هذه الملفات
- **عزل الشركات فى الـAPI** → فحص ١١٢ مساراً بحالات اختبار ذاتية (v3.74.739)
- **عزل الشركات فى القاعدة** → `assert_company_access` على ٨٨ دالة، مُختبَرة سلوكياً (v3.74.729–731)
- **توازن القيد المزدوج** → `trg_enforce_journal_balance` فى قاعدة البيانات

**التغطية موجودة — لكنها تعيش فى CI وفى القاعدة، لا فى هذه الملفات.**

### الخلاصة
ثالث شىء اليوم يُبلغ نجاحاً وهو لا يعمل: أداة الإصلاح، وطبقة الاختبارات المعطَّلة، **والآن العدّاد نفسه**.

### Files
- `tests/critical/{security,invoices,inventory,journal}.test.ts`
- `tests/integration/api-security.test.ts`

---

## [3.74.740] - 2026-07-20 — الاختبارات كانت مُصوَّبة على قاعدة الإنتاج

### البند الذى وافقتَ عليه لم يكن ما ظننّاه
اتفقنا على «تفعيل الاختبارات كبوابة نشر» بإزالة `continue-on-error`. **ففحصت قبل التنفيذ.**

**١ — ست ملفات من عشرة تتخطى نفسها.** مشروطة بـ`RUN_API_INTEGRATION_TESTS` **غير المضبوط فى أى مكان**.

**٢ — والباقى لم يكن يعمل أصلاً.** خطوة فحص المفاتيح تتخطى الطبقتين كلتيهما إن لم تُضبط أسرار GitHub.

> فإزالة `continue-on-error` كانت **لن تُغيّر شيئاً**. تُخفى الراية **لا شىء**، لأن **لا شىء يعمل**.

**٥٨ حالة اختبار معطَّلة، والحارس الحقيقى الوحيد هو `tests/critical`.**

### وما وجدته بدل ذلك — لغم
`createTestClient()` كان يقرأ:
```
NEXT_PUBLIC_SUPABASE_URL
SUPABASE_SERVICE_ROLE_KEY
```

**نفس متغيرَى التطبيق. ونفس الأسرار التى يمرّرها CI.**

وهذه الاختبارات **تُنشئ مستخدمين وشركات وفواتير وقيوداً وتحذفها**.

> فلحظة أن يضيف أحد تلك الأسرار «ليُشغّل الاختبارات»، **يبدأ كل دفع إلى main بإنشاء شركات وهمية داخل قاعدة محاسبتك الحية وحذفها.** ولا شىء فى المشروع يقول «لا».

**تحققت قبل أن أكتب هذا:** الإنتاج فيه **صفر** مستخدم بنمط `test-%@test.com`، و**صفر** شركة بنمط `%test%`.

**فهو لغم لم ينفجر، لا حريق.**

### العلاج
**١ — الاختبارات تُسمّى قاعدتها بنفسها.** `TEST_SUPABASE_URL` و`TEST_SUPABASE_SERVICE_ROLE_KEY` فقط — **بلا أى ارتداد** إلى متغيرات التطبيق.

**٢ — وخط دفاع ثانٍ:** حتى لو وُجِّهت إلى معرّف مشروع الإنتاج، **ترفض العمل صراحة**. معرّف المشروع ليس سراً — يُشحن فى المتصفح — فذكره هنا **يجعل الحادث مستحيلاً لا بعيداً**.

**٣ — و`continue-on-error` أُزيلت.** إما تعمل فعلاً وتُوقف البناء، أو **يُعلَن بوضوح أنها لا تعمل**:

```
::warning:: 58 test cases are NOT executing.
The only suite guarding this build is tests/critical.
```

> **تشغيل شىء تتجاهل نتيجته أسوأ من عدم تشغيله** — يشترى شعوراً بالتغطية ولا يدفع ثمنه.

### وسابع مرة أستنتج من اسم
استنتجت أن هذه الاختبارات **لا تلمس قاعدة البيانات** لأن `grep` عن `createClient` أعطى صفراً. **والسبب أنها مُغلَّفة فى `createTestClient()`.**

لو بنيت التوصية على ذلك، لكنت طمأنتك إلى عكس الحقيقة تماماً.

### ما يلزم لتفعيلها فعلاً
**مشروع Supabase منفصل للاختبارات.** وهو **نفس ما يحتاجه** إثبات أن النسخة المرجعية تُعيد البناء (v3.74.734).

**البندان المتبقيان يلتقيان عند شىء واحد** — وهو قرار تكلفة، قرارك.

### Files
- `tests/helpers/test-setup.ts` · `.github/workflows/ci.yml`

---

## [3.74.739] - 2026-07-20 — القائمة صفر، والقاعدة تُثبت أنها مازالت تعضّ

### الأربعة الباقية — كلها سليمة
| المسار | كيف يُقيّد |
|---|---|
| `billing/invoices/[id]/pdf` | يجلب الصف ثم **يقارن** `invoice.company_id` بشركة الجلسة |
| `permissions/shared-with-me` | يُقيّد بـ`grantee_user_id` = **المستخدم نفسه**، فذكر شركة أخرى لا يوسّع شيئاً |
| `invoices/[id]/record-payment` | يُسلّم الشركة المشتقّة إلى **خدمة أوامر** تتولى التقييد |
| `billing/renew` | الشركة من **رمز HMAC موثّق** |

### توقفت عن مطاردة الأشكال
ست مراجعات للقاعدة، وكل مرة شكل جديد. **مطاردة خاسرة.**

فالسؤال تحت كل هذه الأشكال واحد:

> **هل جاءت الشركة من مصدر لا يختاره المُنادى؟ وهل استُخدمت فعلاً؟**

إن كان كذلك، **فليُعبّر المسار عن التقييد كما يشاء** — بمُرشِّح أو بمقارنة أو بتسليمها لخدمة.

### وللمرة السادسة، اسم لا شكل
`billing/renew` يُسمّى الحقل `payload.cid` لا `companyId`. **وقاعدتى كانت تشترط الاسم** — فرفضت مساراً شركته من **رمز موقَّع**، وهو من أوثق المصادر الممكنة.

**الرمز الموثّق هو التفويض نفسه. واسم حقله لا يخص أحداً.**

### الخطر الذى أنشأه التوسيع — واختبار يمنعه
وسّعت القاعدة **ست مرات**. وكل توسيع يقترب بها من أن تصبح **زينة تقبل كل شىء**.

فأضفت **ست حالات اختبار تعمل مع كل تشغيل**، تُثبّت الطرفين:
- الشكل الذى سبّب الحادثة **يجب أن يُرفض**
- الأشكال التى أُنذرت خطأً **يجب أن تُقبل**

**وتحققت أنها تعضّ فعلاً:** كسرت القاعدة عمداً، فسقطت الحالتان فوراً برسالة واضحة. اختبار لا يفشل حين تُكسر القاعدة **ليس اختباراً**.

### ومصدران للحقيقة أنشأتُهما ثم أزلتهما
كتبت القاعدة مرتين: مرة فى الحلقة الرئيسية، ومرة فى `evaluate()` للاختبار. **وهذا بالضبط ما ينتج «الفحص يمرّ والاختبار يفشل»** — وهو النمط الذى أُنبّه عليه طوال اليوم.

الحلقة تستدعى `evaluate()` الآن. **تعريف واحد.**

### حصيلة المراجعة الكاملة
```
١٣ إنذاراً أصلياً:
  ٢  ثغرات حقيقية   (bonuses GET · audit-log)
  ١  سوء تصنيف      (subscription/create — تسجيل عام)
 ١٠  قاعدتى أضيق من المشروع
```

**القائمة صفر.** وأى مخالفة جديدة **تُوقف البناء**.
...
## [3.74.738] - 2026-07-20 — سجل المراجعة كان يقبل قيوداً مزوّرة من أى أحد

### الثغرة
`POST /api/audit-log` كان يُدخل القيد **ويعود** قبل أن يصل إلى فحص الجلسة المكتوب أسفله. وفى الإنتاج — حيث المفاتيح موجودة دائماً — **ذلك الفحص كود ميت لا يُنفَّذ**.

فأى طلب **بلا مصادقة إطلاقاً** كان يكتب:
```
action · company_id · user_id · user_email · user_name · old_data · new_data · ip_address
```
**بالقيم التى يختارها المُرسِل**، فى سجل مراجعة **أى شركة**.

### ولماذا هذا أخطر مما يبدو
سجل المراجعة هو **ما تعود إليه عند النزاع** لتعرف من فعل ماذا.

> **سجل يستطيع أى أحد الكتابة فيه ليس دليلاً على شىء.**

وقيد مزوَّر يتّهم موظفاً حقيقياً **لا يمكن تمييزه عن قيد صادق**.

**الإصلاح:** الجلسة تُثبَت **أولاً**، و`user_id` من الجلسة، والشركة **لا تُقبل إلا بعد التحقق من العضوية** فيها. حقول الهوية القادمة من الطلب **تُتجاهَل لا تُصدَّق**.

**والسلوك محفوظ:** فشل التسجيل لا يوقف سير العمل — كما كان.

### ستة إنذارات كانت خطأى لا خطأها
`account-lines` و`billing/preview` و`billing/seats` و`bonuses/settings` و`company-logo` و`send-purchase-order` — **كلها سليمة**.

تُقيّد بـ`.eq("id", companyId)` على جدول الشركات، أو عبر مسار مُركَّب `journal_entries.company_id`. **وقاعدتى كانت تعرف شكلاً واحداً فقط.**

**وسّعت القاعدة، لا الكود.** إنذار كاذب ليس مجرد إزعاج — **هو الطريق إلى فحص لا يقرؤه أحد**.

### وللمرة الخامسة: طابقتُ اسماً لا شكلاً
بعد إصلاح `audit-log`، **رفض السكربت إصلاحى أنا**.

السبب: فحص العضوية كان يشترط أن يُسمّى متغير الجلسة `user` أو `authUser` أو `userId` — **وأنا سمّيته `sessionUser`**.

| الإصدار | ما طابقته |
|---|---|
| ٧٢٦ · ٧٢٧ · ٧٣٣ | أسماء دوال ومسارات |
| ٧٣٥ | كلمة `TRIGGER` |
| ٧٣٨ | **اسم متغير** |

الشكل الذى يهم: **ابحث فى `company_members`، مُقيَّداً بالشركة وبالمستخدم.** **واسم المتغير لا يخص أحداً.**
...
### Files
- `app/api/audit-log/route.ts` · `scripts/check-service-role-scoping.js`

---

## [3.74.737] - 2026-07-20 — الفحص الجديد وجد ثغرة حقيقية فى أول تشغيل

### بيانات مكافآت الموظفين كانت مكشوفة للجميع
`GET /api/bonuses` كان يقرأ معرّف الشركة **من عنوان الرابط**، **بلا أى مصادقة إطلاقاً**، ثم يستعلم بالصلاحية الكاملة:

```
GET /api/bonuses?companyId=<أى معرّف>
```

> **أى شخص يصل إلى الرابط كان يقرأ سجل مكافآت أى شركة** — بيانات تعويضات موظفين.

**والمفارقة:** `POST` على **نفس الملف** كان مُحصَّناً بشكل صحيح (`secureApiRequest` + صلاحية + أدوار). **الفتحة فى `GET` وحده.**

وكان التعليق فوق السطر يقول: *«جلب companyId من الـ URL parameters مباشرة»* — **مكتوباً كأنه ميزة.**

**الإصلاح:** الشركة تأتى من **الجلسة** كما فى `POST` تماماً. **ومعامل `companyId` فى الرابط يُتجاهَل** — المُنادى لا يختار بيانات مَن يقرأ.

### وهذا هو المقصود بالفرق
لم يجد هذه الثغرةَ **أحد**. وجدها **فحص عام كُتب بالأمس** فى **أول تشغيل**.

والقائمة كانت تقول عنها «بانتظار المراجعة» — **لا «آمنة»**. وهذا التمييز لم يكن لفظياً.

### `subscription/create` — تصنيف لا إصلاح
راجعتها: **تسجيل عام** يُنشئ **مستأجراً جديداً** (مستخدم + شركة + فرع) **ولا يمس أى شركة قائمة**. نُقلت إلى المستثناة بسبب موثّق.

**لكن لاحظت أمراً آخر ولا أخلطه بالأول:** تستدعى `auth.admin.createUser` **بلا حد معدّل** — وهذا **باب إساءة استخدام** (إنشاء حسابات بالجملة)، لا ثغرة بيانات. سجّلته ليُقرَّر فيه منفصلاً.
...
نقصت باثنتين: واحدة أُصلحت، وواحدة صُنِّفت. **والسكربت يرفض البناء لو بقيت أى منهما فى القائمة بعد نظافتها** — فالنقصان مُلزِم.

### Files
- `app/api/bonuses/route.ts` · `scripts/check-service-role-scoping.js`

---

## [3.74.736] - 2026-07-20 — فحص أمنى يغطى ١١٢ مساراً بدل ثلاثة

### ما كان موجوداً
وظيفة الأمان فى CI كانت تفحص **ثلاثة ملفات بالاسم**، وتتأكد أن حارس الصلاحية **مذكور** فيها.

**عيبان، وكلاهما أثبتَته v3.74.733:**

١. **ثلاثة من ١١٢** مساراً يستخدم الصلاحية الكاملة. الباقى بلا تغطية، ولا أحد وسّع القائمة.

٢. **تفحص أن السؤال طُرح، لا أن الإجابة استُخدمت.** و`fix-negative-payments` **كانت تستدعى الحارس فعلاً**، ثم تُهمل معرّف الشركة وتكتب فى كل الشركات. **كانت ستمرّ.**

### القاعدة الجديدة
> أى مسار يستخدم الصلاحية الكاملة يجب أن يكون مُصادَقاً، وأن يُقيّد عمله بشركة **لا يختارها المُنادى** — أو يتحقق من عضويته فيها إن أرسلها.

### أربع محاولات قبل أن أُصيب
| المحاولة | ما فحصته | ما كسرها |
|---|---|---|
| ١ | «هل يُنادى أحد هذه الحُرّاس؟» | `customers/delete` يتحقق من العضوية بشكل مختلف — **سليم** |
| ٢ | + نمط التحقق من العضوية | `bills/[id]/journal-entry-id` يستخدم `enforceGovernance()` — **حارس لم أكن أعرفه** |
| ٣ | + قائمة أسماء أوسع | `billing/renew` يُصادِق **برمز HMAC موقَّع** — آلية رابعة |
| ٤ | **لا أسأل عن الأسماء إطلاقاً** | ✓ |

**الدرس:** كل مرة كتبت فيها قاعدة **من ذاكرتى عن المشروع**، وجد المشروع آلية مشروعة لم أعرفها. القائمة لا تكتمل أبداً بقدر معرفة كاتبها.

فالنسخة النهائية **لا تسأل أى حارس استُخدم**، بل تسأل السؤالين اللذين يقرران فعلاً: **هل المُنادى مُصادَق؟ وهل الشركة من مصدر لا يتحكم فيه؟**

### النتيجة
```
١١٢ مساراً مفحوصاً
 ١٦ مُستثنى بسبب موثّق (cron · webhooks · دعوات ما قبل الدخول)
 ١٣ بانتظار المراجعة
```

### سقّاطة لا قائمة استثناءات
الـ١٣ **لم أقرأها كلها**. **ولن أُعدّل مسارات محاسبية لم أقرأها** — فهذه هى الطريقة التى تُنتَج بها أخطاء `fix-negative-payments` لا تُزال.

فالسكربت:
- **يرفض البناء فوراً** لأى مخالفة **جديدة**
- **ويرفضه أيضاً** إذا نظُف مسار وبقى فى القائمة — **فالقائمة مُجبَرة على النقصان لا الترهّل**

**وجودك فى القائمة يعنى «لم يُفحَص بعد»، لا «آمن».**

### Files
- `scripts/check-service-role-scoping.js` · `.github/workflows/ci.yml` · `package.json`

---

## [3.74.735] - 2026-07-19 — الرقم الذى كتبته فى النسخة المرجعية كان خاطئاً

### تصحيح فورى لخطأ فى v3.74.734
نتيجة النشر عرضت:
```
triggers=1359
```

**بينما فى قاعدة البيانات ٥٠١ مُشغِّل.**

### كيف وقعت فيه — بينما كنت أُصلحه
فى v3.74.734 عددتُ `CREATE TRIGGER` فحصلت على ٤٩٩. **اكتشفت السبب بالتحقق** — اثنان `constraint triggers` بصيغة مختلفة — وكتبت ذلك فى سجل التغييرات باعتباره نموذجاً للدقة.

**ثم صحّحته بالبحث عن كلمة `TRIGGER` وحدها.**

و`TRIGGER` **صلاحية جدول**. فكل سطر منح يقول:
```
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, ...
```
**فأصبح كل جدول يُحسب كمُشغِّل.**

```
الكلمة المجردة TRIGGER:        ١٣٥٩
CREATE TRIGGER:                 ٤٩٩
CREATE CONSTRAINT TRIGGER:        ٢
الصحيح:                        ٥٠١ ✓
```

### لماذا هذا أخطر من عدّاد مغلوط
الرقم **كُتب داخل ترويسة `schema.sql`** — الملف الذى **وظيفته الوحيدة أن يقول ما تحتويه قاعدة الإنتاج**.

> **مرجع يكذب فى سطره الأول.**

ولو قرأه أحد بعد سنة ليعرف حجم النظام، لصدّق ١٣٥٩.

### النمط الذى تكرر اليوم أربع مرات
| # | الخطأ |
|---|---|
| ٧٢٦ | حارس يطابق **اسم** الدالة فرفض تعليقى |
| ٧٢٧ | حارس يطابق **اسم** `cost_price` فرفض شرحى |
| ٧٣٣ | حارس يطابق **مسار** الـAPI فرفض توثيقى |
| ٧٣٥ | عدّاد يطابق **كلمة** `TRIGGER` فحسب الصلاحيات |

**الكلمة ليست المعنى.** طابِق **الجملة** لا **اللفظ**: `CREATE (CONSTRAINT )?TRIGGER`.

### Files
- `scripts/dump-db-schema.js` · `supabase/schema/schema.sql` (أُعيد توليده)

---

## [3.74.734] - 2026-07-19 — قاعدة البيانات لم تكن فى المستودع أصلاً

### السؤال كان: ما الذى يلزم لمشروع عالمى؟
فحصت قبل أن أُجيب — **وظنّى كان خاطئاً للمرة الرابعة اليوم.** توقعت مشروعاً بلا اختبارات ولا CI. الواقع: **٣٤ ملف اختبار، وCI على كل دفعة، وحارس pre-push، وتدقيق حوكمة، ونشر تلقائى.** البنية موجودة وجيدة.

**لكن الفحص كشف شيئاً آخر.**

### الدفتر لا يطابق المستودع — ولا يقترب
```
ترحيلات مُسجَّلة كمُطبَّقة:  ٦٦١
ملفات ترحيل فى المستودع:   ٥٢٩
المشترك بينهما:            ٤٩ فقط
```

**٤٩ من ٦٦١.** الباقى طُبِّق عبر محرر SQL أو MCP، وكلاهما يُسجّل **طابعاً زمنياً** لا اسم الملف. حتى ترحيلات اليوم مُسجَّلة بأسماء غير أسماء ملفاتها.

فالمجلد **لا يستطيع أن يُخبر أحداً بما تحتويه قاعدة الإنتاج**.

### وما هو محفوظ فعلاً — وما ليس كذلك
`supabase/schema/functions.sql` يحفظ **١١٩٦ دالة**، ويُحدَّث كل إصدار. ممتاز.

**وما لا يحفظه أحد — موجود فى الإنتاج فقط:**

| العنصر | العدد |
|---|---|
| جداول / أعمدة | ٢٤٩ / ٤٣٩٧ |
| **سياسات RLS** | **٧٩٧** |
| **مُشغِّلات (triggers)** | **٥٠١** |
| فهارس | ١٢٠٢ |
| قيود | ١٧٩٥ |
| **صلاحيات الدوال** | كلها |

السياسات هى **نموذج الأمان كله**. والمُشغِّلات هى **حيث تعمل FIFO وCOGS فعلياً**.

و`db_dump.sql` و`db_schema.sql` فى جذر المشروع — **حجم كل منهما صفر بايت** منذ مايو.

### والمفارقة الأقسى
`pg_get_functiondef` **لا يُخرج الصلاحيات**. فملف الدوال — رغم جودته — **لا يستطيع بنيوياً** حفظ ما غيّرناه اليوم.

> إعادة بناء من المستودع كانت ستُنشئ كل دالة بصلاحية **`EXECUTE` لـ PUBLIC** الافتراضية — **فتُلغى بصمت عمل اليوم كله.**

### ما نُفِّذ
`export_public_schema()` + `scripts/dump-db-schema.js` → `supabase/schema/schema.sql`

يحفظ الجداول والقيود والفهارس والمُشغِّلات وسياسات RLS **والصلاحيات**.

**وحارس ضد التصدير المبتور:** لو عاد التصدير ناقصاً، يرفض السكربت الكتابة. **نسخة مبتورة تُحفَظ كمرجع أسوأ من فشل صريح** — تُبلغ عن أقل مما فى الإنتاج، ولا أحد يعلم.

### تفصيلة أمسكتها بالتحقق
عدّ المُشغِّلات أعطى ٤٩٩ لا ٥٠١. لم أتجاهل الفرق: اثنان منها **constraint triggers** فيخرجان بصيغة `CREATE CONSTRAINT TRIGGER` لا `CREATE TRIGGER`.

وهما بالتحديد `trg_enforce_journal_balance` و`trg_recurring_template_balance` — **مُشغِّلا توازن القيد المزدوج**. التصدير سليم، والعدّ كان يُسقطهما.

### ما لم يتحقق بعد — ولا أدّعيه
**لم يُثبَت أن هذا الملف يُعيد بناء القاعدة وحده.** إثباته يحتاج **استعادة فى مشروع تجريبى ومقارنة**.

حتى ذلك الاختبار: **نستطيع رؤية ما فى الإنتاج، لا إعادة إنشائه.** والفرق بينهما هو الفرق بين توثيق وتعافٍ من كارثة.

### Files
- `supabase/migrations/20260719000736_v3_74_734_export_public_schema.sql`
- `scripts/dump-db-schema.js` · `supabase/schema/schema.sql` (مُولَّد)

---

## [3.74.733] - 2026-07-19 — حارسٌ أوقف خطأً كنت على وشك ارتكابه

### اعتراف أولاً
بحثت عن المسارات الثلاثة فلم أجدها، **فأعلنت أنها غير موجودة وأن الصفحتين ميتتان**، وكتبت إصداراً يحذفهما.

**بحثى كان خاطئاً.** استخدمت بحثاً فى **محتوى** الملفات عن اسم المجلد — واسم المجلد لا يُذكر داخل `route.ts` بطبيعة الحال. والمسارات موجودة كلها.

سكربت الرفع **رفض النشر** ووقف عند الفرضية:
```
X these endpoints now EXIST - the pages were deleted on the premise that they did not
```

كنت قد كتبت هذا الحارس ليتحقق من **الفرضية لا من النتيجة**. وهو ما أمسك خطئى أنا.

### والحقيقة أخطر مما خمّنت أصلاً

#### `/api/fix-negative-payments` — يكتب فى كل الشركات
```ts
const { error: authError } = await requireOwnerOrAdmin(req)   // companyId مُهمَل
supabase.from("payments").select("*").lt("amount", 0)          // بلا أى تقييد بشركة
```

ثم يأخذ `company_id` **من صف الدفعة نفسه** لا من جلسة المُنادى، ويُنشئ مرتجعات ويحذف مدفوعات — بـ`service_role`، فلا تنطبق RLS.

> **مسؤول فى أى شركة يضغط الزر، فيُعاد كتابة سجل المدفوعات لدى كل الشركات.**

فحص الصلاحية كان يتأكد أنه **مسؤول فى مكان ما** — ولم يُقيّد **ما يمسّه** أبداً.

**والمقارنة تكشف كل شىء:** توأمه `/api/inspect-negative-payments` يحتفظ بـ`companyId` ويُطبّق `.eq("company_id", companyId)`. **نفس الميزة، وسطر واحد يفرق — والآمن هو الذى يقرأ فقط.**

#### `/api/fix-bill-return` — يمحو الدفاتر بدل عكسها
- **حذف نهائى** لسطور القيود ثم القيود نفسها
- **حذف نهائى** لحركات المخزون
- تصفير `returned_quantity`
- وفرض `status: "paid"` — وبجواره **تعليق كاتبه نفسه**: «أو الحالة المناسبة»

**التصحيح المحاسبى يكون بقيد عكسى يُبقى الأصل ظاهراً، لا بحذفه.** بعد تشغيلها لا يبقى دليل أن المرتجع حدث، ولا أنه حُذف.

وتترك `fifo_lot_consumptions` تشير إلى حركات لم تعد موجودة — فتختلف الدفعات عن الدفتر **بلا ما يُفسّر السبب**.

### القرار
| المسار | الإجراء |
|---|---|
| `POST /api/fix-negative-payments` | `410` |
| `POST /api/fix-bill-return` | `410` |
| `GET /api/inspect-negative-payments` | **يبقى** — مقيَّد بالشركة ويقرأ فقط |

الصفحتان تشرحان السبب بدل أن تختفيا — من فتحها من إشارة مرجعية يستحق تفسيراً لا `404`.

**ولم أُصلح الملفين:** النسخة الصحيحة تنشئ قيوداً عكسية وتستدعى مسار FIFO وتحتسب الحالة من المدفوعات — **لن تكون هذين الملفين مُرقَّعين**، فأُغلقا بدل تعديلهما.

### الدرس
الحارس الذى يفحص **لماذا** قررتُ شيئاً أنقذ الموقف. حارس يفحص **ماذا** فعلت فقط كان سيمرّر الحذف بسعادة.

### وخطأ ثالث فى نفس الجلسة
أول تشغيل بعد الإصلاح رفض الصفحة لأنها «مازالت تُنادى المسار الموقوف» — **بينما كان يلتقط تعليقى أنا** الذى يشرح سبب الإيقاف.

كتبت الحارس يبحث عن **الاسم** بدل **شكل النداء**. وهذه **المرة الثالثة** فى نفس اليوم (v3.74.726 و v3.74.727 و هذه).

القاعدة صارت مكتوبة فى السكربت نفسه: **الحارس يطابق شكل الكود، لا الأسماء** — وإلا رفض توثيقه الخاص.

### Files
- `app/api/fix-negative-payments/route.ts` · `app/api/fix-bill-return/route.ts`
- `app/admin/fix-negative-payments/page.tsx` · `app/admin/fix-bill-return/page.tsx`

---

## [3.74.732] - 2026-07-19 — إغلاق الدَّين الذى خلّفه الحارس

### الفاتورة المؤجَّلة
فى v3.74.730 اخترت `57014` بدل `42501` **عن وعى**، لأن `WHEN OTHERS` يبتلع الثانى. لكن الاختيار ترك ديناً:

> رمز `57014` معناه الحرفى **«أُلغى الاستعلام»** — والمستخدم يقرؤه **«انتهت المهلة»**.

فالرفض الأمنى كان يصل بلباس عطل تقنى.

**والضرر مزدوج:**

1. المستخدم يُخبَر أن العملية **تأخرت**، بينما هى **رُفضت**
2. ونحن كنّا سنُطارد **مشكلة أداء وهمية** لا وجود لها

### الترجمة
`formatSupabaseError` و`handleSupabaseError` يتعرّفان الآن على `57014` ويردّان:

> **«غير مصرح: هذه العملية تخص شركة أخرى»** — بعنوان **«غير مصرح لك»**

وأزلت إلحاق الرمز بين قوسين: **«(57014)» بجوار رسالة صلاحيات ضجيج يدعو لتشخيص خاطئ.**

### لماذا فى إصدار مستقل
كان يمكن دمجها فى v3.74.731. لكن ذلك كان سيخلط **إغلاق ثغرة** بـ**تحسين رسالة** — ولو ظهر عطل لاحقاً لصعُب فصل السبب.
...
## [3.74.731] - 2026-07-19 — العدّاد صفر: ١١٦ ← ٠

### التسعة عشر — كل واحدة بقرارها
لم أُطبّق قاعدة عامة. فحصت **مواضع النداء الفعلية** لكل مجموعة:

| المجموعة | ما وجدته | القرار |
|---|---|---|
| المقاعد والاشتراكات (٨) | كل النداءات عبر `admin.rpc` أى **service_role** | الحارس القياسى — يمرّ بلا أثر ويُغلق باب المتصفح |
| المطابقة والإشعارات (٤) | خادمية أو نيابة عن عضو | الحارس القياسى |
| **التأسيس** (٢) | تُنادى **قبل أن تُوجد العضوية** | حارس خاص |

**حارس التأسيس:** يمرّ إذا كانت الشركة **بلا أعضاء إطلاقاً** — فهذا تأسيس حقيقى لا انتحال.

ولاحظ: `create_branch_atomic` مسارها الحى **آمن أصلاً** (`apiGuard` + `requireRole`، ومعرّف الشركة من الجلسة لا من الطلب). استخدمت حارس التأسيس رغم ذلك — **الصحة لا يجب أن تتوقف على بقاء مسار واحد حذراً.**

### أربع دوال كانت ستُنسى بصمت
بعد الدفعة توقف العدّاد عند **٤**. السبب:

**ثلاث** منها تُخزَّن **بجسم على سطر واحد** — بلا أى فاصل أسطر. والمُرقِّع كان يبحث عن `\nBEGIN`، فلم يجد، **فتخطّاها بلا خطأ**.

> **لا رسالة، لا فشل، لا شىء.** بقيت مكشوفة بينما كل ما حولها يُؤمَّن.

**والرابعة** `increment_usage_metric` بلغة `sql` لا `plpgsql` — لا قائمة أوامر تُسبَق بحارس. أُعيدت كتابتها `plpgsql` بنفس الجسم حرفياً.

**وهذا بالضبط ما يبرر العدّاد:** لولاه لأعلنت النجاح وأربع دوال مفتوحة. **الترحيل الناجح لا يعنى الترحيل الصحيح.**

### الاختبار النهائى
```
الغريب على sync_all_stock_quantities  → مرفوض
العضو على sync_all_stock_quantities   → مسموح
الغريب على increment_usage_metric     → مرفوض
العضو عبر حارس التأسيس                → مسموح
```

### النتيجة
```
دوال محمية: ٨٨
الفاحص:     CLEAN — لا دوال مكشوفة
١١٦ → ٨٧ → ٨١ → ١٩ → ٤ → ٠
```

### ما يبقى مفتوحاً
ترجمة `57014` إلى رسالة مفهومة فى الواجهة (متابعة v3.74.730).

### Files
- `supabase/migrations/20260719000734_*.sql` · `20260719000735_*.sql`

---

## [3.74.730] - 2026-07-19 — الحارس كان يُبتلَع صامتاً: ٨١ ← ١٩

### الدفعة الثالثة
الحارس نفسه على كل دالة متبقية تحمل `p_company_id`. **٦٩ دالة محمية.**

### خمس عشرة دالة استُثنيت عمداً
الحارس يعمل فقط حين توجد جلسة متصفح. لكن بعض الدوال تُنادى **قبل أن يصير المُنادى عضواً أصلاً**:

| المجموعة | السبب |
|---|---|
| `create_branch_atomic` · `seed_default_role_permissions` | **تأسيس الشركة** — العضوية تُنشأ بهذه النداءات نفسها |
| `run_daily_reconciliation` | مهمة نظام تعبر الشركات بحق |
| الاشتراكات والمقاعد | مسارات فوترة، المُنفِّذ ليس عضواً بالضرورة |
| الإشعارات و`idempotency` | بنية تحتية تُنادى من داخل عمليات أخرى |

> حراستها بالطريقة الكسولة كانت **ستُقايض ثغرة أمنية بتسجيل شركة معطوب**. هذه ليست مقايضة.

### وهنا وجدت ما كان سيُشحَن كأمان زائف
اختبرت `validate_three_way_matching` سلوكياً — **فمرّ الغريب إلى صميم العملية**، والحارس أول سطر فيها بالفعل.

**السبب:** الدالة تغلّف جسمها بـ`EXCEPTION WHEN OTHERS`. الحارس رفع الخطأ، **والمُعالِج التقطه وابتلعه، واستمر التنفيذ كأن شيئاً لم يكن**.

**١٥ من الـ٦٩ تحمل هذا المُعالِج.**

> لوحة التحكم تعدّها محمية. الكود يبدو محمياً. **والثغرة مفتوحة.**

ولاحظ: **لا فحص نصى كان ليكتشف هذا** — الحارس موجود، ومكانه صحيح، ومكتوب تماماً كما يجب. **لم يكشفه إلا نداء فعلى بهوية غريب.**

### العلاج — قاعدة تحققت منها ولم أفترضها
```
ERRCODE 42501  →  WHEN OTHERS يبتلعه
ERRCODE 57014  →  ينفذ من خلاله
```

`WHEN OTHERS` **لا يلتقط `query_canceled` ولا `assert_failure`** عمداً. اختبرتها بنفسى قبل الاعتماد عليها.

**التكلفة صراحةً:** `57014` معناه «أُلغى الاستعلام» لا «غير مصرح» — تجاوز دلالى **أدفعه عن وعى**. حارس يستطيع بعض المُنادين إطفاءه ليس حارساً.

### الاختبار بعد الإصلاح
```
نداء الخادم                    → PASS
عضو حقيقى                      → PASS
غريب (دالة بمُعالِج WHEN OTHERS) → PASS — مرفوض
غريب (دالة بلا مُعالِج)          → PASS — مرفوض
```

### متابعة معروفة
PostgREST يترجم `57014` إلى حالة HTTP توحى بانتهاء المهلة. الرسالة العربية تصل صحيحة، لكن رمز الحالة مضلل — يستحق ترجمة مركزية فى معالج أخطاء الواجهة.
...
### Files
- `supabase/migrations/20260719000732_*.sql` · `20260719000733_*.sql`

---

## [3.74.729] - 2026-07-19 — أول فحص عضوية حقيقى: ٨٧ ← ٨١

### القيد الذى شكّل الحل
هذه الدوال **يُناديها التطبيق بطريقتين**:

| المصدر | الهوية |
|---|---|
| المتصفح مباشرة | `auth.uid()` = المستخدم |
| مسارات الـAPI عندنا | **`auth.uid()` فارغ** |

> فحص ساذج «يجب أن تكون عضواً» كان **سيرفض كل نداء من الخادم ويُسقط النظام**.

فصار الفحص:

**إن وُجدت هوية مستخدم، فيجب أن تكون عضواً فى الشركة. وإن لم تُوجد، فالنداء من الخادم وقد فُحص فى طبقة الـAPI.**

يُغلق باب المتصفح دون أن يمسّ مسار الخادم.

### الست الأخطر — مُؤمَّنة
`perform_annual_closing_atomic` · `distribute_dividends_atomic` · `pay_dividend_atomic` · `process_invoice_payment_atomic` · `record_shareholder_drawing_atomic` · `post_payroll_atomic`

**الحقن بالموضع لا بالتعبير النمطى:** أول `BEGIN` بعد `$function$` هو فاتحة الكتلة الرئيسية (المتداخلة تأتى لاحقاً). تعبير نمطى بـ`.*` كان قد يلتقط كتلة أعمق حسب الجشع — **وحارس داخل فرع داخلى حارس لا يعمل**.

### اختبار سلوكى لا شكلى
لم أكتفِ بأن النص موجود. اختبرت المسارات الثلاثة فعلياً:

```
١ نداء الخادم (بلا هوية)      → PASS — مسموح
٢ عضو حقيقى فى الشركة         → PASS — مسموح
٣ مستخدم مسجَّل من خارج الشركة → PASS — مرفوض
```

**الثالث هو الثغرة نفسها، وقد أُغلقت.**

### خطأ كنت سأرتكبه
الفاحص يتعرّف على الحماية بالبحث عن `company_members` أو `auth.uid()`. والحارس الجديد يُنادى `assert_company_access` — **ولا يحوى أياً من هاتين الكلمتين**.

> فالست دوال المُؤمَّنة **كانت ستبقى محسوبة كمكشوفة**.

وهذا **أسوأ أنواع الفشل**: الرقم يرفض التحرك بينما العمل يُنجَز فعلاً — فيبدو التقدم الحقيقى وكأنه لا تقدم. الفاحص والحارس **يتحدثان فى نفس الإصدار**.
...
### Files
- `supabase/migrations/20260719000730_*.sql` · `20260719000731_*.sql`

---

## [3.74.728] - 2026-07-19 — المرحلة الثانية، الدفعة الأولى: ١١٦ ← ٨٧

### لماذا لم أبدأ بالحل «الصحيح»
العلاج النهائى هو فحص عضوية **داخل** كل دالة. لكن ذلك يعنى **تعديل ١١٦ جسم دالة محاسبية حيّة دفعة واحدة** — وهذا بالضبط ما حذّرتَ منه: *«لا نُصلح شيئاً ونُعطّل آخر»*.

فبحثت عن قطع **أصغر وأكثر أماناً** أُنجزه أولاً.

### الملاحظة التى فتحت الطريق
جزء من هذه الدوال **لا يُناديها كود التطبيق إطلاقاً**. تُستدعى من دوال أخرى ومن triggers داخل قاعدة البيانات.

وهنا التفصيلة الحاسمة: دالة `SECURITY DEFINER` **تعمل بهوية مالكها**، فالنداءات الداخلية **لا تنظر فى صلاحيات المُنادى أصلاً**.

> أى أن سحب `authenticated` منها **لا يُغيّر سلوكها ولا حرفاً** — ويُخرجها من سطح الهجوم تماماً.

### كيف بُنيت القائمة — ثلاث مراحل تصفية
| # | الخطوة | النتيجة |
|---|---|---|
| ١ | الدوال المكشوفة | ١١٦ |
| ٢ | استبعاد أى اسم يظهر **فى أى مكان** فى كود التطبيق | ٨٢ مذكورة · **٣٠ لا** |
| ٣ | فحص المنادين `SECURITY INVOKER` داخل القاعدة | **٢٩** |

**الخطوة ٢ أوسع من اللازم عمداً:** بحثت عن **ذكر الاسم فى أى سطر**، لا عن `rpc()` فقط — حتى لا يفلت نداء مبنى ديناميكياً.

**والخطوة ٣ هى التى تمنع الكسر:** المُنادى `SECURITY INVOKER` **يعتمد على صلاحيات المستخدم**، فسحبها يكسره. ظهرت حالة واحدة: `create_audit_log` تُنادى من `test_audit_trail`.

**أخرجتُها من الدفعة** بدل افتراض أنها اختبار غير مؤثر. حالة واحدة مؤجلة أرخص من عطل واحد.

### النتيجة
```
١١٦ → ٨٧    (العدّاد على لوحة التحكم يعرضه)
```

٢٩ دالة صارت **`service_role` فقط** — لا تُنادى من متصفح إطلاقاً، ولا شىء تغيّر لأى مُنادٍ داخلى.

### الباقى ٨٧ — صريحاً
هذه **يُناديها التطبيق فعلاً**، فلا تُعالَج بسحب صلاحية بل **بفحص عضوية داخل كل دالة**، على دفعات.

نمط الفحص المُعتزَم يحترم أن كثيراً منها يُنادى من الخادم بـ`service_role` (حيث `auth.uid()` فارغ):

> **إن وُجدت هوية مستخدم، فيجب أن تكون عضواً فى الشركة. وإن لم تُوجد، فالنداء من الخادم وقد فُحص فى طبقة الـAPI.**

### Files
- `supabase/migrations/20260719000729_v3_74_728_phase2_batch1_revoke_authenticated.sql`

---

## [3.74.727] - 2026-07-19 — ١١٦ دالة كانت تُنادى بلا تسجيل دخول

### الحالة الواحدة كانت طبقة
إصدار v3.74.726 عالج `fix_historical_cogs`. لكن **الشكل** الذى جعلها خطرة ليس خاصاً بها، فبحثت عن الشكل نفسه:

> `SECURITY DEFINER` (تعمل بصلاحيات كاملة فتتجاوز RLS) + `EXECUTE` لـ **PUBLIC** + `company_id` **من المُنادى** + تكتب + **بلا فحص عضوية**.

**النتيجة: ١١٦ دالة.** منها `perform_annual_closing_atomic` و`distribute_dividends_atomic` و`pay_dividend_atomic` و`process_invoice_payment_atomic`.

### تحقّق منع المبالغة
لم أكتفِ بالنمط. فحصت ثلاثاً منها يدوياً:

```
perform_annual_closing_atomic  → any_auth_reference: false
distribute_dividends_atomic    → any_auth_reference: false
process_invoice_payment_atomic → any_auth_reference: false
```

**لا `auth.*` ولا `current_setting` ولا `company_members` ولا أى مساعد صلاحيات.** الحصانة معدومة فعلاً لا ظاهرياً.
...
**`anon` وليس `authenticated` فقط.** ومفتاح `anon` يُشحَن داخل حزمة المتصفح — أى أنه **متاح لكل من يفتح الموقع**.

إقفال السنة المالية، أو توزيع أرباح، على **أى معرّف شركة**، **بلا حساب أصلاً**.

### المرحلة الأولى — نُفِّذت
سحب `PUBLIC` و`anon`، ومنح `authenticated` و`service_role`. **التطبيق يعمل كما هو تماماً** — لا شىء فى الواجهة يُنادى كزائر.

| الدور | قبل | بعد |
|---|---|---|
| `anon` | ١١٦ | **٠** |
| `authenticated` | ١١٦ | ١١٦ *(المرحلة الثانية)* |
| `service_role` | ١١٦ | ١١٦ |

النطاق **محسوب لا مكتوب يدوياً** — نفس استعلام الفحص، فلا تتسرب دالة من قائمة منسوخة.

### المرحلة الثانية — لم تُنفَّذ، ولا أدّعى غير ذلك
موظف **مسجَّل دخوله** فى شركة أ مازال يستطيع تمرير معرّف شركة ب. علاجه فحص عضوية **داخل كل دالة**، ويُنفَّذ على **دفعات مُراجَعة** لا كنسة واحدة.

### فحص جديد يمنع عودتها صامتة
`ic_exposed_definer_functions` — بفئة جديدة **«أمنية»**. السبب الحقيقى لبقاء `fix_historical_cogs` طويلاً أن **لا أحد كان يراقب**. الفحص يرصد:

1. أى دالة تعود لـ `anon` (يجب أن تبقى صفراً)
2. عدّاد المرحلة الثانية — يهبط مع كل دفعة

### وأثناء ذلك: مصدران للحقيقة
إضافة الفئة كشفت أن **الواجهة والـAPI يكتبان الفئات يدوياً سطراً لكل واحدة**. فئة جديدة كانت **ستُحتسب فى الإجمالى ولا تظهر على الشاشة**.

صار الاثنان يدوران على `CATEGORIES` — نفس علاج صف الإجماليات فى v3.74.716.

### Files
- `supabase/migrations/20260719000727_*.sql` · `20260719000728_*.sql`
- `app/api/governance/system-integrity/route.ts`
- `app/dashboard/_widgets/SystemIntegrityWidget.tsx`

---

## [3.74.726] - 2026-07-19 — أداة إصلاح تُفسد الحسابات وتُبلغ بالنجاح

### كيف وصلنا إليها
سأل المالك: **«الاهم ثم الاهم»**. فبدل الترتيب من الذاكرة، فحصت القائمة — **فوجدتها خاطئة**.

بند كنت سأضعه أولاً — «مرتجع المبيعات لا يتبع FIFO» — **مُصلَح منذ v3.74.702**. راجعت الدالتين فعلياً:

```
auto_create_cogs_journal         → consume_fifo_lots            ✓
auto_reverse_cogs_on_sale_return → restore_fifo_lots_on_return  ✓
```

`cost_price` فيهما شبكة أمان للمخزون القديم فقط. **لو صدّقت قائمتى لأنفقنا يوماً على مشكلة غير موجودة.**

### ما وجدته بدلاً منها — ثلاثة عيوب فى دالة واحدة
صفحة `/settings/fix-cogs` تستدعى `fix_historical_cogs`. الدالة فى قاعدة البيانات:

**١ — كتابة عابرة للشركات.** `SECURITY DEFINER` + `EXECUTE` مُتاح لـ **PUBLIC**، و`p_company_id` يأتى من المُنادى **بلا أى تحقق من العضوية**.

أى مستخدم مُصادَق فى **أى شركة** كان يستطيع تمرير معرّف شركة أخرى مباشرة إلى PostgREST وحقن قيود فى دفترها. **فحص الصلاحية فى الـ API لم يكن يحمى شيئاً** — الدالة مكشوفة من خلفه.

**٢ — تكلفة خاطئة.** تحتسب COGS من `products.cost_price`، وهى بالضبط التكلفة التى هجرناها فى v3.74.702 لأنها تُضخّم الربح.

**٣ — انفصال الدفعات عن الدفتر.** تكتب القيد **بلا** `consume_fifo_lots`، فتبقى دفعات FIFO تُظهر مخزوناً صرفه الدفتر بالفعل.

### وفوق ذلك: نجاح كاذب
الخطوة الأولى فى الـ API تستدعى `exec_sql` — **دالة غير موجودة أصلاً**. تفشل، تُسجَّل فى `errors`، ثم يرد النظام:

> «تم تطبيق التصحيحات المحاسبية بنجاح»

**أداة إصلاح تُفسد ما جاءت لتُصلحه، وتُطمئنك وهى تفعل.**

### القرار: حذف لا تعطيل
`DROP FUNCTION` وليس `REVOKE` — حتى لا يبقى لغم يُعاد منحه لاحقاً. الفجوة التى كُتبت لها **مُغلقة أصلاً** بـ FIFO التلقائى.

الصفحة صارت **فحصاً للقراءة فقط**، والـ`POST` يرد `410 Gone`.

وأزلت من الفحص شرط `cost_price > 0` — كان **يُخفى الحركات الجديرة بالنظر**: منتج بطاقته صفر ودفعاته تحمل تكلفة حقيقية كان يُبلَّغ عنه كسليم.

### ما لم أُغلقه — ولا أدّعى أننى أغلقته
مسح أولى وجد **١١٧ دالة مُرشَّحة** لنفس النمط: `SECURITY DEFINER` + `EXECUTE` لـ PUBLIC + وسيط `company_id` + كتابة + بلا `company_members` أو `auth.uid()`.

**هذا عدد مُرشَّحين لا عدد ثغرات** — كثير منها مساعدات trigger لا تُنادى مباشرة. لكنه يستحق فرزاً مقصوداً، وهو **البند التالى**.

### Files
- `supabase/migrations/20260719000726_v3_74_726_drop_fix_historical_cogs.sql`
- `app/api/fix-cogs-accounting/route.ts` · `app/settings/fix-cogs/page.tsx`

---

## [3.74.725] - 2026-07-19 — العميل المشترك + النقل التلقائى للمالك

### Context
ناقش المالك تصميم ملكية العملاء: كل موظف مسؤول حصرياً عن عملائه، ولا يراهم زميل حتى فى نفس الفرع، ومنع تكرار رقم الهاتف يمنع تسجيل عميل قائم. **تصميم مقصود لا خلل.**

وطرح حالتين لم يعالجهما النظام: **العميل الذى يذهب لفرع آخر**، و**الموظف الذى يُنقل ولا بديل فى فرعه**.

### تحقق غيّر الإجابة — العمولة لا تتبع الملكية
```
recipientUserId = booking.current_responsible_user_id || booking.staff_user_id
```
العمولة تُنسب إلى **مَن نفّذ العمل فى المستند**، لا إلى مالك العميل. **فنقل الملكية لا يحرّك عمولة واحدة** — لا ماضية ولا مستقبلية.

هذا يُسقط أثقل قيد كان يبدو قائماً، ويجعل الملكية شأن **مسؤولية ورؤية** لا شأن مال.

### (١) عيب أدخلته أنا فى v3.74.722
الحارس `validate_customer_branch_isolation` **يتساهل عمداً** مع العميل بلا فرع ويقبله فى مستند أى فرع — أى أن «العميل المشترك» **موجود فى النظام منذ أمس**.

لكن الفلتر الذى كتبته يستخدم `branch_id = فرعى` **بالضبط**، فالعميل بلا فرع **لا يطابق ويختفى من الجميع**.

**قاعدة البيانات تسمح بما تُخفيه الواجهة** — فالآلية الوحيدة التى يملكها النظام لحالة «عميل يتعامل مع أكثر من فرع» كانت **غير قابلة للاستخدام**. صار الفلتر: `فرعى أو بلا فرع`.

### (٢) كشف «عميل مشترك» فى النموذج
الحالة الثالثة كانت تُبلَغ فقط بترك الحقل فارغاً — **لا أحد يعرف ذلك، والنموذج يعتبره خطأ تحقق**. صارت خياراً صريحاً: **«عميل مشترك — كل الفروع»**.

### (٣) نقل تلقائى للمالك عند غياب البديل
عند نقل موظف، إن كان فرعه السابق **بلا أى موظف آخر**، تنتقل ملكية عملائه **للمالك فوراً** — فهو يرى كل الفروع.

**لماذا تلقائى:** لا يوجد أحد لتسليمهم إليه، وتركهم أيتاماً يعنى **ألا يراهم أحد ولا يخدمهم أحد**. الإيداع لدى المالك يُبقيهم متاحين حتى يُعيَّن موظف. وحين يوجد بديل، تظهر رسالة النقل كما هى — فالقرار يبقى للمالك حيث يوجد خيار.

### (٤) المالك يظهر فى قائمة «صاحب البيانات» — لاحظه المالك فوراً
النقل التلقائى يودع العملاء لدى المالك، لكن قائمة الموظف المصدر كانت **تُخفى المستخدم الحالى** — وهو المالك نفسه غالباً.

فكان العملاء **يُودَعون حيث لا تصل إليهم أى قائمة**: نقلناهم إلى مكان آمن ثم أغلقنا الباب. صار المستخدم الحالى قابلاً للاختيار كمصدر فى النقل، بشارة «أنت».

**تحقق:** مبدأ العينين فى `/api/permissions/transfer/[id]/approve` يمنع اعتماد طلبك بنفسك، **إلا إذا كنت المسؤول الأوحد** (استثناء v3.74.67) — فلا يوجد قفل.

### ما لم يُنفَّذ بعد
**طلب ضمّ عميل** عبر تكرار رقم الهاتف — يُبنى بعد استقرار هذا الأساس.

### Files
- `lib/customer-scope.ts` · `app/estimates/page.tsx`
- `components/customers/customer-form-dialog.tsx`
- `app/settings/users/page.tsx`

---

## [3.74.724] - 2026-07-19 — سبعة إنذارات بنص واحد لا تُميّز بينها

### Context
ظهرت الإنذارات السبعة التى رصدها فاحص عزل العملاء — **وهذا صحيح ومتوقّع**. لكن اللقطة كشفت خللاً آخر: **سبعة صفوف بنص متطابق**، بلا اسم عميل ولا رقم مستند. المالك يرى أن هناك مشكلة، **ولا يعرف أين ولا يستطيع التصرف**.

### السبب — «مصدرا حقيقة» مرة أخرى
الودجت يعرض `hint` — وهو **جملة واحدة ثابتة لكل فاحص لا لكل نتيجة** — ثم قائمة حقول **مكتوبة يدوياً**: `difference` و`invoice_number` و`product_name` فقط.

فأى فاحص يُخرج مفاتيح أخرى **لا يعرض شيئاً يُميّز نتائجه**. وفاحصنا يُخرج `customer` و`document_no` و`customer_branch` — لا شىء منها فى القائمة.

**ونفس الشكل الذى عالجناه مرتين اليوم:** إضافة فاحص كانت تتطلب تعديل الودجت ضمناً، **ولا شىء يفرض ذلك أو يُنبّه إليه**.

### Change
**قاعدة عامة `subject`:** سطر واحد يُسمّى السجل، يعرضه الودجت **بلا قائمة حقول**. الفواحص القائمة تعمل كما هى، والفاحص القادم يصير قابلاً للتصرف **دون لمس لوحة التحكم**.

**ما كان يُعرض:**
> Customer belongs to one branch while the staff member... *(× ٧ متطابقة)*

**وما صار يُعرض:**
> **العميل «مجمد زيتون» — فرعه: مدينة نصر · منشئه انتقل إلى: الفرع الرئيسي**
> **فاتورة INV-2026-00002 (فرع: الفرع الرئيسي) — العميل «ahmed abuelmagd» من فرع مدينة نصر**
...
## [3.74.723] - 2026-07-19 — إصلاح v3.74.718 جعل النافذة أسوأ (انحدار)

### ما حدث — خطأ منّى
فى v3.74.718 عالجت تكدّس بنود عرض السعر بتوسيع النافذة وفرض عرض أدنى للجدول. **النتيجة كانت أسوأ:** النافذة كلها صارت تنزلق أفقياً — حقول العميل والتاريخ والملاحظات معها — والحقول مبتورة.

### السببان — وكلاهما خطئى
**١. `max-w-5xl` لم يُطبَّق أصلاً.** `DialogContent` يحمل `sm:max-w-lg` فى إعداداته الافتراضية. وما أضفته كان **بلا بادئة `sm:`** — فهما متغيّران مختلفان، يُبقيهما `tailwind-merge` معاً، وتفوز قاعدة `sm:` فوق 640px. **فبقيت النافذة 512px** لا 1024.

**٢. ثم فرضتُ جدولاً عرضه 880px داخل 512px.** و`DialogContent` عنصر `grid`، وابن الـgrid قيمته الافتراضية `min-width:auto` — **يرفض الانكماش دون محتواه**. فبدل أن ينزلق الجدول داخل صندوقه، **دفع النافذة كلها**.

### Change
- `sm:max-w-5xl` ليتغلّب على الافتراضى فعلاً، و`overflow-x-hidden` على النافذة كخط دفاع أخير
- **`min-w-0`** على صندوق التمرير وعلى الحاوية الأب — وهذا هو المفتاح الحقيقى: بدونه لا ينحصر الانزلاق مهما فعلنا
- خفض العرض الأدنى من 880 إلى **820px** ليتّسع بأريحية داخل 1024 ناقص الحشو

### الدرس
**لم أتحقق من الإعدادات الافتراضية للمكوّن قبل تجاوزها.** افترضت أن `max-w-5xl` سيُطبَّق لمجرد أننى كتبته — ولم أفتح `components/ui/dialog.tsx` لأرى ما يحمله أصلاً. الافتراض نفسه الذى تكرر اليوم: التوقف عند أول إجابة مقنعة.
...
## [3.74.722] - 2026-07-19 — قوائم اختيار العميل كانت ما تزال تعرض فروعاً أخرى

### ما فات فى v3.74.719 — خطأ فى نطاق الإصلاح
أصلحتُ **صفحة قائمة العملاء** وحدها. أما **قوائم الاختيار** فى الصفحات الأخرى فلكلٍّ منها استعلامها الخاص، وظلت جميعها على السلوك القديم. لاحظه المالك فوراً: «فى **جميع** مواضع اختيار العميل ما زالت تظهر له جميع العملاء الذين أنشأهم».

**ستة مواضع:** الحجوزات · عروض الأسعار · الفواتير (جديدة وقائمة) · أوامر البيع · إشعارات المدين.

### الجذر الحقيقى — أعمق من قوائم الاختيار
`getAccessFilter` كانت تُرجع للموظف:
```
filterByCreatedBy: true, createdByUserId: userId,
filterByBranch: false, branchId: null      ← الفرع غير متاح أصلاً
```
**فلم يكن لدى أى قائمة اختيار فرعٌ تُضيّق به** حتى لو أرادت. لذلك صار `branchId` يُرجَع للموظف أيضاً، مع إبقاء `filterByBranch: false` — فهو ليس دوراً على مستوى الفرع ولا يصحّ أن يرى الفرع كله فجأة.

**تحقق قبل التغيير:** فُحصت **كل** قراءات `accessFilter.branchId` فى المشروع. جميعها محروسة بـ`filterByBranch &&` عدا واحدة فى صفحة المدفوعات — وتبيّن أنها **بداخل** `else if (accessFilter.filterByBranch)` أصلاً. فلا سلوك قائم يتغيّر.

### دالة مشتركة لا تكرار سداسى
`lib/customer-scope.ts` تعرّف القاعدة **مرة واحدة**، وتُطبَّق فى المواضع الستة. فالصفحة الجديدة التالية ترثها بدل أن تُعيد الخطأ.

**والقاعدة تقاطع:** المنشئ وحده يتبع الشخص عبر الفروع (الخلل)، والفرع وحده يمنح كل مندوب دفتر الفرع كاملاً فيُلغى خصوصيته.

**وشمل الإصلاح العملاء المشتركين عبر «فتح الصلاحيات»** — كانوا يتسرّبون بفروع المانح الأخرى.

### لماذا كان الأثر مزعجاً لا خطراً
حارس v3.74.719 يرفض المستند فى قاعدة البيانات، **فلا شىء خاطئ يُحفَظ**. لكن المستخدم كان يختار عميلاً مسموحاً له برؤيته، ويملأ النموذج كاملاً، **ثم يُرفض عند الحفظ**.

### Files
- `lib/customer-scope.ts` (جديد) · `lib/validation.ts`
- `app/bookings/new/page.tsx` · `app/estimates/page.tsx` · `app/invoices/new/page.tsx` · `app/invoices/page.tsx` · `app/sales-orders/page.tsx` · `app/customer-debit-notes/new/page.tsx`

---

## [3.74.721] - 2026-07-19 — فتح نموذج النقل مباشرة مع الرسالة بداخله

### Context
طلب المالك أن **يُفتح نموذج نقل الصلاحيات فور تغيير فرع الموظف — مع ظهور الرسالة**.

### Change
عند تغيير الفرع بما يترك عملاء بلا راعٍ، يُفتح **نموذج النقل مباشرة** مملوءاً مسبقاً: الموظف المصدر، ونوع المورد «العملاء»، **والفرع المتروك محدَّداً**. والشرح ينتقل من نافذة منفصلة إلى **شريط تنبيه داخل النموذج نفسه**.

### لماذا داخل النموذج لا نافذة قبله
فتح النموذج بلا سياق يُلقى بالمالك فى شاشة لا يعرف سبب ظهورها. ونافذة منفصلة قبله تعنى نقرة زائدة فى المسار الشائع.

**والوضع داخل النموذج يحلّ الاثنين:** الشرح حاضر، **والتحذير ملاصق للحقلين اللذين يحذّر منهما** — تبويب «نقل ملكية» لا «فتح صلاحيات»، وحقل الفرع الذى يجب ألا يُترك على «الكل».

الشريط يذكر: العدد، والفرع، وأن كل شىء مملوء مسبقاً ولا ينقص إلا اختيار الزميل المستلم، وأن **التأجيل آمن** لأن فاحص السلامة يظل يعرضهم.

### تنظيف
حُذفت النافذة المنفصلة بالكامل — **لا كود ميت ولا شرط `false`**. وتُمسح حالة السياق عند إغلاق النموذج، فلا تتسرّب إلى عملية نقل أخرى تُفتح لاحقاً لسبب مختلف.
...
## [3.74.720] - 2026-07-19 — تنبيه إرشادى عند نقل موظف بين الفروع

### Context
اقترح المالك: عند تغيير فرع الموظف، **تظهر رسالة توضّح خطوات نقل عملاء الفرع السابق**.

**الفكرة صائبة لسبب جوهرى:** لحظة تغيير الفرع هى **اللحظة التى تُخلق فيها المشكلة**. وقد ظهرت هذه المشكلة أصلاً بعد أسابيع من وقوعها، صدفةً، حين لاحظ المالك موظفاً يحمل عملاء فرع آخر. **الإمساك بها عند نشوئها يُلغى المسافة الزمنية كلها.**

### ما يحدث فعلاً عند النقل
الموظف يرى العملاء الذين أنشأهم **فى فرعه** (منذ v3.74.719). فنقله يترك كل عميل أنشأه فى الفرع السابق **بلا راعٍ**:
- هو **لم يعد يراهم**
- **ولا يراهم أحد فى ذلك الفرع** — لأن لا أحد منهم أنشأهم
- ومستنداتهم **مرفوضة من فرعه الجديد** بحكم الحارس

### Change
بعد حفظ تغيير الفرع، يُحسب عدد العملاء الذين أنشأهم الموظف **فى الفرع الذى تركه**. فإن وُجدوا، تظهر نافذة تشرح:
- **ماذا حدث** بالأرقام: كم عميلاً، وفى أى فرع
- **خطوات النقل الخمس** بالترتيب — مع التنبيه الحاسم: تبويب **«نقل ملكية»** لا **«فتح صلاحيات»**، و**اختيار الفرع** لأن تركه على «الكل» ينقل عملاء فروعه الأخرى أيضاً
- **زر «انقلهم الآن»** يفتح نافذة النقل **مملوءة مسبقاً** بالموظف ونوع المورد والفرع الصحيح، فلا يُعيد المالك اشتقاق الحالة يدوياً

### قرارات تصميم
**لا يمنع النقل ولا يؤخّره.** نقل الموظف عمل مشروع، والرسالة **تُخبر ولا تعترض** — تظهر بعد نجاح الحفظ.

**لا تظهر بلا سبب:** لا تظهر إن لم يكن للموظف عملاء فى الفرع السابق، ولا عند تعيين فرع لموظف بلا فرع سابق.

**وتؤكد أن التأجيل آمن:** فاحص السلامة يظل يعرض هؤلاء العملاء حتى تُنقل ملكيتهم — فالرسالة تذكير لا إنذار أخير.
...
## [3.74.719] - 2026-07-19 — عزل العملاء بين الفروع: موظف انتقل واحتفظ بعملاء فرعه السابق

### Context
اكتشفه المالك: موظف انتقل من **فرع مدينة نصر** إلى **الفرع الرئيسى**، وظل يرى — **ويستطيع اختيار** — العملاء الذين أنشأهم فى الفرع الذى تركه. قائمته تعرض ٤ عملاء، **٣ منهم يتبعون فرعاً لم يعد يعمل به**.

### السبب الجذرى
الموظف يرى العملاء بفلتر **«من أنشأهم»** لا بفلتر الفرع. و«من أنشأتهم» محور **يتبع الشخص لا البيانات** — فينتقل معه عبر الفروع.

**والفجوة لم تكن نظرية:** يوجد بالفعل **٤ مستندات** عميلها من فرع غير فرع المستند، منها حجز أُنشئ أثناء اختبارات هذا الأسبوع.

### (١) الحارس — الأهم
حارس عزل الفروع لل**منتجات** موجود منذ v3.74.701 على كل جداول البنود (وهو الذى منع إضافة منتج فرع آخر للحجز). **ولا نظير له للعملاء إطلاقاً** — وهذا يفسّر المستندات الأربعة.

`validate_customer_branch_isolation` على: الفواتير · أوامر البيع · العروض · الحجوزات · المرتجعات · إشعارات المدين.

**متساهل عمداً فى حالتين**، مطابقةً لحارس المنتجات: مستند بلا فرع (مستوى شركة)، وعميل بلا فرع (عميل عام يُستخدم من أى فرع). **المخالفة الحقيقية وحدها تُرفض.**

**اختبار على بيانات حقيقية (تراجَع):** أمر بيع فى الفرع الرئيسى بعميل من مدينة نصر ← **مرفوض ✓** · نفس العميل فى فرعه ← **مقبول ✓**

### (٢) العرض: التقاطع لا أحد المحورين
`get_customers_overview` تدعم فلترى الفرع والمنشئ معاً — لكن المُستدعِى كان يمرّر **فلتر المنشئ وحده** للموظف.

صار يمرّر **الاثنين**: يرى ما أنشأه **وفى فرعه الحالى**.

**ولماذا التقاطع لا الفرع وحده؟** لأن الفرع وحده يمنح كل مندوب دفتر الفرع كاملاً، فيُلغى خصوصية المندوب التى بُنى عليها التصميم أصلاً.

**وأثر جانبى مفيد:** «فتح الصلاحيات» كان يشارك عملاء المانح **بلا حدّ فرع**؛ وبما أن فلتر الفرع صار يُمرَّر دائماً للموظف، فالمشاركة صارت محدودة بالفرع تلقائياً.

### (٣) نقل الملكية بالفرع — موجود بالفعل
تبيّن أن الخاصية **مكتملة**: `transferBranchId` فى الواجهة، وفلترة العملاء عند العرض، وإرساله للـAPI، و`AND (v_branch IS NULL OR branch_id = v_branch)` فى `execute_permission_transfer`.

**لكنها تحت تبويب «نقل ملكية»** — والمالك كان فى تبويب **«فتح صلاحيات»** حيث لا يظهر محدِّد الفرع (وهو صحيح: المشاركة ليست نقلاً). **لا حاجة لبناء شىء.**

### (٤) فاحص جديد — «عَزل العُملاء بَين الفُروع»
يرصد: **عميلاً يتيماً** (منشئه لم يعد فى فرعه، فلا أحد فى فرع العميل يراه) و**مستنداً** عميله من فرع آخر.

رصد فعلياً: **٣ عملاء أيتام و٤ مستندات مخالفة**.

### القائم لم يُصحَّح عمداً
المستندات الأربعة لها **قيود محاسبية مُرحَّلة**، والصواب يختلف حالةً بحالة (نقل العميل لفرعه الجديد، أم إبقاء التاريخ كما هو). **الفاحص يعرضها والمالك يقرر** — بقرار المالك.
...
## [3.74.718] - 2026-07-19 — تكدّس بنود عرض السعر

### Context
لاحظ المالك تكدّس بنود العرض السعرى: العناوين متداخلة حتى بدت «خصم %» و«ضريبة %» كلمة واحدة، وحقول الأرقام مبتورة فى منتصف الرقم.

### الأسباب — أربعة مجتمعة
1. **النافذة ضيّقة:** `max-w-3xl` لثمانية أعمدة من حقول الإدخال.
2. **بلا عرض للأعمدة:** لا `colgroup` ولا أى تحديد، فالمتصفح يوزّعها كما اتفق.
3. **بلا تباعد بين الخلايا:** لا `padding` على `th` ولا `td` — ولهذا التصقت العناوين ببعضها.
4. **`w-full` يسحق الجدول:** الغلاف `overflow-auto` موجود، لكن `w-full` يجبر الجدول على الانكماش داخل النافذة بدل التمرير.

### Change
- توسيع النافذة إلى `max-w-5xl`
- `colgroup` بنسب مدروسة: المنتج والوصف أعرض، والأرقام أضيق
- `min-w-[880px]` + `overflow-x-auto` — **فيُمرَّر الجدول أفقيًا على الشاشات الضيّقة بدل أن ينسحق**
- تباعد للخلايا و`whitespace-nowrap` للعناوين
- **`text-start` بدل `text-left`** — الصفحة RTL وكان المحاذاة معكوسة
- عمود الإجمالى `text-end` مع `tabular-nums` ليصطفّ رقمٌ تحت رقم
- زر الحذف صار **أيقونة** بدل كلمة «حذف» التى كانت تستهلك عمودًا كاملًا فى كل صف لإجراء ثانوى — مع `aria-label` و`title` للوضوح وإمكانية الوصول

### Risk
**عرض فقط.** لا تغيير فى منطق ولا حساب ولا بيانات.
...
## [3.74.717] - 2026-07-19 — إشعاران لاعتماد دفعة واحدة + نصفهما لا يُغلق أبدًا

### Context
لاحظ المالك وصول إشعارين لاعتماد **نفس الدفعة**. وفى v3.74.691 افترضتُ التكرار وكنت مخطئًا — فالإشعاران وقتها كانا يفتحان صفحتين لاعتمادين مختلفين. **فتُحقّق من الوجهة أولًا هذه المرة.**

### التحقق
كلاهما لنفس المرجع (`cbb31d48…`) ونفس التصنيف `approvals` ونفس المستلم:

| | «طلب اعتماد دفعة» | «طلب اعتماد دفعة مورد» |
|---|---|---|
| المصدر | خدمة التطبيق | trigger فى قاعدة البيانات |
| النوع | `payment_approval` | `approval_request` |
| الوجهة | `/payments` | `/approvals` |
| الاستهداف | بالدور (مالك + مدير) | بالمستخدم |
| مفتاح الحدث | موجود | **NULL** |

**الوجهتان مختلفتان فعلًا** — لكن بخلاف حالة أمر الشراء، الصفحتان تعتمدان **نفس الدفعة الواحدة**. فهذا ازدواج حقيقى: حدث واحد، مرجع واحد، إجراء واحد.

### الأخطر من الازدواج
منطق الأرشفة بعد القرار يُرشِّح على `reference_type='payment_approval'` **ومفتاح حدث معروف**. ونسخة الـtrigger لا تحمل أيًّا منهما — **فلا تُغلَق أبدًا**.

**النتيجة:** بعد أن يعتمد المالك، يبقى الإشعار الثانى «غير مقروء» بزرَّى **اعتماد/رفض حيَّين** يشيران إلى دفعة **معتمدة بالفعل**. وقد تراكم **ستة** من هذه فى النظام.

### Change
الـtrigger **لم يُحذف** — فهو المُشعِر الوحيد لو دخلت دفعة مورد حالة الانتظار خارج مسار الخدمة. وبما أنه يعمل عند الإدراج **قبل** الخدمة، فنسخته موجودة حين تعمل الخدمة، فتُنهيها الخدمة وتُبقى **إشعارًا حيًّا واحدًا لكل اعتماد**.

وأُضيف ذلك داخل `archiveApprovalNotifications` المُستدعاة عند **الطلب والقرار** معًا — فتُغطّى الحالتان بتعديل واحد.

### Data
إغلاق **٦ إشعارات** مكررة متراكمة، مقصورًا على ما له نظير `payment_approval` فعلى.
...
## [3.74.716] - 2026-07-19 — صف الإجمالى فى المخزون كان ينقصه عمودان (انحدار من 3.74.714)

### ما حدث — خطأ منّى
فى v3.74.714 أضفت عمودَى **استهلاك الخدمات** و**فى عهدة الفنّى** إلى رأس الجدول وصفوفه — **ولم أضفهما إلى صف الإجمالى**.

وصف الإجمالى مبنىٌّ يدويًا خليةً بخلية (`<td>` لكل عمود)، لا مُولَّد من تعريف الأعمدة. فصار **أقصر بخليتين من الرأس**، وانزاح كل رقم بعد «الهالك» ليقع **تحت عنوان غير عنوانه**.

**النتيجة التى رآها المالك:** الصفوف تتّزن (2 − 1 = 1 ✅) بينما صف الإجمالى يعرض أرقامًا فى غير مواضعها.

### Change
إضافة خليتَى الإجمالى الناقصتين فى موضعهما الصحيح بين «الهالك» و«النقل الواردة».

### Risk
**عرض فقط.** لم يكن أى رقم خاطئًا — كانت الأرقام الصحيحة تُعرض تحت عناوين خاطئة.

### درس
الجدول له **مصدرا حقيقة**: تعريف الأعمدة للرأس والصفوف، وصف يدوى للإجمالى. إضافة عمود تتطلب تعديل الاثنين معًا، ولا شىء يفرض ذلك. الحارس الجديد يفرضه: يرفض الرفع إن ذُكر `serviceUseTotals` أو `custodyTotals` فى الأعمدة دون صف الإجمالى.

### تحقق جانبى — السلسلة على فاتورة حقيقية
`BILL-0003` أول فاتورة **شاملة الضريبة** فى النظام (خصم سطر 10% + خصم فاتورة 10% + شحن):

| | |
|---|---|
| دفعتا FIFO | 16.11 + 0.81 = **16.92** |
| قيد المخزون 1140 | **16.92** |
| ضريبة المدخلات 1160 | 2.37 |
| الموردون 2110 | −19.29 |

**الدفعات = القيد بالقرش.** إصلاح v3.74.715 يعمل على أول استخدام حقيقى له.
...
## [3.74.715] - 2026-07-19 — توزيع التكلفة كان يختلّ مع «شامل الضريبة» ونسب ضريبة مختلفة

### Context
سأل المالك عن **خصم مستوى المنتج** فى أمر الشراء، وظنّ أنه غاب عن العمل المحاسبى. **لم يغب** — تتبّعنا السلسلة: نموذج أمر الشراء يحفظه، و`buildBillItemRow` ينقله للفاتورة، ومحرّك التكلفة يقرأه فى موضعين. والدليل من بياناته: `BILL-0001` تحمل خصمَى سطر **10% و5%** وأعاد المحرّك إنتاج قيدها **بالقرش**.

**لكن أرقام الأمر لم تتّزن إلا بتفسير واحد: «شامل الضريبة» مُفعَّل** — و`21.00 ÷ 1.14 × 0.90 = 16.58` بالضبط. وأكّد المالك أن ذلك مقصود. **وهذا التأكيد كشف الخلل الحقيقى.**

### الخلل
`bills.subtotal` يُخزَّن **دائمًا بلا ضريبة**. وأوزان التوزيع كانت تُؤخذ من `unit_price` مباشرة — وهو **يتضمّن الضريبة** فى الفواتير الشاملة.

**بنسبة ضريبة واحدة لا أثر لذلك** — كل الأوزان تتضخّم بنفس المعامل فتُختصر النسبة. **أما بنسب مختلفة** فسطر بـ14% يُوزن **1.14 ضعف** سطر بـ0%، **فيبتلع تكلفة تخصّ المنتج الآخر**.

| المنتج | قبل الإصلاح | الصحيح | الفرق |
|---|---|---|---|
| أ (ضريبة 14%) | 106.54 | 100.00 | **+6.54** |
| ب (ضريبة 0%) | 93.46 | 100.00 | **−6.54** |

**وإجمالى الفاتورة يبقى صحيحًا تمامًا** — ولهذا لم يمسكه أى فاحص. الخطأ فى **التوزيع بين المنتجين** وحده: تكلفة منتج مضخّمة وأخرى منقوصة ⟵ **ربح خاطئ لكل منتج**، والإجمالى سليم.

### Change
يُستخرَج الوزن **بلا ضريبة** حين تكون الفاتورة شاملة: `الصافى ÷ (1 + نسبة الضريبة)`. وبذلك يتطابق أساس التوزيع مع `subtotal` المخزَّن مهما اختلفت النسب.

### Verified
اختبار على فاتورتين (تراجَع بالكامل): سطران صافيهما الحقيقى 100 لكل منهما —
- **شامل الضريبة:** 100.00 / 100.00 ✅
- **قبل الضريبة:** 114.00 / 100.00 ✅ (بلا تغيير — الفواتير القائمة كلها من هذا النوع)
...
## [3.74.714] - 2026-07-19 — جدول المخزون لم يكن يتّزن مع نفسه

### Context
لاحظ المالك على حجز `BKG-2026-00006` (فرع رئيسى — تست) أن صف المنتج **لا يُفسّر نفسه**:

> مشتريات **1** · مبيعات **0** · مرتجعات **0** · هالك **0** · نقل **0** ← فالحساب يقول **1**، والمعروض **0**

### السبب
تصنيف الحركات فى صفحة المخزون سلسلة `if / else if` تغطى: `purchase` · `sale` · `write_off` · `sale_return` · `purchase_return` وما يتصل بها.

**والحركات التى أدخلها نموذج الحجوزات والعهدة — `service_consumption` و`booking_custody_out` و`booking_custody_return` — تسقط من كل الفروع.**

فتُحتسب فى **إجمالى المخزون** (لأنه يجمع كل الحركات بلا تصنيف) ولا تظهر فى **أى عمود**. النتيجة: الرصيد صحيح، لكن التفصيل **لا يقود إليه** — والمستخدم لا يملك طريقة لمعرفة أين ذهبت الوحدة.

### Change
عمودان جديدان يُغلقان المعادلة:

- **استهلاك الخدمات** — مواد استُهلكت فى تنفيذ خدمة. ذهبت بلا رجعة كالمبيعات، لكنها **ليست بيعًا** ولا إيراد لها، فلا يصحّ دمجها فى عمود المبيعات.
- **فى عهدة الفنّى** — محسوب **بالإشارة** لا بالقيمة المطلقة: الخروج سالب والإرجاع موجب، فالناتج هو **ما لا يزال فعلًا فى يد الفنّى**. يعود إلى صفر تلقائيًا عند التنفيذ أو الإرجاع. وفُصل عن الاستهلاك لأن هذه البضاعة **ما زالت مملوكة** — خرجت من الرف ولم تُستهلك.

**تحقق على بيانات الحجز:** الزيت — مشتريات 1، استهلاك خدمات 1، عهدة 0 (خرجت 1 ورجعت 1) ← المتاح **0** ✅ والصف صار يُفسّر نفسه.

### Risk
**لا شىء.** عرض فقط: لا تغيير فى حركة مخزون ولا قيد ولا رصيد. الأرقام كانت صحيحة، وما تغيّر أنها صارت **مفهومة**.
...
## [3.74.713] - 2026-07-19 — حذف صفحات الإصلاح المسمّاة باسم عميل

### Context
بعد حذف نقاط النهاية الخفية فى v3.74.712، بقيت **صفحات ظاهرة** لها عناوين يفتحها أى أحد فى المتصفح — وأخطرها تحمل **اسم عميل حقيقى** فى العنوان.

### المحذوف
| الصفحة | المشكلة |
|---|---|
| `/fix-inv0001-foodcana` | **اسم عميل حقيقى** فى عنوان صفحة بجذر التطبيق |
| `/fix-invoice-inv0001` | فاتورة بعينها |
| `/fix-invoice-return` | تستدعى نفس مسار الإصلاح للفاتورة السابقة |

ومعها المساران اللذان صارا بلا مستدعى: `api/fix-inv0001-foodcana` و`api/fix-invoice-return-sent`، والسكربت `scripts/fix-invoice-inv0001.ts`.

**لماذا الآن:** أول مبرمج عند عميلك يفتح أدوات المتصفح سيسأل «من فودكانا؟ ولماذا اسمها فى نظامى؟». وهى فوق ذلك تحمل قواعد ما قبل نظام العهدة وFIFO والتكلفة الفعلية — فاستدعاؤها اليوم يُصلح بمنطق العام الماضى.

### التحقق قبل الحذف
- **لا رابط تنقّل يشير إليها** — لا فى شريط جانبى ولا قائمة؛ الوصول بكتابة العنوان فقط
- **`middleware.ts` لا يذكرها**
- المسـاران المحذوفان **لا مستدعى لهما بعد حذف الصفحات الثلاث** — تُحقّق آليًا فى سكربت الرفع
- **`ci.yml` لا يذكر أيًّا منها** فالفحوص تبقى سليمة

### ما لم يُحذف عمدًا
`/settings/fix-cogs` و`/admin/fix-negative-payments` و`/admin/fix-bill-return` — **أدوات إدارية عامة** لا تخص عميلًا بعينه، ولها موضع منطقى داخل الإعدادات ولوحة الإدارة. تحتاج مراجعة منفصلة لقواعدها المحاسبية قبل أى قرار بشأنها.

### ملاحظة جانبية
`tooltips.json` ملف مُولَّد بمحتوى عشوائى — مفاتيح مثل `catch` و`supabasekey` بقيم مقتطعة من أسطر كود. ليس مستدعيًا لشىء ولا يتأثر بهذا الحذف، لكنه يستحق مراجعة مستقلة.

### Files
- حذف `app/fix-inv0001-foodcana/` · `app/fix-invoice-inv0001/` · `app/fix-invoice-return/`
- حذف `app/api/fix-inv0001-foodcana/` · `app/api/fix-invoice-return-sent/`
- حذف `scripts/fix-invoice-inv0001.ts`

---

## [3.74.712] - 2026-07-19 — حذف ٨ نقاط إصلاح تاريخية لا يستدعيها شىء

### Context
المالك طلب الحذف **بشرط التأكد التام أن الحذف لا يضرّ بعمل المشروع**. أُجرى الفحص على أربعة محاور قبل لمس أى ملف.

### الأدلة
**١. لا مستدعى فى الكود.** البحث عن الأسماء الثمانية فى المشروع كله لم يُرجع سوى: ملفات المسارات نفسها، وسجل التغييرات، وملفات التوثيق فى `knowledge/` (توثيق **مُولَّد** لا استدعاء). **لا صفحة ولا مكوّن ولا مكتبة ولا سكربت.**

**٢. لا عناوين مبنية ديناميكيًا.** بحثٌ عن أنماط مثل `` `/api/${...}` `` و`"/api/" +` عبر `app` و`lib` و`components` — **لا نتائج**. فلا يمكن أن يكون هناك استدعاء مخفى خلف متغيّر.

**٣. `middleware.ts` لا يذكرها** — ليست فى أى قائمة توجيه أو حماية.

**٤. كل مجلد يحوى `route.ts` وحده** — لا صفحة ولا دالة مشتركة يعتمد عليها غيرها.

**٥. `ci.yml` يفحص ثلاثة مسارات بالاسم** — `fix-inventory` و`fix-sent-invoice-journals` و`repair-invoice` — **وليس أىٌّ منها ضمن المحذوفات**، فالفحوص تبقى سليمة.

### المحذوف
`fix-invoice-0001-status` · `fix-invoice-0028` · `fix-invoice-display` · `fix-missing-payment-journals` · `fix-negative-quantities` · `fix-orphan-invoices` · `fix-nasr-stock` · `repair-shipping-journals`

كلها أدوات لمرة واحدة لبيانات تاريخية بعينها، **والمشكلات التى بُنيت لها انتهت**. وبقاؤها خطر: تحمل قواعد ما قبل نظام العهدة وFIFO والتكلفة الفعلية، فلو استُدعيت اليوم لأصلحت بقواعد العام الماضى. و`fix-nasr-stock` كان مُعطَّلاً بالفعل خلف 410.

### ملاحظة على التسلسل
الإصدار 711 أضاف حارس دور إلى ثلاثة من هذه الثمانية. هذا الإصدار يحذفها. **والتسلسل مقصود لا مُهدَر:** أمّنّاها أولًا لأن التأمين آمن فورًا، ثم تحقّقنا، ثم حذفنا. لو بدأنا بالحذف قبل التحقق لكان قرارًا على الظن.

### الأثر على الأرقام والعمليات
**صفر.** لا شىء فى النظام يستدعيها، ولا تعمل تلقائيًا، ولا تُشغَّل بجدولة.

### Verified
سكربت الرفع **يُعيد التحقق على جهازك لحظة الرفع**: يفشل إن وجد أى ملف كود يذكر أيًّا من الثمانية، وإن كان أى منها مذكورًا فى `ci.yml`.

### Files
- حذف `app/api/{الثمانية}/route.ts`

---

## [3.74.711] - 2026-07-19 — تدقيق حلّ الحسابات عبر المشروع كله

مسح منهجى لكل موضع يبحث فيه الكود عن حساب، ومقارنته بما يشحنه القالب فعليًا — النمط الوحيد الذى **يُرحّل أموالًا لحساب خاطئ بصمت**.

### النتيجة
**29 نوعًا** يبحث عنها الكود · **19 يشحنها القالب بشكل صحيح** ✅ · **10 غير مطابقة**، منها:

| النوع | الحكم |
|---|---|
| `cost_of_goods_sold` · `raw_materials` · `finished_goods` · `revenue` | **سليم** — مقترن ببديل يعمل فى نفس السلسلة |
| `income_summary` | **سليم** — مقترن بالرمز `3300` الموجود فعلًا |
| `wages_payable` | عولج فى v3.74.710 |
| `vat_payable` · `vat_receivable` · `shipping_income` | فى مسارات إصلاح لمرة واحدة فقط — محروسة بالدور، خطر منخفض |
| **`customer_credits`** | ❌ **خلل حقيقى** |

### الخلل الحقيقى — `customer_credits` بصيغة الجمع
```
find(x => x.sub_type === "customer_credits")          ← لا وجود له إطلاقاً
  || find(x => /credit|سلف|دائن.*عميل/i.test(name))   ← فيسقط هنا دائماً
```
والقالب يشحن `customer_credit` **بالمفرد** (2155 رصيد العملاء الدائن). فالسلسلة تسقط دائمًا إلى **بحث بالاسم**، وذلك النمط يطابق **`1170 سلف ومقدمات للموظفين`** (أصل!) و`1180 سلف ومقدمات للموردين` و`2145 سلف من العملاء` بنفس السهولة.

**والأسوأ: النتيجة غير حتمية** — `find` تُرجع أول مطابقة حسب ترتيب الصفوف العائد من قاعدة البيانات بلا ترتيب مُحدَّد. فتسوية فروق العملة على مرتجع نقدى لعميل كانت قد تقع فى حساب مختلف **فى كل مرة**.

لم يظهر بعد لأن مرتجعات العملاء ورصيدهم الدائن **صفر حتى الآن**. أُصلح إلى المفرد مع رمز صريح `2155` ثم بحث بالاسم **الدقيق** بدل النمط الفضفاض.

### اكتشاف جانبى — نقاط نهاية تُعدّل البيانات بلا حارس دور
ظهر أثناء المسح: **17 مسار إصلاح لمرة واحدة** يُشحن للإنتاج، بعضها باسم عميل أو فاتورة بعينها. ثلاثة منها **تُعدّل الفواتير والقيود بلا أى فحص للدور**:

`fix-invoice-0001-status` · `fix-invoice-display` (6 تعديلات) · `fix-missing-payment-journals` (يُنشئ قيودًا)

**الخطورة الدقيقة:** تستخدم جلسة المستخدم فـ RLS يمنع الزائر المجهول — لكن **أى موظف مسجَّل دخوله** كان يستطيع استدعاءها وتجاوز فحوص الدور التى تفرضها الواجهة. أُضيف `requireOwnerOrAdmin` للثلاثة.

(و`fix-nasr-stock` كان **مُعطَّلاً بالفعل** — يرد 410. وثلاثة أخرى تتحقق من تسجيل الدخول دون الدور.)

### Verified
لا وجود لـ`customer_credits` بالجمع فى كود الإنتاج · كل مسارات الإصلاح المُعدِّلة محروسة الآن.

### Files
- `lib/services/customer-refund-command.service.ts`
- `app/api/fix-invoice-0001-status/route.ts` · `app/api/fix-invoice-display/route.ts` · `app/api/fix-missing-payment-journals/route.ts`

---

## [3.74.710] - 2026-07-19 — الحساب 1145 كان يؤدى وظيفتين + التصنيع معطّل فى ٣ شركات من ٤

طلب المالك **مراجعة شاملة قبل أى تعديل** حتى لا نُصلح شيئًا فنُعطّل آخر. المراجعة كشفت **ثلاثة تصادمات**، كلها بنفس الشكل: النظام يحلّ الحسابات بـ`sub_type` أولًا ثم برقم ثابت كملاذ أخير — **وتلك الأرقام الثابتة انحرفت عن الشجرة التى يشحنها القالب فعليًا.**

### (١) الرمز 1145 — عهدة الفنّى مقابل الإنتاج تحت التشغيل ⚠️ *أنا سببت نصفه*
ترحيل مايو حجز `1145` لحساب **الإنتاج تحت التشغيل**. وفى v3.74.685 أضفتُ «مواد فى عهدة الفنّى» على **نفس الرمز**. راجعت شجرة الحسابات وقتها — **ولم أراجع سلسلة البحث فى وحدة التصنيع**.

**فى تست:** حساب واحد اسمه «مواد فى عهدة الفنّى» ونوعه `work_in_process` و`wip_account_id` يشير إليه. لكانت تكاليف الإنتاج تختلط برصيد العُهد — ولا يبقى أى رقم ذا معنى. **وفى الثلاث الأخرى** كان الإنتاج سيُرحَّل إلى حساب العهدة.

**والتصادم فى الاتجاهين:** دالة العهدة التى كتبتها تقبل `work_in_process` كبديل — فمع وجود حساب إنتاج صحيح، كانت العهدة قد تذهب إليه.

**الفصل آمن:** تحقّقت أن 1145 يحمل **قيود عهدة فقط** وصافيه صفر، ولا توجد أى قيود إنتاج فى النظام كله.

### (٢) الرمز 5410 غائب عن القالب تمامًا
نفس ترحيل مايو أنشأ `5410` (أعباء صناعية محملة) **للشركات الموجودة حينها فقط**، ولم يُضِفه أحد إلى `chart_of_accounts_template`. والشركة الجديدة تُبذَر من القالب — فالشركات الثلاث الأحدث **بلا الحساب**، والتصنيع يرمى `MANUFACTURING_ACCOUNTS_NOT_CONFIGURED`. **أى عميل جديد كان سيصطدم بهذا فى يومه الأول.**

### (٣) الرمز 2210 — الأجور المستحقة كانت ستُرحَّل إلى القروض طويلة الأجل
لا حساب فى القالب يحمل `wages_payable` (الأجور المستحقة تُشحن كـ`2130` بنوع `accrued_salaries`)، فالسلسلة تسقط دائمًا إلى `byCode("2210")` — وهو فى القالب **«القروض طويلة الأجل»**.

### القرار والتنفيذ
بقرار المالك: **العهدة تحتفظ بـ1145** (فى القالب وثلاث شركات تستخدمه)، و**الإنتاج يأخذ `1146`**. وأُزيلت الأرقام الثابتة الخطرة من الكود **بدل إعادة توجيهها** — لأن ترحيلًا إلى حساب خاطئ أسوأ من خطأ إعداد واضح. وتُربط حسابات التصنيع صراحةً فى `companies` فلا يحتاج الحل للملاذ الأخير أصلًا.

### الحسابات اليتيمة
رأس مال الشركاء تحت **`3100 رأس المال`** (هى تفصيله ومجموعها يساويه) بترقيم منتظم `3101`، `3102`. و`5410` تحت `5000`. وأُصلح **المنشئان معًا** (صفحة المساهمين وصفحة القيود) — كلاهما كان يستخدم «أكبر رقم ملكية + 1» بلا أب، ولهذا اختلف `3301` عن `3601` لنفس المفهوم.

**أثر النقل على تقاريرك: صفر.** قائمة المركز المالى تُجمّع بـ`account_type` لا بشجرة الآباء — فالإجماليات كانت صحيحة وستبقى؛ يتغير **التبويب** فقط.

### فاحص جديد — `حِسابات نِظامية ناقِصَة`
يقارن كل شركة بالقالب ويرصد حسابًا ناقصًا **أو نوعًا مختلفًا**. النوع لا يقلّ أهمية عن الوجود: الحلّ يبدأ بالنوع، فنوع خاطئ يُرسل القيود لحساب خاطئ **بصمت**. هذا الفاحص كان سيمسك التصادمات الثلاثة جميعًا من اليوم الأول.

### Verified
الشركات الأربع: العهدة `inventory_in_custody` · الإنتاج `1146` · الأعباء `5410` · الروابط تشير للحسابات الصحيحة · **كل الفواحص (51) نظيفة عبر كل الشركات**.

### Files
- `lib/manufacturing/manufacturing-accounting.ts`
- `app/shareholders/page.tsx` · `app/journal-entries/new/page.tsx`
- `supabase/migrations/20260719000710_v3_74_710_custody_wip_collision_and_template.sql`
- `supabase/schema/functions.sql`

---

## [3.74.709] - 2026-07-19 — السبب الحقيقى للحسابات المُضلَّلة + أربعة حسابات يتيمة

طلب المالك التأكد أولًا مما إذا كان مخطط `A1B`/`A1C` مُستخدَمًا فى ترقيم آخر قبل إصلاح الدالة. **التحقق قلب الصورة ثلاث مرات.**

### (أ) المخطط البديل موجود فعلًا — لكنه خامد لا ميت
شجرة على نمط Zoho (`A → A1 → A1B المصرف / A1C النقد`) يُنشئها `seedZohoDefault()` و`scripts/011_seed_custom_coa_ar.sql`. لكن: **الدالة بلا أى موضع استدعاء**، والسكربت **ليس ملف ترحيل**، و**لا شركة تملك صفًا واحدًا** بهذه الرموز. فهو خامد لا ميت — ولذلك **لم أحذف الفرع الأبجدى**، بل صار الكود يتعرّف على **المخططين معًا** مع احتياطى بالنوع لأى مخطط مستقبلى.

### (ب) كنت أُصلح الدالة الخطأ
`normalizeCashBankParents` هى **التنظيف** الذى لم يعمل. أما **السبب** فهو `quickAdd()`: تبحث عن الأب برمز `A1B`/`A1C` غير الموجود فى أى شجرة رقمية، **فيسقط الأب إلى لا شىء**، و**ترمّز الحساب بقيمة ثابتة**: `1010` للبنك و`1000` للخزينة **بغض النظر عن الشجرة**. والحسابات المعطوبة تطابق هذه القيم **حرفيًا**:

| الحساب | المصدر |
|---|---|
| `1010 - حساب بنكى - بنك قناة السويس` | الرمز **والاسم** من القيمة الثابتة |
| `1001 - خزينة الشركة مدينة نصر` | 1000 محجوز فصار 1001 |
| `1001 - خزنة رئيسية` (notniche) | نفس النمط — **و25 سطر قيد** |

أُصلح **من المنبع**: `quickAdd` تحلّ الأب فى المخططين وتشتق الرمز من نطاقه.

### (ج) فرضية الدالة كانت خاطئة — وفحص البيانات قبل التفعيل أثبت ذلك
«كل حساب نوعه نقدية يتبع مجموعة الصندوق» **غير صحيح**. `1185 العهد (عهد الموظفين)` نوعه نقدية ويتبع 1100 فى **الشركات الأربع جميعًا** — فهو عُهد موظفين لا خزينة. **تفعيل الدالة بمنطقها القديم كان سينقل حساب قالب فى كل شركة عند فتح الصفحة.**

صارت **تتبنّى اليتامى فقط** — الحسابات بلا أب إطلاقًا، وهى وحدها التى لم يضعها أحد عمدًا — ولا تُعيد ترتيب حساب له أب بالفعل.

### نقطة عمياء فى الفاحص نفسه
اليتامى يُنشَأون بـ`level = 1` فيبدون كجذور شرعية. شرط `level > 1` **أخفى أربعة من خمسة**. الجذر الحقيقى له أبناء دائمًا — فالاختبار الموثوق هو **«بلا أب وبلا أبناء»**.

### Repair
تبنّى اليتامى **نقدية/بنوك فقط** ثم إعادة ترقيمهم داخل نطاق الأب. آمن: القيود تشير إلى **معرّف** الحساب لا رمزه (تُحقِّق قبل التنفيذ: 25 سطرًا و10 سطور).

**متروك عمدًا للمالك:** `3301` و`3302` و`3601` (رأس مال شركاء) و`5410` (أعباء صناعية محملة) — لها أرصدة حقيقية وموضعها الصحيح **قرار يخصّ شجرته لا تخمينى**. الفاحص يعرضها.

### إصلاح ثالث مساهِم
مربّعا «حساب بنكى» و«نقد بالصندوق» **لم يكونا متنافيين**؛ وعند تعليم الاثنين كانت **آخر نقرة تفوز بالنوع صامتةً** — وهكذا صار حساب بنكى نوعه نقدية فاختفى من كل قوائم البنوك. صارا متنافيين.

### Verified
كل حسابات النقدية/البنوك فى موضعها الصحيح وقيودها سليمة · الفاحص يرصد الأربعة المتبقية ولا شىء غيرها.
...
## [3.74.708] - 2026-07-19 — ترقيم تلقائى لرمز الحساب + فاحص بِنية شجرة الحسابات

### Context
سأل المالك: هل يمكن أن يكون رمز الحساب تلقائيًا ذكيًا، وهل هذا أفضل عمليًا؟ **شجرة حساباته أجابت:** الحسابان اللذان أضافهما يدويًا **كلاهما** فى غير موضعه.

| الحساب | الخلل |
|---|---|
| `1001 - خزينة الشركة مدينة نصر` | الأب صحيح (1110 الصندوق) لكن الرمز يقع عدديًا **قبل أبيه**، داخل نطاق العنوان 1000. تقارير التجميع بالنطاقات تضعه خارج فرعه. |
| `1010 - حساب بنكى - بنك قناة السويس` | **أخطر — وله أثر وظيفى:** رمز 1010 تحت 1000 (شقيق للأصول المتداولة) بدل 1120 البنوك، **ونوعه `cash` لا `bank`**. |

**الأثر الفعلى للثانى:** الشاشات التى تعرض الحسابات البنكية فقط (صرف رصيد العميل، مرتجعات الفواتير) تُرشِّح على `sub_type='bank'` — فالحساب البنكى **كان غير مرئى حيث يُحتاج تحديدًا**.

### لماذا لم يُصلحه النظام تلقائيًا؟
`normalizeCashBankParents()` تُعيد ربط حسابات النقدية/البنوك بمجموعاتها — لكنها تبحث عن المجموعات برمزَى `A1B` / `A1C`. **لا توجد هذه الرموز فى أى شركة**، فالدالة ترتدّ من حارسها الأول فى كل مرة. **كود ميت** أمام مخطط الترقيم الرقمى — ولهذا بقى حساب بنكى تحت 1000 بلا اعتراض.

### Change
1. **ترقيم مقترح لا مفروض:** عند اختيار الحساب الأب يُملأ الرمز بأول رقم فارغ **داخل نطاق الأب** — والنطاق هو ما بين رمز الأب وأول رمز أكبر منه **لا ينتمى لشجرته**. يحافظ على عُرف الأرقام المستديرة (1110، 1120، 1130…) حين تتبعه كل الأبناء. **ويتوقف عن الاقتراح فور أن يكتب المستخدم رمزًا بنفسه.**
2. **قابل للتعديل دائمًا:** الدليل الموحّد المفروض قانونًا فى بعض الدول، والترحيل من نظام قديم، ومتطلبات المراجع الخارجى — كلها حالات يكون فيها الرمز مفروضًا لا مختارًا. لا يُملأ الاقتراح لمخططات غير رقمية أصلًا.
3. **حارس تحذيرى لا مانع:** ينبّه عند رمز عند/قبل الأب، وعند تسمية حساب «بنك» مع نوع نقدية.
4. **فاحص `ic_chart_of_accounts_structure`** فى لوحة التحكم يرصد الحالتين — فيظهر الخلل مبكرًا بدل أن يُكتشف صدفة.

### Repair
`1001 → 1111` تحت الصندوق · `1010 → 1121` تحت البنوك بنوع `bank`. آمن: القيود تشير إلى **معرّف** الحساب لا رمزه، ولا أبناء لأى منهما (تُحقِّق قبل التنفيذ: الخزينة بها 10 سطور قيد، البنك صفر). **idempotent**.

### Verified
الحسابان فى موضعهما الصحيح بمستوى 4 والقيود العشرة سليمة · **الفواحص الـ49 كلها نظيفة** · الفاحص الجديد رصد الحالتين قبل الإصلاح وصفر بعده.
...
## [3.74.707] - 2026-07-19 — «طلب إلغاء خدمة» بدل «طلب مرتجع» فى الفواتير الخدمية البحتة

### Context
لاحظ المالك ظهور زر «طلب مرتجع» على فاتورة حجز لا تحتوى بضاعة مباعة — كل أصنافها مواد استهلكها الموظف المنفّذ.

**السلوك كان سليمًا:** الزر موجود من أجل **سطر الخدمة نفسه** (عكس الإيراد والمديونية لو أُلغيت الخدمة)، والمواد المستهلكة **لا تعود للمخزن إطلاقًا** — حارس v3.74.606 يستثنى بنود `item_type='service'` من حركات المرتجع («العلبة المستهلكة على العميل لا يُعاد رصّها على الرف»). وتكلفة المواد تبقى محمَّلة بحق: الزيت استُهلك ولن يعود.

**المشكلة كانت التسمية فقط.** النظام يعرف بالفعل أنها فاتورة خدمية بحتة ويعرض للمالك والمدير العام «إلغاء الخدمة» (v3.74.610) — لكن **مسار الطلب** الذى يراه محاسب الفرع بقى بالنص العام «طلب مرتجع»، فقد يظن المحاسب أنه يُرجع بضاعة.

### Change
ربط نص زر الطلب بنفس متغيّر `isServiceOnlyInvoice` الذى يقود زر المالك — **نفس المصدر، فلا يمكن أن يفترقا مستقبلًا**.

### النطاق — لا يمسّ فواتير البضائع
الشرط `items.every(it => it.item_type === 'service')`؛ فسطر منتج مباع **واحد** يجعله `false`:

| الفاتورة | التسمية |
|---|---|
| خدمة + مواد مستهلكة | **طلب إلغاء خدمة** |
| خدمة + منتج مباع | طلب مرتجع (بلا تغيير) |
| منتجات مباعة فقط | طلب مرتجع (بلا تغيير) |

وفى الحالة الوسطى تبقى التسمية «مرتجع» بحق، والبضاعة المباعة وحدها تعود للمخزن؛ المواد المستهلكة مستثناة بالحارس نفسه — فالتسمية والسلوك متسقان فى كل الحالات.

### Risk
**لا شىء.** كلمة معروضة فقط: لا تغيير فى المنطق ولا الحركات المخزنية ولا القيود ولا الصلاحيات ولا دورة الاعتماد. لا تغيير فى قاعدة البيانات.
...
## [3.74.706] - 2026-07-19 — تصحيح تكلفة مزدوجة لمواد الخدمة (تراجُع من 3.74.705)

### ما حدث — خطأ منّى لا خلل قديم
فى الإصدار 705 استنتجت أن **لا شىء** يُحمِّل تكلفة استهلاك الخدمة. فحصت `auto_create_cogs_journal` (تعمل عند البيع فقط) و`auto_link_inventory_to_journal` (تربط البيع والشراء فقط) **وتوقفت عند هذا الحد**. ولم أفحص `execute_sales_invoice_accounting` — وهى تُستدعى فى `complete_booking_atomic` **على بُعد أسطر قليلة من الموضع الذى أضفت فيه استدعائى، داخل نفس الدالة التى كنت أعدّلها**.

مواد الخدمة تُكتب فى الفاتورة **كسطور**، وذلك المحرك يُحمِّل تكلفة كل سطر منتج غير خدمى بسعر بطاقة المنتج. فأنتج تنفيذ `BKG-2026-00006` **قيدَى تكلفة لنفس الصنفين**:

| القيد | القيمة | الأساس |
|---|---|---|
| `service_consumption_cogs` | 19.90 | FIFO فعلية — صحيح |
| `invoice_cogs` | 21.00 | سعر البطاقة — مزدوج |

**40.90 تكلفة لمواد ثمنها 19.90**، وثلاثة إنذارات: توازن تكلفة المبيعات −21.00 · انحراف التقييم −21.23 · مطابقة FIFO −21.14.

### ما كان 705 مُصيبًا فيه رغم ذلك
المحرك القديم كان يُقيّم بسعر **البطاقة** (الإجمالى لا الفعلى) **ولا يستهلك دفعات FIFO** — فمشكلة المخزون الوهمى كانت حقيقية. استهلاك FIFO فى موضعه الصحيح. الخطأ كان **إبقاء التقييم الثانى الأخشن بجواره**.

### الإصلاح — تكلفة واحدة لكل سطر، من الدفعات
`execute_sales_invoice_accounting` تتخطى الآن أى سطر استُهلك كمادة خدمة لتلك الفاتورة. **فواتير البيع العادية لا تتأثر** — لا تحمل حركات `service_consumption` فالشرط لا ينطبق عليها أبدًا.

### المعالجة
عكس القيد المزدوج بقيد عكسى نظامى. النظام **يمنع تعديل قيد مُرحَّل** (وهذا صحيح) — والعكس هو المعالجة المحاسبية السليمة ويحفظ أثر المراجعة. مقصور على الفواتير التى **كل** سطورها المُكلَّفة مواد خدمة، فلا تُمَسّ فاتورة تخلط بضاعة مباعة بمواد خدمة.

### Verified
الإنذارات الثلاثة **اختفت** · حساب العهدة 1145 = **0.00** (رجعت بالكامل) · الفارق المتبقى 0.14 هو الأثر التاريخى المعروف السابق لـFIFO.
...
## [3.74.705] - 2026-07-19 — تكلفة المواد المستهلكة في تنفيذ الخدمة

### الثغرة
تنفيذ الحجز كان يكتب حركة مخزون `service_consumption` **ولا شىء غيرها**. آليتان كان يُفترض أن تُحمِّلا التكلفة، وكلتاهما لم تفعل:

- `auto_create_cogs_journal` تعمل عند `transaction_type = 'sale'` **فقط**
- `auto_link_inventory_to_journal` تربط البيع والشراء فقط؛ أى نوع آخر يسقط فى `ELSE` ويحصل على **NULL**

**فالبضاعة تخرج من المخزن بلا قيد وبلا استهلاك للدفعات:**

- الفاتورة تسجّل **الإيراد**، وتكلفة المواد **لا تصل لقائمة الأرباح أبدًا** ⟵ **الربح مبالغ فيه فى كل خدمة مُنفَّذة**
- حساب المخزون يبقى **منتفخًا** مقابل بضاعة اختفت فعليًا
- دفعات FIFO لا تُستنفد ⟵ **مخزون وهمى يتراكم**، والمبيعات القادمة تسحب تكلفتها من دفعات استُهلكت بالفعل

لم تظهر فى الواقع لسبب واحد: **لم يُنفَّذ أى حجز بعد.**

### قرارات التصميم
1. **الحماية من التكرار مبنية على الحركات لا على الفاتورة.** `resync` قد يضيف مواد لفاتورة كُلِّفت بالفعل؛ ولو كان الحارس على مستوى الفاتورة لبقيت الإضافات **بلا تكلفة بصمت** — وهو نفس نوع الخلل الذى نعالجه.
2. **قيد واحد لكل دفعة حركات غير مُرحَّلة.** ليس لكل سطر (فاحص القيود المكررة يرصد أى مرجع يتكرر ⟵ إنذار كاذب)، وليس لكل فاتورة (إضافة `resync` كانت ستصطدم بالقيد الأول على نفس المفتاح).
3. **استهلاك FIFO يقع هنا وهنا فقط.** خروج العهدة **يُقيّم ولا يستهلك** عمدًا — المادة فى يد الفنّى ما زالت مملوكة، تُستهلك عند التنفيذ لا عند التسليم. الاستهلاك فى الموضعين = **ازدواج**.

### Verified
اختبار على بيانات وهمية (تراجَع بالكامل): دفعتان 4@3.00 و10@7.00 → تنفيذ يستهلك 6 = **26.00 ✓** (4×3 + 2×7) · إعادة الاستدعاء = `nothing_to_post` ✓ · إضافة `resync` لمادة ثانية = **11.91 ✓** (سحبت الدفعة الأقدم 1×1.911875 ثم 2×5.00 — ترتيب FIFO صحيح) · لا حركات غير مُرحَّلة · **صفر إنذارات قيود مكررة**.

### ملاحظة على التنفيذ
موضع الاستدعاء فى `resync_booking_invoice` أخطأتُ فيه أولًا فوقع داخل شرط الخروج المبكر — **قبل** تحميل الفاتورة وقبل وجود أى حركة استهلاك. اكتُشف بفحص الموضع فعليًا لا بالافتراض، وصُحِّح ليسبق **الإرجاع النهائى**، وملف الترحيل يُرسى على نص الإرجاع النهائى صراحةً ويفشل بوضوح إن لم يجده.
...
## [3.74.704] - 2026-07-19 — تكلفة الشراء الفعلية (بعد الخصم + الشحن)

### الخلل
دفعة FIFO كانت تُنشأ بسعر **قائمة الأسعار**، بينما القيد يُسجَّل بالمبلغ **الواجب دفعه فعلًا**. فكل خصم شراء وكل مصروف شحن يُبعد الدفاتر عن FIFO — بشكل **دائم ومتراكم**:

| المستند | القيد | دفعة FIFO | الفرق |
|---|---|---|---|
| BILL-0001 | 6.44 | 7.00 | 0.56 |
| BILL-0002 | 19.90 | 21.00 | 1.10 |

ومنذ v3.74.702 صارت FIFO أساس تكلفة البيع — فهذا **ليس تجميليًا**: تكلفة البيع كانت **مبالغًا فيها** والربح المُعلَن **أقل من حقيقته** بمقدار خصم الشراء كاملًا، **فى كل عملية بيع**. والفجوة تكبر بحجم مشترياتك، ولهذا كانت لا تزال أصغر من عتبة الإنذار فلم تظهر.

**المعيار (IAS 2 / ASC 330):** تكلفة المخزون = سعر الشراء **ناقصًا** الخصومات التجارية **زائدًا** النقل والمناولة. **كلا الطرفين كان مفقودًا.**

### المنهج — توزيع لا إعادة اشتقاق
الخصم على مستويين (نسبة السطر، ونسبة أو مبلغ ثابت على رأس الفاتورة قبل الضريبة أو بعدها)، مع الشحن وضرائب مختلفة لكل سطر وأسعار شاملة للضريبة. إعادة اشتقاق كل ذلك فى مكان ثانٍ = **مصدر حقيقة ثانٍ ينتظر أن يختلف مع الأول**.

بدلًا من ذلك تُوزَّع قيمة الفاتورة **المعتمدة نفسها** (الإجمالى + الشحن) على السطور بنسبة قيمتها الصافية. فمهما كانت قواعد التسعير، **مجموع تكاليف الدفعات يساوى قيد المخزون بالضبط — بحكم البناء**. لا يمكن أن يفترقا مجددًا.

الضريبة **مستبعَدة** مطابقةً للدفاتر: الضريبة تذهب لحساب ضريبة المدخلات لا للمخزون.

### أثر جانبى عولج
العهدة القائمة كانت مُقيَّمة بالتكلفة القديمة. لو تُركت، لكان **التنفيذ** يُرجع العهدة بالقيمة الإجمالية ثم يستهلك FIFO بالقيمة الفعلية — **فيبقى الفرق عالقًا فى حساب المخزون للأبد**. أُعيد تقييم العهدة وأُعيد الفرق من 1145 إلى 1140.

### Verified
- التوزيع يُعيد إنتاج قيد كل فاتورة **بالقرش**: 60000.00 · 6.44 · 19.90.
- notniche: الدفاتر 53400.00 = FIFO **53400.00** (تطابق تام).
- تست: الدفاتر 21.67 مقابل FIFO 21.81 — الفارق **0.14** مُفسَّر بالكامل: قيود قديمة سابقة لـFIFO (بيع 3 وحدات بتكلفة البطاقة 3.00 بدل 2.72، ومرتجعان بـ1.67 بدل 1.81).
- العهدة: 18.95 + 0.95 · كل فواحص السلامة **نظيفة**.
...
## [3.74.703] - 2026-07-19 — سحب معتمد لم يخرج من المخزن (فشل صامت)

### Context — اكتشفه المالك
صنفان على نفس الحجز، كلاهما «معتمد للسحب من المخزن». واحد خرج، والآخر **لا** — ولا شىء فى النظام قال ذلك.

حجز `BKG-2026-00006`، اعتماد بفارق **11 ثانية** لنفس المخزن:

| الصنف | وقت الاعتماد | النتيجة |
|---|---|---|
| booto | 20:47:12 | عهدة `out` بقيمة 1.00 ✅ |
| 5 كيلو زيت ماتور | 20:47:01 | عهدة `none` — **لا حركة، لا قيد، لا خطأ، لا إشعار** |

### السبب الجذرى — قيمة محاسبية صفر ألغت حركة مادية
```
v_value := v_qty * v_cost;                     -- سعر بطاقة المنتج
IF NOT tracked OR v_qty <= 0 OR v_value <= 0 THEN
  SET custody_status='none';
  RETURN ok = TRUE;                            -- نجاح! صامت تمامًا
```
وسعر البطاقة كان **صفرًا** لأن السعر أُدخل فى الفاتورة لا فى البطاقة — **نفس جذر مشكلة تكلفة البيع الصفرية فى v3.74.702**. فأبلغت الدالة بالنجاح دون أن تفعل شيئًا: الفنّى يحمل الزيت، والدفاتر تعدّه فى المخزن، والرصيد مبالغ فيه بوحدة، وحساب 1145 ناقص 20 جنيهًا.

**لماذا لم تكشفه فواحص السلامة؟** لأن المخزون والدفاتر **متطابقان** — كلاهما يظن أن الزيت فى المخزن. التطابق ليس صحة؛ الفواحص تقارن ما سُجِّل ولا تسأل هل ما كان يجب أن يحدث حدث.

### Change — القيمة شىء والحركة شىء آخر
1. **القيمة من دفعات FIFO** عبر `calculate_fifo_cost` — تحسب ولا **تستهلك**، فتبقى الدفعات سليمة حتى التنفيذ الفعلى (خروج العهدة يجب ألا يستنفد مخزونًا لم يُستهلك). ترجع لبطاقة المنتج للمخزون القديم.
2. **الحركة المخزنية غير مشروطة** ما دام الصنف مخزنيًا والكمية حقيقية. القيد وحده هو ما يتوقف على القيمة — وهو بالضبط ما كان مسار **الإرجاع** يفعله أصلًا. المساران متماثلان الآن؛ مسار الخروج كان الشاذّ.
3. حركة بلا قيمة تُصدر **تحذيرًا** وتُبلّغ `valued=false` بدل أن تختفى خلف `ok=true`.

### Data repair (بموافقة المالك)
تسجيل السحب المعلّق بتكلفته الحقيقية **20.00**. الإصلاح **idempotent** — إعادة تشغيله لا تغيّر شيئًا.

### فاحص السلامة كان يجهل العهدة
تسجيل السحب أشعل إنذارًا جديدًا فورًا: البضاعة فى عهدة الفنّى تغادر 1140 إلى 1145 لكنها **ما زالت مملوكة وما زالت فى دفعات FIFO** (خروج العهدة يُقيّمها ولا يستهلكها — تُستهلك عند التنفيذ فقط). فكان الفاحص يقارن FIFO بـ1140 وحده ويعدّ **رصيد العهدة كله انحرافًا**. المخزون المملوك = **1140 + 1145**.

### ⚠️ اكتشاف منفصل يحتاج قرارك — خصم فاتورة الشراء لا يدخل تكلفة الدفعة
ظهر أثناء التحقيق: الدفعة تُسجَّل بسعر **السطر قبل الخصم**، بينما القيد يُسجَّل بالمبلغ **بعد الخصم**:

| المستند | القيد (1140) | دفعة FIFO | الفرق |
|---|---|---|---|
| BILL-0001 | 6.44 | 7.00 | 0.56 |
| BILL-0002 | 19.90 | 21.00 | 1.10 |

أى أن **التكلفة الحقيقية هى المدفوع بعد الخصم**، فدفعات FIFO تُبالغ فى التكلفة ⟵ تكلفة بيع مبالغ فيها ⟵ **ربح أقل من الحقيقة**، وفجوة دائمة بين الدفاتر وFIFO. الفارق الآن 1.33 جنيه فقط (دون عتبة الإنذار) لكنه **يكبر مع حجم المشتريات**. لم أغيّره — إصلاحه يمسّ تكلفة كل دفعة قادمة ويحتاج قرارك.

### Verified
`{ok: true, qty: 1, value: 20, valued: true}` · الصنفان الآن `out` · حساب **1145 = 21.00** (20 + 1) · **كل فواحص السلامة نظيفة**.
...
## [3.74.702] - 2026-07-19 — تكلفة البضاعة المباعة من FIFO + تصحيح فواحص السلامة

### Context
أظهرت لوحة التحكم **4 انحرافات**. التحقيق قسّمها إلى نوعين مختلفين تمامًا.

### (أ) ثلاثة إنذارات كاذبة — خلل في الفواحص نفسها
الفواحص الأربعة (`ic_duplicate_journals` / `ic_ap_balance` / `ic_inventory_valuation_drift` / `ic_inventory_gl_vs_fifo`) كانت تربط القيود بشرط `status='posted'` **دون استثناء القيود الملغاة**. فالمستند المُبطَل يحتفظ بقيده القديم معلَّمًا `is_deleted`، فيُحسب مع بديله وتظهر: «قيود مكررة» (عالي)، واختلال ذمم بقيمة القيد الميت، وفجوة GL/FIFO. **هذا كان سيحدث مع أي مستند مُبطَل.** وفاحص التقييم كان يجمع أيضًا حركات مخزون محذوفة.

### (ب) إنذار حقيقي — التكلفة كانت تُحسب من بطاقة المنتج
`products.cost_price` مجرد **قيمة افتراضية** تملأ سعر فاتورة الشراء (والبطاقة هي المصدر كما صُمِّم). لكن حين يُسجَّل منتج بلا سعر شراء، يُدخله المستخدم في الفاتورة **ولا يُكتب في البطاقة** فتبقى صفرًا. وكانت التكلفة تُحسب `الكمية × بطاقة المنتج` — فمنتج اشتُري بـ20.00 وبطاقته صفر **كان يُباع بتكلفة صفر ويضخّم الربح**.

دفعات FIFO تحمل السعر الحقيقي لكل دفعة (تحقّقنا: دفعة الزيت بـ20.00 بالضبط) لكن **لم يستخدمها شيء** — كل قيود التكلفة الـ15 أتت من مسار بطاقة المنتج.

### Change (بقرارك: اعتماد FIFO)
`auto_create_cogs_journal` يستخدم الآن `consume_fifo_lots` — يُرجع تكلفة الدفعة الحقيقية **ويستهلك الدفعة ويسجّل الاستهلاك**، فتُسحب المبيعة التالية من الدفعة التالية. ومع **احتياطي**: لو لم تكن للمنتج دفعات (مخزون قديم) تُستخدم بطاقة المنتج كما قبل، فلا تُصفَّر التكلفة أبدًا.

وتصحيح بيانات: ملء تكلفة البطاقات **الفارغة فقط** من متوسط سعر الشراء الفعلي (لا يُلمَس سعر حدّدته بنفسك).

### (ج) المرتجعات — بيع وشراء، جزئى وكامل
كانت **مرتجعات البيع** تعكس التكلفة من بطاقة المنتج ولا تُعيد الوحدات إلى دفعاتها. و**مرتجعات الشراء لم تكن تمسّ FIFO إطلاقًا** — بضاعة رُدَّت للمورد وتبقى فى الدفعات كرصيد وهمى (ويوجد بالفعل **مرتجعا شراء** فى قاعدة البيانات). الاثنان مُعالَجان الآن:

- **مرتجع بيع:** تعود الوحدات إلى **نفس الدفعات** التى خرجت منها، ويُعكس القيد **بنفس التكلفة الأصلية**. يعمل جزئيًا وكاملًا.
- **مرتجع شراء:** تُخصم الوحدات من دفعة **الفاتورة المرتجعة** أولًا ثم الأحدث. لا تُمسّ وحدات بيعت بالفعل.
- **إصلاح خطأ مُكتشَف:** الحارس القديم كان يتخطّى العكس لو وُجد أى قيد مرتجع سابق لنفس الفاتورة — أى أن **المرتجع الجزئى الثانى كان يمرّ بلا قيد عكس نهائيًا**. الآن لكل حركة مرتجع قيدها.
- مربوط على **حركة المخزون نفسها** لا على إجراء بعينه، فيغطى كل مسارات المرتجع الحالية وأى مسار يُضاف لاحقًا.

### Verified
- الانحرافات الأربعة **اختفت جميعًا**.
- اختبار على بيانات وهمية (تراجَع بالكامل): دفعتان 10@5 و10@8 → بيع 12 = **66.00 ✓** · مرتجع جزئى 3 = **21.00 ✓** · مرتجع كامل 9 = **45.00 ✓** والدفعات رجعت 10/10 بلا سجلات معلّقة · مرتجع شراء 4 = **32.00 ✓** ثم 8 = **58.00 ✓**.
- الاختبار كشف خطأ ترتيب حقيقى (المرتجع الجزئى كان يعكس 15.00 بدل 21.00) وتم إصلاحه.
...
## [3.74.701] - 2026-07-19 — مخالفة حوكمة: منتجات فروع أخرى كانت تظهر في «منتجات إضافية للبيع»

### Context
فشل تنفيذ حجز في الفرع الرئيسي بخطأ قاعدة بيانات `Branch Isolation Violation`. الصنفان المرفقان بالخدمة سليمان؛ السبب منتج **VitaSlims** المُضاف كمنتج إضافي للبيع وهو يخصّ **فرع مدينة نصر**.

### The real gap
قائمة اختيار المنتجات الإضافية كانت تجلب **كل منتجات الشركة** (`company_id` فقط، بلا فلترة فرع) — فرأى الموظف منتجات فروع أخرى وأضافها. لم يمنعه شيء حتى لحظة التنفيذ، حيث أوقف حارس قاعدة البيانات العملية كلها برسالة تقنية **بعد** أن أتمّ كل خطوات الاعتماد. وبيع منتج فرعٍ آخر على فاتورة هذا الفرع يفسد مخزون ومحاسبة الفرعين معًا.

### Change
القائمة صارت **مقيَّدة بفرع الحجز** (مع المنتجات المشتركة التي بلا فرع). فلا يظهر أصلًا ما لا يجوز بيعه. وحارس قاعدة البيانات يبقى خط الدفاع الأخير.

### Verified
على بيانات «تست»: أصناف الفرع الرئيسي الأربعة تظهر، و**VitaSlims وماتور** (مدينة نصر) لا يظهران لحجوزات الفرع الرئيسي.
...
## [3.74.700] - 2026-07-18 — الصنف الاختياري بعد تحديده يُعامَل معاملة الإلزامي

### Context (قاعدة المالك)
«اختياري» تعني فقط أن المنفّذ **قد يختاره أو لا** أثناء تنفيذ الخدمة. لكن **بمجرد تحديده يجب أن يُعامَل معاملة الصنف الإلزامي** — بما في ذلك اعتماد مسؤول المخزن للإخراج.

### What was wrong
بوابة التنفيذ كانت تشترط اعتماد سحب **فقط** إذا كان **المنتج** مفعّلًا عليه «يتطلب اعتماد سحب». فصنفٌ مفعّل عليه «خصم مخزون» وخانته مغلقة (مثل `booto`) كان **يخرج من المخزن عند التنفيذ بلا طلب اعتماد وبلا أي إشعار لمسؤول المخزن**.

للدقة: المشكلة لم تكن في التفرقة بين إلزامي واختياري — البوابة أصلًا لا تنظر لخانة «اختياري». الثغرة أن **خانة واحدة في بطاقة المنتج** كانت تُعفي صنفًا يخرج فعليًا من المخزن.

### Change
أي صنف مرفق بخدمة (`kind <> 'extra'`) سيُخصم فعليًا (`auto_deduct_inventory`) صار **يتطلب سحبًا معتمَدًا** — بصرف النظر عن خانة المنتج. وخانة المنتج تبقى مُحفِّزًا إضافيًا للأصناف التي تحتاج اعتمادًا دون خصم. طُبِّق في **البوابة والواجهة معًا** (زر «طلب سحب من المخزن» صار يظهر لهذه الأصناف).

**التحديد محترَم تلقائيًا:** الدالة لا تُرجع إلا الأصناف الاختيارية التي حدّدها المنفّذ فعلًا، فغير المحدَّد لا يُعطّل شيئًا.

**المنتجات المباعة لم تُمَس:** تخرج عبر دورة الإرسال من الفاتورة («مرسل») لا عبر هذه البوابة.

### Verified
على حجز الاختبار: `booto` (اختياري محدَّد، خانة المنتج مغلقة) أصبح **يتطلب اعتمادًا** ويُعطّل التنفيذ حتى يُعتمد — كالزيت الإلزامي تمامًا.
...
## [3.74.699] - 2026-07-18 — عدّاد المسودات على كرتَي «أوامر البيع» و«أوامر الحجز»

### Change
كل كرت يعرض الآن عدد الأوامر التي **ما زالت مسودة**، بنفس قاعدة صندوق الموافقات: **العدّاد يقرأ من نفس المصدر المُنطَّق الذي تُعرض منه القائمة**، فلا يُعلن رقمًا لا يقابله شيء عند الفتح.

- **أوامر البيع:** يُحسب من `orders` القادمة من `/api/sales-orders` (محكومة بالخادم حسب الدور والفرع).
- **أوامر الحجز:** يُنشره مكوّن `BookingsTab` من صفوفه نفسها (محكومة بـ`/api/bookings`) وبنفس قاعدة «المؤكَّدة فقط» التي يعرضها — فلا يظهر عدد لحجوزات لا تُعرض في التبويب.

العدّاد لا يتأثر بفلاتر البحث/الحالة داخل التبويب — فهو يجيب «كم أمرًا لديّ ما زال مسودة» لا «كم يطابق الفلتر الحالي». ويُخفى العدّاد إن كان صفرًا.

### Note
`BookingsTab` صار يُحمَّل دائمًا (مخفيًا عند عدم النشاط) ليتوفّر عدّاده قبل فتح التبويب — كما هو حال محتوى أوامر البيع أصلًا.
