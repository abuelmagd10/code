...
### 🇪🇬 Paymob EGP-only charging — Multi-currency display preserved

أبلغ المستخدم بأن حسابه على Paymob يستقبل بـ **EGP فقط**.

### ✅ الإصلاح: Display أى عملة، Charge EGP فقط

النموذج مطابق لـ Stripe/Shopify globally:

```
العميل يرى السعر: 38 ريال سعودى
                    ↓ (live FX: 1 USD = 50 EGP, 1 USD = 3.75 SAR)
Paymob يحصّل:    510 جنيه مصرى
```

### 📋 التعديلات

**1. `lib/billing/pricing-engine.ts`**:
- إضافة `CHARGE_CURRENCY = 'EGP'` ثابت
- حقول جديدة فى الـ output:
  - `chargeCurrency: 'EGP'`
  - `chargeExchangeRate: number` (USD → EGP)
  - `chargeTotalEgp: number` (المبلغ الفعلى للتحصيل)

**2. `app/api/billing/seats/route.ts`**:
- يستخدم `calculatePricing()` بدلاً من السعر الثابت
- يُرسل `amountCents` للـ Paymob بـ EGP فقط
- `extras` يحتوى pricing snapshot كامل (display + charge + FX rate)
- يدعم billing_period (monthly/annual) و coupon code

**3. `app/settings/billing/page.tsx`**:
- Banner أصفر واضح يُظهر مبلغ EGP عند العرض بعملة أخرى:
  ```
  💳 سيتم التحصيل بالجنيه المصرى
  المبلغ المُحصَّل عبر Paymob: ج.م 510.00
  سعر الصرف: 1 USD = 50.00 EGP (لحظى)
  ```
- زر الدفع يُظهر مبلغ EGP صراحة
- النص "Paymob (EGP)" أسفل الزر

### 📊 السلوك

| الحالة | الـ Display | الـ Charge |
|---|---|---|
| مستخدم مصرى | EGP 500/شهر | EGP 500 |
| مستخدم سعودى | ر.س 37.50/شهر | EGP 500 (= $10) |
| مستخدم إماراتى | د.إ 36.70/شهر | EGP 500 (= $10) |
| مستخدم أوروبى | €9.20/شهر | EGP 500 (= $10) |

السعر الحقيقى دائماً $10 USD، يُحصَّل بـ EGP بسعر الصرف اللحظى.

### 📋 Files Changed

| المكون | التغيير |
|---|---|
| `lib/billing/pricing-engine.ts` | إضافة chargeCurrency + chargeTotalEgp |
| `app/api/billing/seats/route.ts` | استخدام pricing engine + extras snapshot |
| `app/settings/billing/page.tsx` | Banner EGP charge + amount display |
| `CHANGELOG.md` | توثيق |
