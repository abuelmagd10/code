...
export async function assertCashOutflowAllowed(
  ...
  // v3.74.562 — same idea for expenses. A pending expense pointing at

  // this account is a queued outflow that other approvers can't see

  // via inventory_transactions / GL. Include it in the reservation.
  ...
  try {
    ...
    const { data: qe } = await supabase
      ...
      .select('amount, base_currency_amount, status')

      .eq('payment_account_id', opts.accountId)

      .in('status', ['pending_approval', 'approved'])

    for (const e of (qe || []) as any[]) {

      // "approved" here means approved-but-not-paid: still a reservation

      // until paid_by / journal_entry_id are set.

      const val = useNative ? Number(e.amount || 0) : Number(e.base_currency_amount ?? e.amount ?? 0)
  ...
  // v3.74.561 — same idea for bank_voucher_requests. Vouchers with

  // voucher_type='payment' (or 'صرف') pull from the account; two

  // simultaneously-pending vouchers must not both approve when the
  ...
  try {
    ...
    const { data: qv } = await supabase
      ...
      .select('amount, base_amount, voucher_type, status')

      .eq('account_id', opts.accountId)

          .in('status', ['pending', 'approved'])

    for (const v of (qv || []) as any[]) {
      const t = String(v.voucher_type || '').toLowerCase()
      const isOutflow = t === 'payment' || t === 'صرف' || t.includes('صرف') || t.includes('pay')
      if (!isOutflow) continue
      const val = useNative ? Number(v.amount || 0) : Number(v.base_amount ?? v.amount ?? 0)
      queuedOutflow += Math.abs(val)
    }
  } catch (_) { /* silent fallback */ }
