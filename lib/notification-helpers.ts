...
/**
 * Notify approvers when a Purchase Request is submitted for approval
 */
export async function notifyPurchaseRequestApprovalRequest(params: {
  ...
  const { companyId, requestId, requestNumber, amount, currency, branchId, costCenterId, createdBy, appLang = 'ar' } = params

  const title = appLang === 'en'
    ? 'Purchase Request Approval Required'
    : 'طلب موافقة على طلب شراء'

  const message = appLang === 'en'
    ? `Purchase Request ${requestNumber} (${amount} ${currency}) requires your approval`
    : `طلب شراء ${requestNumber} بقيمة ${amount} ${currency} يحتاج إلى موافقتك`
  ...
  for (const role of roles) {
    await createNotification({
      ...
      assignedToRole: role,
      priority: 'high' as NotificationPriority,
      eventKey: `purchase_request:${requestId}:approval_request:${role}`,
      severity: 'warning',
      category: 'approvals'
    })
  }
...
/**
 * Notify requester when a Purchase Request is approved
 */
export async function notifyPurchaseRequestApproved(params: {
  ...
  const { companyId, requestId, requestNumber, amount, currency, branchId, costCenterId, createdBy, approvedBy, appLang = 'ar' } = params
  ...
  const message = appLang === 'en'
    ? `Your Purchase Request ${requestNumber} (${amount} ${currency}) has been approved. You can now convert it to a Purchase Order.`
    : `تمت الموافقة على طلب الشراء ${requestNumber} بقيمة ${amount} ${currency}. يمكنك الآن تحويله إلى أمر شراء.`
  ...
  await createNotification({
    ...
    costCenterId,
    priority: 'normal' as NotificationPriority,
    eventKey: `purchase_request:${requestId}:approved`,
    severity: 'info',
    category: 'approvals'
  })
}
...
/**
 * Notify requester when a Purchase Request is rejected
 */
export async function notifyPurchaseRequestRejected(params: {
  ...
  const { companyId, requestId, requestNumber, amount, currency, branchId, costCenterId, createdBy, rejectedBy, reason, appLang = 'ar' } = params
  ...
  const message = appLang === 'en'
    ? `Your Purchase Request ${requestNumber} (${amount} ${currency}) was rejected. Reason: ${reason}`
    : `تم رفض طلب الشراء ${requestNumber} بقيمة ${amount} ${currency}. السبب: ${reason}`
  ...
  await createNotification({
    ...
    costCenterId,
    priority: 'high' as NotificationPriority,
    eventKey: `purchase_request:${requestId}:rejected`,
    severity: 'error',
    category: 'approvals'
  })
}
...
/**
 * Notify requester when a Purchase Request is converted to PO
 */
export async function notifyPurchaseRequestConverted(params: {
  ...
  const { companyId, requestId, requestNumber, poId, poNumber, createdBy, appLang = 'ar' } = params

  const title = appLang === 'en'
    ? 'Purchase Request Converted to PO'
    : 'تم تحويل طلب الشراء إلى أمر شراء'

  const message = appLang === 'en'
    ? `Your Purchase Request ${requestNumber} has been converted to Purchase Order ${poNumber}`
    : `تم تحويل طلب الشراء ${requestNumber} إلى أمر شراء ${poNumber}`
  ...
  await createNotification({
    ...
    assignedToUser: createdBy,
    priority: 'normal' as NotificationPriority,
    eventKey: `purchase_request:${requestId}:converted`,
    severity: 'info',
    category: 'approvals'
  })
}
...
// ===========================================================
// 📦 Goods Receipt (GRN) Notifications
// ===========================================================

/**
 * Notify approvers when a Goods Receipt is submitted for approval
 */
export async function notifyGRNApprovalRequest(params: {
  companyId: string
  grnId: string
  grnNumber: string
  supplierName: string
  ...
  const { companyId, grnId, grnNumber, supplierName, warehouseId, branchId, createdBy, appLang = 'ar' } = params

  const title = appLang === 'en'
    ? 'Goods Receipt Approval Required'
    : 'طلب موافقة على إيصال الاستلام'

  const message = appLang === 'en'
    ? `Goods Receipt ${grnNumber} from supplier ${supplierName} requires your approval`
    : `إيصال الاستلام ${grnNumber} من المورد ${supplierName} يحتاج إلى موافقتك`

  const roles = ['admin', 'owner', 'manager', 'store_manager']
  for (const role of roles) {
    await createNotification({
      companyId,
      ...
      assignedToRole: role,
      priority: 'high' as NotificationPriority,
      eventKey: `goods_receipt:${grnId}:approval_request:${role}`,
      severity: 'warning',
      category: 'approvals'
    })
  }
...
/**
 * Notify creator when a Goods Receipt is approved
 */
export async function notifyGRNApproved(params: {
  companyId: string
  grnId: string
  grnNumber: string
  supplierName: string
  ...
  const { companyId, grnId, grnNumber, supplierName, warehouseId, branchId, createdBy, approvedBy, appLang = 'ar' } = params

  const title = appLang === 'en'
    ? 'Goods Receipt Approved'
    : 'تم اعتماد إيصال الاستلام'

  const message = appLang === 'en'
    ? `Goods Receipt ${grnNumber} from supplier ${supplierName} has been approved. Inventory has been updated.`
    : `تم اعتماد إيصال الاستلام ${grnNumber} من المورد ${supplierName}. تم تحديث المخزون.`
  ...
  await createNotification({
    ...
    warehouseId,
    priority: 'normal' as NotificationPriority,
    eventKey: `goods_receipt:${grnId}:approved`,
    severity: 'info',
    category: 'approvals'
  })
}
...
/**
 * Notify creator when a Goods Receipt is rejected
 */
export async function notifyGRNRejected(params: {
  companyId: string
  grnId: string
  grnNumber: string
  supplierName: string
  ...
  const { companyId, grnId, grnNumber, supplierName, warehouseId, branchId, createdBy, rejectedBy, reason, appLang = 'ar' } = params

  const title = appLang === 'en'
    ? 'Goods Receipt Rejected'
    : 'تم رفض إيصال الاستلام'

  const message = appLang === 'en'
    ? `Goods Receipt ${grnNumber} from supplier ${supplierName} was rejected. Reason: ${reason}`
    : `تم رفض إيصال الاستلام ${grnNumber} من المورد ${supplierName}. السبب: ${reason}`
  ...
  await createNotification({
    ...
    warehouseId,
    priority: 'high' as NotificationPriority,
    eventKey: `goods_receipt:${grnId}:rejected`,
    severity: 'error',
    category: 'approvals'
  })
}
