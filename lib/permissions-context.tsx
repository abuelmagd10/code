...
export function canAccessPageSync(resource: string): boolean {
  ...
  if (resource === "product_availability") {
    // إذا كان المستخدم عضو في الشركة (لديه role)، يمكنه الوصول
    return isValid && role !== ""
  }
...
export function PermissionsProvider({ children }: { children: React.ReactNode }) {
  ...
  const canAccessPage = useCallback((resource: string): boolean => {
    // أثناء التحميل، نعود false لمنع الوميض (ما عدا الصفحات العامة)
    if (!isReady) {
      // استثناء: صفحة توفر المنتجات متاحة حتى أثناء التحميل إذا كان المستخدم عضو في الشركة
      if (resource === "product_availability" && role !== "") return true
      return false
    }
