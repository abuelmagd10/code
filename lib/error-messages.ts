...
/**
 * v3.74.954 — الجانبُ العربىُّ لا يكون أفقرَ من الإنجليزى.
 * ---------------------------------------------------------------------------
 * قِيس على المشروع: ٤٣ موضعاً يُرسل فيها الخادمُ سببَ القاعدة فى `error`
 * ويُرسل بجواره `error_ar` نصّاً ثابتاً، والشاشةُ تُفضّل العربىَّ — فيُعرض
 * الأفقر. ويومَ ٤ أغسطس ٢٠٢٦ كلّف هذا ساعاتٍ من التشخيص: القاعدةُ كانت
 * تقول «new row violates row-level security policy» والشاشةُ تقول «فشل في
 * إنشاء أمر الشراء» ولا شىءَ غيرها.
 *
 * وقواعدُ هذا المشروع تتكلّم العربيةَ منذ ٩٥٠ (RAISE EXCEPTION بالعربية)،
 * فأكثرُ الأسبابِ تمرّ كما هى بلا ترجمة. وما لم يكن عربياً يُصنَّف برمزه،
 * ويُلحَق النصُّ الأصلىُّ دائماً — فالمهندسُ يحتاجه والمستخدمُ لا يضرّه.
 *
 * والاحتياطىُّ (fallback) يبقى كما كان: إن عجزنا عن قولٍ أفضل، لا نقول أسوأ.
 */
export const arabicReason = (error: any, fallback: string): string => {
  ...
  // القاعدةُ تتكلّم العربيةَ أصلاً ⇒ جوابُها هو الجواب.
  if (/[\u0600-\u06FF]/.test(text)) return text

  const code = String(error?.code ?? '')

  // P0001 هو RAISE EXCEPTION من دوالِّنا — نصُّه هو المقصود.
  if (code === 'P0001' && text) return text

  const KNOWN: Record<string, string> = {
    '42501': 'لا تسمح صلاحياتُك بهذه العملية على هذا السجل.',
    '23505': 'قيمةٌ مكرَّرة: الرقمُ أو الاسمُ مستعملٌ من قبل.',
    '23502': 'حقلٌ مطلوبٌ وصل فارغاً.',
    '23503': 'مرجعٌ غيرُ موجود: السجلُّ المرتبطُ غيرُ متاح.',
    '23514': 'قيمةٌ ترفضها قواعدُ الصحة المسجَّلة على الجدول.',
    '22P02': 'قيمةٌ غيرُ صالحةٍ فى أحد الحقول.',
    '40001': 'تزاحمٌ لحظىٌّ على السجل — أعد المحاولة.',
    '57014': 'استغرقت العمليةُ وقتاً أطولَ من المسموح.',
    'PGRST116': 'لا سجلَّ مطابقاً — قد يكون محجوباً عنك أو محذوفاً.',
    'PGRST301': 'انتهت صلاحيةُ الجلسة — سجّل الدخولَ من جديد.',
  }

  const head = KNOWN[code]
  if (head) return text ? head + ' (' + text + ')' : head
  return text ? fallback + ' — السبب: ' + text : fallback
}
