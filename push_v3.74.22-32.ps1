...
if ($v -match 'APP_VERSION = "3.74.32"') {
    Write-Host "+ APP_VERSION = 3.74.32" -ForegroundColor Green
} else { Write-Host "X APP_VERSION not 3.74.32" -ForegroundColor Red; exit 1 }

$cl = Get-Content -LiteralPath "CHANGELOG.md" -Raw
foreach ($vv in @('3.74.22','3.74.23','3.74.24','3.74.25','3.74.26','3.74.27','3.74.28','3.74.29','3.74.30','3.74.31','3.74.32')) {
...
# Application-code spot checks (v3.74.22 .. 26)
$v22 = @{
...
if ($srLib -match "SALES_RETURN_LEVEL1_APPROVER_ROLES\s*=\s*\[\s*'owner',\s*'admin',\s*'general_manager',\s*'manager',\s*\]") {
    Write-Host "+ v3.74.26 accountant removed" -ForegroundColor Green
} else { Write-Host "X v3.74.26 still in L1" -ForegroundColor Red; exit 1 }

# DB-only releases (27..32) verify via CHANGELOG markers
foreach ($entry in @(
...
if (-not $staged) {
    ...
    git commit -m "feat: v3.74.22-32 audit closure + sales-return-warehouse-approve chain of hotfixes
...
v3.74.32 HOTFIX inventory_transactions.transaction_date
         (does not exist; removed), payments.reference
         (real name is reference_number), fifo_lot_consumptions
         missing required product_id + consumption_type +
         consumption_date - all aligned with actual schemas.
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "`n+ v3.74.22-32 pushed" -ForegroundColor Green
}
