...
## CP. AmendmentBanner يعرض تفاصيل البنود المعدلة (v3.74.495)

المالك لاحظ إن banner الأمندمنت على عرض الفاتورة كان بيقول
"✏️ 1 بند معدل" فقط بدون اسم المنتج ولا الحقل اللى اتغيّر، بينما
الـ DiffCard على `/approvals` كان بيوضّح "VitaSlims: كمية 4→5".

`BillAmendmentBanner` اتحدّث ليعرض:
- **بنود مضافة**: كل بند مع اسمه + كمية × سعر + خصم% (لو موجود)
- **بنود محذوفة**: نفس التفاصيل
- **بنود معدلة**: كل بند بيوضّح ايه اتغيّر بالظبط —
  كمية 4→5 · سعر 1.00→1.20 · خصم 10%→5%

الـ component واحد ومُستخدم على bill + invoice، فالمبيعات ورثت
التغيير تلقائى.
...
بالمرآة:
- `historyCategoryToTab` بيربط كل `HistoryCategory` بـ `TabKey` مناسب
- `canShowHistory(category)` = `canShow(tabKey)` — نفس المصفوفة
- كل filter chip فى السجل ملفوف بـ `{canShowHistory(...) && ...}`
- عرض "الكل" بيقصر النتائج على الـ categories المرئية للدور، فالعدّاد
  والقائمة يماشوا الـ chips الظاهرة
...
**١. سايدبار — نقل صندوق الموافقات لأعلى**:
- كان تحت مجموعة "🏭 التصنيع" (تاريخى: أول ما اتعمل كان للتصنيع)
- بعد v3.74.481 يوحّد **كل** فئات الاعتماد
- انتقل لـ **مجموعة مستقلة "🔔 صندوق الموافقات"** بعد "لوحة التحكم" مباشرة

**٢. صلاحيات الأدوار (settings/users)**:
- resource `approvals` كان تحت "🏭 التصنيع" مع label غلط
  ("إنتاج، BOM، صرف المواد")
- نقل لمجموعة مستقلة **"🔔 صندوق الموافقات"** مع label شامل يذكر
  كل الفئات

**٣. ترتيب قائمة المشتريات**:
ترتيب طبيعى حسب workflow:
1. الموردين (masters)
2. **طلبات الشراء** (planning) ← كانت مفقودة
3. أوامر الشراء (ordering)
4. فواتير المشتريات (receipt)
5. مرتجعات المشتريات (returns)
6. إشعارات دائن الموردين
7. تصحيح دفعات الموردين

**Bugs fixed**:
- Purchase Requests link كان مفقود → اتضاف
- Purchase Orders كان يعرض badge `purchase_request` (double-counting) → اتشال

### الصفحات الأصلية (لم تُشل)
كل الصفحات المفصولة لها وظائف زيادة عن الاعتماد (create/edit/delete/
filter/void/advanced modes). الـ inbox = مكان القرار السريع. الصفحات =
CRUD كامل. الاتنين محتفظ بيهم.
...
`pending`, `pending_approval_level_1`, `pending_warehouse_approval`.
الكارت يعرض stage badge (اعتماد إدارى / اعتماد مخزن) ويوجه للـ
endpoint الصحيح:
- `/approve` + `/reject` للمستوى الأول (إدارى)
- `/warehouse-approve` + `/warehouse-reject` للمخزن

الحوكمة عبر `secureApiRequest` (permission=`invoices:write`) +
`SALES_RETURN_LEVEL1_APPROVER_ROLES` /
`SALES_RETURN_WAREHOUSE_APPROVER_ROLES` + branch/warehouse gate.
سايدبار + سجل بيشملوا الاتنين.
...
`workflow_status IN ('pending_admin_approval','pending_approval')`.
Approve/Reject يستدعوا `/api/purchase-returns/[id]/approve` (نفس
الـ endpoint اللى `/purchase-returns/[id]` بتستخدمه). Governance
عبر `PurchaseReturnCommandService.processDecision` +
`approve_purchase_return_atomic` (role + branch/warehouse gate + JE).
